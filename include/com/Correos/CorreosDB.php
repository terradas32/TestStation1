...
class CorreosDB
{
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="idTipoCorreo=" . $aux->qstr($cEntidad->getIdTipoCorreo(), false) . " AND idCorreo=" . $aux->qstr($cEntidad->getIdCorreo(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
	...
	*************************************************************************/
	...
	{
		$this->sSQL = $_SESSION['SQLCorreosDB'];
		return $this->sSQL;
	}
