...
class Respuestas_pruebas_itemsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= "WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idCandidato=" . $aux->qstr($cEntidad->getIdCandidato(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getIdOpcionHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idOpcion<=" . $aux->qstr($cEntidad->getIdOpcionHast(), false);
		}
		...
		$this->sSQL=$sql;
		$_SESSION['SQLRespuestas_pruebas_itemsDB'] = $sql;
		return $sql;
	}
	...
	{
		$this->sSQL = $_SESSION['SQLRespuestas_pruebas_itemsDB'];
		return $this->sSQL;
	}
