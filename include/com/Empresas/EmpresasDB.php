...
//****************************
					if ($i == 0){
						...
						if ($cEntidad->getPublico() != ""){
							require_once(constant("DIR_WS_COM") . "Perfiles_empresas/Perfiles_empresasDB.php");
							require_once(constant("DIR_WS_COM") . "Perfiles_empresas/Perfiles_empresas.php");
							$cEntidadPF = new Perfiles_empresas();
							$cEntidadPF->setIdEmpresa($cEntidad->getIdEmpresa());
							$cEntidadPFDB = new Perfiles_empresasDB($aux);
							// 1. Borramos las asignaciones previas de la Empresa con los perfiles
							$cEntidadPFDB->borrar($cEntidadPF);
							// 2. Asignamos la Empresa a todos los perfiles
...
//FIN Modificado Pedro 06/05/2014 para evirar idPadre 0 que sólo puede ser PE
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$and			= false;
		$retorno			= true;
		$retorno = is_file(constant("DIR_WS_COM") . "Perfiles_empresas/Perfiles_empresasDB.php");
		if (!$retorno){
			$retorno=false;
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Class Not Found [" . constant("MNT_BORRAR") . "][EmpresasDB::Perfiles_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->	" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		...
		if ($retorno)
		{
			require_once(constant("DIR_WS_COM") . "Perfiles_empresas/Perfiles_empresasDB.php");
			require_once(constant("DIR_WS_COM") . "Perfiles_empresas/Perfiles_empresas.php");
			$cPerfiles_empresasDB	= new Perfiles_empresasDB($aux);  // Entidad DB
			$cPerfiles_empresas	= new Perfiles_empresas();  // Entidad
			$cPerfiles_empresas->setIdEmpresa($cEntidad->getIdEmpresa());
			if (!$cPerfiles_empresasDB->borrar($cPerfiles_empresas))
			{
				$retorno=false;
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][EmpresasDB::Perfiles_empresasDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->	" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}
