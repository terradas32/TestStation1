...
class OpcionesDB
{
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  .=" WHERE idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
	...
	*************************************************************************/
	...
	{
		$this->sSQL = $_SESSION['SQLOpcionesDB'];
		return $this->sSQL;
	}
