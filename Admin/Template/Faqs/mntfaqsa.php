...
function validaForm()
{
	...
	msg +=vString("<?php echo constant("STR_PREGUNTA");?>:",f.fPregunta.value,16777215,true);
	msg +=vString("<?php echo constant("STR_RESPUESTA");?>:",f.fRespuesta.value,16777215,true);
	aZonaPublicacion = document.forms[0].fZonaPublicacion;
	sId = "";
	for(i=0; i < aZonaPublicacion.length; i++ ){
		if (aZonaPublicacion[i].type == "radio" && aZonaPublicacion[i].name == "fZonaPublicacion"){
	...
	msg +=vNumber("<?php echo constant("STR_ZONA_PUBLICACION");?>:",sId,11,true);
	msg +=vNumber("<?php echo constant("STR_ORDEN");?>:",f.fOrden.value,11,true);
	msg +=vNumber("<?php echo constant("STR_PUBLICAR");?>:",f.fPublicar.value,1,true);
if (msg != "") {
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	<div id="envoltura">
		...
		<div style="width: 100%">
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fPregunta"><?php echo constant("STR_PREGUNTA");?></label>&nbsp;</td>
					<td><textarea id="fPregunta" name="fPregunta" rows="6" cols="1" class="obliga"  onchange="javascript:trim(this);"><?php echo $cEntidad->getPregunta();?></textarea></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fRespuesta"><?php echo constant("STR_RESPUESTA");?></label>&nbsp;</td>
					<td><textarea id="fRespuesta" name="fRespuesta" rows="6" cols="1" class="obliga"  onchange="javascript:trim(this);"><?php echo $cEntidad->getRespuesta();?></textarea></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fZonaPublicacion"><?php echo constant("STR_ZONA_PUBLICACION");?></label>&nbsp;</td>
					<td><input type="radio" name="fZonaPublicacion" id="fZonaPublicacion0" value="0"  <?php echo ($cEntidad->getZonaPublicacion() != "" && strtoupper($cEntidad->getZonaPublicacion()) == "0") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fZonaPublicacion0">Empresa</label>&nbsp;<input type="radio" name="fZonaPublicacion" id="fZonaPublicacion1" value="1"  <?php echo ($cEntidad->getZonaPublicacion() != "" && strtoupper($cEntidad->getZonaPublicacion()) == "1") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fZonaPublicacion1">Candidato</label>&nbsp;<input type="hidden" name="fZonaPublicacionHast" value="<?php echo $cEntidad->getZonaPublicacion();?>" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
				...
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fPublicar"><?php echo constant("STR_PUBLICAR");?></label>&nbsp;</td>
					
				    <td>
					   <select id="fPublicar" name="fPublicar" size="1">
	...
	<input type="hidden" name="fIdFaq" value="<?php echo (empty($_POST['AddIdioma'])) ? $cEntidad->getIdFaq() : $_POST['fIdFaq'];?>" />
	...
	<input type="hidden" name="faqs_next_page" value="<?php echo (isset($_POST['faqs_next_page'])) ? $cUtilidades->validaXSS($_POST['faqs_next_page']) : "1";?>" /></div>
