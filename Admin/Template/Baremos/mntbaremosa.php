...
function listapuntuaciones(){
	...
	$("div#listapuntuaciones").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBaremo: f.fIdBaremo.value,fIdBloque: f.fIdBloque.value,fIdEscala: f.fIdEscala.value,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
}
function aniadepunt(){
	var f = document.forms[0];
	var paginacargada = "Baremos.php";
	$("div#listapuntuaciones").hide().load(paginacargada,{fAniade:"1",fIdBaremo: f.fIdBaremo.value,fIdPrueba: f.fIdPrueba.value,fIdBloque: f.fIdBloque.value,fIdEscala: f.fIdEscala.value,fPuntMinima: f.fPuntMinima.value,fPuntMaxima: f.fPuntMaxima.value,fPuntBaremada: f.fPuntBaremada.value,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	f.fPuntMinima.value="";
	f.fPuntMaxima.value="";
...
function borrarresult(idResultado,idBaremo,idPrueba,idBloque,idEscala){
	...
	if(confirm('<?php echo constant("DEL_GENERICO");?>')){
		var paginacargada = "Baremos.php";
		$("div#listapuntuaciones").hide().load(paginacargada,{fBorra:"1",fIdResultado: idResultado,fIdBaremo: idBaremo,fIdPrueba: idPrueba,fIdBloque: idBloque,fIdEscala: idEscala,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	}
}
				...
				<td width="50%" valign="top">
					...
					<table cellspacing="0" cellpadding="0" border="0">
						<tr>
							...
							<td><input type="button" class="botones" <?php echo ($_bModificar) ? '' : 'disabled="disabled"';?> id="bid-ok" name="fBtnOk" value="<?php echo constant("STR_ANIADIR");?>" onclick="javascript:aniadepunt();document.forms[0].fPuntMinima.focus();"/></td>
						</tr>
					</table>
				</td>
				<td width="50%" valign="top">
