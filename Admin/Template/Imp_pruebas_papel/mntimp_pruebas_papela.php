<?php
    ...
    //De momento sólo lo han solicitado para prisma, cuando sena más irá cambiando
    //Hay que hacerlo
    $aPruebasValidadasParaPapel= array(24);
    $sPruebaDefecto=24;
    $cEntidad->setIdPrueba($sPruebaDefecto);
...
function validaForm()
{
	...
	msg +=vString("<?php echo constant("STR_NOMBRE");?>:",f.fProceso.value,255,true);
	
	if (eval("document.forms[0].cfFichero") !=null){
		if (f.cfFichero.checked == true)
			bObliga=true;
		else	bObliga=false;
	}else bObliga=true;
	msg +=vString("<?php echo constant("STR_FICHERO");?>:",f.fFichero.value,255,bObliga);
if (msg != "") {
...
<!--
 ...
 -->
					<td><input type="hidden" name="fIdPrueba" value="<?php echo $cEntidad->getIdPrueba();?>" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
				...
				<tr>
					...
					<td><input type="text" name="fProceso" value="<?php echo $cEntidad->getProceso();?>" class="obliga"  onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fFichero"><?php echo constant("STR_FICHERO");?></label>&nbsp;</td>
					<td><input onkeydown="blur();" onkeypress="blur();" type="file" id="fFichero" name="fFichero" class="obliga"  /></td>
				</tr>
					<?php if ($cEntidad->getFichero() != "")
					{
						$img=@getimagesize(constant("HTTP_SERVER") . $cEntidad->getFichero());
						$bIimg = (empty($img)) ? 0 : 1;
					?>
						<tr>
							<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
							<td width="140" class="negrob" valign="top">&nbsp;</td>
							<td>&nbsp;<a class="azul" href="#" onclick="javascript:abrirVentana('<?php echo $bIimg;?>','<?php echo base64_encode(constant("HTTP_SERVER") . $cEntidad->getFichero());?>');"><?php echo str_replace("_","&nbsp;", basename($cEntidad->getFichero()));?></a>&nbsp;<input onmouseover="this.style.cursor='pointer'" type="checkbox" name="cfFichero" />&nbsp;<?php echo constant("STR_QUITAR");?></td>
						</tr>
					<?php } ?>
	...
	<table cellspacing="0" cellpadding="0" border="0">
		<tr>
			<td>Formato del fichero de carga <b>(.csv)</b> :<br /><br /></td>
		</tr>
		<tr>
			<td class="negrob">CODIGO;EDAD;SEXO;NOMBRE;APELLIDO1;APELLIDO2;RESULTADO;ORDEN<br /><br /></td>
		</tr>

	</table>
	<table cellspacing="0" cellpadding="0" border="0">
		<tr>
			<td class="rojo"><?php echo $sMSG;?><br /><br /></td>
		</tr>
	</table>
...
<!-- 		<td><input type="button" class="botones" id="bid-volver" name="btnAdd" value="<?php echo constant("STR_VOLVER");?>" onclick="javascript:document.forms[0].MODO.value=document.forms[0].ORIGEN.value;document.forms[0].submit();" /></td> -->
	...
	<input type="hidden" name="fIdFicheroSabana" value="<?php echo (empty($_POST['AddIdioma'])) ? $cEntidad->getIdFicheroSabana() : $_POST['fIdFicheroSabana'];?>" />
	...
	<input type="hidden" name="imp_pruebas_papel_next_page" value="<?php echo (isset($_POST['imp_pruebas_papel_next_page'])) ? $cUtilidades->validaXSS($_POST['imp_pruebas_papel_next_page']) : "1";?>" /></div>
