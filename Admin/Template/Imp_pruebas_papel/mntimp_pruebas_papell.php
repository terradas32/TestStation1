...
function setPK(idFicheroSabana)
{
	var f=document.forms[0];
	f.fIdFicheroSabana.value=idFicheroSabana;
}
...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="2" border="0" width="100%">
		<tr>
			...
			$sPinta = "";
			$sOrden = "ASC";
			if ($cEntidad->getOrderBy() == "idFicheroSabana"){
			...
			?>
			<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities(constant("STR_ID_FICHERO_SABANA"), ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='idFicheroSabana';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes(constant("STR_ID_FICHERO_SABANA"));?>';return true"><?php echo constant("STR_ID_FICHERO_SABANA");?><?php echo $sPinta;?></a></td>
			<td class="tdnaranjab" align="center" valign="top"><?php echo constant("STR_PRUEBA");?></td>
			<?php 
			$sPinta = "";
			$sOrden = "ASC";
			if ($cEntidad->getOrderBy() == "fichero"){
				if ($cEntidad->getOrder() == "ASC"){
					$sOrden = "DESC";
					$sPinta = $strOrderASC;
				}else{
			...
			?>
			<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities(constant("STR_FICHERO"), ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='fichero';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes(constant("STR_FICHERO"));?>';return true"><?php echo constant("STR_FICHERO");?><?php echo $sPinta;?></a></td>
			<?php 
			$sPinta = "";
			$sOrden = "ASC";
		...
		<tr onmouseover="this.bgColor='<?php echo constant("ONMOUSEOVER");?>'" onmouseout="this.bgColor='<?php echo constant("ONMOUSEOUT");?>'" bgcolor="<?php echo constant("ONMOUSEOUT");?>">
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['idFicheroSabana']);?>"><?php echo substr(str_replace("\n","<br />",strip_tags($lista->fields['idFicheroSabana'], "<b><i><u><strong><br><br />")),0,11);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['idPrueba']);?>"><?php echo $comboPRUEBAS->getDescripcionCombo($lista->fields['idPrueba']);?></td>
		<?php	if (!empty($lista->fields['fichero']))
			{
				$img=@getimagesize(constant("HTTP_SERVER") . $lista->fields['fichero']);
				$bIimg = (empty($img)) ? 0 : 1;
				$sLlamada = '<td class="tddatoslista" valign="top" onclick="javascript:abrirVentana(\'' . $bIimg . '\',\'' . base64_encode(constant("HTTP_SERVER") . $lista->fields['fichero']) . '\');" title="' . basename($lista->fields['fichero']) . '">' . basename($lista->fields['fichero']) . '</td>';
			}else{
				$sLlamada = '<td class="tddatoslista" valign="top" onclick="javascript:setPK(\'' . addslashes($lista->fields['idFicheroSabana']) . '\');enviar(' . constant("MNT_CONSULTAR") . ');" title="">&nbsp;</td>';
			}
		echo $sLlamada; 
		?>
			<td class="tddatoslista" valign="top" align="center" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['fecAlta']);?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecAlta'];?>'));</script></td>
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['usuAlta']);?>"><?php echo $comboWI_USUARIOS->getDescripcionCombo($lista->fields['usuAlta']);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['usuMod']);?>"><?php echo $comboWI_USUARIOS->getDescripcionCombo($lista->fields['usuMod']);?></td>
			<td class="negro" align="center" valign="middle"><?php if($_bBorrar) {?><a href="#" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFicheroSabana']);?>');confBorrar(<?php echo constant("MNT_BORRAR");?>,'<?php echo constant("DEL_GENERICO");?>');"><img src="<?php echo constant('DIR_WS_GRAF');?>menos.gif" width="9" height="9" border="0" alt="<?php echo constant("STR_BORRAR");?>" /></a><?php } ?></td>
		</tr>
		<?php $i++;
		$lista->MoveNext();
		} ?>
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width="10"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="10" height="1" border="0" alt="" /></td>
			<td width="100%" colspan="2" class="naranjab"><?php echo sprintf(constant("STR_LISTA_DE_"),str_replace('_', ' ', constant("STR_IMPPRUEBASPAPEL")));?><a href="#_" onclick="javascript:document.forms[0].action='sqlToExcel.php';document.forms[0].submit();document.forms[0].action='<?php echo $_SERVER['PHP_SELF'];?>';"><img src="<?php echo constant('DIR_WS_GRAF');?>excel.gif" width="34" height="35" align="right" border="0" alt="<?php echo constant("STR_EXPORTAR_A_EXCEL");?>" /></a></td>
		</tr>
		<tr>
			<td width="10"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="10" height="1" border="0" alt="" /></td>
	...
	<br />
	<input type="hidden" name="fIdFicheroSabana" value="" />
	<input type="hidden" name="ORIGEN" value="<?php echo constant("MNT_LISTAR");?>" />
	<input type="hidden" name="fReordenar" value="" />
	<input type="hidden" name="fSQLtoEXCEL" value="<?php echo base64_encode($sql . constant("CHAR_SEPARA") . "imp_pruebas_papel");?>" />
	<input type="hidden" name="LSTIdFicheroSabanaHast" value="<?php echo (isset($_POST['LSTIdFicheroSabanaHast'])) ? $cUtilidades->validaXSS($_POST['LSTIdFicheroSabanaHast']) : "";?>" />
	<input type="hidden" name="LSTIdFicheroSabana" value="<?php echo (isset($_POST['LSTIdFicheroSabana'])) ? $cUtilidades->validaXSS($_POST['LSTIdFicheroSabana']) : "";?>" />
	...
	<input type="hidden" name="imp_pruebas_papel_next_page" value="<?php echo (isset($_POST['imp_pruebas_papel_next_page'])) ? $cUtilidades->validaXSS($_POST['imp_pruebas_papel_next_page']) : "1";?>" />
</div>
