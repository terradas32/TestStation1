...
******************************************************************/
	...
	function textoDefinicion($puntuacion){
		$str="";
		if($puntuacion ==0 || $puntuacion ==1 || $puntuacion==2){
	...
	function textoPuntuacion($puntuacion){
		$str="";
		if($puntuacion ==0 || $puntuacion ==1 || $puntuacion==2){
			$str="Área con clara necesidad de Desarrollo";	//"ÁREA CLAVE DE MEJORA";
		}
		if($puntuacion ==3 || $puntuacion==4){
			$str="Área de Desarrollo";	//"ÁREA POTENCIAL DESARROLLO";
		}
		if($puntuacion ==5 || $puntuacion==6){
			$str="Nivel Adecuado para el puesto";	//"ÁREA EN DESARROLLO";
		}
		if($puntuacion ==7 || $puntuacion==8){
			$str="Área de Fortaleza";	//"ÁREA POTENCIAL FORTALEZA";
		}
		if($puntuacion ==9 || $puntuacion==10){
			$str="Área de clara Fortaleza";	//"ÁREA DE FORTALEZA";
		}
		return $str;
	}
...
//		print_r($aAptCCT84);
		...
		$sHtml.= '	<table id="caja_tit_puntos" border="0">
		        		<tr>
		        		...
		        		<tr>
							...
							<td class="puntos_escala">
								<table width="100%" cellspacing="0" cellpadding="0">
    								...
    								<tr>
    									<td width="81%" style="vertical-align:middle;">
    										<img src="'.constant('DIR_WS_GESTOR') . constant('DIR_WS_GRAF'). 'bodoque_gigante.jpg'.'" style="width:'.$iWhidth*$iPDirectaX.'%;height:25px;">
    									</td>
    								</tr>
    							</table>
							</td>
		        		...
		        		<tr>
							...
							<td class="puntos_escala">
								<table width="100%" cellspacing="0" cellpadding="0">
    								...
    								<tr>
    									<td width="81%" style="vertical-align:middle;">
    										<img src="'.constant('DIR_WS_GESTOR') . constant('DIR_WS_GRAF'). 'bodoque_gigante.jpg'.'" style="width:'.$iWhidth*$iPercentil.'%;height:25px;">
    									</td>
    								</tr>
    							</table>
								<table width="100%" cellspacing="0" cellpadding="0" border="0">
									<tr>
    									...
    									<td width="34%" align="center" style="height:30px;border-right:2px solid #FF8000;">
    										<font style="font-weight: bold;">' . mb_strtoupper(constant("STR_BAJO_POTENCIAL"), 'UTF-8').'</font>
    									<td>
    									<td width="37%" align="center" style="height:30px;border-right:2px solid #FF8000;">
    										<font style="font-weight: bold;">' . mb_strtoupper(constant("STR_MEDIO_POTENCIAL"), 'UTF-8').'</font>
    									<td>
    									<td width="27%" align="center" style="height:30px;">
				...
				<div class="desarrollo">
        		    ...
        		    <div class="caja">
						...
						<p class="textos"><font style="font-weight: bold;">Ejemplo:<br /><br /></font>
							<img src="'.constant("DIR_WS_GESTOR").'graf/seas/' . $_POST['fCodIdiomaIso2']. '/ejemploConsistenciaMB.jpg" alt="Escala Consistencia" title="Escala Consistencia" />
						</p>
...
$sHtml.='
			...
			$sHtml.='
		        		<tr>
							...
							<td class="pl_img">' . '<img src="' . constant("DIR_WS_GESTOR") . 'graf/seas/cpc' . $iPBaremada . '.jpg" title="' . $iPBaremada . '" />' . '</td>
			...
			$sHtml.='
		        		<tr>
							...
							<td class="pl_img">' . '<img src="' . constant("DIR_WS_GESTOR") . 'graf/seas/cpc' . $consistencia . '.jpg" title="' . $iPBaremada . '" />' . '</td>
...
$sHtml.='
						<table width="100%" border="0">
		        			<tr>
								<td width="350" class="cajaColor dato-informe" >Orientación al Cliente</td>
								<td width="7">&nbsp;</td>
...
//---------------------------***
	...
	function generarAcademia($aPuntuaciones, $aPuntuacionesCompetencias, $sHtmlCab, $idIdioma)
	{
		...
		$sHtml.='
					...
					<div class="caja" style="margin-top:20px !important;">
	        			<table width="96%" border="0" style="margin-left: 16px;">
			        		<tr>
								<td width="30%">Orientación al Cliente</td>
					';
		$iPuntacion = $aPuntuacionesCompetencias["1-1"];	// Orientación al Cliente
		$sHtml.='
								<td class="caja">' . $iPuntacion . ' - ' . textoDefinicion($iPuntacion) . '</td>
	        				</tr>
					';
		$iPuntacion = $iPEspecializacion;	// Especialización
		$iPuntacion = TransformarEspecializacion($cCandidato->getEspecialidadMB(), $iPEspecializacion);
		...
		$sHtml.='
					...
					<h2 class="avisoFormacion"> * NOTA DE AVISO: El siguiente paso es, realizar en el plazo de un mes, el test de IQ-Box a través de SABA, previa inscripción del coordinador de Formación.</h2>
					<p>Por favor, le rogamos envíe esta ficha a la siguiente dirección de correo electrónico para que se proceda a la inscripción de la persona en los cursos recomendados:</p>
					<p>&nbsp;</p>
...
//--
	...
	function TransformarEspecializacion($especialidad, $aciertoscono)
	{
		...
		// 1 - Turismos Mercedes-Benz + Smart
		//34 - Industriales Ligeros y Pesados
		//if (($especialidad == "VIL+VIP") or ($especialidad == "MB") or ($especialidad == "CJD") or ($especialidad == "VIL") or ($especialidad == "VIP") or ($especialidad == "MB+VIL") or ($especialidad == "MB+CJD"))
		if (($especialidad == "1") or ($especialidad == "34"))
		{

			switch ($aciertoscono)
		    {

		        //area con clara necesidad de desarrollo
		        //Case 0, 1, 2, 3, 4, 5, 6, 7:
		        case ($aciertoscono >=0) && ($aciertoscono <=  7):
		        	$especializacionfinal = 1;
		        	break;
		        //area con clara necesidad de desarrollo
		        //Case 8, 9, 10, 11, 12:
		        case ($aciertoscono >=8) && ($aciertoscono <=  12):
		        	$especializacionfinal = 2;
		        	break;
		        //area de desarrollo
		        //Case 13, 14, 15, 16, 17:
		        case ($aciertoscono >=13) && ($aciertoscono <=  17):
		        	$especializacionfinal = 3;
		        	break;
		        //nivel adecuado para el puesto
		        //Case 18, 19, 20, 21, 22:
		        case ($aciertoscono >=18) && ($aciertoscono <=  22):
		        	$especializacionfinal = 4;
		        	break;
		        //area de fortaleza
		        //Case 23, 24, 25, 26, 27:
		        case ($aciertoscono >=23) && ($aciertoscono <=  27):
		        	$especializacionfinal = 5;
		        	break;
		        //area de clara fortaleza
		        //Case 28, 29, 30:
		        case ($aciertoscono >=28) && ($aciertoscono <=  30):
		        	$especializacionfinal = 6;
		        	break;

		    }
		}else{
		    if ($especialidad == "MB+VIL+CJD")
		    {
		        switch ($aciertoscono)
		        {
		            ...
		            case ($aciertoscono >=0) && ($aciertoscono <=  10):
		            	$especializacionfinal = 1;
		            	break;
		            //area con clara necesidad de desarrollo
		            //Case 11, 12, 13, 14, 15, 16, 17, 18:
		            case ($aciertoscono >=11) && ($aciertoscono <=  18):
		            	$especializacionfinal = 2;
		            	break;
		            //area de desarrollo
		            //Case 19, 20, 21, 22, 23, 24, 25:
		            case ($aciertoscono >=19) && ($aciertoscono <=  25):
		            	$especializacionfinal = 3;
		            	break;
		            //nivel adeacuado para el puesto
		            //Case 26, 27, 28, 29, 30, 31, 32, 33:
		            case ($aciertoscono >=26) && ($aciertoscono <=  33):
		            	$especializacionfinal = 4;
		            	break;
		            //area de fortaleza
		            //Case 34, 35, 36, 37, 38, 39, 40:
		            case ($aciertoscono >=34) && ($aciertoscono <=  40):
		            	$especializacionfinal = 5;
		            	break;
		            //area de clara fortaleza
		            //Case 41, 42, 43, 44, 45:
		            case ($aciertoscono >=41) && ($aciertoscono <=  45):
		            	$especializacionfinal = 6;
		            	break;
