		...
		$sHtmlInicio='
			<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
				...
				<head>
					...
					<link rel="stylesheet" type="text/css" href="'.constant("DIR_WS_GESTOR").'estilosInformes/seasMazda/resetCSS.css"/>';
					if($_POST['fIdTipoInforme'] != 11){
						$sHtmlInicio.= 		'<link rel="stylesheet" type="text/css" href="'.constant("DIR_WS_GESTOR").'estilosInformes/seasMazda/style.css"/>';
					}else{
						$sHtmlInicio.= 		'<link rel="stylesheet" type="text/css" href="'.constant("DIR_WS_GESTOR").'estilosInformes/seasMazda/styleNarrativos.css"/>';
					}
		$sHtmlInicio.='
					<title>AS</title>
					<style type="text/css">
	...
	$sHtmlCab .='<div class="cabecera">
					<table width="100%" border="0">
						<tr>
						    ...
						    <td class="logo">
						    	<img src="'.constant("DIR_WS_GESTOR").'estilosInformes/seasMazda/img/sp.gif" title="logo"/>
						    </td>
						    <td class="fecha">
...
******************************************************************/
	...
	{
		if ($pd<=9){$baremo_C=1;}
		if ($pd>=10 && $pd<=16){$baremo_C=2;}
		if ($pd>=17 && $pd<=22){$baremo_C=3;}
		if ($pd>=23 && $pd<=29){$baremo_C=4;}
		if ($pd>=30 && $pd<=35){$baremo_C=5;}
		if ($pd>=36 && $pd<=42){$baremo_C=6;}
		if ($pd>=43 && $pd<=48){$baremo_C=7;}
		if ($pd>=49 && $pd<=55){$baremo_C=8;}
		if ($pd>=56 && $pd<=61){$baremo_C=9;}
		if ($pd>=62){$baremo_C=10;}
		return $baremo_C;
	}
                    ...
                    <table width="666" cellspacing="0" cellpadding="0">
                        <tr>
                          <td style="height:45px;">
                            <img src="'.constant('DIR_WS_GESTOR') . constant('DIR_WS_GRAF'). 'numeritosEstandar.jpg'.'" style="width:666px;">
                          </td>
                        </tr>
                        <tr>
                          <td width="81%" style="vertical-align:middle;">
                            <img src="'.constant('DIR_WS_GESTOR') . constant('DIR_WS_GRAF'). 'bodoque_gigante_OLD.jpg'.'" style="width:'.str_replace(",", ".", (($iPercentil*666)/100)).'px;height:25px;">
                          </td>
                        </tr>
                    </table>
...
// 									<tr>
// 										<td rowspan="' . ($nTCompetencias+1) . '" class="blancob" style="text-align: center;background-color: #413D3E; width: 184px;">Competencias<br />Asesor de Servicio<br /></td>
// 									</tr>';
// 								$listaTipoCompetencia->Move(0);
// 								while(!$listaTipoCompetencia->EOF)
// 								{
// 									$sHtml.='			    <tr>
// 																			<td style="text-align: center;width: 50px;"><img src="'.constant("DIR_WS_GESTOR").'graf/seasMazda/flechita.jpg" title="Flecha" /></td>
				...
				$sHtml.='
      			<tr>';
						if ($iTC == 0){
							...
							$sHtml.='
							<td style="font-size: 11px;padding-left: 3px;padding-right: 3px;background-color: gainsboro;border: 1px solid #fff;white-space: nowrap;">' . $listaCompetencias->fields['nombre'] . '</td>
...
$sHtml.='
            ...
            <table border="0" width="100%" style="font-size: 14px;border-collapse: separate;border: 1px solid #ffff;">
					...
					<table id="caja_tit" border="0">
						<tr>
							<td>AJUSTE AL PERFIL DE ASESOR DE SERVICIO</td>
						</tr>
					</table>
					<div class="caja">
						<p class="textos">Se considera que el grado de ajuste al perfil de Asesor de Servicio es adecuado cuando es igual o superior a 76%.</p>
					</div>
					<div class="cajaTextos-resultados">
            <table border="0" width="100%" style="font-size: 14px;border-collapse: separate;border: 1px solid #ffff;">
							<tr>
...
$sHtml.='
								<table width="100%" border="0">
									...
									<tr>
										<td colspan="2" style="border: 1px solid #000;padding: 10px;line-height: 1.5;text-align: justify;"><p class="textos">' . $definiciónTC . '</p></td>
									</tr>
									<tr>
									...
									<tr>
										<td colspan="2" style="border: 1px solid #000;padding: 10px;line-height: 1.5;text-align: justify;">
												...
												<table width="100%" border="0">
													...
													<tr>
														<td colspan="2" style="border: 1px solid #000;padding: 10px;line-height: 1.5;text-align: justify;">
															...
															<p class="textos_preguntas">¿Cómo haces para asegurar que tu trabajo tiene la calidad esperada? Cuéntame un caso concreto en el que hayas tenido que hacer un esfuerzo para asegurar la calidad?, ¿qué pasó?, ¿qué dificultades encontraste?, ¿qué esfuerzo tuviste que hacer?, ¿cuál fue el resultado?</p>

															<p class="textos_preguntas">Cuéntame un caso contrario en el que no hayas logrado asegurar la calidad esperada. ¿Qué pasó?, ¿cómo actuaste?, ¿cuál fue el resultado? Si volvieses a vivir una situación similar, ¿qué harías diferente?</p>
										...
										<table width="100%" border="0">
											...
											<tr>
												<td colspan="2" style="border: 1px solid #000;padding: 10px;line-height: 1.5;text-align: justify;">
													...
													<p class="textos_preguntas">¿Cuál es la situación más estresante que has vivido en el trabajo?, ¿cómo la gestionaste?, ¿qué impacto tuvo en tu trabajo? Si volvieses a vivir una situación similar, ¿qué harías diferente?</p>
												</td>
											</tr>
										</table><br />
						';
  ...
  $cEmpresas = $cEmpresasDB->readEntidad($cEmpresas);
	...
	$sHtml= '
			<div class="pagina portada">
		    	<img src="'.constant("DIR_WS_GESTOR").'graf/seasMazda/portada.jpg" alt="Psicólogos Empresariales" title="Psicólogos Empresariales" />
		    	<h1 class="titulo"></h1>';
	if($_POST['fIdTipoInforme']!=11){
		$sHtml.= 		'<div id="txt_infome"><p>Informe Selección<br />Asesor de Servicio</p></div>';
	}else{
		$sHtml.= 		'<div id="txt_infome_narrativo"><p>Informe Selección<br />Asesor de Servicio</p></div>';
	}
