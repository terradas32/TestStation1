...
// 	echo $sqlProceso_pruebas_candidato;
    $listaProcesosPruebas = $conn->Execute($sqlProceso_pruebas_candidato);
    $iListaPruebas = $listaProcesosPruebas->recordCount();

    generaPortada($_POST['fCodIdiomaIso2'], $cPruebas->getIdPrueba());
    generaIntroduccion($_POST['fCodIdiomaIso2'], $cPruebas->getIdPrueba());
    //error_log("\n**********22222222222***********PEDRO ->IdPrueba::" . $cPruebas->getIdPrueba() . " iListaPruebas::" . $iListaPruebas . " sLlamada:" . $sLlamada, 3, constant("DIR_FS_PATH_NAME_LOG"));
    if($iListaPruebas > 0 && $sLlamada != "Candidato")
    {	while(!$listaProcesosPruebas->EOF)
		{
			$cPruebas2Informe = new Pruebas();
			$cPruebas2Informe->setIdPrueba($listaProcesosPruebas->fields['idPrueba']);
			$cPruebas2Informe->setCodIdiomaIso2($listaProcesosPruebas->fields['codIdiomaIso2']);
			$cPruebas2Informe = $cPruebasDB->readEntidad($cPruebas2Informe);

			if (in_array($listaProcesosPruebas->fields['idPrueba'], $aAleatoriasV))
			{
				$iVERBAL = getPDPrueba($cCandidato, $cPruebas2Informe);
// 				generaPortada($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
// 				generaIntroduccion($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
				generaPrueba($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
			}
			if (in_array($listaProcesosPruebas->fields['idPrueba'], $aAleatoriasN))
			{
				$iNUMERICO = getPDPrueba($cCandidato, $cPruebas2Informe);
				generaPrueba($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
			}
			if (in_array($listaProcesosPruebas->fields['idPrueba'], $aAleatoriasL))
			{
				$iLOGICO = getPDPrueba($cCandidato, $cPruebas2Informe);
				generaPrueba($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
			}
			if (in_array($listaProcesosPruebas->fields['idPrueba'], $aAleatoriasI))
			{
				$iINGLES = getPDPrueba($cCandidato, $cPruebas2Informe);
				generaPrueba($listaProcesosPruebas->fields['codIdiomaIso2'], $listaProcesosPruebas->fields['idPrueba']);
			}
			if (in_array($listaProcesosPruebas->fields['idPrueba'], $aAleatoriasIL))
			{
				$iINGLESL = getPDPrueba($cCandidato, $cPruebas2Informe);
    	...
    	//error_log("\n**********33333333***********PEDRO ->IdPrueba::" . $cPruebas->getIdPrueba() . " iListaPruebas::" . $iListaPruebas . " sLlamada:" . $sLlamada, 3, constant("DIR_FS_PATH_NAME_LOG"));
   		$aTemporal = array(48,58);
   		for ($i=0; $i<count($aTemporal); $i++) {

   			$cPruebas2Informe = new Pruebas();
   			$cPruebas2Informe->setIdPrueba($aTemporal[$i]);
   			$cPruebas2Informe->setCodIdiomaIso2("en");
   			$cPruebas2Informe = $cPruebasDB->readEntidad($cPruebas2Informe);

	    	if (in_array($aTemporal[$i], $aAleatoriasI))
	    	{
	    		$iINGLES = getPDPrueba($cCandidato, $cPruebas2Informe);
	    		generaPrueba("en", $aTemporal[$i]);
	    	}
	    	if (in_array($aTemporal[$i], $aAleatoriasIL))
	    	{
	    		$iINGLESL = getPDPrueba($cCandidato, $cPruebas2Informe);
...
function generaPortada($sCodIdiomaIso2, $sIdPrueba){
		global $sHtml;
		global $sHtmlCab;
		global $conn;
		global $cTextos_seccionesDB;
		global $cCandidato;
		global $sNombre_infome;
		global $cUtilidades;
   		//PORTADA
		$cTextos_secciones = new Textos_secciones();
		$cTextos_secciones->setIdSeccion("3");
		$cTextos_secciones->setCodIdiomaIso2($sCodIdiomaIso2);
...
function generaIntroduccion($sCodIdiomaIso2, $sIdPrueba){
		global $sHtml;
		global $sHtmlCab;
		global $conn;
		global $cTextos_seccionesDB;
		global $cCandidato;
		global $cRangos_textos;
		...
		{
	    	$sHtml.='					<li style="list-style: square;padding:10px;">
...
-->
    							...
    							<table class="resetTable" cellspacing="30" cellpadding="15" border="0" width="85%">
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_NUM_TOTAL_PREGUNTAS").'</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'. $listaItemsPrueba->recordCount().'</font>
    									</td>
    								</tr>
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_NUM_PREGUNTAS_ACERTADAS").'(P.D.)</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'.$iPDirecta.'</font>
    									</td>
    								</tr>
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_PUNTUACION_PERCENTIL").'(P.C.)</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'.$iPercentil.'</font>
    									</td>
    								</tr>
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_INDICE_RAPIDEZ").'</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'.$sIR.'</font>
    									</td>
    								</tr>
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_INDICE_PRECISION").'</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'.$sIP.'</font>
    									</td>
    								</tr>
    								<tr>
    									<td width="90%" style="border:3px solid #6e6e6e;">
    										<font style="font-size: 18px;color: #cc3300;font-weight: bold;">'.constant("STR_PRODUCTO_RENDIMIENTO").'</font>
    									</td>
    									<td width="10%" style="border:3px solid #6e6e6e;" align="center"><font style="font-size: 18px;color: #6e6e6e;font-weight: bold;">'.$sPOR.'</font>
    									</td>
    								</tr>
    							</table>
    						</td>
    	...
    	$sHtml.='		<tr>
    						<td width="80%" align="center">
    							<table width="90%" cellspacing="0" cellpadding="0" style="background:#c0c0c0;border-left:3px solid #6e6e6e;border-right:3px solid #6e6e6e;">
    								<tr>
    									<td width="19%" align="center" style="height:60px;border:2px solid #6e6e6e;">
    										<font style="font-size: 18px;font-weight: bold;">'.constant("STR_PUNT_INFORMES").'</font>
    									<td>
    									<td width="27%" align="center" style="height:60px;border:2px solid #6e6e6e;">
    										<font style="font-size: 18px;font-weight: bold;">' . mb_strtoupper(constant("STR_BAJO_POTENCIAL"), 'UTF-8').'</font>
    									<td>
    									<td width="27%" align="center" style="height:60px;border:2px solid #6e6e6e;">
    										<font style="font-size: 18px;font-weight: bold;">' . mb_strtoupper(constant("STR_MEDIO_POTENCIAL"), 'UTF-8').'</font>
    									<td>
    									<td width="27%" align="center" style="height:60px;border:2px solid #6e6e6e;">
    										<font style="font-size: 18px;font-weight: bold;">' . mb_strtoupper(constant("STR_ALTO_POTENCIAL"), 'UTF-8').'</font>
    									<td>
    								</tr>
    							</table>
    							<table width="90%" cellspacing="0" cellpadding="0" style="border-left:4px solid #6e6e6e;border-bottom:3px solid #6e6e6e;border-right:4px solid #6e6e6e;">
    								<tr>
...
//		echo "<br />" . $sqlRangosTextos;
    	...
    	$sHtml.='		<tr>
    						<td>
    							<p style="text-align:center;background:#6e6e6e;height:25px;padding:5px;">
...
*/
	...
	function grafResumen($width, $height, $sCadena, $_PathImgInforme, $sImg, $iVERBAL=0, $iLOGICO=0, $iNUMERICO=0, $iINGLES=0, $iINGLESL=0)
	{
		$bIdeal=false;

		include_once("KPMGGrafResumen.php");
	...
	function grafResumen2($width, $height, $sCadena, $_PathImgInforme, $sImg, $iVERBAL=0, $iLOGICO=0, $iNUMERICO=0, $iINGLES=0)
	{
		...
		$datay=array($iVERBAL, $iLOGICO, $iNUMERICO, $iINGLES);
		$datax=array("VERBAL", "LOGICA", "CALCULO", "INGLES");
