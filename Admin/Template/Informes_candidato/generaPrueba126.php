...
//		echo "<br />" . $sqlInversos;
		...
		$sDescInforme=constant("STR_PRISMA_PERFIL_DE_COMPETENCIAS_105");
...
//		echo "<br />AA:sqlEscalas_items::" . $sqlEscalas_items . "";
...
//					       			$iPd = $iPd + $cRespuestas_pruebas_items->getIdOpcion();
					        ...
					        if($nBaremos > 0){
					       	...
					       	$scolor="#000";
									$aBloquesBIP = array(75,76,77,78,79,80,81,82,83,84,85,86,87,88);
					       	if (in_array($listaBloques->fields['idBloque'], $aBloquesBIP))
									{
					       		$scolor="green";
					       	}
		...
		$sHtmlInicio='
			<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
				...
				<head>
					...
					<link rel="stylesheet" type="text/css" href="'.constant("DIR_WS_GESTOR").'estilosInformes/prisma126/resetCSS.css"/>';
		$sHtmlInicio.= '<link rel="stylesheet" type="text/css" href="'.constant("DIR_WS_GESTOR").'estilosInformes/prisma126/style.css"/>';
	...
	$sHtmlCab .='<div class="cabecera">
					<table width="100%" border="0">
						<tr>
						    ...
						    <td class="logo">
						    	<img src="'.constant("DIR_WS_GESTOR").'estilosInformes/prisma126/img/logo-pequenio.jpg" title="logo"/>
						    </td>
						    <td class="fecha">
		...
		$sHtml.= '
			<div class="pagina portada">
		    	<img src="' . constant("DIR_WS_GESTOR").'graf/prisma126/portada.jpg" alt="Psicólogos Empresariales" title="Psicólogos Empresariales" />
		    	<h1 class="titulo"><!-- <img src="'.constant("DIR_WS_GESTOR") . 'estilosInformes/prisma126/img/logo.jpg" /> --></h1>';

		$sHtml.= '<div id="txt_infome"><p></p></div>';
		$sHtml.='
				<div id="informe">
					<p class="textos">'. $cCandidato->getNombre() . ' ' . $cCandidato->getApellido1() . ' ' . $cCandidato->getApellido2() .'</p>
					<p class="textos">'.date("d/m/Y").'</p>
				</div>
...
//		$sHtml.=	constant("_NEWPAGE");
		...
		// {
		// 	case(5);//Informe competencias
		 		$sHtml.= informeSintesisCompetencias($aPuntuaciones,$sHtmlCab,$_POST['fCodIdiomaIso2']);
				$sHtmlNoPintar= informeCompetencias($aPuntuaciones,$sHtmlCab,$_POST['fCodIdiomaIso2']);
				$aSQLPuntuacionesPPL = array();
...
//		echo "LLEGO PDF";exit;
    ...
    $footer_html    =  mb_strtoupper("PEOPLE EXPERTS", 'UTF-8') . str_repeat(" ", 70) . "©PRISMA Psicólogos Empresariales y Asociados SA.";
...
//		echo "<br />1111-->sBloques::" . $sBloques;
		...
		$sHtml.='
				<div class="desarrollo">
					<h2 class="subtitulo">' . constant("STR_PRISMA_SINTESIS_DEL_PERFIL_DE_COMPETENCIAS_105") . '</h2>
        			<div class="caja" style="margin-bottom:20px;">
		            	<h3 class="encabezado">' . mb_strtoupper(constant("STR_INTRODUCCION"), 'UTF-8') . '</h3>
		            	<p class="textos">' . constant("STR_BIP_INTRO_P1") . '</p>
									<p class="textos">' . constant("STR_BIP_INTRO_P2") . '</p>
									<p class="textos">' . constant("STR_BIP_INTRO_P3") . '</p>
			         </div><!--FIN DIV CAJA-->
							 <div class="caja">
		...
		if($nBloques > 0){
			$sHtml.='
							<tr>
			                  <td colspan="8" style="background:#fff;"><h2 class="subtitulo">' . mb_strtoupper(constant("STR_COMPETENCIAS"), 'UTF-8') . '</h2></td>
			                </tr>
			         ';
			$sHtml.='
			                <tr>
			                  <td colspan="3" style="background:#6a6a6b;">&nbsp;</td>
			                  <td class="cel">' . constant("STR_PRISMA_BAJO") . '</td>
			                  <td class="cel">' . constant("STR_PRISMA_ME_BA") . '</td>
			                  <td class="cel">' . constant("STR_PRISMA_MEDIO") . '</td>
			                  <td class="cel">' . constant("STR_PRISMA_ME_AL") . '</td>
			                  <td class="cel">' . constant("STR_PRISMA_ALTO") . '</td>
			                </tr>';
			$iPBloque = 0;
			$iPGlobal= 0;
			$iPBloque1to5 = 0;
...
//				echo "<br />sqlEscalas:: " . $sqlEscalas;
				...
				if($nEscalas > 0){
					while(!$listaEscalas->EOF){
						...
						{
							$iEscala = 0;
							if ($listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala'] == "66-2"){
								//Escala inversa REDES DE COLABORACIÓN - AUTONOMIÍA
//								echo "<br />INV ->[" . $listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala'] . "] ESCALA inv ANTES: " . $aPuntuaciones[$listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala']];
//								echo "<br />INV ->[" . $listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala'] . "] ESCALA inv DESPUES: " . getInversoEscala($aPuntuaciones[$listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala']]);
...
//				 			echo "<br />iPGlobal::($iEscala - 5.5) * ($iEscala - 5.5)" . $iPGlobal;
					...
					$aBloquesBIP = array(75,76,77,78,79,80,81,82,83,84,85,86,87,88);
					if (in_array($listaBloques->fields['idBloque'], $aBloquesBIP))
					{	//Solo pintamos las competencias que son especificas de Banco Hipotecario
				        ...
				        $sHtml.='
					        <tr>
			                  <td class="tablaTitu" style="text-align: left;padding-left: 5px;">' . mb_strtoupper($listaBloques->fields['nombre'], 'UTF-8') . '</td>
												<td class="number" ><p>'.$iPBloque1to5.'</p></td>
			                  <td class="descripcion"><p>' . $listaBloques->fields['descripcion'] . '</p></td>';
			 				//if($iPBloque==1 || $iPBloque==2){
			 				if($iPBloque1to5==1){
								//$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasBajo.JPG" alt="' . getEscala_1to5($iPBloque) . '" title="' . getEscala_1to5($iPBloque) . '" /></td>';
			 					$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasBajo.JPG" alt="' . $iPBloque1to5 . '" title="' . $iPBloque1to5 . '" /></td>';
					       	}else{
					       		$sHtml.='<td class="celS">&nbsp;</td>';
					       	}
					       	//if($iPBloque==3 || $iPBloque==4){
					       	if($iPBloque1to5==2){
								//$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMB.JPG" alt="' . getEscala_1to5($iPBloque) . '" title="' . getEscala_1to5($iPBloque) . '" /></td>';
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMB.JPG" alt="' . $iPBloque1to5 . '" title="' . $iPBloque1to5 . '" /></td>';
					       	}else{
					       		$sHtml.='<td class="celS">&nbsp;</td>';
					       	}
					       	//if($iPBloque==5 || $iPBloque==6){
					       	if($iPBloque1to5==3){
								//$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMedio.JPG" alt="' . getEscala_1to5($iPBloque) . '" title="' . getEscala_1to5($iPBloque) . '" /></td>';
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMedio.JPG" alt="' . $iPBloque1to5 . '" title="' . $iPBloque1to5 . '" /></td>';
					       	}else{
					       		$sHtml.='<td class="celS">&nbsp;</td>';
					       	}
					       	//if($iPBloque==7 || $iPBloque==8){
					       	if($iPBloque1to5==4){
								//$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMA.JPG" alt="' . getEscala_1to5($iPBloque) . '" title="' . getEscala_1to5($iPBloque) . '" /></td>';
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMA.JPG" alt="' . $iPBloque1to5 . '" title="' . $iPBloque1to5 . '" /></td>';
					       	}else{
					       		$sHtml.='<td class="celS">&nbsp;</td>';
					       	}
					       	//if($iPBloque==9 || $iPBloque==10){
					       	if($iPBloque1to5==5){
								//$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasAlto.JPG" alt="' . getEscala_1to5($iPBloque) . '" title="' . getEscala_1to5($iPBloque) . '" /></td>';
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasAlto.JPG" alt="' . $iPBloque1to5 . '" title="' . $iPBloque1to5 . '" /></td>';
					       	}else{
					       		$sHtml.='<td class="celS">&nbsp;</td>';
					       	}
		               	$sHtml.='
		 ...
		 ////////////////
		 // Se toman las 32 escalas del PRISM@ normal para la consistencia.
		 //$consistencia = baremo_C(number_format(sqrt($iPGlobal/18)*100 ,0));
//		 echo "<br />A----------->iPGlobal::" . $iPGlobal;
//		 echo "<br />A----------->number_format(sqrt(iPGlobal/32)*100 ,0)= " . number_format(sqrt($iPGlobal/32)*100 ,0);
		 $consistencia = baremo_C(number_format(sqrt($iPGlobal/32)*100 ,0));
//		 echo "<br />A----------->consistencia::" . $consistencia;
		 //$consistencia1to5 = baremo_C(number_format(sqrt($iPGlobal1to5/18)*100 ,0));
		 //$consistencia1to5 = baremo_C(number_format(sqrt($iPGlobal1to5/32)*100 ,0));
		 $consistencia1to5 = getEscala_1to5($consistencia);
		 ///////////////


		$sHtml.='
		<tr>
		<td colspan="8" class="celS" >&nbsp;</td>
		</tr>
			<tr>
				<td class="tablaTitu" style="text-align: left;padding-left: 5px;background: #548dd4;" >CONSISTENCIA</td>
				<td class="number" ><p>'.$consistencia.'</p></td>
				<td class="descripcion" style="background: #548dd4;"><p>Grado de congruencia en las respuestas</p></td>';

							if($consistencia1to5==1){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasBajo.JPG" alt="' . $consistencia1to5 . '" title="' . $consistencia1to5 . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($consistencia1to5==2){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMB.JPG" alt="' . $consistencia1to5 . '" title="' . $consistencia1to5 . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($consistencia1to5==3){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMedio.JPG" alt="' . $consistencia1to5 . '" title="' . $consistencia1to5 . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($consistencia1to5==4){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMA.JPG" alt="' . $consistencia1to5 . '" title="' . $consistencia1to5 . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($consistencia1to5==5){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasAlto.JPG" alt="' . $consistencia1to5 . '" title="' . $consistencia1to5 . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
		               	$sHtml.='
	...
	function informeCompetencias($aPuntuaciones , $sHtmlCab, $idIdioma){
		...
		$sHtml.='
				<div class="desarrollo">
					<h2 class="subtitulo">' . constant("STR_105_DETALLE DIMENSIONES_COMPETENCIAS") . '</h2>
        			<div class="caja" style="margin-bottom:10px;">
		            	<p class="textos">' . constant("STR_105_INFORME_ORIENTADO_A_COMPETENCIAS_INTRO_P1") . '</p>
			          </div><!--FIN DIV CAJA-->
			          <table class="sintesis" border="0" cellspacing="0" cellpadding="0">';
...
//				 			echo "<br />iPGlobal::($iEscala - 5.5) * ($iEscala - 5.5)" . $iPGlobal;
					...
					{	//Solo pintamos las competencias que son especificas de Banco Hipotecario
						...
						while(!$listaEscalas->EOF)
						{
							...
							$sHtml.='
							        <tr>
					                  <td class="tablaTitu" style="background: #595959;text-align: left;padding-left: 5px;">' . mb_strtoupper($listaEscalas->fields['nombre'], 'UTF-8') . '</td>
					                  <td class="descripcion" style="width: 250px;"><p>' . $listaEscalas->fields['descripcion'] . '</p></td>';

							if($iPEscala==1 || $iPEscala==2){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasBajo.JPG" alt="' . getEscala_1to5($iPEscala) . '" title="' . getEscala_1to5($iPEscala) . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($iPEscala==3 || $iPEscala==4){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMB.JPG" alt="' . getEscala_1to5($iPEscala) . '" title="' . getEscala_1to5($iPEscala) . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($iPEscala==5 || $iPEscala==6){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMedio.JPG" alt="' . getEscala_1to5($iPEscala) . '" title="' . getEscala_1to5($iPEscala) . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($iPEscala==7 || $iPEscala==8){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasMA.JPG" alt="' . getEscala_1to5($iPEscala) . '" title="' . getEscala_1to5($iPEscala) . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							if($iPEscala==9 || $iPEscala==10){
								$sHtml.='<td class="celS"><img src="'.constant("DIR_WS_GESTOR").'graf/prisma105/graficasAlto.JPG" alt="' . getEscala_1to5($iPEscala) . '" title="' . getEscala_1to5($iPEscala) . '" /></td>';
							}else{
								$sHtml.='<td class="celS">&nbsp;</td>';
							}
							$sHtml.='
