...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdBaremo()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdBaremo";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdBaremo"]) && $_POST[$sPrefijo . "IdBaremo"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idBaremo=" . $conn->qstr($_POST[$sPrefijo . "IdBaremo"], false);
		}
		...
		if ($bJoin){
			//$comboBAREMOS= new Combo($conn, $sPrefijo . "IdBaremo", "DISTINCT(s.idBaremo)", "s.nombre", "descripcion", "baremos s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idBaremo", "fecMod");
		}else{
			$comboBAREMOS= new Combo($conn, $sPrefijo . "IdBaremo","idBaremo","nombre","descripcion","baremos","",constant("SLC_OPCION"),$sWhere, "idBaremo", "fecMod");
		}
		...
		$comboBAREMOS->setNombre($sNombreCampo);
echo $comboBAREMOS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>