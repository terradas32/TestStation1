...
function setPK(idPerfil,idUsuario,idEmpresa)
{
	document.forms[0].fIdPerfil.value=idPerfil;
	document.forms[0].fIdUsuario.value=idUsuario;
	document.forms[0].fIdEmpresa.value=idEmpresa;
}
...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="0" border="0" width="100%">
		<tr>
			...
			?>
			<td class="tdnaranjab" align="center"><?php echo constant("STR_EMPRESA");?></td>
			<td class="tdnaranjab" align="center"><?php echo constant("STR_USUARIO");?></td>
		...
		<tr onmouseover="this.bgColor='<?php echo constant("ONMOUSEOVER");?>'" onmouseout="this.bgColor='<?php echo constant("ONMOUSEOUT");?>'" bgcolor="<?php echo constant("ONMOUSEOUT");?>">
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['idEmpresa']));?>"><?php echo substr($comboEMPRESAS->getDescripcionCombo($lista->fields['idEmpresa']),0,50);?></td>
      <?php $comboUSUARIOS	= new Combo($conn,"fIdUsuario","idUsuario","usuario","Descripcion","empresas_usuarios","",constant("SLC_OPCION"),"idEmpresa=" . $lista->fields['idEmpresa']);  ?>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['idUsuario']));?>"><?php echo substr($comboUSUARIOS->getDescripcionCombo($lista->fields['idUsuario']),0,50);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['idPerfil']));?>"><?php echo substr($comboPERFILES->getDescripcionCombo($lista->fields['idPerfil']),0,25);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['fecAlta']));?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecAlta'];?>'));</script></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['fecMod']));?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecMod'];?>'));</script></td>
			<td class="tddatoslista" align="center"><?php if (!empty($lista->fields['idUsuario'])){;?><a href="javascript:setPK('<?php echo $lista->fields['idPerfil'];?>','<?php echo $lista->fields['idUsuario'];?>','<?php echo $lista->fields['idEmpresa'];?>');confBorrar(<?php echo constant("MNT_BORRAR");?>,'<?php echo constant("DEL_GENERICO");?>');"><?php echo ($_bBorrar) ? "<img src='" . constant("DIR_WS_GRAF") . "menos.gif' width='9' height='9' border='0' alt=\"" . constant("STR_BORRAR") . "\">" : "";?></a><?php }else{echo("&nbsp;");};?></td>
		</tr>
		<?php $i++;
		$lista->MoveNext();
		} ?>
...
<div id="contenedor">
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width='10'><img src="<?php echo constant("DIR_WS_GRAF");?>sp.gif" width="10" height="1" border="0" alt="" /></td>
			<td width="100%" colspan="2" align='left' class="naranjab"><?php echo sprintf(constant("STR_LISTA_DE_"),str_replace('_', ' ', 'empresas_usuarios_perfiles'));?></td>
		</tr>
		<tr>
			<td width='10'><img src="<?php echo constant("DIR_WS_GRAF");?>sp.gif" width="10" height="1" border="0" alt="" /></td>
	...
	<input type="hidden" name="empresas_usuarios_perfiles_next_page" value="<?php echo (isset($_POST['empresas_usuarios_perfiles_next_page'])) ? $cUtilidades->validaXSS($_POST['empresas_usuarios_perfiles_next_page']) : "1";?>" />
</div>
