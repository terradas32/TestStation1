...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdZona()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdZona";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdMunicipio"])){
		...
		if ($bJoin){
			//$comboWI_ZONAS= new Combo($conn, $sPrefijo . "IdZona", "DISTINCT(s.idZona)", "s.descripcion", "descripcion", "wi_zonas s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idZona", "fecMod");
		}else{
			$comboWI_ZONAS= new Combo($conn, $sPrefijo . "IdZona","idZona","descripcion","descripcion","wi_zonas","",constant("SLC_OPCION"),$sWhere, "idZona", "fecMod");
		}
		...
		$comboWI_ZONAS->setNombre($sNombreCampo);
echo $comboWI_ZONAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>