...
function enviar()
{
	...
	if (validaForm()){
		lon();
		f.fIdEscala.value=f.elements['fIdEscala'].value;
		return true;
	}else	return false;
}
function validaForm()
{
	...
	msg +=vNumber("<?php echo constant("STR_ID_BLOQUE");?>:",f.fIdBloque.value,11,true);
	msg +=vNumber("<?php echo constant("STR_ID_ESCALA");?>:",f.elements['fIdEscala'].value,11,true);
	msg +=vNumber("<?php echo constant("STR_PUNT_MIN");?>:",f.fPuntMin.value,11,true);
	msg +=vNumber("<?php echo constant("STR_PUNT_MAX");?>:",f.fPuntMax.value,11,true);
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
		...
		<div style="width: 100%">
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					...
					<td><?php $comboPRUEBAS->setNombre("fIdPrueba");?><?php echo $comboPRUEBAS->getHTMLCombo("1","obliga",$cEntidad->getIdPrueba()," ","");?></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fIdBloque"><?php echo constant("STR_ID_BLOQUE");?></label>&nbsp;</td>
					<td><?php $comboBLOQUES->setNombre("fIdBloque");?><?php echo $comboBLOQUES->getHTMLCombo("1","obliga",$cEntidad->getIdBloque()," onchange=\"javascript:cambiaIdBloque()\"","");?></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fIdEscala"><?php echo constant("STR_ID_ESCALA");?></label>&nbsp;</td>
						<td><div id="comboIdEscala"><?php $comboESCALAS->setNombre("fIdEscala");?><?php echo $comboESCALAS->getHTMLComboNull("1","obliga",$cEntidad->getIdEscala(),"onchange=\"javascript:cambiaIdEscala()\" ","");?></div></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fPuntMin"><?php echo constant("STR_PUNT_MIN");?></label>&nbsp;</td>
					<td><input type="text" id="fPuntMin" name="fPuntMin" value="<?php echo $cEntidad->getPuntMin();?>" class="obliga"  onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fPuntMax"><?php echo constant("STR_PUNT_MAX");?></label>&nbsp;</td>
					<td><input type="text" id="fPuntMax" name="fPuntMax" value="<?php echo $cEntidad->getPuntMax();?>" class="obliga"  onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fTexto"><?php echo constant("STR_TEXTO");?></label>&nbsp;</td>
					<td><textarea id="fTexto" name="fTexto" rows="6" cols="1" class="obliga"  onchange="javascript:trim(this);"><?php echo $cEntidad->getTexto();?></textarea></td>
				</tr>
	...
	<input type="hidden" name="fIdTexto" value="<?php echo (empty($_POST['AddIdioma'])) ? $cEntidad->getIdTexto() : $_POST['fIdTexto'];?>" />
	...
	<input type="hidden" name="textos_escalas_next_page" value="<?php echo (isset($_POST['textos_escalas_next_page'])) ? $cUtilidades->validaXSS($_POST['textos_escalas_next_page']) : "1";?>" />
						<script language="javascript" type="text/javascript">
							...
							function cambiaIdBloque(){
								var f= document.forms[0];
								$("#comboIdEscala").show().load("jQuery.php",{sPG:"comboescalas",bBus:"0",multiple:"0",nLineas:"1",bObliga:"1",bgColor:"<?php echo constant("BG_COLOR")?>",fNombreCampo:"fIdEscala",fJSProp:"cambiaIdEscala",fCodIdiomaIso2:f.fCodIdiomaIso2.value,fIdBloque:f.fIdBloque.value,vSelected:"<?php echo $cEntidad->getIdEscala();?>",sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"}).fadeIn("slow");
							}
							//]]>
						</script>
						<script language="javascript" type="text/javascript">
