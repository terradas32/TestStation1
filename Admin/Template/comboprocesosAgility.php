...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		if (!empty($_POST[$sPrefijo . "IdEmpresa"])){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idEmpresa=" . $conn->qstr($_POST[$sPrefijo . "IdEmpresa"], false);
			//miramos todos aquellos procesos para la empresa que tengan CML y PRISM@
			$sWhere0 = "SELECT * FROM proceso_pruebas WHERE idEmpresa = " . $conn->qstr($_POST[$sPrefijo . "IdEmpresa"], false) . " AND idPrueba = 13";
			$rs = $conn->Execute($sWhere0);
			$sINProcesos= "";
			$sInCML= "";
			if ($rs->recordCount() > 0){
				while (!$rs->EOF)
				{
					$sInCML .="," . $rs->fields['idProceso'];
					$rs->MoveNext();
				} 
			}
			if (!empty($sInCML)){
				//Miro que tambien tengan PRISMA
				$sWhere0 = "SELECT * FROM proceso_pruebas WHERE idEmpresa = " . $conn->qstr($_POST[$sPrefijo . "IdEmpresa"], false) . " AND idPrueba = 24 AND idProceso IN (" . substr($sInCML, 1) . ") ";
				$rs = $conn->Execute($sWhere0);
				if ($rs->recordCount() > 0){
					while (!$rs->EOF)
					{
						$sINProcesos .="," . $rs->fields['idProceso'];
						$rs->MoveNext();
					} 
				}
			}
			if (!empty($sINProcesos)){
				$sWhere.= " AND idProceso IN (" . substr($sINProcesos, 1) . ") ";
			}else{
				//No hay ningun proceso que se pueda generar agility
				$sWhere.= " AND idProceso=-1 ";
			}
		}
		...
		if ($bJoin){
			...
			$comboPROCESOS= new Combo($conn, $sPrefijo . "IdProceso","idProceso","nombre","descripcion","procesos","",constant("SLC_OPCION"),$sWhere, "idProceso", "fecMod");
		}
