...
function listapuntuaciones(){
	...
	$("div#listapuntuaciones").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBaremo: f.fIdBaremo.value,fIdTipoCompetencia: f.fIdTipoCompetencia.value,fIdCompetencia: f.fIdCompetencia.value,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
}
function aniadepunt(){
	var f = document.forms[0];
	var paginacargada = "Baremos_competencias.php";
	$("div#listapuntuaciones").hide().load(paginacargada,{fAniade:"1",fIdBaremo: f.fIdBaremo.value,fIdPrueba: f.fIdPrueba.value,fIdTipoCompetencia: f.fIdTipoCompetencia.value,fIdCompetencia: f.fIdCompetencia.value,fPuntMinima: f.fPuntMinima.value,fPuntMaxima: f.fPuntMaxima.value,fPuntBaremada: f.fPuntBaremada.value,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	f.fPuntMinima.value="";
	f.fPuntMaxima.value="";
...
function borrarresult(idResultado,idBaremo,idPrueba,idTipoCompetencia,idCompetencia){
	var f = document.forms[0];
	if(confirm('<?php echo constant("DEL_GENERICO");?>')){
		var paginacargada = "Baremos_competencias.php";
		$("div#listapuntuaciones").hide().load(paginacargada,{fBorra:"1",fIdResultado: idResultado,fIdBaremo: idBaremo,fIdPrueba: idPrueba,fIdTipoCompetencia: idTipoCompetencia,fIdCompetencia: idCompetencia,MODO:"<?php echo constant('MNT_LISTAPUNTBAREMOS')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	}
}
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<input type="hidden" name="fIdTipoCompetencia" value="<?php echo (empty($_POST['AddIdioma'])) ? $cEntidad->getIdTipoCompetencia() : $_POST['fIdTipoCompetencia'];?>" />
