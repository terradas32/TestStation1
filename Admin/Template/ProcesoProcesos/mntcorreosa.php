...
function nuevaplantilla()
{
	...
	$("div#muestracorreo").hide().load(paginacargada,{fNuevo:"1",MODO:"<?php echo constant('MNT_NUEVOCORREO')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
}
function cancela()
{
	var f = document.forms[0];
...
function cargaplantilla()
{
	...
	if(f.fIdCorreo.value!=""){
		$("div#muestracorreo").hide().load(paginacargada,{fConsulta:"1",fIdTipoCorreo:f.fIdTipoCorreo.value,fIdCorreo:f.fIdCorreo.value,MODO:"<?php echo constant('MNT_NUEVOCORREO')?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	}else{
		$("div#muestracorreo").empty();
	}
}
				...
				<tr>
					<td colspan="2" width="100%" style="border-bottom: 1px solid #000000; height:35px">
						<ul class="listaProceso">
							...
							<li class="mArrow">&nbsp;</li>
							<li class="mActivo"><span><img src="./graf/iconsMenu/mail.png" title="Comunicación" alt="Comunicación" /></span><b style="color:#FFB200">Comunicación</b></li>
						</ul>
					</td>
				</tr>
				<tr>
					<td width="20%" valign="top" style="border-right: 2px solid #000000;">
						<ul>
							...
							<li>
								<a href="#" onclick="javascript:enviarNuevo(<?php echo constant('MNT_ANIADECANDIDATOS')?>);" class="negrob">Volver a candidatos</a>
							</li>
						</ul>
					</td>
					<td width="80%">
					...
					<table cellspacing="0" cellpadding="0" border="0">
						<tr>
							...
							<td> <input type="button" class="botonesgrandes" <?php echo ($_bModificar) ? '' : 'disabled="disabled"';?> name="fBtnEnviarCorreos" value="<?php echo constant("STR_ENVIAR_CORREOS");?>" onclick="document.forms[0].action='EnviarCorreos.php';document.forms[0].MODO.value=<?php echo constant('MNT_NUEVO')?>;block('4');document.forms[0]._clicado.value=29;document.forms[0].submit();"/></td>
