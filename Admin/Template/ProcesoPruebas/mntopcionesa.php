...
function validaForm()
{
	...
	msg +=vNumber("<?php echo constant("STR_ITEM");?>:",f.fIdItem.value,11,true);
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
		...
		<div style="width: 100%">
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					...
					<td><input type="text" name="fDescripcion" value="<?php echo $cOpciones->getDescripcion();?>" class="cajatexto" onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
				...
				<?php if ($cOpciones->getPathOpcion() != "")
				{
					$img=@getimagesize(constant("HTTP_SERVER") . $cOpciones->getPathOpcion());
					$bIimg = (empty($img)) ? 0 : 1;
				?>
					<tr>
						<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
						<td width="140" class="negrob" valign="top">&nbsp;</td>
						<td>&nbsp;<a class="azul" href="#" onclick="abrirVentana('<?php echo $bIimg;?>','<?php echo base64_encode(constant("HTTP_SERVER") . $cOpciones->getPathOpcion());?>');"><?php echo str_replace("_","&nbsp;", basename($cOpciones->getPathOpcion()));?></a>&nbsp;<input onmouseover="this.style.cursor='pointer'" type="checkbox" name="cfPathOpcion" />&nbsp;<?php echo constant("STR_QUITAR");?></td>
					</tr>
				<?php } ?>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><?php echo constant("STR_CODIGO");?>&nbsp;</td>
					<td><input type="text" name="fCodigo" value="<?php echo $cOpciones->getCodigo();?>" class="obliga"  onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
				    ...
				    <td>
					   <select name="fBajaLog" size="1">
						  <option value="0" <?php echo ($cOpciones->getBajaLog() == "0") ? "selected" : "";?>><?php echo constant("STR_ACTIVO");?></option>
						  <option value="1" <?php echo ($cOpciones->getBajaLog() == "1") ? "selected" : "";?>><?php echo constant("STR_NO_ACTIVO");?></option>
					   </select>
				    </td>
	...
	<table cellspacing="0" cellpadding="0" border="0">
		<tr>
			<td><input type="button" class="botones" id="bid-volver" name="btnAdd" value="<?php echo constant("STR_VOLVER");?>" onclick="document.forms[0].MODO.value=document.forms[0].ORIGEN.value;document.forms[0].fOpcion.value=1;document.forms[0].fCodIdiomaIso2.disabled=false;document.forms[0].fIdPrueba.disabled=false;document.forms[0].submit();" /></td>
	...
	<input type="hidden" name="fIdOpcion" value="<?php echo $cOpciones->getIdOpcion();?>" />
	...
	<input type="hidden" name="fFecAlta" value="<?php echo $cOpciones->getFecAlta();?>" />
	<input type="hidden" name="fFecMod" value="<?php echo $cOpciones->getFecMod();?>" />
	<input type="hidden" name="fUsuAlta" value="<?php echo $cOpciones->getUsuAlta();?>" />
	<input type="hidden" name="fUsuMod" value="<?php echo $cOpciones->getUsuMod();?>" />
