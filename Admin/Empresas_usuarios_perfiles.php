...
include_once ('include/conexion.php');
    ...
    $cEntidadDB	= new Empresas_usuarios_perfilesDB($conn);  // Entidad DB
	$cEntidad	= new Empresas_usuarios_perfiles();  // Entidad
    ...
    $comboUSUARIOS	= new Combo($conn,"fIdUsuario","idUsuario","usuario","Descripcion","empresas_usuarios","",constant("SLC_OPCION"),"");
	$comboPERFILES	= new Combo($conn,"fIdPerfil","idPerfil","descripcion","Descripcion","emp_perfiles","",constant("SLC_OPCION"),"");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			$newId	= $cEntidadDB->insertar(readEntidad($cEntidad));
			if (!empty($newId)){
			?><script>alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					if (!empty($_POST['empresas_usuarios_perfiles_next_page']) && $_POST['empresas_usuarios_perfiles_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'empresas_usuarios_perfiles');
					if ($bInit)	$pager->curr_page=1;
					$pager->showPageLinks = true;
					$lista=$pager->getRS();
					include('Template/Empresas_usuarios_perfiles/mntempresas_usuarios_perfilesl.php');
				}else{
					$cEntidad	= new Empresas_usuarios_perfiles();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Empresas_usuarios_perfiles/mntempresas_usuarios_perfilesa.php');
				}
			}else{
				?><script>alert("<?php echo constant("ERR_FORM_ERROR");?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php
				$_POST['MODO']=constant("MNT_ALTA");
				include('Template/Empresas_usuarios_perfiles/mntempresas_usuarios_perfilesa.php');
			}
			break;
		...
		case constant("MNT_CONSULTAR"):
			$cEntidad = readEntidad($cEntidad);		$cEntidad = $cEntidadDB->readEntidad($cEntidad);
			$_POST['MODO']    = constant("MNT_MODIFICAR");
			include('Template/Empresas_usuarios_perfiles/mntempresas_usuarios_perfilesa.php');
			break;
		case constant("MNT_BUSCAR"):
			...
			include('Template/Empresas_usuarios_perfiles/mntempresas_usuarios_perfiles.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdEmpresa((isset($_POST["LSTIdEmpresa"])) ? $_POST["LSTIdEmpresa"] : "");	$cEntidad->setBusqueda(constant("STR_EMPRESA"), (isset($_POST["LSTIdEmpresa"])) ? $comboEMPRESAS->getDescripcionCombo($_POST["LSTIdEmpresa"]) : "");
		$cEntidad->setIdUsuario((isset($_POST["LSTIdUsuario"])) ? $_POST["LSTIdUsuario"] : "");	$cEntidad->setBusqueda(constant("STR_USUARIO"), (isset($_POST["LSTIdUsuario"])) ? $comboUSUARIOS->getDescripcionCombo($_POST["LSTIdUsuario"]) : "");
		global $comboPERFILES;
		$cEntidad->setIdPerfil((isset($_POST["LSTIdPerfil"])) ? $_POST["LSTIdPerfil"] : "");	$cEntidad->setBusqueda(constant("STR_PERFIL"), (isset($_POST["LSTIdPerfil"])) ? $comboPERFILES->getDescripcionCombo($_POST["LSTIdPerfil"]) : "");
