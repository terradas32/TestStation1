...
class EmpresasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(idUsuario) AS Max FROM empresas_usuarios ";
		$sql .= "WHERE ";
		$sql .= "UPPER(usuario)=" . $aux->qstr(strtoupper($cEntidad->getUsuario()), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			...
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Usuario Existe [" . constant("MNT_ALTA") . "][EmpresasDB]see[Empresas_usuarios]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
		...
		$sql .= "power_bi_token" . ",";
		$sql .= "power_bi_active" . ",";
		$sql .= "power_bi_token_fit" . ",";
		$sql .= "power_bi_active_fit" . ",";
		//$sql .= "texto_fit_comp" . ",";


		$sql .= "idsPruebas" . ",";
		$sql .= "idsPruebasAleatorias" . ",";
		...
		$sql .= "fecNacimientoMB" . ",";
		$sql .= "especialidadMB" . ",";
		$sql .= "nivelConocimientoMB" . ",";
		$sql .= "srvTPV" . ",";
		$sql .= "idTipoTpv" . ",";

		$sql .= "puestoEvaluar" . ",";
		$sql .= "responsableDirecto" . ",";
		$sql .= "categoriaForjanor" . ",";
		$sql .= "verCorreo" . ",";
		$sql .= "altaCiega" . ",";
		$sql .= "altaPrecargada" . ",";
		...
		$sql .= "candidatosRepetidores" . ",";


		$sql .= "descuentaMatriz" . ",";
		$sql .= "solicitarUnidadesA" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getAvisoLegal(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPrepago(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNcandidatos(), false) . ",";
		$sql .= $aux->qstr(($cEntidad->getDongles() !="") ? $cEntidad->getDongles() : "0", false) . ",";
		...
		$sql .= $aux->qstr($cEntidad->getTimezone(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDireccion(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getTlfContacto(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPersonaContacto(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getUmbral_aviso(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getpower_bi_token(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getpower_bi_active(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getpower_bi_token_fit(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getpower_bi_active_fit(), false) . ",";
		//$sql .= $aux->qstr($cEntidad->gettexto_fit_comp(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIdsPruebas(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIdsPruebasAleatorias(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getEdad(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getSexo(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNivel(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getFormacion(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getArea(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getTelefono(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNombreCan(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getApellido1(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getApellido2(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getMailCan(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNifCan(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getSectorMB(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getCodIso2PaisProcedencia(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getConcesionMB(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getBaseMB(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getFecNacimientoMB(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getEspecialidadMB(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNivelConocimientoMB(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getSrvTPV(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIdTipoTpv(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getPuestoEvaluar(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getResponsableDirecto(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getCategoriaForjanor(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getVerCorreo(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getAltaCiega(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getAltaPrecargada(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getProcesoConfidencial(), false) . ",";

		$sql .= $aux->qstr($cEntidad->getCandidatosRepetidores(), false) . ",";


		$sql .= $aux->qstr($cEntidad->getDescuentaMatriz(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getSolicitarUnidadesA(), false) . ",";
...
**************************************************************************************************/
	function modificar($cEntidad)
	{
		...
		//Miramos si es PE en ese caso no reposicionamos ni colocamos para que no haya
		//Ningún cambio de orden
		if ($cEntidad->getIdEmpresa() == constant("EMPRESA_PE")){
            $cEntidad->setDentroDe("");
            $cEntidad->setDespuesDe("1");
        }
			...
			($cEntidad->getDespuesDe() == $cEntidadBack->getOrden()) )
		{
			...
			$sql .= "avisoLegal=" . $aux->qstr($cEntidad->getAvisoLegal(), false) . ", ";
			...
			$sql .= "power_bi_active=" . $aux->qstr($cEntidad->getpower_bi_active(), false) . ", ";
			$sql .= "power_bi_token_fit=" . $aux->qstr($cEntidad->getpower_bi_token_fit(), false) . ", ";
			$sql .= "power_bi_active_fit=" . $aux->qstr($cEntidad->getpower_bi_active_fit(), false) . ", ";
			//$sql .= "texto_fit_comp=" . $aux->qstr($cEntidad->gettexto_fit_comp(), false) . ", ";
			$sql .= "idsPruebas=" . $aux->qstr($cEntidad->getIdsPruebas(), false) . ", ";
			$sql .= "idsPruebasAleatorias=" . $aux->qstr($cEntidad->getIdsPruebasAleatorias(), false) . ", ";

			$sql .= "edad=" . $aux->qstr($cEntidad->getEdad(), false) . ", ";
			$sql .= "sexo=" . $aux->qstr($cEntidad->getSexo(), false) . ", ";
			$sql .= "nivel=" . $aux->qstr($cEntidad->getNivel(), false) . ", ";
			$sql .= "formacion=" . $aux->qstr($cEntidad->getFormacion(), false) . ", ";
			$sql .= "area=" . $aux->qstr($cEntidad->getArea(), false) . ", ";
			$sql .= "telefono=" . $aux->qstr($cEntidad->getTelefono(), false) . ", ";
			$sql .= "nombreCan=" . $aux->qstr($cEntidad->getNombreCan(), false) . ", ";
			$sql .= "apellido1=" . $aux->qstr($cEntidad->getApellido1(), false) . ", ";
			$sql .= "apellido2=" . $aux->qstr($cEntidad->getApellido2(), false) . ", ";
			$sql .= "mailCan=" . $aux->qstr($cEntidad->getMailCan(), false) . ", ";
			$sql .= "nifCan=" . $aux->qstr($cEntidad->getNifCan(), false) . ", ";
			...
			$sql .= "fecNacimientoMB=" . $aux->qstr($cEntidad->getFecNacimientoMB(), false) . ", ";
			...
			$sql .= "srvTPV=" . $aux->qstr($cEntidad->getSrvTPV(), false) . ", ";
			...
			$sql .= "verCorreo=" . $aux->qstr($cEntidad->getVerCorreo(), false) . ", ";
			$sql .= "altaCiega=" . $aux->qstr($cEntidad->getAltaCiega(), false) . ", ";
			$sql .= "altaPrecargada=" . $aux->qstr($cEntidad->getAltaPrecargada(), false) . ", ";

			$sql .= "procesoConfidencial=" . $aux->qstr($cEntidad->getProcesoConfidencial(), false) . ", ";

			$sql .= "candidatosRepetidores=" . $aux->qstr($cEntidad->getCandidatosRepetidores(), false) . ", ";

			$sql .= "descuentaMatriz=" . $aux->qstr($cEntidad->getDescuentaMatriz(), false) . ", ";
			$sql .= "solicitarUnidadesA=" . $aux->qstr($cEntidad->getSolicitarUnidadesA(), false) . ", ";
...
//Modificado Pedro 06/05/2014 para evirar idPadre 0 que sólo puede ser PE
//							$sql .= "idPadre=" . 0 . ", ";
//							$sql .= "indentacion=" . 0 . ", ";
//FIN Modificado Pedro 06/05/2014 para evirar idPadre 0 que sólo puede ser PE
						}else{
...
//				echo $sql;
		...
		if($retorno){
			...
			if($cEntidad->getDistribuidor()=="1") {
				if ($cEntidad->getIdEmpresa() == "5140"){
					$cEmpPerfil->setIdPerfil("6");
				}else{
					$cEmpPerfil->setIdPerfil("0");
				}
			}else{
				require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Empresas/Empresas.php");
				$cEntidadEmp	= new Empresas();
				$cEntidadEmp->setIdEmpresa($cEntidad->getIdEmpresa());
				$cEntidadEmp = $this->readEntidad($cEntidadEmp);
				if ($cEntidadEmp->getIdPadre() == "5140"){	//FormacionMB
					$cEmpPerfil->setIdPerfil("8"); //Perfil Para concesionarios MB
				}else{
					$cEmpPerfil->setIdPerfil("1");
				}
			}
			$cEmpPerfil->setUsuAlta($cEntidad->getUsuMod());
			$cEmpPerfil->setUsuMod($cEntidad->getUsuMod());
			$cEmpPerfilDB->insertar($cEmpPerfil);
		}
		return $retorno;
	} // //fin modificar
/*************************************************************************************************
	...
	$iHijosPosFinal --> Entero con el numero de hijos da la posiscion final a la que vamos a mover la empresa.
**************************************************************************************************/
	function modificarSinPass($cEntidad)
	{
		$aux			= $this->conn;
		$i=0;
		require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Upload.php");
...
//				echo $sql;
		...
		if($retorno){
			...
			$cEmpPerfil->setIdEmpresa($cEntidad->getIdEmpresa());
			//Hay más perfiles con lo que no se borra ahora 22/01/2016
			//$cEmpPerfilDB->borrar($cEmpPerfil);
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdCliente($arr['idCliente']);
					...
					$cEntidad->setTimezone($arr['Timezone']);

					$cEntidad->setDireccion($arr['direccion']);

					$cEntidad->setTlfContacto($arr['tlfContacto']);
					...
					//$cEntidad->settexto_fit_comp($arr['texto_fit_comp']);
					$cEntidad->setIdsPruebas($arr['idsPruebas']);
					$cEntidad->setIdsPruebasAleatorias($arr['idsPruebasAleatorias']);

					$cEntidad->setEdad($arr['edad']);
					$cEntidad->setSexo($arr['sexo']);
					$cEntidad->setNivel($arr['nivel']);
					$cEntidad->setFormacion($arr['formacion']);
					$cEntidad->setArea($arr['area']);
					$cEntidad->setTelefono($arr['telefono']);
					$cEntidad->setNombreCan($arr['nombreCan']);
					$cEntidad->setApellido1($arr['apellido1']);
					$cEntidad->setApellido2($arr['apellido2']);
					$cEntidad->setMailCan($arr['mailCan']);
					$cEntidad->setNifCan($arr['nifCan']);
					...
					$cEntidad->setVerCorreo($arr['verCorreo']);
					$cEntidad->setAltaCiega($arr['altaCiega']);
					$cEntidad->setAltaPrecargada($arr['altaPrecargada']);

					$cEntidad->setProcesoConfidencial($arr['procesoConfidencial']);

					$cEntidad->setCandidatosRepetidores($arr['candidatosRepetidores']);
					$cEntidad->setDescuentaMatriz($arr['descuentaMatriz']);
					$cEntidad->setSolicitarUnidadesA($arr['solicitarUnidadesA']);
...
//			$cEntidadPadre = $cEntidad;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getTimezone() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(Timezone) LIKE UPPER('%" . $cEntidad->getTimezone() . "%')";
		}
		...
		if ($cEntidad->getTlfContacto() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(tlfContacto) LIKE UPPER('%" . $cEntidad->getTlfContacto() . "%')";
		}
		if ($cEntidad->getPersonaContacto() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(personaContacto) LIKE UPPER('%" . $cEntidad->getPersonaContacto() . "%')";
		}
		...
		if ($cEntidad->getIdsPruebasAleatorias() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(idsPruebasAleatorias) LIKE UPPER('%" . $cEntidad->getIdsPruebasAleatorias() . "%')";
		}

		if ($cEntidad->getEdad() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(edad) LIKE UPPER('%" . $cEntidad->getEdad() . "%')";
		}
		if ($cEntidad->getSexo() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(sexo) LIKE UPPER('%" . $cEntidad->getSexo() . "%')";
		}
		if ($cEntidad->getNivel() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(nivel) LIKE UPPER('%" . $cEntidad->getNivel() . "%')";
		}
		if ($cEntidad->getFormacion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(formacion) LIKE UPPER('%" . $cEntidad->getFormacion() . "%')";
		}
		if ($cEntidad->getArea() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(area) LIKE UPPER('%" . $cEntidad->getArea() . "%')";
		}
		if ($cEntidad->getTelefono() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(telefono) LIKE UPPER('%" . $cEntidad->getTelefono() . "%')";
		}

		if ($cEntidad->getNombreCan() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(nombreCan) LIKE UPPER('%" . $cEntidad->getNombreCan() . "%')";
		}
		if ($cEntidad->getMailCan() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(mailCan) LIKE UPPER('%" . $cEntidad->getMailCan() . "%')";
		}
		if ($cEntidad->getNifCan() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(nifCan) LIKE UPPER('%" . $cEntidad->getNifCan() . "%')";
		}

		if ($cEntidad->getSectorMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(sectorMB) LIKE UPPER('%" . $cEntidad->getSectorMB() . "%')";
		}

		if ($cEntidad->getCodIso2PaisProcedencia() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(codIso2PaisProcedencia) LIKE UPPER('%" . $cEntidad->getCodIso2PaisProcedencia() . "%')";
		}
		if ($cEntidad->getConcesionMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(concesionMB) LIKE UPPER('%" . $cEntidad->getConcesionMB() . "%')";
		}
		if ($cEntidad->getBaseMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(baseMB) LIKE UPPER('%" . $cEntidad->getBaseMB() . "%')";
		}

		if ($cEntidad->getFecNacimientoMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(fecNacimientoMB) LIKE UPPER('%" . $cEntidad->getFecNacimientoMB() . "%')";
		}
		if ($cEntidad->getEspecialidadMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(especialidadMB) LIKE UPPER('%" . $cEntidad->getEspecialidadMB() . "%')";
		}
		if ($cEntidad->getNivelConocimientoMB() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(nivelConocimientoMB) LIKE UPPER('%" . $cEntidad->getNivelConocimientoMB() . "%')";
		}

		if ($cEntidad->getSrvTPV() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(srvTPV) LIKE UPPER('%" . $cEntidad->getSrvTPV() . "%')";
		}
		if ($cEntidad->getIdTipoTpv() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(idTipoTpv) LIKE UPPER('%" . $cEntidad->getIdTipoTpv() . "%')";
		}
		if ($cEntidad->getPuestoEvaluar() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(puestoEvaluar) LIKE UPPER('%" . $cEntidad->getPuestoEvaluar() . "%')";
		}

		if ($cEntidad->getVerCorreo() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(verCorreo) LIKE UPPER('%" . $cEntidad->getVerCorreo() . "%')";
		}
		if ($cEntidad->getAltaCiega() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(altaCiega) LIKE UPPER('%" . $cEntidad->getAltaCiega() . "%')";
		}
		if ($cEntidad->getAltaPrecargada() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(altaPrecargada) LIKE UPPER('%" . $cEntidad->getAltaPrecargada() . "%')";
		}

		if ($cEntidad->getProcesoConfidencial() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(procesoConfidencial) LIKE UPPER('%" . $cEntidad->getProcesoConfidencial() . "%')";
		}
		if ($cEntidad->getCandidatosRepetidores() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(candidatosRepetidores) LIKE UPPER('%" . $cEntidad->getCandidatosRepetidores() . "%')";
		}
		if ($cEntidad->getDescuentaMatriz() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(descuentaMatriz) LIKE UPPER('%" . $cEntidad->getDescuentaMatriz() . "%')";
		}
		if ($cEntidad->getSolicitarUnidadesA() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(solicitarUnidadesA) LIKE UPPER('%" . $cEntidad->getSolicitarUnidadesA() . "%')";
		}

		if ($cEntidad->getResponsableDirecto() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(responsableDirecto) LIKE UPPER('%" . $cEntidad->getResponsableDirecto() . "%')";
		}
		if ($cEntidad->getCategoriaForjanor() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(categoriaForjanor) LIKE UPPER('%" . $cEntidad->getCategoriaForjanor() . "%')";
		}
