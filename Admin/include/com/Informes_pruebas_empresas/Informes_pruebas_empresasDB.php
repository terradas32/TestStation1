...
* la tabla Informes_pruebas_empresas
**/
class Informes_pruebas_empresasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(idEmpresa) AS Max FROM informes_pruebas_empresas ";
		$sql .= "WHERE ";
		$sql .=" AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idTipoInforme=" . $aux->qstr($cEntidad->getIdTipoInforme(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Informes_pruebas_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	
		$sql = "INSERT INTO informes_pruebas_empresas (";
        $sql .= "idEmpresa" . ",";
		$sql .= "idPrueba" . ",";
		$sql .= "codIdiomaIso2" . ",";
		$sql .= "idTipoInforme" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getPrecio(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getNVecesDescargado(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Informes_pruebas_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM informes_pruebas_empresas ";
		$sql  .=" WHERE idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idTipoInforme=" . $aux->qstr($cEntidad->getIdTipoInforme(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Informes_pruebas_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE informes_pruebas_empresas SET ";
		...
		$sql .= "precio=" . $aux->qstr($cEntidad->getPrecio(), false) . ", ";
		$sql .= "nVecesDescargado=" . $aux->qstr($cEntidad->getNVecesDescargado(), false) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false) ;
		$sql .= " WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idTipoInforme=" . $aux->qstr($cEntidad->getIdTipoInforme(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Informes_pruebas_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM informes_pruebas_empresas ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Informes_pruebas_empresasDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM informes_pruebas_empresas WHERE ";
		$sql  .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idTipoInforme=" . $aux->qstr($cEntidad->getIdTipoInforme(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setPrecio($arr['precio']);
					$cEntidad->setNVecesDescargado($arr['nVecesDescargado']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Informes_pruebas_empresasDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM informes_pruebas_empresas ";
		if ($cEntidad->getIdEmpresa() != ""){
		...
		if ($cEntidad->getPrecio() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="precio=" . $aux->qstr($cEntidad->getPrecio(), false);
		}		
		if ($cEntidad->getNVecesDescargado() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="nVecesDescargado>=" . $aux->qstr($cEntidad->getNVecesDescargado(), false);
		}
		if ($cEntidad->getNVecesDescargadoHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="nVecesDescargado<=" . $aux->qstr($cEntidad->getNVecesDescargadoHast(), false);
		}
	...
	*  en la base de datos
	************************************************************************/
	function addContInforme($cEntidad)
	{
		$aux			= $this->conn;
	
		$sql = "UPDATE informes_pruebas_empresas SET ";
		$sql .= "nVecesDescargado=nVecesDescargado + 1 ";
		$sql .= " WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idTipoInforme=" . $aux->qstr($cEntidad->getIdTipoInforme(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
...
}//Fin de la Clase Informes_pruebas_empresasDB
?>