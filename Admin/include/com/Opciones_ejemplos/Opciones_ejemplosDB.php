...
* la tabla opciones_ejemplos
**/
class Opciones_ejemplosDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		if($cEntidad->getIdOpcion() != ""){
        	$newId = $cEntidad->getIdOpcion();
        }else{
        	$newId = $this->getSiguienteId($cEntidad);
        }
	
		require_once(constant("DIR_WS_COM") . "Upload.php");
		$img0 = new Upload("fPathOpcion");
		...
		$img0->image_x = 140;	// Ancho por defecto
		$img0->jpeg_quality = 75;
		$sDirImg="imgOpciones_ejemplos/" . $cEntidad->getIdPrueba() . "/" . $cEntidad->getCodIdiomaIso2();
		...
		if (!$img0->UploadFile($spath . $sDirImg))
		{
			...
			$cEntidad->setPathOpcion($img0->getPath_WS());
		}		
		        
		$sql = "INSERT INTO opciones_ejemplos (";
		$sql .= "codIdiomaIso2" . ",";
		$sql .= "idPrueba" . ",";
		$sql .= "idEjemplo" . ",";
		$sql .= "idOpcion" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getIdEjemplo(), false) . ",";
		$sql .= $aux->qstr($newId, false) . ",";
		$sql .= $aux->qstr($cEntidad->getDescripcion(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPathOpcion(), false) . ",";
...
//		echo $sql;
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Opciones_ejemplosDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idOpcion) AS Max FROM opciones_ejemplos ";
		$sql  .=" WHERE codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idEjemplo=" . $aux->qstr($cEntidad->getIdEjemplo(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Opciones_ejemplosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE opciones_ejemplos SET ";
		...
		if ($cEntidad->getPathOpcion() != "")
			$sql .= "pathOpcion=" . $aux->qstr($cEntidad->getPathOpcion(), false) . ", ";
		...
		$sql .= " WHERE ";
		$sql .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idEjemplo=" . $aux->qstr($cEntidad->getIdEjemplo(), false) . " AND idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Opciones_ejemplosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		$sql .= "bajaLog='1'";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Opciones_ejemplosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM opciones_ejemplos WHERE ";
		$sql  .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idEjemplo=" . $aux->qstr($cEntidad->getIdEjemplo(), false) . " AND idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " ";
		echo $sql; 
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setPathOpcion($arr['pathOpcion']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Opciones_ejemplosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM opciones_ejemplos ";
		if ($cEntidad->getCodIdiomaIso2() != ""){
		...
		if ($cEntidad->getPathOpcion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(pathOpcion) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getPathOpcion() . "%") . ")";
		}
	...
	function quitaImagen($cEntidad)
	{
		...
		$sql = "UPDATE opciones_ejemplos SET  ";
		if (strtolower($cEntidad->getPathOpcion()) == "on"){
			$sql .= "pathOpcion='', ";
		}
		...
		if($aux->Execute($sql) === false){
			...
			if (strtolower($cEntidad->getPathOpcion()) == "on"){
				@unlink($spath . $cEntidadAnterior->getPathOpcion());
				//Borramos la miñatura
				$name= basename($cEntidadAnterior->getPathOpcion());
				$namePath=str_replace($name,"m" . $name, $cEntidadAnterior->getPathOpcion());
				@unlink($spath . $namePath);
			}
		}
		return $retorno;
	}
}//Fin de la Clase Opciones_ejemplosDB
?>