...
* la tabla sec_ponderaciones_fa
**/
class Sec_ponderacionesfaDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad) 
	{
		...
		$newId = $cEntidad->getIDPERFILPUESTO();
		$iCont = 0;
		$sql = "SELECT COUNT(IDPERFILPUESTO) AS Max FROM sec_ponderaciones_fa ";
		$sql .= "WHERE ";
		$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDDIMENSION=" . $aux->qstr($cEntidad->getIDDIMENSION(), false) . " ";
			$rs = $aux->Execute($sql);
			if ($rs){
			...
			if ($iCont > 0 ){
				//Existe el registro
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Sec_ponderacionesBD]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				return 0;
			}
		...
		$sql = "INSERT INTO sec_ponderaciones_fa (";
		$sql .= "IDPERFILPUESTO" . ",";
		$sql .= "IDCOMPETENCIA" . ",";
		$sql .= "IDTIPOPRUEBA" . ",";
		$sql .= "IDDIMENSION" . ",";
		...
		$sql .= "usuMod" . ")";
		$sql .= " VALUES (";
		$sql .= $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . ",";
		if($cEntidad->getIDDIMENSION() == ""){
			$sql .= "0" . ",";
		}else{
			$sql .= $aux->qstr($cEntidad->getIDDIMENSION(), false) . ",";
		}
		$sql .= $aux->qstr($cEntidad->getDESCPERFILPUESTO(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getCODCOMPETENCIA(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDESCCOMPETENCIA(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getCODDIMENSION(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDESCDIMENSION(), false) . ",";
		$sql .= $aux->qstr(str_replace("'", "", $cEntidad->getPONDERACION())) . ",";
	...
	*****************************************************************************************/
	...
	{
		$aux			= $this->conn;
		$sql = "SELECT MAX() AS Max FROM sec_ponderaciones_fa ";
		$sql .=" WHERE IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " ";
		$newId=0;
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_ponderaciones_fa SET ";
		$sql .= "IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . ", ";
		...
		$sql .= "DESCPERFILPUESTO=" . $aux->qstr($cEntidad->getDESCPERFILPUESTO(), false) . ", ";
		$sql .= "CODCOMPETENCIA=" . $aux->qstr($cEntidad->getCODCOMPETENCIA(), false) . ", ";
		$sql .= "DESCCOMPETENCIA=" . $aux->qstr($cEntidad->getDESCCOMPETENCIA(), false) . ", ";
		$sql .= "CODDIMENSION=" . $aux->qstr($cEntidad->getCODDIMENSION(), false) . ", ";
		$sql .= "DESCDIMENSION=" . $aux->qstr($cEntidad->getDESCDIMENSION(), false) . ", ";
		$sql .= "PONDERACION=" . $aux->qstr($cEntidad->getPONDERACION(), false) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false) ;
		$sql .= " WHERE ";
		$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
	...
	************************************************************************/
	function borrar($cEntidad) 
	{
		...
		$retorno			= true;
			...
			$sql ="DELETE FROM sec_ponderaciones_fa ";
			$sql .=" WHERE ";
	...
	*************************************************************************/
	function consultar($cEntidad)
	{
		...
		$sql = "SELECT IDPERFILPUESTO,IDCOMPETENCIA,IDTIPOPRUEBA,IDDIMENSION,DESCPERFILPUESTO,CODCOMPETENCIA,DESCCOMPETENCIA,CODDIMENSION,DESCDIMENSION,PONDERACION," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_ponderaciones_fa WHERE ";
		$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " ";
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setPONDERACION($arr['PONDERACION']);
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function consultarConDimension($cEntidad) 
	{
		...
		if($cEntidad->getIDDIMENSION() == ""){
			...
			$idDimension =$cEntidad->getIDDIMENSION();
		}
		
		$sql = "SELECT IDPERFILPUESTO,IDCOMPETENCIA,IDTIPOPRUEBA,IDDIMENSION,DESCPERFILPUESTO,CODCOMPETENCIA,DESCCOMPETENCIA,CODDIMENSION,DESCDIMENSION,PONDERACION," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_ponderaciones_fa WHERE ";
		$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDDIMENSION=" . $aux->qstr(idDimension, false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_ponderaciones_fa ";
		...
		if ($cEntidad->getDESCPERFILPUESTO() != null && $cEntidad->getDESCPERFILPUESTO() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(DESCPERFILPUESTO) LIKE UPPER('%" . $cEntidad->getDESCPERFILPUESTO() . "%')";
		}
		...
		if ($cEntidad->getPONDERACION() != null && $cEntidad->getPONDERACION() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="PONDERACION>=" . $aux->qstr($cEntidad->getPONDERACION());
		}
		if ($cEntidad->getPONDERACIONHast() != null && $cEntidad->getPONDERACIONHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="PONDERACION<=" . $aux->qstr($cEntidad->getPONDERACIONHast());
		}
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function readListaHast($cEntidad)
	{
		...
		if ($cEntidad->getIDCOMPETENCIAHast() != null && $cEntidad->getIDCOMPETENCIAHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="IDCOMPETENCIA<=" . $aux->qstr($cEntidad->getIDCOMPETENCIAHast(), false);
		}
		if ($cEntidad->getIDTIPOPRUEBA() != null && $cEntidad->getIDTIPOPRUEBA() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
...
}//Fin de la Clase Sec_ponderacionesfaDB
?>