...
* la tabla sec_respuestas_fm
**/
class Sec_respuestasfmDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql = "";
			...
			//$newId = String.valueOf((Integer.parseInt(getSiguienteId(conConexion,"SELECT " . SQL.cGetId("IDRESPUESTA") . " FROM sec_respuestas_fm"))));
		$newId = $this->getSiguienteId($cEntidad);		
	
		$sql = "INSERT INTO sec_respuestas_fm (";
		$sql .= "IDRESPUESTA" . ",";
		$sql .= "IDCANDIDATO" . ",";
		$sql .= "IDPROCESO" . ",";
		$sql .= "DNI" . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(IDRESPUESTA) AS Max FROM sec_respuestas_fm ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_respuestas_fm SET ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_respuestas_fm ";
			$sql  .=" WHERE ";
	...
	************************************************************************/
	function borrarFm($cEntidad)
	{
		...
		$retorno			= true;
			...
			if ($cEntidad->getDESCCOMPETENCIA() != ""){
				$sql .= this.getSQLAnd($and);
				$and = true;
				$sql  .="DESCCOMPETENCIA=" . $aux->qstr($cEntidad->getDESCCOMPETENCIA()) . " ";
			}
	...
	*************************************************************************/
	function consultar($cEntidad)
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,IDCOMPETENCIA,CODCOMPETENCIA,DESCCOMPETENCIA,PUNTUACIONDIRECTA,PUNTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_fm WHERE ";
		$sql  .="IDRESPUESTA=" . $aux->qstr($cEntidad->getIDRESPUESTA()) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function consultarPorCod($cEntidad)
	{
		...
		$sql = "SELECT IDCANDIDATO,IDPROCESO,DNI,IDCOMPETENCIA,CODCOMPETENCIA,DESCCOMPETENCIA,PUNTSOBRE10,PUNTUACIONDIRECTA,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_fm WHERE ";
		$sql  .="IDCANDIDATO=" . $aux->qstr($cEntidad->getIDCANDIDATO()) . " AND IDPROCESO=" . $aux->qstr($cEntidad->getIDPROCESO()) . " AND CODCOMPETENCIA=" . $aux->qstr($cEntidad->getCODCOMPETENCIA())+ " AND UPPER(DNI) LIKE UPPER('%" . $cEntidad->getDNI() . "%') ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_respuestas_fm ";
...
}//Fin de la Clase Sec_respuestasfmDB
?>