...
class NivelesjerarquicosDB
{
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idNivel=" . $aux->qstr($cEntidad->getIdNivel(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getOrden() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(orden) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getOrden() . "%") . ")";
		}
