...
* la tabla sec_pruebas_factores
**/
 class Sec_pruebasfactoresDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql = "";
			...
			$newId = $cEntidad->getIDTIPOPRUEBA();
			$iCont = 0;
			
			$sql  = "SELECT COUNT(IDTIPOPRUEBA) AS Max FROM sec_pruebas_factores ";
			$sql .= "WHERE ";
			$sql .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBA(), false) . " AND IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . " ";
		$rs = $aux->Execute($sql);
			if ($rs){
			...
			if ($iCont > 0 ){
				//Existe el registro
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Sec_ponderacionesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				return 0;
			}
			...
			$sql = "INSERT INTO sec_pruebas_factores (";
			$sql .= "IDTIPOPRUEBA" . ",";
			$sql .= "IDPRUEBA" . ",";
			$sql .= "IDFACTOR" . ",";
			$sql .= "CODFACTOR" . ",";
			...
			$sql .= $aux->qstr($cEntidad->getIDFACTOR(), false) . ",";
			$sql .= $aux->qstr($cEntidad->getCODFACTOR(), false) . ",";
			...
			//Inserta la Entidad 
			if($this->cuentaPorPrueba($cEntidad) > 0 || $this->cuentaPorFactor($cEntidad) > 0){
				if($this->cuentaPorPrueba($cEntidad) > 0){
					$this->msg_Error	= array();
					$sTypeError	=	date('d/m/Y H:i:s') . "Esta prueba ya está asignada a un factor, \\n revise las pruebas asignadas hasta el momento \\n para comprobar su correcta asignación";
					$this->msg_Error[]	= $sTypeError;
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					return 0;
				}
				if($this->cuentaPorFactor($cEntidad) > 0){
					$this->msg_Error	= array();
					$sTypeError	=	date('d/m/Y H:i:s') . "Este factor / competencia ya está asignado/a a una prueba, \\n revise los/las factores/competencias asignados/as hasta el momento \\n para comprobar su correcta asignación";
					$this->msg_Error[]	= $sTypeError;
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					return 0;
				}
	...
	*****************************************************************************************/
	function  getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM sec_pruebas_factores ";
		$sql  .=" WHERE IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBA(), false) . " AND IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
				$newId = $arr['Max'] + 1;
	...
	************************************************************************/
	function  modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_pruebas_factores SET ";
		$sql .= "IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . ", ";
		$sql .= "IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBA(), false) . ", ";
		$sql .= "IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . ", ";
		$sql .= "CODFACTOR=" . $aux->qstr($cEntidad->getCODFACTOR(), false) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false) ;
		$sql .= " WHERE ";
		$sql .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBAANTERIOR(), false) . " AND IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBAANTERIOR(), false) . " AND IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTORANTERIOR(), false) . " ";
		log.write("SQL de Ejecucion: " . $sql);
					
		if(cuentaPorPrueba($cEntidad)>1 || cuentaPorFactor($cEntidad)>1){
			if($this->cuentaPorPrueba($cEntidad) > 1){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . "Esta prueba ya está asignada a un factor, \\n revise las pruebas asignadas hasta el momento \\n para comprobar su correcta asignación";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			...
			if($this->cuentaPorFactor($cEntidad) > 1){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . "Este factor / competencia ya está asignado/a a una prueba, \\n revise los/las factores/competencias asignados/as hasta el momento \\n para comprobar su correcta asignación";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_pruebas_factores ";
			$sql  .=" WHERE ";
	...
	*************************************************************************/
	function consultar($cEntidad)
	{
		...
		$sql = "SELECT IDTIPOPRUEBA,IDPRUEBA,IDFACTOR,CODFACTOR," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_pruebas_factores WHERE ";
		$sql  .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBA(), false) . " AND IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . " ";
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
				...
				$cEntidad->setIDFACTOR($arr['IDFACTOR']);
				$cEntidad->setCODFACTOR($arr['CODFACTOR']);
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function consultarPorFactor($cEntidad)
	{
		...
		$sql = "SELECT IDTIPOPRUEBA,IDPRUEBA,IDFACTOR,CODFACTOR," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_pruebas_factores WHERE ";
		$sql  .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " AND IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . " ";
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function consultarPorPrueba($cEntidad)
	{
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function cuentaPorPrueba($cEntidad)
	{
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function cuentaPorFactor($cEntidad)
	{
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_pruebas_factores ";
		if ($cEntidad->getIDTIPOPRUEBA() != null && $cEntidad->getIDTIPOPRUEBA() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false);
		}
		if ($cEntidad->getIDPRUEBA() != null && $cEntidad->getIDPRUEBA() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="IDPRUEBA=" . $aux->qstr($cEntidad->getIDPRUEBA(), false);
		}
		if ($cEntidad->getIDFACTOR() != null && $cEntidad->getIDFACTOR() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false);
		}
		if ($cEntidad->getCODFACTOR() != null && $cEntidad->getCODFACTOR() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="UPPER(CODFACTOR) LIKE UPPER('%" . $cEntidad->getCODFACTOR() . "%')";
		}
...
}//Fin de la Clase Sec_pruebasfactoresDB
?>