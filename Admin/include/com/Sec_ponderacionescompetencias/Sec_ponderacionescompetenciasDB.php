...
* la tabla sec_ponderaciones_competencias
**/
class Sec_ponderacionescompetenciasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad) 
	{
		...
		$sql = "";
			...
			$iCont = 0;
			$sql = "SELECT COUNT(IDPERFILPUESTO) AS Max FROM sec_ponderaciones_competencias ";
			$sql .= "WHERE ";
			...
			$sql = "INSERT INTO sec_ponderaciones_competencias (";
			$sql .= "IDPERFILPUESTO" . ",";
			$sql .= "IDCOMPETENCIA" . ",";
			$sql .= "IDTIPOPRUEBA" . ",";
			...
			$sql .= $aux->qstr(str_replace("'", "", $cEntidad->getPONDERACION()), false) . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql = "SELECT MAX() AS Max FROM sec_ponderaciones_competencias ";
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_ponderaciones_competencias SET ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql ="DELETE FROM sec_ponderaciones_competencias ";
			$sql .=" WHERE ";
	...
	*************************************************************************/
	function readEntidad($cEntidad) 
	{
		...
		$aux			= $this->conn;
		$sql = "SELECT IDPERFILPUESTO,IDCOMPETENCIA,IDTIPOPRUEBA,DESCPERFILPUESTO,CODCOMPETENCIA,DESCCOMPETENCIA,PONDERACION," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_ponderaciones_competencias WHERE ";
		$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " AND IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDTIPOPRUEBA(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_ponderaciones_competencias ";
		...
		if ($cEntidad->getPONDERACIONHast() != null && $cEntidad->getPONDERACIONHast() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="PONDERACION<=" . $aux->qstr($cEntidad->getPONDERACIONHast(), false);
		}
...
}//Fin de la Clase Sec_ponderacionescompetenciasDB
?>