...
* la tabla sec_ponderaciones
**/
class Sec_ponderacionesDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
			...
			$sql = "SELECT COUNT(IDPERFILPUESTO) AS Max FROM sec_ponderaciones ";
			$sql .= "WHERE ";
			$sql .="IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " ";
			$rs = $aux->Execute($sql);
			if ($rs){
			...
			$sql = "INSERT INTO sec_ponderaciones (";
			$sql .= "IDPERFILPUESTO" . ",";
			$sql .= "IDCOMPETENCIA" . ",";
			$sql .= "IDTIPOPRUEBA" . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql = "SELECT MAX() AS Max FROM sec_ponderaciones ";
		$sql .=" WHERE IDPERFILPUESTO=" . $aux->qstr($cEntidad->getIDPERFILPUESTO(), false) . " AND IDCOMPETENCIA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false) . " ";
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
				$newId = $arr['Max'] + 1;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_ponderaciones SET ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql ="DELETE FROM sec_ponderaciones ";
			$sql .=" WHERE ";
	...
	*************************************************************************/
	function readEntidad($cEntidad) 
	{
		...
		$sql = "SELECT IDPERFILPUESTO,IDCOMPETENCIA,IDTIPOPRUEBA,DESCPERFILPUESTO,CODCOMPETENCIA,DESCCOMPETENCIA,PONDERACION," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_ponderaciones WHERE ";
	...
	*************************************************************************/
	function readLista($cEntidad) 
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_ponderaciones ";
		...
		if ($cEntidad->getIDTIPOPRUEBA() != null && $cEntidad->getIDTIPOPRUEBA() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="IDTIPOPRUEBA=" . $aux->qstr($cEntidad->getIDCOMPETENCIA(), false);
		}
...
}//Fin de la Clase Sec_ponderacionesDB
