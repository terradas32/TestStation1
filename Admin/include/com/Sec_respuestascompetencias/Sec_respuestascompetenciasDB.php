...
* la tabla sec_respuestas_competencias
**/
class Sec_respuestascompetenciasDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		//$newId = String.valueOf((Integer.parseInt(getSiguienteId(conConexion,"SELECT " . SQL.cGetId("IDRESPUESTA") . " FROM sec_respuestas_competencias"))));
		$newId = $this->getSiguienteId($cEntidad);	
	
		$sql = "INSERT INTO sec_respuestas_competencias (";
		$sql .= "IDRESPUESTA" . ",";
		$sql .= "IDCANDIDATO" . ",";
		$sql .= "IDPROCESO" . ",";
		$sql .= "DNI" . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(IDRESPUESTA) AS Max FROM sec_respuestas_competencias ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_respuestas_competencias SET ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_respuestas_competencias ";
			$sql  .=" WHERE ";
	...
	*************************************************************************/
	function consultar($cEntidad)
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,IDCOMPETENCIA,CODCOMPETENCIA,DESCCOMPETENCIA,PUNTUACIONDIRECTA,PUNTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_competencias WHERE ";
		$sql  .="IDRESPUESTA=" . $aux->qstr($cEntidad->getIDRESPUESTA()) . " ";
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql.="SELECT * FROM sec_respuestas_competencias ";
...
}//Fin de la Clase Sec_respuestascompetenciasDB
