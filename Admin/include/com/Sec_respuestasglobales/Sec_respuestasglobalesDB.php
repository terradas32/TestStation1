...
* la tabla sec_respuestas_globales
**/
class Sec_RespuestasglobalesDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		//newId = String.valueOf((Integer.parseInt(getSiguienteId(conConexion,"SELECT " . SQL.cGetId("IDRESPUESTA") . " FROM sec_respuestas_globales"))));
		$newId = $this->getSiguienteId($cEntidad);		
	
		$sql = "INSERT INTO sec_respuestas_globales (";
		$sql .= "IDRESPUESTA" . ",";
		$sql .= "IDCANDIDATO" . ",";
		$sql .= "IDPROCESO" . ",";
		$sql .= "DNI" . ",";
		...
		$sql .= $aux->qstr(str_replace("'", "", $cEntidad->getRANKING())) . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(IDRESPUESTA) AS Max FROM sec_respuestas_globales ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_respuestas_globales SET ";
		...
		$sql .= "RANKING=" . $aux->qstr(str_replace("'", "", $cEntidad->getRANKING())) . ", ";
		$sql .= "PUNTSOBRE10=" . $aux->qstr(str_replace("'", "", $cEntidad->getPUNTSOBRE10())) . ", ";
		$sql .= "MEDIAPONDERADA=" . $aux->qstr(str_replace("'", "", $cEntidad->getMEDIAPONDERADA())) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr("0") ;
		$sql .= " WHERE ";
		$sql .="IDRESPUESTA=" . $aux->qstr($cEntidad->getIDRESPUESTA()) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_respuestas_globales ";
			$sql  .=" WHERE ";
	...
	*************************************************************************/
	function consultar($cEntidad)
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,RANKING,PUNTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_globales WHERE ";
		$sql  .="IDRESPUESTA=" . $aux->qstr($cEntidad->getIDRESPUESTA()) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setRANKING($arr['RANKING']);
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function consultarComp($cEntidad)
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,RANKING,PUNTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_globales WHERE ";
		$sql  .="IDCANDIDATO=" . $aux->qstr($cEntidad->getIDCANDIDATO()) . " ";
		$sql  .="AND IDPROCESO=" . $aux->qstr($cEntidad->getIDPROCESO()) . " AND UPPER(DNI) LIKE UPPER('%" . $cEntidad->getDNI() . "%') ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_respuestas_globales ";
		...
		if ($cEntidad->getRANKING() != null && $cEntidad->getRANKING() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="RANKING>=" . $aux->qstr($cEntidad->getRANKING());
		}
		if ($cEntidad->getRANKINGHast() != null && $cEntidad->getRANKINGHast() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="RANKING<=" . $aux->qstr($cEntidad->getRANKINGHast());
		}
...
}//Fin de la Clase Sec_RespuestasglobalesDB
?>