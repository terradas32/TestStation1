...
* la tabla Sec_factoresactitudinalesDB
**/
class Sec_factoresactitudinalesDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql = "INSERT INTO sec_factores_actitudinales (";
		$sql .= "IDFACTOR" . ",";
		$sql .= "CODIGO" . ",";
		$sql .= "DESCRIPCION" . ",";
		$sql .= "IDCOMPETENCIA" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getINTELIGENCIA(), false) . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)  
	{
		...
		$sql = "SELECT MAX(IDFACTOR) AS Max FROM sec_factores_actitudinales ";
		$sql .="";
		$newId=0;
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad) 
	{
		...
		$sql = "UPDATE sec_factores_actitudinales SET ";
		...
		$sql .= "INTELIGENCIA=" . $aux->qstr($cEntidad->getINTELIGENCIA(), false) . ",";
	...
	************************************************************************/
	function borrar($cEntidad) 
	{
		...
		$retorno			= true;
			...
			$sql ="DELETE FROM sec_factores_actitudinales ";
			$sql .=" WHERE ";
	...
	*************************************************************************/
	function readEntidad($cEntidad) 
	{
		...
		$sql = "SELECT IDFACTOR,CODIGO,DESCRIPCION,IDCOMPETENCIA,IDTIPOPRUEBA,INTELIGENCIA,fecAlta,fecMod,usuAlta,usuMod FROM sec_factores_actitudinales WHERE ";
		$sql .="IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_factores_actitudinales ";
		...
		if ($cEntidad->getCODIGO() != null && $cEntidad->getCODIGO() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(CODIGO) LIKE UPPER('%" . $cEntidad->getCODIGO() . "%')";
		}
		...
		if ($cEntidad->getIDTIPOPRUEBAHast() != null && $cEntidad->getIDTIPOPRUEBAHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="IDTIPOPRUEBA<=" . $aux->qstr($cEntidad->getIDTIPOPRUEBAHast(), false) . "";
		}
...
}//Fin de la Clase Sec_factoresactitudinalesDB
?>