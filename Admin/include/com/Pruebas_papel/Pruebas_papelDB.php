...
* la tabla pruebas_papel
**/
class Pruebas_papelDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr($cEntidad->getRESULTADO(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Pruebas_papelDB]";
	...
	/***********************************************************************
	* Devuelve la query para insercion
	* @param entidad Entidad a insertar con Datos
	* @return string query
	* @exception Exception Error al ejecutar la acción
	*  en la base de datos
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(id) AS Max FROM pruebas_papel ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Pruebas_papelDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE pruebas_papel SET ";
		$sql .= "id=" . $aux->qstr($cEntidad->getId(), false) . ", ";
		...
		$sql .= "RESULTADO=" . $aux->qstr($cEntidad->getRESULTADO(), false) . ", ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Pruebas_papelDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Pruebas_papelDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM pruebas_papel WHERE ";
		$sql  .="id=" . $aux->qstr($cEntidad->getId(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setRESULTADO($arr['RESULTADO']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Pruebas_papelDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getEDAD() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(EDAD) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getEDAD() . "%") . ")";
		}
		if ($cEntidad->getSEXO() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getRESULTADO() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(RESULTADO) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getRESULTADO() . "%") . ")";
		}
	...
	function quitaImagen($cEntidad)
	{
		...
		$cEntidadAnterior	= new Pruebas_papel();
		$cEntidadAnterior->setId($cEntidad->getId());
		$cEntidadAnterior = $this->readEntidad($cEntidadAnterior);

		$sql = "UPDATE pruebas_papel SET  ";
		return $retorno;
	}
}//Fin de la Clase Pruebas_papelDB
?>