...
* la tabla sec_resultados_informesdim
**/
class Sec_resultadosinformesdimDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(IDCANDIDATO) AS Max FROM sec_resultados_informesdim ";
		$sql .= "WHERE ";
		...
		$sql = "INSERT INTO sec_resultados_informesdim (";
		$sql .= "IDCANDIDATO" . ",";
		$sql .= "IDPROCESO" . ",";
		$sql .= "IDDIMENSION" . ",";
		$sql .= "CODDIMENSION" . ",";
		...
		$sql .= $aux->qstr(str_replace("'", "", $cEntidad->getRESULTDIMENSION())) . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM sec_resultados_informesdim ";
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_resultados_informesdim SET ";
		...
		$sql .= "RESULTDIMENSION=" . $aux->qstr($cEntidad->getRESULTDIMENSION()) . ", ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_resultados_informesdim ";
			$sql  .=" WHERE ";
	...
	*************************************************************************/
	function consultar($cEntidad) 
	{
		...
		$sql = "SELECT IDCANDIDATO,IDPROCESO,IDDIMENSION,CODDIMENSION,DESCDIMENSION,RESULTDIMENSION,RESULTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_resultados_informesdim WHERE ";
		$sql  .="IDCANDIDATO=" . $aux->qstr($cEntidad->getIDCANDIDATO()) . " AND IDPROCESO=" . $aux->qstr($cEntidad->getIDPROCESO()) . " ";
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setRESULTDIMENSION($arr['RESULTDIMENSION']);
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function consultarPorDim($cEntidad)
	{
		...
		$sql = "SELECT IDCANDIDATO,IDPROCESO,IDDIMENSION,CODDIMENSION,DESCDIMENSION,RESULTDIMENSION,RESULTSOBRE10,MEDIAPONDERADA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_resultados_informesdim WHERE ";
		$sql  .="IDCANDIDATO=" . $aux->qstr($cEntidad->getIDCANDIDATO()) . " AND IDPROCESO=" . $aux->qstr($cEntidad->getIDPROCESO()) . " AND IDDIMENSION=" . $aux->qstr($cEntidad->getIDDIMENSION()) . " ";
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM sec_resultados_informesdim ";
		...
		if ($cEntidad->getRESULTDIMENSION() != null && $cEntidad->getRESULTDIMENSION() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="RESULTDIMENSION>=" . $aux->qstr($cEntidad->getRESULTDIMENSION());
		}
		if ($cEntidad->getRESULTDIMENSIONHast() != null && $cEntidad->getRESULTDIMENSIONHast() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="RESULTDIMENSION<=" . $aux->qstr($cEntidad->getRESULTDIMENSIONHast());
		}
...
}//Fin de la Clase Sec_resultadosinformesdimDB
?>