...
class Empresas_usuarios_perfilesDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $cEntidad->getIdEmpresa() . '-' . $cEntidad->getIdUsuario() . '-' . $cEntidad->getIdPerfil();
		//Se recogen los datos de la select de multiselecion
		//guardados como a,b,c,d,e,f
		$aIdPerfil = explode(",",$cEntidad->getIdPerfil());
		//Borramos todos los registros de ese Usuario
		$sql  ="DELETE FROM empresas_usuarios_perfiles ";
		$sql  .=" WHERE idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " ";
		...
		for ($i=0, $maxPF = sizeof($aIdPerfil); $i<$maxPF; $i++){
			...
			$sql .= $aux->qstr($aIdPerfil[$i], false) . ",";
