...
class ItemsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][ItemsDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idItem) AS Max FROM items ";
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][ItemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][ItemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		$sql .= "bajaLog='1'";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][ItemsDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getTipoItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="tipoItem=" . $aux->qstr($cEntidad->getTipoItem(), false);
		}
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function listaItemsNoAsignados($cEntidad)
	{
		$aux			= $this->conn;
		...
		//$sql .=" AND idItem NOT IN (SELECT idItem FROM escalas_items WHERE idprueba= ". $cEntidad->getIdPrueba()." AND idEscala=". $_POST['idEscala'] ." AND idBloque=". $_POST['idBloque'] .")";
		$sql .=" AND idItem NOT IN (SELECT idItem FROM escalas_items WHERE idprueba= ". $cEntidad->getIdPrueba().")";
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function listaItemsNoAsignadosCompetencias($cEntidad)
	{
		$aux			= $this->conn;
		...
		//$sql .=" AND idItem NOT IN (SELECT idItem FROM competencias_items WHERE idprueba= ". $cEntidad->getIdPrueba()." AND idTipoCompetencia=". $_POST['idTipoCompetencia'] ." AND idCompetencia=". $_POST['idCompetencia'] .")";
		$sql .=" AND idItem NOT IN (SELECT idItem FROM competencias_items WHERE idprueba= ". $cEntidad->getIdPrueba().")";
		if ($cEntidad->getOrderBy() != ""){
			$sql .=" ORDER BY " . $cEntidad->getOrderBy();
			if ($cEntidad->getOrder() != ""){
				$sql .=" " . $cEntidad->getOrder();
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function listaItemsNoInversos($cEntidad)
	{
		$aux			= $this->conn;
		...
		//$sql .=" AND idItem NOT IN (SELECT idItem FROM escalas_items WHERE idprueba= ". $cEntidad->getIdPrueba()." AND idEscala=". $_POST['idEscala'] ." AND idBloque=". $_POST['idBloque'] .")";
		$sql .=" AND idItem NOT IN (SELECT idItem FROM items_inversos WHERE idprueba= ". $cEntidad->getIdPrueba().")";
	...
	function quitaImagen($cEntidad)
	{
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][ItemsDBDB::quitaImagen()]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}else{
