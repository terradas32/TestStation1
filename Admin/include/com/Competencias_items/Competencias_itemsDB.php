...
* la tabla competencias_items
**/
class Competencias_itemsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $cEntidad->getIdTipoCompetencia();
		$iCont = 0;
		$sql  = "SELECT COUNT(idTipoCompetencia) AS Max FROM competencias_items ";
		$sql .= "WHERE ";
		$sql .="idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Competencias_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	
		$sql = "INSERT INTO competencias_items (";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Competencias_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	...
	{
		$aux			= $this->conn;
		$sql  = "SELECT MAX() AS Max FROM competencias_items ";
		$sql  .=" WHERE idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Competencias_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE competencias_items SET ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Competencias_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM competencias_items ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Competencias_itemsDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM competencias_items WHERE ";
		$sql  .="idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Competencias_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function readListaGroupTipo($cEntidad)
	{
		$aux			= $this->conn;
		...
		$sql.= " GROUP BY idTipoCompetencia ";
...
}//Fin de la Clase Competencias_itemsDB
?>