...
class FaqsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $this->getSiguienteId($cEntidad);
                ...
                if($cEntidad->getIdFaq() != ""){
                    $newId = $cEntidad->getIdFaq();
        		}else{
                    $newId = $this->getSiguienteId($cEntidad);
        		}
		...
		$sql .= $aux->qstr($cEntidad->getPregunta(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getRespuesta(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][FaqsDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idFaq) AS Max FROM faqs ";
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][FaqsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE faqs SET ";
		$sql .= "idFaq=" . $aux->qstr($cEntidad->getIdFaq(), false) . ", ";
		$sql .= "codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . ", ";
		$sql .= "pregunta=" . $aux->qstr($cEntidad->getPregunta(), false) . ", ";
		$sql .= "respuesta=" . $aux->qstr($cEntidad->getRespuesta(), false) . ", ";
		...
		$sql .= " WHERE ";
		$sql .="idFaq=" . $aux->qstr($cEntidad->getIdFaq(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][FaqsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		$sql .= "publicar='1'";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][FaqsDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdFaq($arr['idFaq']);
					$cEntidad->setCodIdiomaIso2($arr['codIdiomaIso2']);
					$cEntidad->setPregunta($arr['pregunta']);
					$cEntidad->setRespuesta($arr['respuesta']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][FaqsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getPregunta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(pregunta) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getPregunta() . "%") . ")";
		}
		if ($cEntidad->getRespuesta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(respuesta) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getRespuesta() . "%") . ")";
		}
	...
	function quitaImagen($cEntidad)
	{
		...
		$cEntidadAnterior	= new Faqs();
		$cEntidadAnterior->setIdFaq($cEntidad->getIdFaq());
