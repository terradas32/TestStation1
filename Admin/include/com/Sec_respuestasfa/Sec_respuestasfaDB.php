...
* la tabla sec_respuestas_fa
**/
class Sec_respuestasfaDB 
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		//newId = String.valueOf((Integer.parseInt(getSiguienteId(conConexion,"SELECT " . SQL.cGetId("IDRESPUESTA") . " FROM sec_respuestas_fa"))));
		$newId = $this->getSiguienteId($cEntidad);
			
			
	
		$sql = "INSERT INTO sec_respuestas_fa (";
		$sql .= "IDRESPUESTA" . ",";
		$sql .= "IDCANDIDATO" . ",";
		$sql .= "IDPROCESO" . ",";
		$sql .= "DNI" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getIDFACTOR()) . ",";
		$sql .= $aux->qstr($cEntidad->getCODFACTOR()) . ",";
		$sql .= $aux->qstr($cEntidad->getDESCFACTOR()) . ",";
		...
		$sql .= $aux->qstr($cEntidad->getINTELIGENCIA()) . ",";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(IDRESPUESTA) AS Max FROM sec_respuestas_fa ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE sec_respuestas_fa SET ";
		...
		$sql .= "IDFACTOR=" . $aux->qstr($cEntidad->getIDFACTOR()) . ", ";
		$sql .= "CODFACTOR=" . $aux->qstr($cEntidad->getCODFACTOR()) . ", ";
		$sql .= "DESCFACTOR=" . $aux->qstr($cEntidad->getDESCFACTOR()) . ", ";
		...
		$sql .= "INTELIGENCIA=" . $aux->qstr($cEntidad->getINTELIGENCIA()) . ", ";
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$retorno			= true;
			...
			$sql  ="DELETE FROM sec_respuestas_fa ";
			$sql  .=" WHERE ";
	...
	************************************************************************/
	function borrarFa($cEntidad)
	{
		...
		$retorno			= true;
			...
			if ($cEntidad->getCODFACTOR() != ""){
				$sql .= this.getSQLAnd($and);
				$and = true;
				$sql  .="CODFACTOR=" . $aux->qstr($cEntidad->getCODFACTOR()) . " ";
			}
			if ($cEntidad->getDESCFACTOR() != ""){
				$sql .= this.getSQLAnd($and);
				$and = true;
				$sql  .="DESCFACTOR=" . $aux->qstr($cEntidad->getDESCFACTOR()) . " ";
			}
	...
	*************************************************************************/
	function consultar($cEntidad) 
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,IDFACTOR,CODFACTOR,DESCFACTOR,PUNTUACIONDIRECTA,PUNTSOBRE10,MEDIAPONDERADA,INTELIGENCIA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_fa WHERE ";
		$sql  .="IDRESPUESTA=" . $aux->qstr($cEntidad->getIDRESPUESTA()) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setDESCFACTOR($arr['DESCFACTOR']);
					...
					$cEntidad->setINTELIGENCIA($arr['INTELIGENCIA']);
	...
	*************************************************************************/
	function consultaCompleta($cEntidad)
	{
		...
		$sql = "SELECT IDRESPUESTA,IDCANDIDATO,IDPROCESO,DNI,IDFACTOR,CODFACTOR,DESCFACTOR,PUNTUACIONDIRECTA,PUNTSOBRE10,MEDIAPONDERADA,INTELIGENCIA," . "fecAlta" . "," . "fecMod" . ",usuAlta,usuMod FROM sec_respuestas_fa WHERE ";
		$sql  .="IDCANDIDATO=" . $aux->qstr($cEntidad->getIDCANDIDATO()) . " AND IDPROCESO=" . $aux->qstr($cEntidad->getIDPROCESO()) . "AND CODFACTOR=" . $aux->qstr($cEntidad->getCODFACTOR()) +" AND UPPER(DESCFACTOR) LIKE UPPER('%" . $cEntidad->getDESCFACTOR() . "%') AND UPPER(DNI) LIKE UPPER('%" . $cEntidad->getDNI() . "%') ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql.="SELECT * FROM sec_respuestas_fa ";
		...
		if ($cEntidad->getIDFACTORHast() != null && $cEntidad->getIDFACTORHast() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="IDFACTOR<=" . $aux->qstr($cEntidad->getIDFACTORHast());
		}
		...
		if ($cEntidad->getDESCFACTOR() != null && $cEntidad->getDESCFACTOR() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="UPPER(DESCFACTOR) LIKE UPPER('%" . $cEntidad->getDESCFACTOR() . "%')";
		}
		...
		if ($cEntidad->getINTELIGENCIA() != null && $cEntidad->getINTELIGENCIA() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="UPPER(INTELIGENCIA) LIKE UPPER('%" . $cEntidad->getINTELIGENCIA() . "%')";
		}
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function readListaNoIC($cEntidad)
	{
		...
		if ($cEntidad->getINTELIGENCIA() != null && $cEntidad->getINTELIGENCIA() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
			$sql .="UPPER(INTELIGENCIA) NOT LIKE UPPER('%" . $cEntidad->getINTELIGENCIA() . "%')";
		}
		if ($cEntidad->getFecAlta() != null && $cEntidad->getFecAlta() != ""){
			$sql .= this.getSQLWhere($and);
			$and = true;
...
}//Fin de la Clase Sec_respuestasfaDB
?>