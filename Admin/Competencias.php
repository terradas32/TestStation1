...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new CompetenciasDB($conn);  // Entidad DB
	$cEntidad	= new Competencias();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['competencias_next_page']) && $_POST['competencias_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'competencias');
					...
					$lista=$pager->getRS();
					include('Template/Competencias/mntcompetenciasl.php');
				}else{
					//$cEntidad	= new Competencias();  // inicializamos la Entidad
					$cEntidad->setNombre("");
					$cEntidad->setDescripcion("");
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Competencias/mntcompetenciasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Competencias/mntcompetencias.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdTipoCompetencia((isset($_POST["LSTIdTipoCompetencia"]) && $_POST["LSTIdTipoCompetencia"] != "") ? $_POST["LSTIdTipoCompetencia"] : "");	$cEntidad->setBusqueda(constant("STR_TIPO_DE_COMPETENCIA"), (isset($_POST["LSTIdTipoCompetencia"]) && $_POST["LSTIdTipoCompetencia"] != "" ) ? $comboTIPOS_COMPETENCIAS->getDescripcionCombo($_POST["LSTIdTipoCompetencia"]) : "");
		$cEntidad->setIdCompetencia((isset($_POST["LSTIdCompetencia"]) && $_POST["LSTIdCompetencia"] != "") ? $_POST["LSTIdCompetencia"] : "");	$cEntidad->setBusqueda(constant("STR_ID_COMPETENCIA"), (isset($_POST["LSTIdCompetencia"]) && $_POST["LSTIdCompetencia"] != "" ) ? $_POST["LSTIdCompetencia"] : "");
		$cEntidad->setIdCompetenciaHast((isset($_POST["LSTIdCompetenciaHast"]) && $_POST["LSTIdCompetenciaHast"] != "") ? $_POST["LSTIdCompetenciaHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_COMPETENCIA") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdCompetenciaHast"]) && $_POST["LSTIdCompetenciaHast"] != "" ) ? $_POST["LSTIdCompetenciaHast"] : "");
		...
		$cEntidad->setIdPrueba((isset($_POST["LSTIdPrueba"]) && $_POST["LSTIdPrueba"] != "") ? $_POST["LSTIdPrueba"] : "");	$cEntidad->setBusqueda(constant("STR_PRUEBA"), (isset($_POST["LSTIdPrueba"]) && $_POST["LSTIdPrueba"] != "" ) ? $_POST["LSTIdPrueba"] : "");
