...
//	echo "<br />" . $sSQLCandidatos;exit;
	...
	while (!$vItems->EOF  )
	{
		...
		switch ($_idPrueba)
		{
			...
			case 105:	//Prisma
			case 126:	//Prisma
			case 117:	//MAZDA CUESTIONARIO DE COMPETENCIAS AC
			case 121: //MAZDA CUESTIONARIO DE COMPETENCIAS AS
			case 123: //MAZDA CUESTIONARIO DE COMPETENCIAS ACD
				$vVectorCandidatos->Move(0);
				while (!$vVectorCandidatos->EOF){
...
//						 	echo $nEscalas;exit;
						 	if($nEscalas > 0){
						 		while(!$listaEscalas->EOF){
								    ...
								    $sHtml.='<tr>';
									while (!$vVectorCandidatos->EOF){
												$_pkCandidato= $vVectorCandidatos->fields['idEmpresa'] . $vVectorCandidatos->fields['idProceso'] . $vVectorCandidatos->fields['idCandidato'];
								        $iPBaremada = $aPuntuaciones[$_pkCandidato][$listaBloques->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala']];
								        if ($bPrimeraVuelta){
									        $sHtml.='
					...
					$sHtml.='<tr>';
    						while (!$vVectorCandidatos->EOF){
    					       ...
    					       if ($bPrimeraVuelta){
    					        ...
    					        $sHtml.='
    					        	<td align="center"><strong>' . $aPuntuacionesConsistencia[$_pkCandidato] . '</strong></td>
    								';
    				       		$bPrimeraVuelta = false;
    							$vVectorCandidatos->MoveNext();
    						}
					 	...
					 	if($nCompetencias >0){
					 		while(!$listaCompetencias->EOF){
								...
								while (!$vVectorCandidatos->EOF){
								    ...
								    $sHtml.='
								    	<td align="center"><strong>' . $aPuntuacionesCompetencias[$_pkCandidato][$sPosiCompetencias] . '</strong></td>
									';
							       	$bPrimeraVuelta = false;
									$vVectorCandidatos->MoveNext();
								}
...
//		echo "<br />id::" . $valor .  " - valor::" . $inv;
	...
	function calculosGlobalesEscala($_idCandidato, $_idProceso){
		...
		$_pkCandidato="";
		$_pkCandidato=$_idEmpresa . $_idProceso . $_idCandidato;
		$iPGlobal = 0;
		$iTotalEscalas=0;
...
//				       	echo "<br />---------->[" . $sPosi . "][" . $iPBaremada . "]";
				       	$aPuntuaciones[$_pkCandidato][$sPosi] =  $iPBaremada;
...
//		 echo "<br />" . $iTotalEscalas;exit;
		 ...
		 $aPuntuacionesConsistencia[$_pkCandidato] =  $consistencia;
...
//				        echo $iPdCompetencias . "<br />";
				        ...
				        if($nBaremosC>0){
				       	...
				       	$sPosiCompetencias = $listaTipoCompetencia->fields['idTipoCompetencia'] . "-" . $listaCompetencias->fields['idCompetencia'];
				       	$aPuntuacionesCompetencias[$_pkCandidato][$sPosiCompetencias] =  $iPdCompetencias;
