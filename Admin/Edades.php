...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Edades/EdadesDB.php");
	require_once(constant("DIR_WS_COM") . "Edades/Edades.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new EdadesDB($conn);  // Entidad DB
	$cEntidad	= new Edades();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['edades_next_page']) && $_POST['edades_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'edades');
					...
					$lista=$pager->getRS();
					include('Template/Edades/mntedadesl.php');
				}else{
					$cEntidad	= new Edades();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Edades/mntedadesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Edades/mntedades.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdEdad((isset($_POST["LSTIdEdad"]) && $_POST["LSTIdEdad"] != "") ? $_POST["LSTIdEdad"] : "");	$cEntidad->setBusqueda(constant("STR_ID_EDAD"), (isset($_POST["LSTIdEdad"]) && $_POST["LSTIdEdad"] != "" ) ? $_POST["LSTIdEdad"] : "");
		$cEntidad->setIdEdadHast((isset($_POST["LSTIdEdadHast"]) && $_POST["LSTIdEdadHast"] != "") ? $_POST["LSTIdEdadHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_EDAD") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdEdadHast"]) && $_POST["LSTIdEdadHast"] != "" ) ? $_POST["LSTIdEdadHast"] : "");
