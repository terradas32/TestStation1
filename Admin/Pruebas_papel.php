...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Pruebas_papelDB($conn);  // Entidad DB
	$cEntidad	= new Pruebas_papel();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['pruebas_papel_next_page']) && $_POST['pruebas_papel_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'pruebas_papel');
					...
					$lista=$pager->getRS();
					include('Template/Pruebas_papel/mntpruebas_papell.php');
				}else{
					$cEntidad	= new Pruebas_papel();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Pruebas_papel/mntpruebas_papela.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Pruebas_papel/mntpruebas_papel.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setRESULTADO((isset($_POST["fRESULTADO"])) ? $_POST["fRESULTADO"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setEDAD((isset($_POST["LSTEDAD"]) && $_POST["LSTEDAD"] != "") ? $_POST["LSTEDAD"] : "");	$cEntidad->setBusqueda(constant("STR_EDAD"), (isset($_POST["LSTEDAD"]) && $_POST["LSTEDAD"] != "" ) ? $_POST["LSTEDAD"] : "");
		...
		$cEntidad->setRESULTADO((isset($_POST["LSTRESULTADO"]) && $_POST["LSTRESULTADO"] != "") ? $_POST["LSTRESULTADO"] : "");	$cEntidad->setBusqueda(constant("STR_RESULTADO"), (isset($_POST["LSTRESULTADO"]) && $_POST["LSTRESULTADO"] != "" ) ? $_POST["LSTRESULTADO"] : "");
