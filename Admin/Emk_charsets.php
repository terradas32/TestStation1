...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Emk_charsets/Emk_charsetsDB.php");
	require_once(constant("DIR_WS_COM") . "Emk_charsets/Emk_charsets.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Emk_charsetsDB($conn);  // Entidad DB
	$cEntidad	= new Emk_charsets();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['emk_charsets_next_page']) && $_POST['emk_charsets_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'emk_charsets');
					...
					$lista=$pager->getRS();
					include('Template/Emk_charsets/mntemk_charsetsl.php');
				}else{
					$cEntidad	= new Emk_charsets();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Emk_charsets/mntemk_charsetsa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Emk_charsets/mntemk_charsets.php');
			break;
