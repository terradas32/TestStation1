...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Correos_procesoDB($conn);  // Entidad DB
	$cEntidad	= new Correos_proceso();  // Entidad
	...
	$comboCORREOS	= new Combo($conn,"fIdCorreo","idCorreo","nombre","Descripcion","correos","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					if (!empty($_POST['correos_proceso_next_page']) && $_POST['correos_proceso_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'correos_proceso');
					...
					$lista=$pager->getRS();
					include('Template/Correos_proceso/mntcorreos_procesol.php');
				}else{
					$cEntidad	= new Correos_proceso();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Correos_proceso/mntcorreos_procesoa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Correos_proceso/mntcorreos_proceso.php');
			break;
		case constant("MNT_LISTAR"):
			...
			if (isset($_POST["fReordenar"]) &&  !empty($_POST["fReordenar"])){
				$bInit=true;
				$sql = $cEntidadDB->readListaNI($cEntidad);
			}else{
				if (!empty($_POST['correos_proceso_next_page']) && $_POST['correos_proceso_next_page'] > 1){
					$bInit=false;
					$sql = $cEntidadDB->readListaNI($cEntidad);
	...
	*/
	function readLista($cEntidad){
		...
		global $comboCORREOS;
		$cEntidad->setIdCorreo((isset($_POST["LSTIdCorreo"]) && $_POST["LSTIdCorreo"] != "") ? $_POST["LSTIdCorreo"] : "");	$cEntidad->setBusqueda(constant("STR_CORREO"), (isset($_POST["LSTIdCorreo"]) && $_POST["LSTIdCorreo"] != "" ) ? $comboCORREOS->getDescripcionCombo($_POST["LSTIdCorreo"]) : "");
