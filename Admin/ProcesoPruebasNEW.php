...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "ProcesoPruebas/ProcesoPruebasDB.php");
	require_once(constant("DIR_WS_COM") . "ProcesoPruebas/ProcesoPruebas.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new ProcesoPruebasDB($conn);  // Entidad DB
	$cEntidad	= new ProcesoPruebas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['procesopruebas_next_page']) && $_POST['procesopruebas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'procesopruebas');
					...
					$lista=$pager->getRS();
					include('Template/ProcesoPruebas/mntprocesopruebasl.php');
				}else{
					$cEntidad	= new ProcesoPruebas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/ProcesoPruebas/mntprocesopruebasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/ProcesoPruebas/mntprocesopruebas.php');
			break;
