	...
	switch ($_POST['MODO'])
	{
            ...
            if (!$bEncontradoPassword)
            {
                if (!$bEncontradoUsuario)
                {
        			...
        			?><script>alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
        				...
        				{
        					if (!empty($_POST['usuarios_next_page']) && $_POST['usuarios_next_page'] > 1 ){
        						$bInit=false;
        					...
        					$pager = new ADODB_Pager($conn,$sql,'usuarios');
        					if ($bInit)	$pager->curr_page=1;
        					$pager->showPageLinks = true;
        					$lista=$pager->getRS();
        					include('Template/Usuarios/mntusuariosl.php');
        				}else{
        					$cEntidad	= new Usuarios();  // inicializamos la Entidad
        					$_POST['MODO']    = constant("MNT_ALTA");
        					include('Template/Usuarios/mntusuariosa.php');
        				}
        			}else{
      ...
      $cEntidad = $cEntidadDB->readEntidad($cEntidad);
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Usuarios/mntusuarios.php');
			break;
