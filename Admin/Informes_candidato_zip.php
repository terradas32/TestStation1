...
//				echo "<br />Fichero encontrado:: " . $_ficheroPDF;
				...
				$sql = "INSERT INTO informes_generar (";
				$sql .= "idCandidato" . ",";
				$sql .= "idEmpresa" . ",";
				$sql .= "idProceso" . ",";
				$sql .= "idPrueba" . ",";
				...
				$sql .= "usuMod" . ")";
				$sql .= " VALUES (";
				$sql .= $conn->qstr($cCandidato->getIdCandidato(), false) . ",";
				$sql .= $conn->qstr($lista->fields['idEmpresa'], false) . ",";
				$sql .= $conn->qstr($lista->fields['idProceso'], false) . ",";
				$sql .= $conn->qstr($cPrueba->getIdPrueba(), false) . ",";
				$sql .= $conn->qstr($rsProceso_informes->fields['codIdiomaIso2'], false) . ",";
				$sql .= $conn->qstr($cPrueba->getNombre(), false) . ",";
				$sql .= $conn->qstr($cCandidato->getNombre(), false) . ",";
				$sql .= $conn->qstr($cCandidato->getApellido1(), false) . ",";
				$sql .= $conn->qstr($cCandidato->getMail(), false) . ",";
				$sql .= $conn->qstr($rsProceso_informes->fields['idTipoInforme'], false) . ",";
				$sql .= $conn->qstr($rsProceso_informes->fields['codIdiomaInforme'], false) . ",";
				$sql .= $conn->qstr($_sIdBaremo, false) . ",";
				$sql .= $conn->qstr("0", false) . ",";
				$sql .= $conn->sysTimeStamp . ",";
				$sql .= $conn->sysTimeStamp . ",";
				$sql .= $conn->qstr($_cEntidadUsuarioTK->getIdUsuario(), false) . ",";
				$sql .= $conn->qstr($_cEntidadUsuarioTK->getIdUsuario(), false) . ")";
				
				if($conn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [Informes_candidatos_zip][Admin]";
...
//		echo "<br />IDROCESO::" . $_POST["LSTIdProceso"];
		$cEntidad->setIdProceso((isset($_POST["LSTIdProceso"]) && $_POST["LSTIdProceso"] != "") ? $_POST["LSTIdProceso"] : "");
		...
		$cEntidad->setBusqueda(constant("STR_PROCESO"), (isset($_POST["LSTIdProceso"]) && $_POST["LSTIdProceso"] != "" ) ? $comboPROCESOS->getDescripcionCombo($_POST["LSTIdProceso"]) : "");
