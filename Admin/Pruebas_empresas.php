...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Pruebas_empresasDB($conn);  // Entidad DB
	$cEntidad	= new Pruebas_empresas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['pruebas_empresas_next_page']) && $_POST['pruebas_empresas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'pruebas_empresas');
					...
					$lista=$pager->getRS();
					include('Template/Pruebas_empresas/mntpruebas_empresasl.php');
				}else{
					...
					$cEntidad	= new Pruebas_empresas();  // inicializamos la Entidad
					...
					include('Template/Pruebas_empresas/mntpruebas_empresasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Pruebas_empresas/mntpruebas_empresas.php');
			break;
