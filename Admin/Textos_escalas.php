...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Textos_escalas/Textos_escalasDB.php");
	require_once(constant("DIR_WS_COM") . "Textos_escalas/Textos_escalas.php");
	
include_once ('include/conexion.php');
	
	$cUtilidades	= new Utilidades();
	
	$cEntidadDB	= new Textos_escalasDB($conn);  // Entidad DB
	$cEntidad	= new Textos_escalas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['textos_escalas_next_page']) && $_POST['textos_escalas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'textos_escalas');
					...
					$lista=$pager->getRS();
					include('Template/Textos_escalas/mnttextos_escalasl.php');
				}else{
					$cEntidad	= new Textos_escalas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Textos_escalas/mnttextos_escalasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Textos_escalas/mnttextos_escalas.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		global $cUtilidades;
		$cEntidad->setIdTexto((isset($_POST["fIdTexto"])) ? $_POST["fIdTexto"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdTexto((isset($_POST["LSTIdTexto"]) && $_POST["LSTIdTexto"] != "") ? $_POST["LSTIdTexto"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TEXTO"), (isset($_POST["LSTIdTexto"]) && $_POST["LSTIdTexto"] != "" ) ? $_POST["LSTIdTexto"] : "");
		$cEntidad->setIdTextoHast((isset($_POST["LSTIdTextoHast"]) && $_POST["LSTIdTextoHast"] != "") ? $_POST["LSTIdTextoHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TEXTO") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdTextoHast"]) && $_POST["LSTIdTextoHast"] != "" ) ? $_POST["LSTIdTextoHast"] : "");
		global $comboPRUEBAS;
		...
		global $comboBLOQUES;
		$cEntidad->setIdBloque((isset($_POST["LSTIdBloque"]) && $_POST["LSTIdBloque"] != "") ? $_POST["LSTIdBloque"] : "");	$cEntidad->setBusqueda(constant("STR_ID_BLOQUE"), (isset($_POST["LSTIdBloque"]) && $_POST["LSTIdBloque"] != "" ) ? $comboBLOQUES->getDescripcionCombo($_POST["LSTIdBloque"]) : "");
		global $comboESCALAS;
		$cEntidad->setIdEscala((isset($_POST["LSTIdEscala"]) && $_POST["LSTIdEscala"] != "") ? $_POST["LSTIdEscala"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ESCALA"), (isset($_POST["LSTIdEscala"]) && $_POST["LSTIdEscala"] != "" ) ? $comboESCALAS->getDescripcionCombo($_POST["LSTIdEscala"]) : "");
		$cEntidad->setPuntMin((isset($_POST["LSTPuntMin"]) && $_POST["LSTPuntMin"] != "") ? $_POST["LSTPuntMin"] : "");	$cEntidad->setBusqueda(constant("STR_PUNT_MIN"), (isset($_POST["LSTPuntMin"]) && $_POST["LSTPuntMin"] != "" ) ? $_POST["LSTPuntMin"] : "");
		$cEntidad->setPuntMinHast((isset($_POST["LSTPuntMinHast"]) && $_POST["LSTPuntMinHast"] != "") ? $_POST["LSTPuntMinHast"] : "");	$cEntidad->setBusqueda(constant("STR_PUNT_MIN") . " " . constant("STR_HASTA"), (isset($_POST["LSTPuntMinHast"]) && $_POST["LSTPuntMinHast"] != "" ) ? $_POST["LSTPuntMinHast"] : "");
		$cEntidad->setPuntMax((isset($_POST["LSTPuntMax"]) && $_POST["LSTPuntMax"] != "") ? $_POST["LSTPuntMax"] : "");	$cEntidad->setBusqueda(constant("STR_PUNT_MAX"), (isset($_POST["LSTPuntMax"]) && $_POST["LSTPuntMax"] != "" ) ? $_POST["LSTPuntMax"] : "");
		$cEntidad->setPuntMaxHast((isset($_POST["LSTPuntMaxHast"]) && $_POST["LSTPuntMaxHast"] != "") ? $_POST["LSTPuntMaxHast"] : "");	$cEntidad->setBusqueda(constant("STR_PUNT_MAX") . " " . constant("STR_HASTA"), (isset($_POST["LSTPuntMaxHast"]) && $_POST["LSTPuntMaxHast"] != "" ) ? $_POST["LSTPuntMaxHast"] : "");
