...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Perfiles_ideales/Perfiles_idealesDB.php");
	require_once(constant("DIR_WS_COM") . "Perfiles_ideales/Perfiles_ideales.php");
	
include_once ('include/conexion.php');
	
	$cUtilidades	= new Utilidades();
	
	$cEntidadDB	= new Perfiles_idealesDB($conn);  // Entidad DB
	$cEntidad	= new Perfiles_ideales();  // Entidad
	...
	$comboTIPOS_COMPETENCIAS	= new Combo($conn,"fIdTipoCompetencia","idTipoCompetencia","nombre","Descripcion","tipos_competencias","",constant("SLC_OPCION"),"","","fecMod");
	$comboCOMPETENCIAS	= new Combo($conn,"fIdCompetencia","idCompetencia","nombre","Descripcion","competencias","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['perfiles_ideales_next_page']) && $_POST['perfiles_ideales_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'perfiles_ideales');
					...
					$lista=$pager->getRS();
					include('Template/Perfiles_ideales/mntperfiles_idealesl.php');
				}else{
					$cEntidad	= new Perfiles_ideales();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Perfiles_ideales/mntperfiles_idealesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Perfiles_ideales/mntperfiles_ideales.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setPuntuacionMin((isset($_POST["fPuntuacionMin"])) ? $_POST["fPuntuacionMin"] : "");
		$cEntidad->setPuntuacionMax((isset($_POST["fPuntuacionMax"])) ? $_POST["fPuntuacionMax"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setPuntuacionMin((isset($_POST["LSTPuntuacionMin"]) && $_POST["LSTPuntuacionMin"] != "") ? $_POST["LSTPuntuacionMin"] : "");	$cEntidad->setBusqueda(constant("STR_PUNTUACION_MIN"), (isset($_POST["LSTPuntuacionMin"]) && $_POST["LSTPuntuacionMin"] != "" ) ? $_POST["LSTPuntuacionMin"] : "");
		$cEntidad->setPuntuacionMinHast((isset($_POST["LSTPuntuacionMinHast"]) && $_POST["LSTPuntuacionMinHast"] != "") ? $_POST["LSTPuntuacionMinHast"] : "");	$cEntidad->setBusqueda(constant("STR_PUNTUACION_MIN") . " " . constant("STR_HASTA"), (isset($_POST["LSTPuntuacionMinHast"]) && $_POST["LSTPuntuacionMinHast"] != "" ) ? $_POST["LSTPuntuacionMinHast"] : "");
		$cEntidad->setPuntuacionMax((isset($_POST["LSTPuntuacionMax"]) && $_POST["LSTPuntuacionMax"] != "") ? $_POST["LSTPuntuacionMax"] : "");	$cEntidad->setBusqueda(constant("STR_PUNTUACION_MAX"), (isset($_POST["LSTPuntuacionMax"]) && $_POST["LSTPuntuacionMax"] != "" ) ? $_POST["LSTPuntuacionMax"] : "");
		$cEntidad->setPuntuacionMaxHast((isset($_POST["LSTPuntuacionMaxHast"]) && $_POST["LSTPuntuacionMaxHast"] != "") ? $_POST["LSTPuntuacionMaxHast"] : "");	$cEntidad->setBusqueda(constant("STR_PUNTUACION_MAX") . " " . constant("STR_HASTA"), (isset($_POST["LSTPuntuacionMaxHast"]) && $_POST["LSTPuntuacionMaxHast"] != "" ) ? $_POST["LSTPuntuacionMaxHast"] : "");
