...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Rangos_textos/Rangos_textosDB.php");
	require_once(constant("DIR_WS_COM") . "Rangos_textos/Rangos_textos.php");
	
include_once ('include/conexion.php');
	
	$cUtilidades	= new Utilidades();
	
	$cEntidadDB	= new Rangos_textosDB($conn);  // Entidad DB
	$cEntidad	= new Rangos_textos();  // Entidad
	...
	$comboINFORMES_PRUEBAS	= new Combo($conn,"fIdTipoInforme","idTipoInforme","idTipoInforme","Descripcion","informes_pruebas","",constant("SLC_OPCION"),"","","fecMod");
	$comboRANGOS_IR	= new Combo($conn,"fIdIr","idRangoIr",$conn->Concat("rangoSup" , '" ---- "' , "rangoInf"),"Descripcion","rangos_ir","",constant("SLC_OPCION"),"","","fecMod");
	$comboRANGOS_IP	= new Combo($conn,"fIdIp","idRangoIp",$conn->Concat("rangoSup" , '" ---- "' , "rangoInf"),"Descripcion","rangos_ip","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['rangos_textos_next_page']) && $_POST['rangos_textos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'rangos_textos');
					...
					$lista=$pager->getRS();
					include('Template/Rangos_textos/mntrangos_textosl.php');
				}else{
					$cEntidad	= new Rangos_textos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Rangos_textos/mntrangos_textosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Rangos_textos/mntrangos_textos.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setIdIr((isset($_POST["fIdIr"])) ? $_POST["fIdIr"] : "");
		$cEntidad->setIdIp((isset($_POST["fIdIp"])) ? $_POST["fIdIp"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		global $comboINFORMES_PRUEBAS;
		$cEntidad->setIdTipoInforme((isset($_POST["LSTIdTipoInforme"]) && $_POST["LSTIdTipoInforme"] != "") ? $_POST["LSTIdTipoInforme"] : "");	$cEntidad->setBusqueda(constant("STR_TIPO_INFORME"), (isset($_POST["LSTIdTipoInforme"]) && $_POST["LSTIdTipoInforme"] != "" ) ? $comboINFORMES_PRUEBAS->getDescripcionCombo($_POST["LSTIdTipoInforme"]) : "");
		global $comboRANGOS_IR;
		$cEntidad->setIdIr((isset($_POST["LSTIdIr"]) && $_POST["LSTIdIr"] != "") ? $_POST["LSTIdIr"] : "");	$cEntidad->setBusqueda(constant("STR_IR"), (isset($_POST["LSTIdIr"]) && $_POST["LSTIdIr"] != "" ) ? $comboRANGOS_IR->getDescripcionCombo($_POST["LSTIdIr"]) : "");
		global $comboRANGOS_IP;
		$cEntidad->setIdIp((isset($_POST["LSTIdIp"]) && $_POST["LSTIdIp"] != "") ? $_POST["LSTIdIp"] : "");	$cEntidad->setBusqueda(constant("STR_IP"), (isset($_POST["LSTIdIp"]) && $_POST["LSTIdIp"] != "" ) ? $comboRANGOS_IP->getDescripcionCombo($_POST["LSTIdIp"]) : "");
		...
		$cEntidad->setTextoHast((isset($_POST["LSTTextoHast"]) && $_POST["LSTTextoHast"] != "") ? $_POST["LSTTextoHast"] : "");	$cEntidad->setBusqueda(constant("STR_TEXTO") . " " . constant("STR_HASTA"), (isset($_POST["LSTTextoHast"]) && $_POST["LSTTextoHast"] != "" ) ? $_POST["LSTTextoHast"] : "");
