...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Nivelesjerarquicos/NivelesjerarquicosDB.php");
	require_once(constant("DIR_WS_COM") . "Nivelesjerarquicos/Nivelesjerarquicos.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new NivelesjerarquicosDB($conn);  // Entidad DB
	$cEntidad	= new Nivelesjerarquicos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['nivelesjerarquicos_next_page']) && $_POST['nivelesjerarquicos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'nivelesjerarquicos');
					...
					$lista=$pager->getRS();
					include('Template/Nivelesjerarquicos/mntnivelesjerarquicosl.php');
				}else{
					$cEntidad	= new Nivelesjerarquicos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Nivelesjerarquicos/mntnivelesjerarquicosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Nivelesjerarquicos/mntnivelesjerarquicos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdNivel((isset($_POST["LSTIdNivel"]) && $_POST["LSTIdNivel"] != "") ? $_POST["LSTIdNivel"] : "");	$cEntidad->setBusqueda(constant("STR_ID_NIVEL"), (isset($_POST["LSTIdNivel"]) && $_POST["LSTIdNivel"] != "" ) ? $_POST["LSTIdNivel"] : "");
		$cEntidad->setIdNivelHast((isset($_POST["LSTIdNivelHast"]) && $_POST["LSTIdNivelHast"] != "") ? $_POST["LSTIdNivelHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_NIVEL") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdNivelHast"]) && $_POST["LSTIdNivelHast"] != "" ) ? $_POST["LSTIdNivelHast"] : "");
