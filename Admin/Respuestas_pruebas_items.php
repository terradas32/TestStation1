...
//	echo('modo:' . $_POST['MODO']);
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					$lista=$pager->getRS();
					include('Template/Respuestas_pruebas_items/mntrespuestas_pruebas_itemsl.php');
				}else{
					$cEntidad	= new Respuestas_pruebas_items();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Respuestas_pruebas_items/mntrespuestas_pruebas_itemsa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Respuestas_pruebas_items/mntrespuestas_pruebas_items.php');
			break;
		case constant("MNT_LISTAR"):
			...
			$cOpcionesDB	= new OpcionesDB($conn);  // Entidad DB
			$cItemsDB	= new ItemsDB($conn);  // Entidad DB
			$sValor="";
			while (!$lista->EOF)
			{
				$cOpciones	= new Opciones();  // Entidad			
				$cOpciones->setCodIdiomaIso2($lista->fields['codIdiomaIso2']);
				$cOpciones->setIdPrueba($lista->fields['idPrueba']);
				$cOpciones->setIdItem($lista->fields['idItem']);
				$cOpciones->setIdOpcion($lista->fields['idOpcion']);
				$cOpciones = $cOpcionesDB->readEntidad($cOpciones);
				
				$cItems	= new Items();  // Entidad			
				$cItems->setCodIdiomaIso2($lista->fields['codIdiomaIso2']);
				$cItems->setIdPrueba($lista->fields['idPrueba']);
				$cItems->setIdItem($lista->fields['idItem']);
				$cItems = $cItemsDB->readEntidad($cItems);
				$sValor="";
				$sValor=$cUtilidades->getValorCalculadoPRUEBAS($lista, $cItems, $conn);
				//Monto la query de UPDATE
				$sWHERE = ""; 
				$sWHERE .= " WHERE idEmpresa='" . $lista->fields['idEmpresa'] . "' ";
				$sWHERE .= " AND idProceso='" . $lista->fields['idProceso'] . "' ";
				$sWHERE .= " AND idCandidato='" . $lista->fields['idCandidato'] . "' ";
				$sWHERE .= " AND codIdiomaIso2='" . $lista->fields['codIdiomaIso2'] . "' ";
				$sWHERE .= " AND idPrueba='" . $lista->fields['idPrueba'] . "' ";
				$sWHERE .= " AND idItem='" . $lista->fields['idItem'] . "' ";
				$sWHERE .= " AND idOpcion='" . $lista->fields['idOpcion'] . "' ";
		
				$sSQL = " UPDATE respuestas_pruebas_items SET ";
				$sSQL .= " valor='" . $sValor . "', ";
				$sSQL .= " codigo='" . $cOpciones->getCodigo() . "'";
				$sSQL .= $sWHERE;
				if ( $lista->fields['idPrueba'] != 13){
					// Si no es la CML
					$conn->Execute($sSQL);
				}
...
//				echo "<br />" . $sSQL;
		...
		default:
//			$cEntidad->setCodIdiomaIso2($sLang);
			$cEntidad->setOrderBy("idEmpresa,idProceso,codIdiomaIso2,idPrueba,idCandidato,orden");
			$cEntidad->setBusqueda(constant("STR_ORDENAR_POR"), "Varios");
