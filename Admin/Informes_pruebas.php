...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Informes_pruebasDB($conn);  // Entidad DB
	$cEntidad	= new Informes_pruebas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php
				...
				{
					if (!empty($_POST['informes_pruebas_next_page']) && $_POST['informes_pruebas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'informes_pruebas');
					...
					$lista=$pager->getRS();
					include('Template/Informes_pruebas/mntinformes_pruebasl.php');
				}else{
					...
					$cEntidad	= new Informes_pruebas();  // inicializamos la Entidad
					...
					include('Template/Informes_pruebas/mntinformes_pruebasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Informes_pruebas/mntinformes_pruebas.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setPrecio((isset($_POST["fPrecio"])) ? $_POST["fPrecio"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setPrecio((isset($_POST["LSTPrecio"]) && $_POST["LSTPrecio"] != "") ? $_POST["LSTPrecio"] : "");	$cEntidad->setBusqueda("Precio", (isset($_POST["LSTPrecio"]) && $_POST["LSTPrecio"] != "" ) ? $_POST["LSTPrecio"] : "");
