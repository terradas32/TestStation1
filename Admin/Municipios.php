...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Municipios/MunicipiosDB.php");
	require_once(constant("DIR_WS_COM") . "Municipios/Municipios.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new MunicipiosDB($conn);  // Entidad DB
	$cEntidad	= new Municipios();  // Entidad
	...
	$comboWI_PAISES	= new Combo($conn,"fIdPais","idPais","descripcion","Descripcion","wi_paises","",constant("SLC_OPCION"),"","","fecMod");
	$comboWI_CCAA	= new Combo($conn,"fIdComunidad","idComunidad","descripcion","Descripcion","wi_ccaa","",constant("SLC_OPCION"),"","","fecMod");
	$comboWI_PROVINCIAS	= new Combo($conn,"fIdProvincia","idProvincia","descripcion","Descripcion","wi_provincias","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['municipios_next_page']) && $_POST['municipios_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'municipios');
					...
					$lista=$pager->getRS();
					include('Template/Municipios/mntmunicipiosl.php');
				}else{
					$cEntidad	= new Municipios();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Municipios/mntmunicipiosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Municipios/mntmunicipios.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setIdComunidad((isset($_POST["fIdComunidad"])) ? $_POST["fIdComunidad"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdMunicipio((isset($_POST["LSTIdMunicipio"]) && $_POST["LSTIdMunicipio"] != "") ? $_POST["LSTIdMunicipio"] : "");	$cEntidad->setBusqueda(constant("STR_ID_MUNICIPIO"), (isset($_POST["LSTIdMunicipio"]) && $_POST["LSTIdMunicipio"] != "" ) ? $_POST["LSTIdMunicipio"] : "");
		...
		global $comboWI_CCAA;
		$cEntidad->setIdComunidad((isset($_POST["LSTIdComunidad"]) && $_POST["LSTIdComunidad"] != "") ? $_POST["LSTIdComunidad"] : "");	$cEntidad->setBusqueda(constant("STR_COMUNIDAD"), (isset($_POST["LSTIdComunidad"]) && $_POST["LSTIdComunidad"] != "" ) ? $comboWI_CCAA->getDescripcionCombo($_POST["LSTIdComunidad"]) : "");
