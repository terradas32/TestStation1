...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Empresas_conf_tpvDB($conn);  // Entidad DB
	$cEntidad	= new Empresas_conf_tpv();  // Entidad
	
	$_cEntidadUsuarioTK = getUsuarioToken($conn);
	$sCol1='';
	$sCol2='';
	
	$comboEMPRESAS	= new Combo($conn,"fIdEmpresa","idEmpresa","nombre","Descripcion","empresas","",constant("SLC_OPCION"),"","","fecMod");
	$comboTIPOS_TPV	= new Combo($conn,"fIdTipoTpv","idTipoTpv","descripcion","Descripcion","tipos_tpv","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php
				...
				{
					if (!empty($_POST['empresas_conf_tpv_next_page']) && $_POST['empresas_conf_tpv_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'empresas_conf_tpv');
					...
					$lista=$pager->getRS();
					include('Template/Empresas_conf_tpv/mntempresas_conf_tpvl.php');
				}else{
					$cEntidad	= new Empresas_conf_tpv();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Empresas_conf_tpv/mntempresas_conf_tpva.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Empresas_conf_tpv/mntempresas_conf_tpv.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setBUSINESS_NAME((isset($_POST["fBUSINESS_NAME"])) ? $_POST["fBUSINESS_NAME"] : "");
		$cEntidad->setBUSINESS_CODE((isset($_POST["fBUSINESS_CODE"])) ? $_POST["fBUSINESS_CODE"] : "");
		$cEntidad->setBUSINESS_PASS((isset($_POST["fBUSINESS_PASS"])) ? $_POST["fBUSINESS_PASS"] : "");
		$cEntidad->setBUSINESS_COINC((isset($_POST["fBUSINESS_COINC"])) ? $_POST["fBUSINESS_COINC"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		global $comboTIPOS_TPV;
		$cEntidad->setIdTipoTpv((isset($_POST["LSTIdTipoTpv"]) && $_POST["LSTIdTipoTpv"] != "") ? $_POST["LSTIdTipoTpv"] : "");	$cEntidad->setBusqueda(constant("STR_TIPO_TPV"), (isset($_POST["LSTIdTipoTpv"]) && $_POST["LSTIdTipoTpv"] != "" ) ? $comboTIPOS_TPV->getDescripcionCombo($_POST["LSTIdTipoTpv"]) : "");
		$cEntidad->setBUSINESS_NAME((isset($_POST["LSTBUSINESS_NAME"]) && $_POST["LSTBUSINESS_NAME"] != "") ? $_POST["LSTBUSINESS_NAME"] : "");	$cEntidad->setBusqueda(constant("STR_BUSINESS_NAME"), (isset($_POST["LSTBUSINESS_NAME"]) && $_POST["LSTBUSINESS_NAME"] != "" ) ? $_POST["LSTBUSINESS_NAME"] : "");
		$cEntidad->setBUSINESS_CODE((isset($_POST["LSTBUSINESS_CODE"]) && $_POST["LSTBUSINESS_CODE"] != "") ? $_POST["LSTBUSINESS_CODE"] : "");	$cEntidad->setBusqueda(constant("STR_BUSINESS_CODE"), (isset($_POST["LSTBUSINESS_CODE"]) && $_POST["LSTBUSINESS_CODE"] != "" ) ? $_POST["LSTBUSINESS_CODE"] : "");
		$cEntidad->setBUSINESS_PASS((isset($_POST["LSTBUSINESS_PASS"]) && $_POST["LSTBUSINESS_PASS"] != "") ? $_POST["LSTBUSINESS_PASS"] : "");	$cEntidad->setBusqueda(constant("STR_BUSINESS_PASS"), (isset($_POST["LSTBUSINESS_PASS"]) && $_POST["LSTBUSINESS_PASS"] != "" ) ? $_POST["LSTBUSINESS_PASS"] : "");
		$cEntidad->setBUSINESS_COINC((isset($_POST["LSTBUSINESS_COINC"]) && $_POST["LSTBUSINESS_COINC"] != "") ? $_POST["LSTBUSINESS_COINC"] : "");	$cEntidad->setBusqueda(constant("STR_BUSINESS_COINC"), (isset($_POST["LSTBUSINESS_COINC"]) && $_POST["LSTBUSINESS_COINC"] != "" ) ? $_POST["LSTBUSINESS_COINC"] : "");
