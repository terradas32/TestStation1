...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Escalas/EscalasDB.php");
	require_once(constant("DIR_WS_COM") . "Escalas/Escalas.php");
	
include_once ('include/conexion.php');
	
	$cUtilidades	= new Utilidades();
	
	$cEntidadDB	= new EscalasDB($conn);  // Entidad DB
	$cEntidad	= new Escalas();  // Entidad
	...
	$comboBLOQUES	= new Combo($conn,"fIdBloque","idBloque","nombre","Descripcion","bloques","",constant("SLC_OPCION"),"codIdiomaIso2=" . $conn->qstr($sLang, false),"","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['escalas_next_page']) && $_POST['escalas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'escalas');
					...
					$lista=$pager->getRS();
					include('Template/Escalas/mntescalasl.php');
				}else{
					//$cEntidad	= new Escalas();  // inicializamos la Entidad
					$cEntidad->setNombre("");
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Escalas/mntescalasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Escalas/mntescalas.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdEscala((isset($_POST["LSTIdEscala"]) && $_POST["LSTIdEscala"] != "") ? $_POST["LSTIdEscala"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ESCALA"), (isset($_POST["LSTIdEscala"]) && $_POST["LSTIdEscala"] != "" ) ? $_POST["LSTIdEscala"] : "");
		$cEntidad->setIdEscalaHast((isset($_POST["LSTIdEscalaHast"]) && $_POST["LSTIdEscalaHast"] != "") ? $_POST["LSTIdEscalaHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ESCALA") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdEscalaHast"]) && $_POST["LSTIdEscalaHast"] != "" ) ? $_POST["LSTIdEscalaHast"] : "");
