...
//	echo('modo:' . $_POST['MODO']);
	...
	switch ($sPaso)
	{
		case 1:
			...
			if (!empty($sDesde)){
				$sql.=" AND c.fechaFinalizado >= '" . $sDesde . " 00:00:00' ";
			}
			if (!empty($sHasta)){
				$sql.=" AND c.fechaFinalizado <= '" . $sHasta . " 23:59:59' ";
			}
			if (!empty($sProceso)){
			...
			if ($rsRESPUESTAS_PRUEBAS->RecordCount() > 0 ){
				...
				$sMensaje .= '<script language="javascript" type="text/javascript">setTimeout(function(){enviar(); }, 10000);</script>';
			}else {
		...
		ini_set("default_socket_timeout", "6000");
			...
			if (sizeof($aSQLD) > 0){
				...
				$k=0;
				$sSQLDAux="";
				for ($m=0, $maxm = sizeof($aSQLD); $m < $maxm; $m++){
					if ($k > 1500){
						$sSQLDAux = substr($sSQLDAux, 0, strlen($sSQLDAux)-1) . ";";
						if($conn->Execute($sSQL . $sSQLDAux) === false){
							$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Expecial_export]";
							error_log($sTypeError . " ->\t" . $sSQL . $sSQLDAux . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
...
//		echo "<br />::->" . $ultima_pregunta;
		$iPercentil = getPercentil($puntuacion, $cProceso_informes->getIdBaremo(), $rowRESPUESTAS_PRUEBAS->fields['idPrueba']);
