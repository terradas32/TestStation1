...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new BloquesDB($conn);  // Entidad DB
	$cEntidad	= new Bloques();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['bloques_next_page']) && $_POST['bloques_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'bloques');
					...
					$lista=$pager->getRS();
					include('Template/Bloques/mntbloquesl.php');
				}else{
					$cEntidad	= new Bloques();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Bloques/mntbloquesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Bloques/mntbloques.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdBloque((isset($_POST["LSTIdBloque"]) && $_POST["LSTIdBloque"] != "") ? $_POST["LSTIdBloque"] : "");	$cEntidad->setBusqueda(constant("STR_ID_BLOQUE"), (isset($_POST["LSTIdBloque"]) && $_POST["LSTIdBloque"] != "" ) ? $_POST["LSTIdBloque"] : "");
		$cEntidad->setIdBloqueHast((isset($_POST["LSTIdBloqueHast"]) && $_POST["LSTIdBloqueHast"] != "") ? $_POST["LSTIdBloqueHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_BLOQUE") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdBloqueHast"]) && $_POST["LSTIdBloqueHast"] != "" ) ? $_POST["LSTIdBloqueHast"] : "");
