...
include_once ('include/conexion.php');
	...
	$comboSOLICITAR_UNIDADES_A 	= new	Combo($conn,"fSolicitarUnidadesA","idEmpresa","nombre","Descripcion","empresas",""," ","idEmpresa IN(" . $sHijos . ")","","orden");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			$cEntidadTPV->setUsuAlta($cEntidad->getUsuAlta());
			$cEntidadTPV->setUsuMod($cEntidad->getUsuMod());
			...
			if (!empty($newId)){
				...
				//Inicio de revisar baremos para la empresa
				...
				while (!$rsPRUEBASGROUP->EOF)
				{
					...
					while(!$listaBaremos->EOF){
						$cBaremos_empresas = new Baremos_empresas();
						$cBaremos_empresas->setIdEmpresa($newId);
						...
						//echo $sqlBaremos_empresas . "<br />";
						$listaBaremos_empresas = $conn->Execute($sqlBaremos_empresas);

						if ($listaBaremos_empresas->recordCount() > 0 )
						{
							//Miramos Si ahora sigue teniendo el baremo activo para Esta Empresa
							$bActivoBar = false;
							if (!empty($_POST["fIdBarPrue" . $k . $rsPRUEBASGROUP->fields[$sAsIdPRUEBASGROUP]])){
								//viene chequeado
								//echo "<br /> A:: Viene chequeado";
								$bActivoBar=true;
							}
							if (!$bActivoBar){
								//Ahora no lo tiene quequeado
								//echo "<br />A:: Ahora no lo tiene quequeado";
								//Sacamos a la empresa
								$cBaremos_empresasDB->borrar($cBaremos_empresas);
							}
						}else{
							//Miramos si hay que añadir la Empresa
							$bActivoBar = false;
							if (!empty($_POST["fIdBarPrue" . $k . $rsPRUEBASGROUP->fields[$sAsIdPRUEBASGROUP]])){
								//viene chequeado
								//echo "<br /> B:: Viene chequeado";
								$bActivoBar=true;
							}
							if ($bActivoBar){
								//Añadimos la empresa
								//echo "<br />B:: Añadimos la empresa";
								$cBaremos_empresas->setUsuAlta($_cEntidadUsuarioTK->getIdUsuario());
		...
		case constant("MNT_MODIFICAR"):
			...
			$cEntidadTPV->setIdTipoTpv($cEntidad->getIdTipoTpv());
			if ($cEntidadTPV->getIdEmpresa() != "" && $cEntidadTPV->getIdTipoTpv() != ""){
				$cEntidadTPV_OLD	= new Empresas_conf_tpv();  // Entidad
				$cEntidadTPV_OLD->setIdEmpresa($cEntidad->getIdEmpresa());
				$cEntidadTPV_OLD->setIdTipoTpv($cEntidad->getIdTipoTpv());

				$cEntidadTPV_OLD = $cEntidadTPVDB->readEntidad($cEntidadTPV_OLD);
				if ($cEntidadTPV_OLD->getBUSINESS_NAME() != ""){
					$bOkTpv = $cEntidadTPVDB->modificar($cEntidadTPV);
				}else {
					$cEntidadTPV->setIdEmpresa($cEntidad->getIdEmpresa());
					$cEntidadTPV->setIdTipoTpv($cEntidad->getIdTipoTpv());
					$newIdTpv = $cEntidadTPVDB->insertar($cEntidadTPV);
...
//			$cEntidad->setBusqueda(constant("STR_ORDEN"), "ASC");
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidadTPV->setIdTipoTpv((isset($_POST["fIdTipoTpv"])) ? $_POST["fIdTipoTpv"] : "");
		$cEntidadTPV->setBUSINESS_NAME((isset($_POST["fBUSINESS_NAME"])) ? $_POST["fBUSINESS_NAME"] : "");
		$cEntidadTPV->setBUSINESS_CODE((isset($_POST["fBUSINESS_CODE"])) ? $_POST["fBUSINESS_CODE"] : "");
		$cEntidadTPV->setBUSINESS_PASS((isset($_POST["fBUSINESS_PASS"])) ? $_POST["fBUSINESS_PASS"] : "");
		$cEntidadTPV->setBUSINESS_COINC((isset($_POST["fBUSINESS_COINC"])) ? $_POST["fBUSINESS_COINC"] : "");
		...
		$cEntidad->setVerCorreo((isset($_POST["fVerCorreo"])) ? $_POST["fVerCorreo"] : "");
		$cEntidad->setAltaCiega((isset($_POST["fAltaCiega"])) ? $_POST["fAltaCiega"] : "");
		$cEntidad->setAltaPrecargada((isset($_POST["fAltaPrecargada"])) ? $_POST["fAltaPrecargada"] : "");
		$cEntidad->setProcesoConfidencial((isset($_POST["fProcesoConfidencial"])) ? $_POST["fProcesoConfidencial"] : "");

		$cEntidad->setCandidatosRepetidores((isset($_POST["fCandidatosRepetidores"])) ? $_POST["fCandidatosRepetidores"] : "");


		$cEntidad->setDescuentaMatriz((isset($_POST["fDescuentaMatriz"])) ? $_POST["fDescuentaMatriz"] : "");
		$cEntidad->setSolicitarUnidadesA((isset($_POST["fSolicitarUnidadesA"])) ? $_POST["fSolicitarUnidadesA"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdTipoTpv((isset($_POST["LSTIdTipoTpv"]) && $_POST["LSTIdTipoTpv"] != "") ? $_POST["LSTIdTipoTpv"] : "");	$cEntidad->setBusqueda("IdTipoTpv", (isset($_POST["LSTIdTipoTpv"]) && $_POST["LSTIdTipoTpv"] != "" ) ? $_POST["LSTIdTipoTpv"] : "");

		$cEntidadTPV->setBUSINESS_NAME((isset($_POST["LSTBUSINESS_NAME"]) && $_POST["LSTBUSINESS_NAME"] != "") ? $_POST["LSTBUSINESS_NAME"] : "");	$cEntidad->setBusqueda("BUSINESS_NAME", (isset($_POST["LSTBUSINESS_NAME"]) && $_POST["LSTBUSINESS_NAME"] != "" ) ? $_POST["LSTBUSINESS_NAME"] : "");
		$cEntidadTPV->setBUSINESS_CODE((isset($_POST["LSTBUSINESS_CODE"]) && $_POST["LSTBUSINESS_CODE"] != "") ? $_POST["LSTBUSINESS_CODE"] : "");	$cEntidad->setBusqueda("BUSINESS_CODE", (isset($_POST["LSTBUSINESS_CODE"]) && $_POST["LSTBUSINESS_CODE"] != "" ) ? $_POST["LSTBUSINESS_CODE"] : "");
		$cEntidadTPV->setBUSINESS_PASS((isset($_POST["LSTBUSINESS_PASS"]) && $_POST["LSTBUSINESS_PASS"] != "") ? $_POST["LSTBUSINESS_PASS"] : "");	$cEntidad->setBusqueda("BUSINESS_PASS", (isset($_POST["LSTBUSINESS_PASS"]) && $_POST["LSTBUSINESS_PASS"] != "" ) ? $_POST["LSTBUSINESS_PASS"] : "");
		$cEntidadTPV->setBUSINESS_COINC((isset($_POST["LSTBUSINESS_COINC"]) && $_POST["LSTBUSINESS_COINC"] != "") ? $_POST["LSTBUSINESS_COINC"] : "");	$cEntidad->setBusqueda("BUSINESS_COINC", (isset($_POST["LSTBUSINESS_COINC"]) && $_POST["LSTBUSINESS_COINC"] != "" ) ? $_POST["LSTBUSINESS_COINC"] : "");
