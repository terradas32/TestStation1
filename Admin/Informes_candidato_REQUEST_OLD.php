...
if ($_proceso == ""){
	...
	// fIdBaremo
	$_bIsProceso = true;
	$_REQUEST['MODO'] = $argv[2];
	$_REQUEST['fIdTipoInforme'] = $argv[3];
	$_REQUEST['fCodIdiomaIso2'] = $argv[4];
	$_REQUEST['fIdPrueba'] = $argv[5];
	$_REQUEST['fIdEmpresa'] = $argv[6];
	$_REQUEST['fIdProceso'] = $argv[7];
	$_REQUEST['fIdCandidato'] = $argv[8];
	$_REQUEST['fCodIdiomaIso2Prueba'] = $argv[9];
	$_REQUEST['fIdBaremo'] = $argv[10];
...
include_once ('include/conexion.php');
	...
	if (isset($_REQUEST['fIdBaremo']) && isset($_REQUEST['fIdPrueba']))
	{
		$cBaremos = new Baremos();
		$cBaremosDB = new BaremosDB($conn);
		$cBaremos->setIdBaremo(isset($_REQUEST['fIdBaremo']) ? $_REQUEST['fIdBaremo'] : "1");
		$cBaremos->setIdPrueba($_REQUEST['fIdPrueba']);
		$cBaremos = $cBaremosDB->readEntidad($cBaremos);
		$_sBaremo = $cBaremos->getNombre();
	}	
	$sHijos = "";
	if (empty($_REQUEST["fHijos"]))
	{
		require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . 'Empresas/Empresas.php');
		require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . 'Empresas/EmpresasDB.php');
	...
	$sLlamada="Candidato";
		if (!empty($_SERVER["HTTP_REFERER"])){
			if (strpos($_SERVER["HTTP_REFERER"], '/Admin/') === false) {
				$_EmpresaLogada = $_REQUEST['fIdEmpresa'];
				$sLlamada="Empresa";
	...
	switch ($_REQUEST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				if ($_REQUEST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					$cEntidad = readLista($cEntidad);
					if ($cEntidad->getIdEmpresa() == ""){
						$cEntidad->setIdEmpresa($sHijos);
					}
					if (!empty($_REQUEST['respuestas_pruebas_next_page']) && $_REQUEST['respuestas_pruebas_next_page'] > 1 ){
						$bInit=false;
					...
					$_REQUEST['MODO']    = constant("MNT_ALTA");
		...
		case constant("MNT_MODIFICAR"):
			...
			if ($cEntidadDB->modificar($cEntidad))
			{
				...
				?><script language="javascript" type="text/javascript">alert("<?php echo constant("ERR_FORM_ERROR");?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				$_REQUEST['MODO']=constant("MNT_MODIFICAR");
				include('Template/Informes_candidato/mntrespuestas_pruebasa.php');
			}
			break;
		...
		case constant("MNT_NUEVO"):
			...
			$_REQUEST["LSTOrderBy"] = "fecMod";
			$_REQUEST["LSTOrder"] = "DESC";
			$_REQUEST['MODO']    = constant("MNT_ALTA");
			include('Template/Informes_candidato/mntrespuestas_pruebasa.php');
			break;
		...
		case constant("MNT_BUSCAR"):
			$_REQUEST['MODO']    = constant("MNT_LISTAR");
			$cEntidad = readLista($cEntidad);
		...
		case constant("MNT_LISTAR"):
			...
			if (isset($_REQUEST["fReordenar"]) &&  !empty($_REQUEST["fReordenar"])){
				$bInit=true;
				$sql = $cEntidadDB->readListaIN($cEntidad);
			}else{
				if (!empty($_REQUEST['respuestas_pruebas_next_page']) && $_REQUEST['respuestas_pruebas_next_page'] > 1){
		...
		case constant("MNT_LISTAIDIOMAS"):
			...
			if((isset($_REQUEST['fIdPrueba']) && $_REQUEST['fIdPrueba']!="") && (isset($_REQUEST['fIdTipoInforme']) && $_REQUEST['fIdTipoInforme']!="")){
				$cInformes_pruebas	= new Informes_pruebas();
				$cInformes_pruebas->setIdPrueba($_REQUEST['fIdPrueba']);
				$cInformes_pruebas->setIdTipoInforme($_REQUEST['fIdTipoInforme']);
		...
		case constant("MNT_LISTATIPOS"):
			...
			if(isset($_REQUEST['fIdPrueba']) && $_REQUEST['fIdPrueba']!=""){
				$cInformes_pruebas	= new Informes_pruebas();
				$cInformes_pruebas->setIdPrueba($_REQUEST['fIdPrueba']);
		...
		case constant("MNT_LISTABAREMOS"):
			if(isset($_REQUEST['fIdPrueba']) && $_REQUEST['fIdPrueba']!=""){
				...
				$cEscalas_items->setIdPrueba($_REQUEST['fIdPrueba']);
				...
				$cBaremos	= new Baremos();
				$cBaremos->setIdPrueba($_REQUEST['fIdPrueba']);
				$cBaremos->setIdPruebaHast($_REQUEST['fIdPrueba']);
		...
		case constant("MNT_EXPORTA"):
    			...
    			$cInformesPruebasTrf->setIdPrueba($_REQUEST['fIdPrueba']);
    			$cInformesPruebasTrf->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2']);
    			$cInformesPruebasTrf->setIdTipoInforme($_REQUEST['fIdTipoInforme']);
    			$cInformesPruebasTrf->setIdEmpresa($_REQUEST['fIdEmpresa']);
				...
				$cEmpresaDng->setIdEmpresa($_REQUEST['fIdEmpresa']);
...
//				echo "<br />HTTP_REFERER::" . $_SERVER["HTTP_REFERER"];
				$cCandidato = new Candidatos();
				$cCandidato->setIdEmpresa($_REQUEST['fIdEmpresa']);
				$cCandidato->setIdProceso($_REQUEST['fIdProceso']);
				$cCandidato->setIdCandidato($_REQUEST['fIdCandidato']);
				
				$cCandidato =  $cCandidatosDB->readEntidad($cCandidato);
				
				$cPruebas = new Pruebas();
				$cPruebas->setIdPrueba($_REQUEST['fIdPrueba']);
				$cPruebas->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
				...
				if ($_sPrepago == 0 || $_EmpresaLogada == constant("EMPRESA_PE") || $sLlamada == "Candidato"){
					...
					$sNombre = $cUtilidades->SEOTitulo($cPruebas->getNombre() . "_" . $cCandidato->getNombre() . "_" . $cCandidato->getApellido1() . "_" . $cCandidato->getMail() . "_" . $_REQUEST['fIdEmpresa']. "_" .$_REQUEST['fIdProceso'] . "_" . $_REQUEST['fIdTipoInforme'] . "_" . $_REQUEST['fCodIdiomaIso2'] . "_" . $_REQUEST['fIdBaremo']);
					$sDirImg="imgInformes/";
...
//					$sNombre = $cUtilidades->SEOTitulo($cPruebas->getNombre() . "_" . $cCandidato->getNombre() . "_" . $cCandidato->getApellido1() . "_" . $cCandidato->getMail() . "_" . $_REQUEST['fIdEmpresa']. "_" .$_REQUEST['fIdProceso'] . "_" . $_REQUEST['fIdTipoInforme'] . "_" . $_REQUEST['fCodIdiomaIso2']);
					$sNombre = $cUtilidades->SEOTitulo($cPruebas->getNombre() . "_" . $cCandidato->getNombre() . "_" . $cCandidato->getApellido1() . "_" . $cCandidato->getMail() . "_" . $_REQUEST['fIdEmpresa']. "_" .$_REQUEST['fIdProceso'] . "_" . $_REQUEST['fIdTipoInforme'] . "_" . $_REQUEST['fCodIdiomaIso2'] . "_" . $_REQUEST['fIdBaremo']);
					$sDirImg="imgInformes/";
					$spath = (substr(constant("DIR_FS_DOCUMENT_ROOT"), -1, 1) != '/') ? constant("DIR_FS_DOCUMENT_ROOT") . '/' : constant("DIR_FS_DOCUMENT_ROOT");
					$_ficheroPDF = $spath . $sDirImg . $sNombre . ".pdf";
					...
					if (!$bPDFGenerado)
					{
						...
						$idTipoInforme=$_REQUEST['fIdTipoInforme'];
						
						$cRespuestasPruebasItems = new Respuestas_pruebas_items();
						
						$cRespuestasPruebasItems->setIdCandidato($_REQUEST['fIdCandidato']);
						$cRespuestasPruebasItems->setIdPrueba($_REQUEST['fIdPrueba']);
						$cRespuestasPruebasItems->setIdEmpresa($_REQUEST['fIdEmpresa']);
						$cRespuestasPruebasItems->setIdProceso($_REQUEST['fIdProceso']);
						$cRespuestasPruebasItems->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
						$cRespuestasPruebasItems->setOrderBy("idItem");
						$cRespuestasPruebasItems->setOrder("ASC");
						
						$cIt = new Items();
						$cIt->setIdPrueba($_REQUEST['fIdPrueba']);
						$cIt->setIdPruebaHast($_REQUEST['fIdPrueba']);
						$cIt->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
						...
						if($listaRespItems->recordCount()>0)
						{
							while(!$listaRespItems->EOF){
								...
								$cItem->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
								...
								$cOpcion->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
								...
								if(strtoupper($cItem->getCorrecto()) == strtoupper($cOpcion->getCodigo())){
									...
									$sSQLValor .= " WHERE";
									$sSQLValor .= " idEmpresa='" . $_REQUEST['fIdEmpresa'] . "'";
									$sSQLValor .= " AND idProceso='" . $_REQUEST['fIdProceso'] . "'";
									$sSQLValor .= " AND idCandidato='" . $_REQUEST['fIdCandidato'] . "'";
									$sSQLValor .= " AND codIdiomaIso2='" . $_REQUEST['fCodIdiomaIso2Prueba'] . "'";
...
//									echo "<br />//A*****--->correcta:: " . $sSQLValor;
							...
							$cBaremos_resultados->setIdBaremo($_REQUEST['fIdBaremo']);
							$cBaremos_resultados->setIdPrueba($_REQUEST['fIdPrueba']);
...
// 							echo "pPercentil: " . $iPercentil . "<br />";
							...
							include('constantesInformes/' .	$_REQUEST['fCodIdiomaIso2'] .'.php');
...
//							echo "<br />TIPO::" . $cPruebas->getIdTipoPrueba();
							switch ($cPruebas->getIdTipoPrueba())
							{
								...
								case 11:	//FLASH tipo Personalidad
									...
									if ($sLlamada == "Candidato" && ($cPruebas->getIdTipoPrueba() == 7 ) )
 									{
										if (isset($aSQLPuntuacionesPPL) || isset($aSQLPuntuacionesC))
										{
											...
											$sSQL = "INSERT INTO export_personalidad (idEmpresa, descEmpresa, idProceso, descProceso, idCandidato, nombre, apellido1, apellido2, email, dni, idPrueba, descPrueba, fecPrueba,idBaremo,descBaremo,fecAltaProceso, idSexo,idEdad, idFormacion, idNivel, idArea, fecAlta) VALUES ";
											$sSQL .= "(" . $conn->qstr($cRespPruebas->getIdEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getDescEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getIdProceso(), false) . "," . $conn->qstr($cRespPruebas->getDescProceso(), false) . "," . $conn->qstr($cRespPruebas->getIdCandidato(), false) . "," . $conn->qstr($cCandidato->getNombre(), false) . "," . $conn->qstr($cCandidato->getApellido1(), false) . "," . $conn->qstr($cCandidato->getApellido2(), false) . "," . $conn->qstr($cCandidato->getMail(), false) . "," . $conn->qstr($cCandidato->getDni(), false) . "," . $conn->qstr($cRespPruebas->getIdPrueba(), false) . "," . $conn->qstr($cRespPruebas->getDescPrueba(), false) . "," . $conn->qstr($cRespPruebas->getFecAlta(), false) . "," . $conn->qstr($_REQUEST["fIdBaremo"], false) . "," . $conn->qstr($_sBaremo, false) . "," . $conn->qstr($cProceso->getFecAlta(), false) . "," . $conn->qstr($cCandidato->getIdSexo(), false) . "," . $conn->qstr($cCandidato->getIdEdad(), false) . "," . $conn->qstr($cCandidato->getIdFormacion(), false) . "," . $conn->qstr($cCandidato->getIdNivel(), false) . "," . $conn->qstr($cCandidato->getIdArea(), false) . ",now());";
											//echo "<br />" . $sSQL;
											...
											$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Guardando puntuaciones [" . $_SERVER['REMOTE_ADDR'] . "][" . $sLlamada . "][" . constant("MNT_EXPORTA") . "] Sin calculos en informe: idEmpresa: " . $_REQUEST['fIdEmpresa'] . " idProceso: " . $_REQUEST['fIdProceso'] . " idCandidato: " . $_REQUEST['fIdCandidato'] . " idPrueba: " . $_REQUEST['fIdPrueba'];
											error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
										}
 									}
									break;
								default:
									...
									switch ($_REQUEST['fIdPrueba'])
									{
										...
										case 60:	//KPMG
											...
											break;
										case 8:	//ELT
											include('Template/Informes_candidato/generaPrueba' . $_REQUEST['fIdPrueba'] . '.php');
											break;
										case 32:	//CIP
											include('Template/Informes_candidato/generaTipo' . $idTipoInforme . '.php');
											
											//Para las de Personalidad
											if ($sLlamada == "Candidato" && ($cPruebas->getIdTipoPrueba() == 9 ) )
											{
												if (isset($aSQLPuntuacionesPPL) || isset($aSQLPuntuacionesC))
												{
									...
									if ($sLlamada == "Candidato" && ($cPruebas->getIdTipoPrueba() == 2 || $cPruebas->getIdTipoPrueba() == 5) ){
 									
 										//Se guradan los resultados calculados en respuestas pruebas
										...
										//$_sBaremo = $cBaremos->getNombre();
 										...
 										$sSQL = "INSERT INTO export_aptitudinales (idEmpresa, descEmpresa, idProceso, descProceso, idCandidato, nombre, apellido1, apellido2, email, dni, idPrueba, descPrueba, fecPrueba,idBaremo,descBaremo,fecAltaProceso, correctas, contestadas,percentil, ir, ip, por, estilo, idSexo,idEdad, idFormacion, idNivel, idArea, fecAlta) VALUES ";
 										$sSQL .= "(" . $conn->qstr($cRespPruebas->getIdEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getDescEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getIdProceso(), false) . "," . $conn->qstr($cRespPruebas->getDescProceso(), false) . "," . $conn->qstr($cRespPruebas->getIdCandidato(), false) . "," . $conn->qstr($cCandidato->getNombre(), false) . "," . $conn->qstr($cCandidato->getApellido1(), false) . "," . $conn->qstr($cCandidato->getApellido2(), false) . "," . $conn->qstr($cCandidato->getMail(), false) . "," . $conn->qstr($cCandidato->getDni(), false) . "," . $conn->qstr($cRespPruebas->getIdPrueba(), false) . "," . $conn->qstr($cRespPruebas->getDescPrueba(), false) . "," . $conn->qstr($cRespPruebas->getFecAlta(), false) . "," . $conn->qstr($_REQUEST["fIdBaremo"], false) . "," . $conn->qstr($_sBaremo, false) . "," . $conn->qstr($cProceso->getFecAlta(), false) . "," . $conn->qstr($iPDirecta, false) . "," . $conn->qstr($listaRespItems->recordCount(), false) . "," . $conn->qstr($iPercentil, false) . "," . $conn->qstr($IR, false) . "," . $conn->qstr($IP, false) . "," . $conn->qstr($POR, false) . "," . $conn->qstr($sRan_test, false) . "," . $conn->qstr($cCandidato->getIdSexo(), false) . "," . $conn->qstr($cCandidato->getIdEdad(), false) . "," . $conn->qstr($cCandidato->getIdFormacion(), false) . "," . $conn->qstr($cCandidato->getIdNivel(), false) . "," . $conn->qstr($cCandidato->getIdArea(), false) . ",now());";
 										$conn->Execute($sSQL);
 										//echo "<br />2::" . $sSQL;
...
// 										echo "<br />" . $sSQLUPDATE;
							...
							$dTotalCoste = getDescuentoDongles($cCandidato->getIdEmpresa(), $cCandidato->getIdProceso(), $cCandidato->getIdCandidato(), $cPruebas->getCodIdiomaIso2(), $cPruebas->getIdPrueba(), $cInformesPruebasTrf->getCodIdiomaIso2(), $cInformesPruebasTrf->getIdTipoInforme(), $_REQUEST['fIdBaremo'], $dTotalCoste, false);
		...
		//					print  ( $binDatosExcel);
							...
							if ($sLlamada == "Candidato" && ($cPruebas->getIdTipoPrueba() == 2 || $cPruebas->getIdTipoPrueba() == 5) ){
								...
								$cRespPruebas = new Respuestas_pruebas();
								$cRespPruebasDB = new Respuestas_pruebasDB($conn);
								$cRespPruebas->setIdEmpresa($_REQUEST['fIdEmpresa']);
								$cRespPruebas->setIdProceso($_REQUEST['fIdProceso']);
								$cRespPruebas->setIdCandidato($_REQUEST['fIdCandidato']);
								$cRespPruebas->setIdPrueba($_REQUEST['fIdPrueba']);
								$cRespPruebas->setCodIdiomaIso2($_REQUEST['fCodIdiomaIso2Prueba']);
								$cRespPruebas = $cRespPruebasDB->readEntidad($cRespPruebas);
							
								$cProceso = new Procesos();
								$cProcesoDB	= new ProcesosDB($conn);
								$cProceso->setIdEmpresa($_REQUEST['fIdEmpresa']);
								$cProceso->setIdProceso($_REQUEST['fIdProceso']);
								$cProceso = $cProcesoDB->readEntidad($cProceso);
								...
								$sSQL = "INSERT INTO export_aptitudinales (idEmpresa, descEmpresa, idProceso, descProceso, idCandidato, nombre, apellido1, apellido2, email, dni, idPrueba, descPrueba, fecPrueba,idBaremo,descBaremo,fecAltaProceso, correctas, contestadas,percentil, ir, ip, por, estilo, idSexo,idEdad, idFormacion, idNivel, idArea, fecAlta) VALUES ";
								$sSQL .= "(" . $conn->qstr($cRespPruebas->getIdEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getDescEmpresa(), false) . "," . $conn->qstr($cRespPruebas->getIdProceso(), false) . "," . $conn->qstr($cRespPruebas->getDescProceso(), false) . "," . $conn->qstr($cRespPruebas->getIdCandidato(), false) . "," . $conn->qstr($cCandidato->getNombre(), false) . "," . $conn->qstr($cCandidato->getApellido1(), false) . "," . $conn->qstr($cCandidato->getApellido2(), false) . "," . $conn->qstr($cCandidato->getMail(), false) . "," . $conn->qstr($cCandidato->getDni(), false) . "," . $conn->qstr($cRespPruebas->getIdPrueba(), false) . "," . $conn->qstr($cRespPruebas->getDescPrueba(), false) . "," . $conn->qstr($cRespPruebas->getFecAlta(), false) . "," . $conn->qstr($_REQUEST["fIdBaremo"], false) . "," . $conn->qstr($_sBaremo, false) . "," . $conn->qstr($cProceso->getFecAlta(), false) . "," . $conn->qstr($iPDirecta, false) . "," . $conn->qstr($contestadas, false) . "," . $conn->qstr($iPercentil, false) . "," . $conn->qstr($IR, false) . "," . $conn->qstr($IP, false) . "," . $conn->qstr($POR, false) . "," . $conn->qstr($sRan_test, false) . "," . $conn->qstr($cCandidato->getIdSexo(), false) . "," . $conn->qstr($cCandidato->getIdEdad(), false) . "," . $conn->qstr($cCandidato->getIdFormacion(), false) . "," . $conn->qstr($cCandidato->getIdNivel(), false) . "," . $conn->qstr($cCandidato->getIdArea(), false) . ",now());";
								$conn->Execute($sSQL);
								//echo "<br />1::" . $sSQL;
								$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Guardando puntuaciones [" . $_SERVER['REMOTE_ADDR'] . "][" . $sLlamada . "][" . constant("MNT_EXPORTA") . "] SQL: " . $sSQL;
								error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
...
//							echo "<br />D" . $sqlBaremosResultados . "<br />";
							...
							//Miramos si hay que descontar y cuanro para informar al usuario.
							$dTotalCoste = getDescuentoDongles($cCandidato->getIdEmpresa(), $cCandidato->getIdProceso(), $cCandidato->getIdCandidato(), $cPruebas->getCodIdiomaIso2(), $cPruebas->getIdPrueba(), $cInformesPruebasTrf->getCodIdiomaIso2(), $cInformesPruebasTrf->getIdTipoInforme(), $_REQUEST['fIdBaremo'], $dTotalCoste, true);
...
//						echo "<a href=\"#_\" title=\"" . constant("STR_DESCARGAR") . "\" onclick=\"javascript:abrirVentana('0','". base64_encode(constant('HTTP_SERVER'). $sDirImg . $sNombre)."');\">" . constant("STR_DESCARGAR") . "</a>";
	...
	*/
	function readEntidad($cEntidad){
		...
		global $cUtilidades;
		$cEntidad->setIdEmpresa((isset($_REQUEST["fIdEmpresa"])) ? $_REQUEST["fIdEmpresa"] : "");
		global $comboDESC_EMPRESAS;
		$sIdEmpresa = (isset($_REQUEST["fIdEmpresa"])  ? $_REQUEST["fIdEmpresa"] : "");
		$cEntidad->setDescEmpresa($comboDESC_EMPRESAS->getDescripcionCombo($sIdEmpresa));
		$cEntidad->setIdProceso((isset($_REQUEST["fIdProceso"])) ? $_REQUEST["fIdProceso"] : "");
		global $comboDESC_PROCESOS;
		$sIdProceso = (isset($_REQUEST["fIdProceso"])  ? $_REQUEST["fIdProceso"] : "");
		$cEntidad->setDescProceso($comboDESC_PROCESOS->getDescripcionCombo($sIdProceso));
		$cEntidad->setIdCandidato((isset($_REQUEST["fIdCandidato"])) ? $_REQUEST["fIdCandidato"] : "");
		global $comboDESC_CANDIDATOS;
		$sIdCandidato = (isset($_REQUEST["fIdCandidato"])  ? $_REQUEST["fIdCandidato"] : "");
		$cEntidad->setDescCandidato($comboDESC_CANDIDATOS->getDescripcionCombo($sIdCandidato));
		$cEntidad->setCodIdiomaIso2((isset($_REQUEST["fCodIdiomaIso2"])) ? $_REQUEST["fCodIdiomaIso2"] : "");
		global $comboDESC_WI_IDIOMAS;
		$sCodIdiomaIso2 = (isset($_REQUEST["fCodIdiomaIso2"])  ? $_REQUEST["fCodIdiomaIso2"] : "");
		$cEntidad->setDescIdiomaIso2($comboDESC_WI_IDIOMAS->getDescripcionCombo($sCodIdiomaIso2));
		$cEntidad->setIdPrueba((isset($_REQUEST["fIdPrueba"])) ? $_REQUEST["fIdPrueba"] : "");
		global $comboDESC_PRUEBAS;
		$sIdPrueba = (isset($_REQUEST["fIdPrueba"])  ? $_REQUEST["fIdPrueba"] : "");
		$cEntidad->setDescPrueba($comboDESC_PRUEBAS->getDescripcionCombo($sIdPrueba));
		$cEntidad->setFinalizado((isset($_REQUEST["fFinalizado"])) ? $_REQUEST["fFinalizado"] : "");
		$cEntidad->setLeidoInstrucciones((isset($_REQUEST["fLeidoInstrucciones"])) ? $_REQUEST["fLeidoInstrucciones"] : "");
		$cEntidad->setLeidoEjemplos((isset($_REQUEST["fLeidoEjemplos"])) ? $_REQUEST["fLeidoEjemplos"] : "");
		$cEntidad->setMinutos_test((isset($_REQUEST["fMinutos_test"])) ? $_REQUEST["fMinutos_test"] : "");
		$cEntidad->setSegundos_test((isset($_REQUEST["fSegundos_test"])) ? $_REQUEST["fSegundos_test"] : "");
		$cEntidad->setFecAlta((isset($_REQUEST["fFecAlta"])) ? $_REQUEST["fFecAlta"] : "");
		$cEntidad->setFecMod((isset($_REQUEST["fFecMod"])) ? $_REQUEST["fFecMod"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdEmpresa((isset($_REQUEST["LSTIdEmpresa"]) && $_REQUEST["LSTIdEmpresa"] != "") ? $_REQUEST["LSTIdEmpresa"] : "");	$cEntidad->setBusqueda(constant("STR_EMPRESA"), (isset($_REQUEST["LSTIdEmpresa"]) && $_REQUEST["LSTIdEmpresa"] != "" ) ? $comboEMPRESAS->getDescripcionCombo($_REQUEST["LSTIdEmpresa"]) : "");
		$cEntidad->setDescEmpresa((isset($_REQUEST["LSTDescEmpresa"]) && $_REQUEST["LSTDescEmpresa"] != "") ? $_REQUEST["LSTDescEmpresa"] : "");	$cEntidad->setBusqueda(constant("STR_EMPRESA"), (isset($_REQUEST["LSTDescEmpresa"]) && $_REQUEST["LSTDescEmpresa"] != "" ) ? $_REQUEST["LSTDescEmpresa"] : "");
		...
		$cEntidad->setIdProceso((isset($_REQUEST["LSTIdProceso"]) && $_REQUEST["LSTIdProceso"] != "") ? $_REQUEST["LSTIdProceso"] : "");	$cEntidad->setBusqueda(constant("STR_PROCESO"), (isset($_REQUEST["LSTIdProceso"]) && $_REQUEST["LSTIdProceso"] != "" ) ? $comboPROCESOS->getDescripcionCombo($_REQUEST["LSTIdProceso"]) : "");
		$cEntidad->setDescProceso((isset($_REQUEST["LSTDescProceso"]) && $_REQUEST["LSTDescProceso"] != "") ? $_REQUEST["LSTDescProceso"] : "");	$cEntidad->setBusqueda(constant("STR_PROCESO"), (isset($_REQUEST["LSTDescProceso"]) && $_REQUEST["LSTDescProceso"] != "" ) ? $_REQUEST["LSTDescProceso"] : "");
		...
		$cEntidad->setIdCandidato((isset($_REQUEST["LSTIdCandidato"]) && $_REQUEST["LSTIdCandidato"] != "") ? $_REQUEST["LSTIdCandidato"] : "");	$cEntidad->setBusqueda(constant("STR_CANDIDATO"), (isset($_REQUEST["LSTIdCandidato"]) && $_REQUEST["LSTIdCandidato"] != "" ) ? $comboCANDIDATOS->getDescripcionCombo($_REQUEST["LSTIdCandidato"]) : "");
		$cEntidad->setDescCandidato((isset($_REQUEST["LSTDescCandidato"]) && $_REQUEST["LSTDescCandidato"] != "") ? $_REQUEST["LSTDescCandidato"] : "");	$cEntidad->setBusqueda(constant("STR_CANDIDATO"), (isset($_REQUEST["LSTDescCandidato"]) && $_REQUEST["LSTDescCandidato"] != "" ) ? $_REQUEST["LSTDescCandidato"] : "");
		global $comboWI_IDIOMAS;
		$cEntidad->setCodIdiomaIso2((isset($_REQUEST["LSTCodIdiomaIso2"]) && $_REQUEST["LSTCodIdiomaIso2"] != "") ? $_REQUEST["LSTCodIdiomaIso2"] : "");	$cEntidad->setBusqueda(constant("STR_IDIOMA"), (isset($_REQUEST["LSTCodIdiomaIso2"]) && $_REQUEST["LSTCodIdiomaIso2"] != "" ) ? $comboWI_IDIOMAS->getDescripcionCombo($_REQUEST["LSTCodIdiomaIso2"]) : "");
		$cEntidad->setDescIdiomaIso2((isset($_REQUEST["LSTDescIdiomaIso2"]) && $_REQUEST["LSTDescIdiomaIso2"] != "") ? $_REQUEST["LSTDescIdiomaIso2"] : "");	$cEntidad->setBusqueda(constant("STR_IDIOMA"), (isset($_REQUEST["LSTDescIdiomaIso2"]) && $_REQUEST["LSTDescIdiomaIso2"] != "" ) ? $_REQUEST["LSTDescIdiomaIso2"] : "");
		global $comboPRUEBAS;
		$cEntidad->setIdPrueba((isset($_REQUEST["LSTIdPrueba"]) && $_REQUEST["LSTIdPrueba"] != "") ? $_REQUEST["LSTIdPrueba"] : "");	$cEntidad->setBusqueda(constant("STR_PRUEBA"), (isset($_REQUEST["LSTIdPrueba"]) && $_REQUEST["LSTIdPrueba"] != "" ) ? $comboPRUEBAS->getDescripcionCombo($_REQUEST["LSTIdPrueba"]) : "");
		$cEntidad->setDescPrueba((isset($_REQUEST["LSTDescPrueba"]) && $_REQUEST["LSTDescPrueba"] != "") ? $_REQUEST["LSTDescPrueba"] : "");	$cEntidad->setBusqueda(constant("STR_PRUEBA"), (isset($_REQUEST["LSTDescPrueba"]) && $_REQUEST["LSTDescPrueba"] != "" ) ? $_REQUEST["LSTDescPrueba"] : "");
		$cEntidad->setFinalizado("1");
		$cEntidad->setFinalizadoHast("1");
		$cEntidad->setLeidoInstrucciones((isset($_REQUEST["LSTLeidoInstrucciones"]) && $_REQUEST["LSTLeidoInstrucciones"] != "") ? $_REQUEST["LSTLeidoInstrucciones"] : "");	$cEntidad->setBusqueda(constant("STR_LEIDO_INSTRUCCIONES"), (isset($_REQUEST["LSTLeidoInstrucciones"]) && $_REQUEST["LSTLeidoInstrucciones"] != "" ) ? $_REQUEST["LSTLeidoInstrucciones"] : "");
		$cEntidad->setLeidoInstruccionesHast((isset($_REQUEST["LSTLeidoInstruccionesHast"]) && $_REQUEST["LSTLeidoInstruccionesHast"] != "") ? $_REQUEST["LSTLeidoInstruccionesHast"] : "");	$cEntidad->setBusqueda(constant("STR_LEIDO_INSTRUCCIONES") . " " . constant("STR_HASTA"), (isset($_REQUEST["LSTLeidoInstruccionesHast"]) && $_REQUEST["LSTLeidoInstruccionesHast"] != "" ) ? $_REQUEST["LSTLeidoInstruccionesHast"] : "");
		$cEntidad->setLeidoEjemplos((isset($_REQUEST["LSTLeidoEjemplos"]) && $_REQUEST["LSTLeidoEjemplos"] != "") ? $_REQUEST["LSTLeidoEjemplos"] : "");	$cEntidad->setBusqueda(constant("STR_LEIDO_EJEMPLOS"), (isset($_REQUEST["LSTLeidoEjemplos"]) && $_REQUEST["LSTLeidoEjemplos"] != "" ) ? $_REQUEST["LSTLeidoEjemplos"] : "");
		$cEntidad->setLeidoEjemplosHast((isset($_REQUEST["LSTLeidoEjemplosHast"]) && $_REQUEST["LSTLeidoEjemplosHast"] != "") ? $_REQUEST["LSTLeidoEjemplosHast"] : "");	$cEntidad->setBusqueda(constant("STR_LEIDO_EJEMPLOS") . " " . constant("STR_HASTA"), (isset($_REQUEST["LSTLeidoEjemplosHast"]) && $_REQUEST["LSTLeidoEjemplosHast"] != "" ) ? $_REQUEST["LSTLeidoEjemplosHast"] : "");
		$cEntidad->setMinutos_test((isset($_REQUEST["LSTMinutos_test"]) && $_REQUEST["LSTMinutos_test"] != "") ? $_REQUEST["LSTMinutos_test"] : "");	$cEntidad->setBusqueda(constant("STR_MINUTOS_TEST"), (isset($_REQUEST["LSTMinutos_test"]) && $_REQUEST["LSTMinutos_test"] != "" ) ? $_REQUEST["LSTMinutos_test"] : "");
		$cEntidad->setSegundos_test((isset($_REQUEST["LSTSegundos_test"]) && $_REQUEST["LSTSegundos_test"] != "") ? $_REQUEST["LSTSegundos_test"] : "");	$cEntidad->setBusqueda(constant("STR_SEGUNDOS_TEST"), (isset($_REQUEST["LSTSegundos_test"]) && $_REQUEST["LSTSegundos_test"] != "" ) ? $_REQUEST["LSTSegundos_test"] : "");
		$cEntidad->setFecAlta((isset($_REQUEST["LSTFecAlta"]) && $_REQUEST["LSTFecAlta"] != "") ? $_REQUEST["LSTFecAlta"] : "");	$cEntidad->setBusqueda(constant("STR_FECHA_DE_ALTA"), (isset($_REQUEST["LSTFecAlta"]) && $_REQUEST["LSTFecAlta"] != "" ) ? $conn->UserDate($_REQUEST["LSTFecAlta"],constant("USR_FECHA"),false) : "");
		$cEntidad->setFecAltaHast((isset($_REQUEST["LSTFecAltaHast"]) && $_REQUEST["LSTFecAltaHast"] != "") ? $_REQUEST["LSTFecAltaHast"] : "");	$cEntidad->setBusqueda(constant("STR_FECHA_DE_ALTA") . " " . constant("STR_HASTA"), (isset($_REQUEST["LSTFecAltaHast"]) && $_REQUEST["LSTFecAltaHast"] != "" ) ? $conn->UserDate($_REQUEST["LSTFecAltaHast"],constant("USR_FECHA"),false) : "");
		$cEntidad->setFecMod((isset($_REQUEST["LSTFecMod"]) && $_REQUEST["LSTFecMod"] != "") ? $_REQUEST["LSTFecMod"] : "");	$cEntidad->setBusqueda(constant("STR_FECHA_DE_MODIFICACION"), (isset($_REQUEST["LSTFecMod"]) && $_REQUEST["LSTFecMod"] != "" ) ? $conn->UserDate($_REQUEST["LSTFecMod"],constant("USR_FECHA"),false) : "");
		$cEntidad->setFecModHast((isset($_REQUEST["LSTFecModHast"]) && $_REQUEST["LSTFecModHast"] != "") ? $_REQUEST["LSTFecModHast"] : "");	$cEntidad->setBusqueda(constant("STR_FECHA_DE_MODIFICACION") . " " . constant("STR_HASTA"), (isset($_REQUEST["LSTFecModHast"]) && $_REQUEST["LSTFecModHast"] != "" ) ? $conn->UserDate($_REQUEST["LSTFecModHast"],constant("USR_FECHA"),false) : "");
		global $comboWI_USUARIOS;
		$cEntidad->setUsuAlta((isset($_REQUEST["LSTUsuAlta"]) && $_REQUEST["LSTUsuAlta"] != "") ? $_REQUEST["LSTUsuAlta"] : "");	$cEntidad->setBusqueda(constant("STR_USUARIO_DE_ALTA"), (isset($_REQUEST["LSTUsuAlta"]) && $_REQUEST["LSTUsuAlta"] != "" ) ? $comboWI_USUARIOS->getDescripcionCombo($_REQUEST["LSTUsuAlta"]) : "");
		$cEntidad->setUsuMod((isset($_REQUEST["LSTUsuMod"]) && $_REQUEST["LSTUsuMod"] != "") ? $_REQUEST["LSTUsuMod"] : "");	$cEntidad->setBusqueda(constant("STR_USUARIO_DE_MODIFICACION"), (isset($_REQUEST["LSTUsuMod"]) && $_REQUEST["LSTUsuMod"] != "" ) ? $comboWI_USUARIOS->getDescripcionCombo($_REQUEST["LSTUsuMod"]) : "");
		$cEntidad->setOrderBy((!empty($_REQUEST["LSTOrderBy"])) ? $_REQUEST["LSTOrderBy"] : "fecMod");	$cEntidad->setBusqueda(constant("STR_ORDENAR_POR"), (!empty($_REQUEST["LSTOrderBy"])) ? $_REQUEST["LSTOrderBy"] : "fecMod");
		$cEntidad->setOrder((!empty($_REQUEST["LSTOrder"])) ? $_REQUEST["LSTOrder"] : "DESC");	$cEntidad->setBusqueda(constant("STR_ORDEN"), (!empty($_REQUEST["LSTOrder"])) ? $_REQUEST["LSTOrder"] : "DESC");
		$cEntidad->setLineasPagina((!empty($_REQUEST["LSTLineasPagina"]) && is_numeric($_REQUEST["LSTLineasPagina"])) ? $_REQUEST["LSTLineasPagina"] : constant("CNF_LINEAS_PAGINA"));
		$_REQUEST["LSTLineasPagina"] = $cEntidad->getLineasPagina();
		return $cEntidad;
