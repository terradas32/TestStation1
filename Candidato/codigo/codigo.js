...
function getFormatDate(sFecha){
	sFec="";
	sFec+=sFecha.substring(sFecha.lastIndexOf("/")+1,sFecha.lastIndexOf("/")+5);//AÑO
	sFec+="-";
...
function formatThisDate(sFecha){
	...
	if (sFecha != "" && sFecha != "0000-00-00")
	{
		...
		sFec+=sFecha.substring(sFecha.indexOf("-")+1,sFecha.lastIndexOf("-"));//MES
		sFec+="/";
		sFec+=sFecha.substring(0,sFecha.indexOf("-"));//AÑO
	}
	return sFec;
}
...
function cQuitaAcentos(sCadena){ 
	sChars = "áéíóúÁÉÍÓÚäëïöüÄËÏÖÜàèìòùÀÈÌÒÙâêîôûÂÊÎÔÛñ";
