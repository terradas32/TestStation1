...
class ItemsDB
{
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  .=" WHERE idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
	...
	************************************************************************/
	...
	require_once(constant("DIR_WS_COM") . "Upload.php");
		...
		$sql .= " WHERE ";
		$sql .="idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
	...
	*************************************************************************/
	function readEntidadOrden($cEntidad)
	{
		...
		$sql = "SELECT *  FROM items WHERE ";
		$sql  .="orden=" . $aux->qstr($cEntidad->getOrden(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdItem($arr['idItem']);
