...
class Utilidades
{
	...
	*/
	function isCurrent2Dates($fecInicio, $fecFin, $timezone){
...
// 		exit;
	...
	*/
	...
	function backgroundPostOld($url){
		$parts=parse_url($url);
		...
		function backgroundPost($url){
			...
			$goURL = $parts['scheme'] . "://" . $parts['host'] . $parts['path'];

			$postfields = $parts['query'];
			$ch = curl_init();
...
//				echo "<br />Empresa::" . $rsLine->fields['idEmpresa'] . " Proceso::" . $rsLine->fields['idProceso'] . " Prueba::" . $rsLine->fields['idPrueba'] . " Candidato::" . $rsLine->fields['idCandidato'] . " **->orden::" . $rsLine->fields['orden'] . " " . $cItem->getCorrecto() . " == " . $cOpcion->getCodigo() . " ->valor::" . $sValor;
	 ...
	 */
	function getOptiosSelect_timezone_list($sKeySelected="") {
