...
* la tabla empresas_conf_tpv
**/
class Empresas_conf_tpvDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(idEmpresa) AS Max FROM empresas_conf_tpv ";
		$sql .= "WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idTipoTpv=" . $aux->qstr($cEntidad->getIdTipoTpv(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Empresas_conf_tpvDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	
		$sql = "INSERT INTO empresas_conf_tpv (";
		$sql .= "idEmpresa" . ",";
		$sql .= "idTipoTpv" . ",";
		$sql .= "BUSINESS_NAME" . ",";
		$sql .= "BUSINESS_CODE" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getBUSINESS_NAME(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getBUSINESS_CODE(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getBUSINESS_PASS(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getBUSINESS_COINC(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Empresas_conf_tpvDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM empresas_conf_tpv ";
		$sql  .=" WHERE idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idTipoTpv=" . $aux->qstr($cEntidad->getIdTipoTpv(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Empresas_conf_tpvDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE empresas_conf_tpv SET ";
		$sql .= "idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . ", ";
		$sql .= "idTipoTpv=" . $aux->qstr($cEntidad->getIdTipoTpv(), false) . ", ";
		$sql .= "BUSINESS_NAME=" . $aux->qstr($cEntidad->getBUSINESS_NAME(), false) . ", ";
		$sql .= "BUSINESS_CODE=" . $aux->qstr($cEntidad->getBUSINESS_CODE(), false) . ", ";
		$sql .= "BUSINESS_PASS=" . $aux->qstr($cEntidad->getBUSINESS_PASS(), false) . ", ";
		$sql .= "BUSINESS_COINC=" . $aux->qstr($cEntidad->getBUSINESS_COINC(), false) . ", ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Empresas_conf_tpvDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
			...
			if (empty($HistId)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error [GUARDANDO HISTORICO][" . constant("MNT_ALTA") . "][Empresas_conf_tpvDBDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				header('Location: ' . constant("HTTP_SERVER") . 'index.php');
				exit;
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM empresas_conf_tpv ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Empresas_conf_tpvDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM empresas_conf_tpv WHERE ";
		$sql  .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idTipoTpv=" . $aux->qstr($cEntidad->getIdTipoTpv(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setBUSINESS_NAME($arr['BUSINESS_NAME']);
					$cEntidad->setBUSINESS_CODE($arr['BUSINESS_CODE']);
					$cEntidad->setBUSINESS_PASS($arr['BUSINESS_PASS']);
					$cEntidad->setBUSINESS_COINC($arr['BUSINESS_COINC']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Empresas_conf_tpvDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM empresas_conf_tpv ";
		if ($cEntidad->getIdEmpresa() != ""){
		...
		if ($cEntidad->getBUSINESS_NAME() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(BUSINESS_NAME) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getBUSINESS_NAME() . "%") . ")";
		}
		if ($cEntidad->getBUSINESS_CODE() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(BUSINESS_CODE) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getBUSINESS_CODE() . "%") . ")";
		}
		if ($cEntidad->getBUSINESS_PASS() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(BUSINESS_PASS) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getBUSINESS_PASS() . "%") . ")";
		}
		if ($cEntidad->getBUSINESS_COINC() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(BUSINESS_COINC) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getBUSINESS_COINC() . "%") . ")";
		}
	...
	function quitaImagen($cEntidad)
	{
		...
		$cEntidadAnterior	= new Empresas_conf_tpv();
...
}//Fin de la Clase Empresas_conf_tpvDB
?>