...
//		$img0->image_x = 148;	// Ancho por defecto
//		$img0->jpeg_quality = 75;
//		$sDirImg="imgOpciones/" . $cEntidad->getIdPrueba() . "/" . $cEntidad->getCodIdiomaIso2();
...
//		echo $sql;
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idOpcion) AS Max FROM opciones ";
		$sql  .=" WHERE codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
