...
* la tabla correos_proceso
**/
class Correos_procesoDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(idEmpresa) AS Max FROM correos_proceso ";
		$sql .= "WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idTipoCorreo=" . $aux->qstr($cEntidad->getIdTipoCorreo(), false) . " AND idCorreo=" . $aux->qstr($cEntidad->getIdCorreo(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Correos_procesoDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}

		$sql = "INSERT INTO correos_proceso (";
		$sql .= "idEmpresa" . ",";
		$sql .= "idProceso" . ",";
		$sql .= "idTipoCorreo" . ",";
		$sql .= "idCorreo" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getIdCorreo(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Correos_procesoDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM correos_proceso ";
		$sql  .=" WHERE idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idTipoCorreo=" . $aux->qstr($cEntidad->getIdTipoCorreo(), false) . " AND idCorreo=" . $aux->qstr($cEntidad->getIdCorreo(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Correos_procesoDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE correos_proceso SET ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Correos_procesoDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if($listaInformados->recordCount()>0){
			...
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	= "No se ha podido eliminar esta asignación de correo en el proceso '" . $cProceso->getNombre() ."',\\n ya tiene candidatos informados.";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM correos_proceso ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Correos_procesoDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM correos_proceso WHERE ";
		$sql  .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idTipoCorreo=" . $aux->qstr($cEntidad->getIdTipoCorreo(), false) . " AND idCorreo=" . $aux->qstr($cEntidad->getIdCorreo(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Correos_procesoDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM correos_proceso ";
		if ($cEntidad->getIdEmpresa() != ""){
	...
	*************************************************************************/
	...
	function parseaHTML($cEntidad, &$cCandidatos, &$cProcesos, &$cEmpresas, $sUsuario, $sPassword)
	{
		$aux			= $this->conn;
		...
		while (!$rsLiterales->EOF){
			...
			if ($sLiteral == "@empresa_colaboradora@"){
				//Miramos si es de Banco davivienda
...
// 				$cEmpresaPadreDB = new EmpresasDB($aux);
// 				$_EmpresaEspecial = "5467";	//Banco davivienda
// 				$sHijos = $cEmpresaPadreDB->getHijos($_EmpresaEspecial);
// 				if (!empty($sHijos)){
// 					$sHijos .= $_EmpresaEspecial;
// 				}else{
// 					$sHijos = $_EmpresaEspecial;
// 				}
				$sHijos = '5467,5491,5499,5506,5492,5493,5494,5495,5496,5497,5498,5500,5501,5502,5503,5504,5505,5507,5508,5509,5510,5511,5512,5513,5514,5515,5516,5517,5518,5519,5520,5521,5522,5523,5524,5525,5526,5564';
				//echo $sHijos;exit;
				$_aEmpresaEspecial = explode(",", $sHijos);
				if (in_array($cEmpresas->getIdEmpresa(), $_aEmpresaEspecial)) {
					if (strpos($cEntidad->getCuerpo(), $sLiteral)){
						$cEntidad->setCuerpo(str_replace($sLiteral, "BANCO DAVIVIENDA S.A.",$cEntidad->getCuerpo()));
					}
					if (strpos($cEntidad->getAsunto(), $sLiteral)){
						$cEntidad->setAsunto(str_replace($sLiteral, "BANCO DAVIVIENDA S.A.",$cEntidad->getAsunto()));
					}
				}else{
					if (strpos($cEntidad->getCuerpo(), $sLiteral)){
...
}//Fin de la Clase Correos_procesoDB
?>
