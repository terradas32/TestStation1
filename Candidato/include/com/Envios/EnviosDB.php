...
//		$sql .= $aux->qstr($newId, false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][EnviosDB]";
			...
			$_cEntidadUsuariosDB	= new EmpresasDB($aux);
			$_cEntidadUsuarios		= new Empresas();
			$_cEntidadUsuarios->setIdEmpresa($cEntidad->getUsuAlta());
			...
			$cEntidadHistorico_cambios->setIdUsuario($_cEntidadUsuarios->getIdEmpresa());
			$cEntidadHistorico_cambios->setIdUsuarioTipo(1);
			$cEntidadHistorico_cambios->setLogin($_cEntidadUsuarios->getUsuario());
			$cEntidadHistorico_cambios->setNombre($_cEntidadUsuarios->getNombre());
			$cEntidadHistorico_cambios->setApellido1("");
			$cEntidadHistorico_cambios->setApellido2("");
			$cEntidadHistorico_cambios->setEmail($_cEntidadUsuarios->getMail());
			$cEntidadHistorico_cambios->setUsuAlta($cEntidad->getIdEmpresa());
			$cEntidadHistorico_cambios->setUsuMod($cEntidad->getIdEmpresa());
			$HistId	= $cEntidadHistorico_cambiosDB->insertar($cEntidadHistorico_cambios);
			if (empty($HistId)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error [GUARDANDO HISTORICO][" . constant("MNT_ALTA") . "][EmpresasDB]";
				$this->msg_Error[]	= $sTypeError;
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idEnvio) AS Max FROM envios ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][EnviosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= "idEnvio=" . $aux->qstr($cEntidad->getIdEnvio(), false) . ", ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][EnviosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][EnviosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdEnvio($arr['idEnvio']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][EnviosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
