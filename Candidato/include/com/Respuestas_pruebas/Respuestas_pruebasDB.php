...
class Respuestas_pruebasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr($cEntidad->getMinutos2_test(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getSegundos2_test(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPantalla(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getCampoLibre(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			...
			if (empty($HistId)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error [GUARDANDO HISTORICO][" . constant("MNT_ALTA") . "][respuestas_pruebasDB]";
				$this->msg_Error[]	= $sTypeError;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= "minutos2_test=" . $aux->qstr($cEntidad->getMinutos2_test(), false) . ", ";
		$sql .= "segundos2_test=" . $aux->qstr($cEntidad->getSegundos2_test(), false) . ", ";
		$sql .= "pantalla=" . $aux->qstr($cEntidad->getPantalla(), false) . ", ";
		$sql .= "campoLibre=" . $aux->qstr($cEntidad->getCampoLibre(), false) . ", ";
	...
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setMinutos2_test($arr['minutos2_test']);
					$cEntidad->setSegundos2_test($arr['segundos2_test']);
					$cEntidad->setPantalla($arr['pantalla']);
					$cEntidad->setCampoLibre($arr['campoLibre']);
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getMinutos_test() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="minutos_test LIKE " . $aux->qstr("%" . $cEntidad->getMinutos_test() . "%") . "";
		}
		if ($cEntidad->getSegundos_test() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="segundos_test LIKE " . $aux->qstr("%" . $cEntidad->getSegundos_test() . "%") . "";
		}
	...
	*************************************************************************/
	function readListaIN($cEntidad)
	{
		...
		if ($cEntidad->getMinutos_test() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="minutos_test LIKE UPPER" . $aux->qstr("%" . $cEntidad->getMinutos_test() . "%") . "";
		}
		if ($cEntidad->getSegundos_test() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="segundos_test LIKE UPPER" . $aux->qstr("%" . $cEntidad->getSegundos_test() . "%") . "";
		}
		if ($cEntidad->getFecAlta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
