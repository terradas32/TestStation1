...
class Respuestas_pruebas_itemsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr(($cEntidad->getUsuMod() != "") ? $cEntidad->getUsuMod() : $cEntidad->getIdCandidato(), false) . ")";
	...
	*  en la base de datos
	***********************************************************************/
	function insertarPruebaFLASH($cEntidad)
	{
		$aux			= $this->conn;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= "usuMod=" . $aux->qstr(($cEntidad->getUsuMod() != "") ? $cEntidad->getUsuMod() : $cEntidad->getIdCandidato(), false) ;
		$sql .= " WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idCandidato=" . $aux->qstr($cEntidad->getIdCandidato(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
