...
<table cellspacing="5" cellpadding="0" border="0" width="95%">
		<tr>
			<td class="negrob" colspan="<?php echo $i_Opciones+1;?>">
				<?php echo  mb_strtoupper(constant("STR_PAGINA"), 'UTF-8');?>&nbsp;<?php echo intval($OrdenHast/$sPreguntasPorPagina)?> <?php echo constant("STR_DE");?> <?php echo $iPaginas?>
				<br />
				<br />
			</td>
		</tr>
...
<?php 
		...
		{
			$sOpciones .= ',' . $listaOpciones->fields['idItem'];
		?>
				...
				if($listaOpciones->recordCount()>0){
					...
					?>
						...
						<?php 
							...
							?>
 							<input type="radio" <?php echo $sChecked;?> name="fIdOpcion<?php echo $listItems->fields['idItem'];?>" value="<?php echo $listaOpciones->fields['codigo'] . "@" . $listaOpciones->fields['idOpcion'];?>" id="opcion<?php echo $listaOpciones->fields['codigo']. "@" . $listItems->fields['idItem'];?>" onclick="javascript:guardarespuesta('<?php echo $listItems->fields['idItem'];?>','<?php echo $listaOpciones->fields['idOpcion'];?>','<?php echo $listItems->fields['orden'];?>',<?php echo $listaOpciones->recordCount()?>);" /> 
						</td>
