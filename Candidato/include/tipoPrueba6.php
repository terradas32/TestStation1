				...
				if($listaOpciones->recordCount()>0){
					...
					?>
						...
						<?php
							...
							?>
							<input type="checkbox"  <?php echo $sChecked;?> name="fIdOpcion<?php echo $listaOpciones->fields['idItem'];?>" value="<?php echo $listaOpciones->fields['codigo'] . "@" . $listaOpciones->fields['idOpcion']?>" id="opcion<?php echo $listaOpciones->fields['codigo']. "@" . $listItems->fields['idItem']?>" onclick="javascript:guardarespuestatipo6('<?php echo $listItems->fields['idItem'];?>','<?php echo $listaOpciones->fields['idOpcion']?>','<?php echo $listItems->fields['orden'];?>',<?php echo $sPreguntasPorPagina?>,this,<?php echo $cItems->getOrden()?>,<?php echo ($cItems->getOrden() + $sPreguntasPorPagina)-1?>,'<?php echo $sOpciones;?>');" />
						</td>
...
<?php $listItems->MoveNext();
	...
	<script>
		...
		if ("<?php echo $_POST['fIdPrueba']?>" == "13")
		{
			...
			//Pongo los checkeados a disabled
