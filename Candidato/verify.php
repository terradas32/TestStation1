...
// 	 	$aH[1] -> Proceso
	...
	if (isset($_POST['fGo'])){
				...
				)
		{
			...
			if (!$bEncontradoNombre && !$bEncontradoApellido1)
			{
				if(strlen($_POST['fPasswordCan']) > 7){
					$bEncontradoUsuario = $cUtilidades->chkChar($_POST['fMailCan']);
					$bEncontradoPassword = $cUtilidades->chkChar($_POST['fPasswordCan']);
					if (!$bEncontradoPassword && !$bEncontradoUsuario)
	        {
			 			if ( (!empty($_POST['fIdEmpresa'])) && (!empty($_POST['fIdProceso'])))
			 			{
							...
							{	//El candidato existe
								//Miramos que la contraseña no se repita con ese correo en ningún Proceso
								$sQL = " SELECT * FROM candidatos ";
								$sQL .= " WHERE mail = " . $conn->qstr($_POST['fMailCan'], false);
								$sQL .= " AND password = " . $conn->qstr(md5($_POST['fPasswordCan']), false);

								$rsCuantos = $conn->Execute($sQL);
								$iCuantos=$rsCuantos->NumRows();
								// echo "<br>". $iCuantos;
								while (!$rsCuantos->EOF)
								{
									 if($rsCuantos->fields['idEmpresa'] == $cEntidad->getIdEmpresa() &&
									 $rsCuantos->fields['idProceso'] == $cEntidad->getIdProceso() &&
									 $rsCuantos->fields['idCandidato'] == $cEntidad->getIdCandidato() ){
										$iCuantos--;
									}
									$rsCuantos->MoveNext();
								}
			// echo "<br>" . $iCuantos;exit;
				...
				// 	 								}
								...
								if ($sLang="es"){
									...
									$strMensaje = "404:: The data provided do not match any record in our database.";
								}
						...
						$strMensaje = "403-4044:: " . constant("ERR_FORM_LOGIN");
					}

				}else{
					$strMensaje = "403-1:: " . constant("STR_LONG_MIN_PASSWORD");
				}
			}else{
				$strMensaje = "403-4044:: " . constant("ERR_FORM_LOGIN");
			}
