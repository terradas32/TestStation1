...
echo date("Y-m-d H:i:s");
	...
	//$sSQL = "SELECT * FROM `candidatos` WHERE concat(idEmpresa,idProceso, idCandidato) NOT IN (SELECT concat(idEmpresa,idProceso, idCandidato) FROM respuestas_pruebas) LIMIT 0,5000";
	//$sSQL = "SELECT * FROM respuestas_pruebas WHERE concat(idEmpresa,idProceso, idCandidato) NOT IN (SELECT concat(idEmpresa,idProceso, idCandidato) FROM candidatos)";
	$sSQL = "SELECT * FROM respuestas_pruebas_items WHERE concat(idEmpresa,idProceso, idCandidato) NOT IN (SELECT concat(idEmpresa,idProceso, idCandidato) FROM respuestas_pruebas) LIMIT 0,5000";
	echo "<br />" . $sSQL;
	...
	while(!$rsRespuestas_pruebasApti->EOF)
	{
			...
			$sSQL .= "idEmpresa=" . $rsRespuestas_pruebasApti->fields['idEmpresa'] . " AND ";
			$sSQL .= "idProceso=" . $rsRespuestas_pruebasApti->fields['idProceso'] . " AND ";
			$sSQL .= "idCandidato=" . $rsRespuestas_pruebasApti->fields['idCandidato'] . "  ";
			$conn->Execute($sSQL);
			echo "<br />" .  $sSQL;
