...
* FROM respuestas_pruebas WHERE fecAlta >= NOW() - INTERVAL 1 YEAR AND idEmpresa= 4267 AND idPrueba IN (16,21,20,26,8,28,29,30,31,37,38,36,34,35,40,43,44,45,46,47,50,51,52,53,54,55,56,57,48,58,59,60,66,61,62,63,64,65,67,68,69,70,75,81,82,84,85,86,102,103,119,110,111,118,116,115,122) and finalizado=1
*
*/

	//Miramos si queda algo por lanzar
	$sSQL = "SELECT count(*) AS cuantos FROM `respuestas_pruebas_resultados2` WHERE veces=0";
	echo "<br />" . $sSQL;
	...
	if ($cuantos == 0)
	{
			...
			$sSQL = "TRUNCATE TABLE `respuestas_pruebas_resultados2`";
			$conn->Execute($sSQL);
			//Cargamoso la tabla con los guardados en la úlitmas 4 horas
			$sSQL = " INSERT INTO `respuestas_pruebas_resultados2` (`idEmpresa`, `descEmpresa`, `idProceso`, `descProceso`, `idCandidato`, `descCandidato`, `codIdiomaIso2`, `descIdiomaIso2`, `idPrueba`, `descPrueba`, `finalizado`, `leidoInstrucciones`, `leidoEjemplos`, `minutos_test`, `segundos_test`, `minutos2_test`, `segundos2_test`, `pantalla`, `campoLibre`, `veces`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod`)";
			...
			$sSQL .= " FROM respuestas_pruebas WHERE fecAlta >= NOW() - INTERVAL 4 HOUR AND idPrueba IN (16,21,20,26,8,28,29,30,31,37,38,36,34,35,40,43,44,45,46,47,50,51,52,53,54,55,56,57,48,58,59,60,66,61,62,63,64,65,67,68,69,70,75,81,82,84,85,86,102,103,119,110,111,118,116,115,122) and finalizado=1";
			$conn->Execute($sSQL);
	}
