...
//-------------------------------------------
	...
	//Para las pruebas de VitaAV, verificamos que se inician y se crea registro
	// en proceso_pruebas si no está creado.
	$bVitaAV=0;	//Identificador de si la prueba es de la plataforma Vita AV
	if (isset($_REQUEST["fVitaAV"]) && $_REQUEST["fVitaAV"] == "1")
	{

		include_once ('include/conexionVitaAV.php');

		//Es una llamada de finalización de prueba
		$cPVitaAV = new Pruebas();
		$cPVitaAVDB = new PruebasDB($conn);


		$cPPVitaAV = new Proceso_pruebas();
		$cPPVitaAVDB = new Proceso_pruebasDB($conn);
		$cPPVitaAV->setIdEmpresa($_cEntidadCandidatoTK->getIdEmpresa());
		$cPPVitaAV->setIdProceso($_cEntidadCandidatoTK->getIdProceso());
		$cPPVitaAV->setOrderBy("orden");
		$cPPVitaAV->setOrder("ASC");

		$sqlPPVitaAV = $cPPVitaAVDB->readLista($cPPVitaAV);
//		echo "<br />" . $sqlPPVitaAV;
		$lPPVitaAV = $conn->Execute($sqlPPVitaAV);
		//Buscamos la prueba que estamos, la 1ª sin finalizar
		$cRPVitaAVDB = new Respuestas_pruebasDB($conn);
	    while(!$lPPVitaAV->EOF){
			$cRPVitaAV = new Respuestas_pruebas();

			$cRPVitaAV->setIdPrueba($lPPVitaAV->fields['idPrueba']);
			$cRPVitaAV->setCodIdiomaIso2($lPPVitaAV->fields['codIdiomaIso2']);
			$cRPVitaAV->setIdProceso($_cEntidadCandidatoTK->getIdProceso());
			$cRPVitaAV->setIdEmpresa($_cEntidadCandidatoTK->getIdEmpresa());
			$cRPVitaAV->setIdCandidato($_cEntidadCandidatoTK->getIdCandidato());

			$lRPVitaAV = $cRPVitaAVDB->readLista($cRPVitaAV);
			if ($lPPVitaAV->NumRows() > 0){
				while(!$lPPVitaAV->EOF){
		       		if($cRPVitaAV->getFinalizado()== '0' || $cRPVitaAV->getFinalizado()==""){
//		       			echo "<br />" . $lPPVitaAV->fields['idPrueba'];
				    	$cPVitaAV->setIdPrueba($lPPVitaAV->fields['idPrueba']);
			    		$cPVitaAV->setCodIdiomaIso2($lPPVitaAV->fields['codIdiomaIso2']);
		       			break;
		       		}
					$lPPVitaAV->MoveNext();
				}
    	...
    	$cPVitaAV= $cPVitaAVDB->readEntidad($cPVitaAV);
    	if ($cPVitaAV->getCodigo() == ""){
    		echo  "Ha ocurrido un error, No se ha especificado el código de ejercicio de Vita AV.";
			exit;
    	}
    	//Buscamos la simulación
    	$sSQL= "SELECT * FROM evaluations WHERE name = " . $connVitaAV->qstr($cPVitaAV->getCodigo(), false);
		$rsEjercicioVitaAV = $connVitaAV->Execute($sSQL);
		$_ClientVitaAV = "";
		$_EvaluationVitaAV = "";
//		echo "<br />sSQL::" . $sSQL;
		if ($rsEjercicioVitaAV){
			while (!$rsEjercicioVitaAV->EOF)
			{
//				echo "<br />client_id::" . $rsEjercicioVitaAV->fields['client_id'];
//				echo "<br />evaluation_id::" . $rsEjercicioVitaAV->fields['id'];
				$_ClientVitaAV = $rsEjercicioVitaAV->fields['client_id'];
				$_EvaluationVitaAV = $rsEjercicioVitaAV->fields['id'];
				$rsEjercicioVitaAV->MoveNext();
			}
		}else{
			$e = ADODB_Pear_Error();
			echo "<br />ERROR: " . $e->message;
		...
		if (empty($_ClientVitaAV) || empty($_EvaluationVitaAV)){
			echo  "Ha ocurrido un error, No se ha detectado el cliente y/o evaluación de Vita AV asignada.";
			exit;
		}
    	//Buscamos al usuario
    	$sSQL= "SELECT * FROM users WHERE email = " . $connVitaAV->qstr($_cEntidadCandidatoTK->getMail(), false) . " AND id_cliente=" . $connVitaAV->qstr($_ClientVitaAV, false);
		$rsUsrVitaAV = $connVitaAV->Execute($sSQL);
		$_UsrVitaAV = "";
    $_PerfilUsrVitaAV = "";
		if ($rsUsrVitaAV){
			while (!$rsUsrVitaAV->EOF)
			{
//				echo "<br />user_id::" . $rsUsrVitaAV->fields['id'];
				$_UsrVitaAV = $rsUsrVitaAV->fields['id'];
        $_PerfilUsrVitaAV = $rsUsrVitaAV->fields['role_id'];
				$rsUsrVitaAV->MoveNext();
			}
		}else{
			$e = ADODB_Pear_Error();
			echo "<br />ERROR: " . $e->message;
		...
		if (empty($_UsrVitaAV)){
			echo  "Ha ocurrido un error, No se ha detectado el usuario Vita AV.";
			exit;
		}
		//Miramos si ha finalizado
    //Puede ser un Participante role_id 3 o un Manager role_id 6
    if ($_PerfilUsrVitaAV == 3){
    	$sSQL= "SELECT * FROM evaluation_exercise_competitors WHERE evaluation_id = " . $connVitaAV->qstr($_EvaluationVitaAV, false) . " AND competitor_id=" . $connVitaAV->qstr($_UsrVitaAV, false);
    }elseif ($_PerfilUsrVitaAV == 6){
      $sSQL= "SELECT * FROM evaluation_exercise_competitors WHERE evaluation_id = " . $connVitaAV->qstr($_EvaluationVitaAV, false) . " AND competitor_id=" . $connVitaAV->qstr($_UsrVitaAV, false);
    }else{
      echo  "Ha ocurrido un error, usuario NO AUTORIZADO.";
			exit;
    }
		$rsFinVitaAV = $connVitaAV->Execute($sSQL);
		$_FinVitaAV = "0";
    $_iFinalizadosVitaAV = "0";
		if ($rsFinVitaAV){
			while (!$rsFinVitaAV->EOF)
			{
          if ($_PerfilUsrVitaAV == 3){
    	      //				echo "<br />finalizado::" . $rsFinVitaAV->fields['competitor_status'];
          	if ($rsFinVitaAV->fields['competitor_status'] == "3"){
    					//En VitaAV en estado "3" es FINALIZADO
    					$_iFinalizadosVitaAV++;
    				}
          }elseif ($_PerfilUsrVitaAV == 6){
             //				echo "<br />finalizado::" . $rsFinVitaAV->fields['manager_status'];
           	if ($rsFinVitaAV->fields['manager_status'] == "3"){
    					//En VitaAV en estado "3" es FINALIZADO
    					$_iFinalizadosVitaAV++;
    				}
          }
				$rsFinVitaAV->MoveNext();
			}
      if ($_iFinalizadosVitaAV == $rsFinVitaAV->NumRows()){
          $_FinVitaAV = 1;
      }
		}else{
			$e = ADODB_Pear_Error();
		...
		if ($_FinVitaAV == "1")
		{
		    ...
		    while(!$lPPVitaAV->EOF){
				...
				$cRPVitaAV = $cRPVitaAVDB->readEntidad($cRPVitaAV);

	       		if($cRPVitaAV->getFinalizado()== '0' || $cRPVitaAV->getFinalizado()==""){
	       			//Esta prueba es la que hay que finalizar
	       			$cRPVitaAV->setFinalizado("1");
                    $cRPVitaAVDB->insertar($cRPVitaAV);
	       			$cRPVitaAVDB->modificar($cRPVitaAV);
	       			$bVitaAV=0;
	       			$_REQUEST["fVitaAV"]=0;
	       			break;
	       		}
	...
	//FIN - Para las pruebas de VitaAV,
...
//		echo "<br />" . $sqlRespItems;
    	...
    	if(isset($_POST['fFinalizarPorTiempo']) && $_POST['fFinalizarPorTiempo']=="1")
    	{
	    	...
	    	// o la prueba permite finalizar dejando SIN CONTESTAR
				if ($_POST['fIdPrueba'] == 32 && $listaRespItems->recordCount() == 252){
					$cPruebasD->setPermiteBlancos("1");
				}
	    	if(($cPruebasD->getPermiteBlancos() == "1") || ($listaRespItems->recordCount() == $listaItems->recordCount()) )
	    	{
	    		$iContestadas24=0;
	    		$k=1;
	    		if ($_POST['fIdPrueba'] == 24 || $_POST['fIdPrueba'] == 105 || $_POST['fIdPrueba'] == 83){
	    			...
	    			if ($rsIPE->NumRows() > 0){
						...
						if ($iConsRead <= 0)
						{
							...
							if (!empty($sDescuentaMatriz)){
								...
								$sSQL .= "nomDescuentaMatriz=" . $conn->qstr($cEmpresaConsumo->getNombre(), false) . " ";
								$sSQL .= "WHERE ";
								$sSQL .= "idEmpresa=" . $cConsumos->getIdEmpresa() . " AND ";
								$sSQL .= "idProceso=" . $cConsumos->getIdProceso() . " AND ";
								$sSQL .= "idCandidato=" . $cConsumos->getIdCandidato() . " AND ";
...
///////////////////////////////////////////////////////////////////
	    	...
	    	$sSQLUPDATE = "UPDATE consumos c, candidatos ca SET c.idSexo=ca.idSexo,c.idEdad=ca.idEdad,c.idFormacion=ca.idFormacion,c.idNivel=ca.idNivel,c.idArea=ca.idArea,c.codIso2PaisProcedencia=ca.codIso2PaisProcedencia WHERE ca.idEmpresa=" . $conn->qstr($_POST['fIdEmpresa'], false) . " AND ca.idProceso=" . $conn->qstr($_POST['fIdProceso'], false) . " AND ca.idCandidato=" . $conn->qstr($_POST['fIdCandidato'], false) . " AND c.idEmpresa=ca.idEmpresa  AND c.idProceso=ca.idProceso AND c.idCandidato=ca.idCandidato;";
    	...
    	if (!$bGame){
            ...
            if (isset($_POST['fCodIso2PaisProcedencia']) && $_POST['fCodIso2PaisProcedencia'] != ""){
            	$cCandidato->setCodIso2PaisProcedencia($_POST['fCodIso2PaisProcedencia']);
            }
				...
				{	if (in_array($listaProcesosPruebas->fields['idPrueba'], $aPruebasAleatorias))
					{
						...
						{
							if (in_array($listaAleatorios->fields['idPrueba'], $aPruebasAleatorias)){
...
//						echo "<br >ALEATORIAS::" . $sAleatorias;
						...
						//Marcamos el registro con la prueba con la que se dio de alta en el proceso
						$sSQLMarcar = "UPDATE proceso_pruebas_candidato SET idPruebaProceso=" . $listaProcesosPruebas->fields['idPrueba'];
						$sSQLMarcar .= " WHERE idEmpresa=" . $listaProcesosPruebas->fields['idEmpresa'];
						$sSQLMarcar .= " AND idProceso=" . $listaProcesosPruebas->fields['idProceso'];
						$sSQLMarcar .= " AND codIdiomaIso2='" . $listaProcesosPruebas->fields['codIdiomaIso2'] . "'";
						$sSQLMarcar .= " AND idPrueba=" . $sPruebaAleatoria;
						$sSQLMarcar .= " AND idCandidato=" . $_POST['fIdCandidato'];
						//echo $sSQLMarcar;
						$conn->Execute($sSQLMarcar);
...
<?php include('include/metatags.php');?>
	...
	<link rel="stylesheet" type="text/css" href="estilos/animation.css">
    <!--if IE 7
    	link(rel='stylesheet', href='<?php echo constant("HTTP_SERVER");?>estilos/fontello-ie7.css')
    -->
...
function checkVitaAV() {
	...
	} else setTimeout("checkVitaAV()",1);
}
function pan_VitaAV(see){
	...
	var posicionx=0;
	var posiciony=0;
	var direccion="<?php echo Constant("HTTP_SERVER");?>VitaAV.php?see=" + see;
	adWin = window.open("",'ventana','fullscreen=1,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=yes,resizable=0');
	adWin.resizeTo(tamanox,tamanoy);
	adWin.moveTo(posicionx,posiciony);
	adWin.location=direccion;
			...
			<div style="margin-left:20%;">
	        			<div id="triggers">
							<table cellpadding="5" cellspacing="0" border="1" width="80%;">
								<tr style="background-color:#FF7B0D;color:#ffffff;font-weight:bold;">
									...
									<td align="center"><?php echo constant("STR_LO_QUE_NECESITARA");?></td>
...
//        			    	$iListaPruebas = $listaProcesosPruebas->recordCount();
        			    	if($iListaPruebas > 0)
        			    	{
				        		...
				        		while(!$listaProcesosPruebas->EOF){
									...
									$rsReplica = $conn->Execute($cRespuestasPruebasDB->readLista($cRespuestas));
									$_bPintar=true;
									if (!empty($rsReplica->fields['replicada'])){
										$_bPintar=false;
									}
				        			...
				        			?>
							        	...
							        	<tr>
											<td align="center">
							        			<?php echo $cPruebas->getNombre() . " - " . $cPruebas->getDescripcion();?>
							        		</td>
								        	<td align="center">
								        		<?php
                            ...
                            ?>
								        	...
								        	<td align="center">
								        		<?php
														switch ($cPruebas->getIdPrueba())
														{
															...
															case "39":
															case "105":
																if (!empty($cPruebas->getPreguntasPorPagina()))
																{
																	$iQuestions = $listaItems->recordCount()/$cPruebas->getPreguntasPorPagina();
																	if (!empty($sLang) && $sLang == 'es' ){
																			echo $iQuestions. " preguntas, con " .$cPruebas->getPreguntasPorPagina() . " frases cada una";
																	}elseif ($sLang == 'ca'){
																		echo $iQuestions. " preguntes, de " .$cPruebas->getPreguntasPorPagina() . " frases cadascuna";
																	}
																	else{
																			echo $iQuestions. " questions, with " .$cPruebas->getPreguntasPorPagina() . " phrases each";
																	}
								        	...
								        	<td align="left">
								        		<?php
								        			$aIconos = explode(",", $cPruebas->getNecesitara());
								        			$sIconos="";
									        		for ($ico=0, $max = sizeof($aIconos); $ico < $max; $ico++){
									        			$sIconos .= "<span class=\"icon-" . $aIconos[$ico] . " ico-24\" alt=\"" . $aIconos[$ico] . "\" title=\"" . $aIconos[$ico] . "\"></span>";
													}
													echo $sIconos;
								        		?>
								        	</td>
							        			...
							        			if($iPintadoBoton==0){
							       					switch ($cPruebas->getIdTipoPrueba())
													{
														...
														case "18":	//Vita AV
																...
																$sLLAMADA = "ruta=" . constant("HTTP_SERVER_AV") . "&proceso=" . $cCandidato->getIdProceso() . "&prueba=" . $cPruebas->getIdPrueba() . "&usuario=" . $cCandidato->getToken();
									        					?>
									        					<input type="button" class="botones" value="<?php echo constant("STR_COMENZAR");?>" style="vertical-align: text-bottom;margin-bottom: 15px;" onclick="javascript:pan_VitaAV('<?php echo base64_encode($sLLAMADA);?>');"/>
									        					<?php
																break;
...
<?php	if ($iPruebasFinalizadas == $iListaPruebas){	?>
				<div style="margin-left:16%;padding-top:20px;">
					<?php
						if (!empty($sLang) && $sLang == 'es' ){
						...
						if ($cCandidato->getIdEmpresa() == '5826'){
							if (!empty($sLang) && $sLang == 'es' ){
								$sMSG_GRACIAS_PRUEBAS_REALIZADAS = "";
								$sMSG_DPTO_ADMISIONES_CONTACTO = "Ha completado todas sus pruebas. Para que el examen sea registrado debe de ir a la pestaña “ProctorExam” y hacer click en &lt;Finalizar Examen&gt;.";
							}else {
								$sMSG_GRACIAS_PRUEBAS_REALIZADAS = "";
								$sMSG_DPTO_ADMISIONES_CONTACTO = "You have completed all your tests. You should go to “ProctorExam” website tab and click on &lt;Finish Exam&gt; button for the exam to be registered.";
							}
						}
					 ?>
						<?php echo $sMSG_GRACIAS_PRUEBAS_REALIZADAS;?>
					<br /><br />
					<?php echo $sMSG_DPTO_ADMISIONES_CONTACTO;?>
					<br /><br />
				</div>
...
<input type="hidden" name="fVitaAV" value="" />
