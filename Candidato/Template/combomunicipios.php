...
include_once ('include/conexion.php');
	...
	ob_start();
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:top.cambiamunicipios()\"";
		$sNombreCampo	= "fIdMunicipio";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST["fDescProvincia"]) && $_POST["fDescProvincia"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "descProvincia=" . $conn->qstr($_POST["fDescProvincia"], false);
		}
		...
		if (empty($sWhere)){
			$sWhere = "idMunicipio='0'";
		}
		if ($bJoin){
			//$comboMUNICIPIOS= new Combo($conn, "fIdMunicipio", "DISTINCT(s.idMunicipio)", "s.descripcion" , "descripcion", "municipios s, suscriptorestemasboletins stm","","Seleccione una opción",$sWhere, "idMunicipio");
		}else{
			$comboMUNICIPIOS= new Combo($conn,"fIdMunicipio","idMunicipio","descripcion","descripcion","wi_municipios","","Seleccione una opción",$sWhere, "idMunicipio");
		}
		...
		if (isset($_POST["bBus"]) && $_POST["bBus"]){
			$sNombreCampo="LSTIdMunicipio";
			$sValor=explode(",",isset($_POST["LSTIdMunicipio"]) ? $_POST["LSTIdMunicipio"] : "");
		}else{
			//$sOption		= "<option value='-1'>Todos</option>";
			$sValor=explode(",",isset($_POST["fIdMunicipio"]) ? $_POST["fIdMunicipio"] : "");
		}
		...
		$comboMUNICIPIOS->setNombre($sNombreCampo);
    echo $comboMUNICIPIOS->getHTMLCombo($lineas, "cajatexto", $sValor, $sComboProp, $sOption);
ob_end_flush();
?>