    	...
    	while ($arr = $rs->FetchRow()){
			...
			<script type="text/javascript">
				...
				$(document).ready(function()
				{
					...
					$("#jquery_jplayer_1").jPlayer({
						ready: function (event) {
							$(this).jPlayer("setMedia", {
                                ...
                                if ($nVeces < 2) {
								?>
									title: "You have played the audio <?php echo $nVeces; ?> times of 2",
									m4a: "locucion<?php echo $_POST['fIdPrueba']?>.mp3"
								<?php
                                }else{
								?>
									title: "You have listened the audio times allowed, it is not possible to play it again.",
									m4a: "graf/sp.gif"
								<?php	
								}
								?>
					...
					$('#audio').css({'padding-bottom': '20px'});
					$(".jp-play").click( function() {				  
						var current = $('#jquery_jplayer_1').data("jPlayer").status.currentTime;
						if(current == "0"){
							if (nv < 2){
								nv = nv+1;
								var sTitle =  "You have played the audio "+nv+" times of 2";
								$("#jp_container_1 .jp-title").text(sTitle);
								setListening();
							}else{
								var sTitle =  "You have listened the audio times allowed, it is not possible to play it again.";
								$("#jp_container_1 .jp-title").text(sTitle);
