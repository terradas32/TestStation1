...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Tipos_prueba/Tipos_pruebaDB.php");
	require_once(constant("DIR_WS_COM") . "Tipos_prueba/Tipos_prueba.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Tipos_pruebaDB($conn);  // Entidad DB
	$cEntidad	= new Tipos_prueba();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">jAlert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php 
				...
				{
					if (!empty($_POST['tipos_prueba_next_page']) && $_POST['tipos_prueba_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'tipos_prueba');
					...
					$lista=$pager->getRS();
					include('Template/Tipos_prueba/mnttipos_pruebal.php');
				}else{
					$cEntidad	= new Tipos_prueba();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Tipos_prueba/mnttipos_pruebaa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Tipos_prueba/mnttipos_prueba.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdTipoPrueba((isset($_POST["LSTIdTipoPrueba"]) && $_POST["LSTIdTipoPrueba"] != "") ? $_POST["LSTIdTipoPrueba"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TIPO_PRUEBA"), (isset($_POST["LSTIdTipoPrueba"]) && $_POST["LSTIdTipoPrueba"] != "" ) ? $_POST["LSTIdTipoPrueba"] : "");
		$cEntidad->setIdTipoPruebaHast((isset($_POST["LSTIdTipoPruebaHast"]) && $_POST["LSTIdTipoPruebaHast"] != "") ? $_POST["LSTIdTipoPruebaHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TIPO_PRUEBA") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdTipoPruebaHast"]) && $_POST["LSTIdTipoPruebaHast"] != "" ) ? $_POST["LSTIdTipoPruebaHast"] : "");
