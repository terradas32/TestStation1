...
include_once ('include/conexion.php');
	...
	if ($_POST['MODO'] == constant("MNT_CONSULTAR") && $_EmpresaLogada == $_POST["fIdEmpresa"] ){
		$sHijos = constant("EMPRESA_PE") . "," . $sHijos;
	}
	$sSQLPruebaIN = "";
...
//				$cEntidad = readLista($cEntidad);
				...
				$_idsPruebas = $cEntidad->getIdsPruebas();
				//echo $_idsPruebas;exit;
				/**
				 * Reviso los baremos a los que tiene acceso la empresa Padre
				 */
				$cEmpresaNew = new Empresas();
				$cEmpresaPadre = new Empresas();
 				$cEmpresaPadreDB = new EmpresasDB($conn);
 				$cEmpresaPadre->setIdPadre($cEntidad->getIdPadre());
 				$cEmpresaPadre = $cEmpresaPadreDB->readEntidadPadre($cEmpresaPadre);
				$_IdsPruebasAleatorias = $cEmpresaPadre->getIdsPruebasAleatorias();
				$cEmpresaNew->setIdEmpresa($newId);
				$cEmpresaNew = $cEntidadDB->readEntidad($cEmpresaNew);
				$cEmpresaNew->setIdsPruebasAleatorias($_IdsPruebasAleatorias);
				// $cEntidadDB->modificar($cEmpresaNew);
				$cEntidadDB->modificarSinPass($cEmpresaNew);
				//Revisamos si la prueba tiene escalas (Detectamos así si es de personalidad)
				$_idBloque="";	$_idEscala="";
				$cEscalasItems = new Escalas_items();
				//Recorremos TODAS las pruebas.
				$_aidsPruebas= explode(",", $_idsPruebas);
				for ($i=0, $max = sizeof($_aidsPruebas); $i < $max; $i++){

					$cBaremos_empresas = new Baremos_empresas();
					$cBaremos_empresas->setIdEmpresa($cEmpresaPadre->getIdEmpresa());
					$cBaremos_empresas->setIdPrueba($_aidsPruebas[$i]);

					$sqlBaremos_empresas= $cBaremos_empresasDB->readLista($cBaremos_empresas);
					//echo "<br >" . $sqlBaremos_empresas;
					$listaBaremos_empresas = $conn->Execute($sqlBaremos_empresas);
					//Replicamos los baremos de la empresa Padre a la empresa que se crea.
					while (!$listaBaremos_empresas->EOF)
					{
						$cBaremos_empresasNew = new Baremos_empresas();
						$cBaremos_empresasNew->setIdEmpresa($newId);
						$cBaremos_empresasNew->setIdPrueba($listaBaremos_empresas->fields['idPrueba']);
						$cBaremos_empresasNew->setIdBaremo($listaBaremos_empresas->fields['idBaremo']);
						$cBaremos_empresasDB->borrar($cBaremos_empresasNew);
						$cBaremos_empresasNew->setUsuAlta($_cEntidadUsuarioTK->getIdEmpresa());
						$cBaremos_empresasNew->setUsuMod($_cEntidadUsuarioTK->getIdEmpresa());
						$cBaremos_empresasDB->insertar($cBaremos_empresasNew);
...
///////////////////////////////////////////////////////////////////
		...
		case constant("MNT_MODIFICAR"):
			...
			if ($cEntidadDB->modificar($cEntidad))
			{
				...
				$cEntidad->setIdsPruebasAleatorias($_IdsPruebasAleatorias);
				//$cEntidadDB->modificar($cEntidad);
				$cEntidadDB->modificarSinPass($cEntidad);
				...
				for ($i=0, $max = sizeof($_aidsPruebas); $i < $max; $i++){
					...
					{
						$cBaremos_empresasNew = new Baremos_empresas();
						$cBaremos_empresasNew->setIdEmpresa($cEntidad->getIdEmpresa());
						$cBaremos_empresasNew->setIdPrueba($listaBaremos_empresas->fields['idPrueba']);
						$cBaremos_empresasNew->setIdBaremo($listaBaremos_empresas->fields['idBaremo']);
						$cBaremos_empresasDB->borrar($cBaremos_empresasNew);
						$cBaremos_empresasNew->setUsuAlta($_cEntidadUsuarioTK->getIdEmpresa());
