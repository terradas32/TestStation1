...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Formaciones/FormacionesDB.php");
	require_once(constant("DIR_WS_COM") . "Formaciones/Formaciones.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new FormacionesDB($conn);  // Entidad DB
	$cEntidad	= new Formaciones();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['formaciones_next_page']) && $_POST['formaciones_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'formaciones');
					...
					$lista=$pager->getRS();
					include('Template/Formaciones/mntformacionesl.php');
				}else{
					$cEntidad	= new Formaciones();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Formaciones/mntformacionesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Formaciones/mntformaciones.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdFormacion((isset($_POST["LSTIdFormacion"]) && $_POST["LSTIdFormacion"] != "") ? $_POST["LSTIdFormacion"] : "");	$cEntidad->setBusqueda(constant("STR_ID_FORMACION"), (isset($_POST["LSTIdFormacion"]) && $_POST["LSTIdFormacion"] != "" ) ? $_POST["LSTIdFormacion"] : "");
		$cEntidad->setIdFormacionHast((isset($_POST["LSTIdFormacionHast"]) && $_POST["LSTIdFormacionHast"] != "") ? $_POST["LSTIdFormacionHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_FORMACION") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdFormacionHast"]) && $_POST["LSTIdFormacionHast"] != "" ) ? $_POST["LSTIdFormacionHast"] : "");
