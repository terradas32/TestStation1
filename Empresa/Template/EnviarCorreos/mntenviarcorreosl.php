...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="2" border="0" width="99%">
		...
		<tr onmouseover="this.bgColor='<?php echo constant("ONMOUSEOVER");?>'" onmouseout="this.bgColor='<?php echo constant("ONMOUSEOUT");?>'" bgcolor="<?php echo constant("ONMOUSEOUT");?>">
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idCandidato']);?>','<?php echo addslashes($lista->fields['idEmpresa']);?>','<?php echo addslashes($lista->fields['idProceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['informado']);?>">
				<?php
				$cProcesos->setIdProceso($lista->fields['idProceso']);
				$cProcesos->setIdEmpresa($lista->fields['idEmpresa']);
				$cProcesos = $cProcesosDB->readEntidad($cProcesos);
				if ($cProcesos->getFecEnvioProgramado() !="" && $lista->fields['informado'] == 0){
					echo constant("STR_ENVIO_PROGRAMADO") . " " . $cProcesos->getFecEnvioProgramado();
				}else{
          switch ($lista->fields['informado'])
          {
            case 0:
              echo constant("STR_NO");
              break;
            case 1:
              echo constant("STR_SI");
              break;
            case 2:
              echo constant("STR_REPLICADA");
              break;
            default:
            		echo constant("STR_NO");
              break;
          } // end switch
					//echo ($lista->fields['informado'] > 0 ) ? constant("STR_SI") : constant("STR_NO");
				}
				?>
			</td>
...
<!--
 ...
 -->
	...
	<input type="hidden" name="fSQLtoEXCEL" value="<?php echo base64_encode($sql . constant("CHAR_SEPARA") . "candidatos");?>" />
