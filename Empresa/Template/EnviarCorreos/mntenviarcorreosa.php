...
function chkTodos(){
	var f=document.forms[0];
	var sTodo= "<?php echo $cEntidad->getIdCandidato();?>";
	var reenviar = "<?php echo $bReenviar;?>";
	if (sTodo == "-1"){
		if (reenviar == "1")
		{
			lon();
			setTimeout('selTodos()',3000);
		}
	}
}
function selTodos(){
	var f=document.forms[0];
	f.fIdCandidato.value="-1";
	if (f.fIdCandidato.value == "-1"){
...
function msgReenvio(){
	jAlert("¡¡<?php echo constant("STR_ATENCION");?>!!:\n\n<?php echo constant("MSG_REENVIO_CONTRASENIA");?>","<?php echo constant("STR_NOTIFICACION");?>");
}
		...
		<div style="width: 100%">
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				?>
					...
					<td>
						<input type="radio" onclick="javascript:msgReenvio();cambiaIdProceso();" name="fInformado" id="fInformado1" value="1"  <?php echo ($cEntidad->getInformado() != "" && strtoupper($cEntidad->getInformado()) == "1") ? "checked=\"checked\"" : "";?> />
						&nbsp;
						<label for="fInformado1"><?php echo constant("STR_SI");?></label>&nbsp;
						...
						<label for="fInformado0"><?php echo constant("STR_NO");?></label>&nbsp;
...
<input type="hidden" name="fiTotalCandidatosInicio" value="<?php echo (!empty($_POST['fiTotalCandidatosInicio'])) ? $_POST['fiTotalCandidatosInicio'] : '0';?>" />
<input type="hidden" name="fiTotalEnviados" value="<?php echo (!empty($_POST['fiTotalEnviados'])) ? $_POST['fiTotalEnviados'] : '0';?>" />
<input type="hidden" name="fMailsNOEnviados" value="<?php echo (!empty($_POST['fMailsNOEnviados'])) ? $_POST['fMailsNOEnviados'] : '';?>" />
<input type="hidden" name="fMailsNOEnviadosLessYear" value="<?php echo (!empty($_POST['fMailsNOEnviadosLessYear'])) ? $_POST['fMailsNOEnviadosLessYear'] : '';?>" />
	...
	<input type="hidden" name="candidatos_next_page" value="<?php echo (isset($_POST['candidatos_next_page'])) ? $cUtilidades->validaXSS($_POST['candidatos_next_page']) : "1";?>" />
						<script language="javascript" type="text/javascript">
							...
							function cambiaIdEmpresa(){
								var f= document.forms[0];
								$("#comboIdProceso").show().load("jQuery.php",{sPG:"comboprocesos",bBus:"0",multiple:"0",nLineas:"1",bObliga:"1",bgColor:"<?php echo constant("BG_COLOR")?>",fNombreCampo:"fIdProceso",fJSProp:"cambiaIdProceso",fIdEmpresa:f.fIdEmpresa.value,vSelected:"<?php echo $cEntidad->getIdProceso();?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"},function(){$("#comboIdCandidato").show().load("jQuery.php",{sPG:"combocandidatosenvio",bBus:"0",multiple:"0",nLineas:"20",bObliga:"1",multiple:"1",fNombreCampo:"fIdCandidato",fJSProp:"cambiaIdCandidato",fIdEmpresa:f.fIdEmpresa.value,fIdProceso:f.fIdProceso.value,fInformado:f.fInformadoHast.value,vSelected:"<?php echo $cEntidad->getIdCandidato();?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"}).fadeIn("slow");}).fadeIn("slow");
								cambiaIdTipoCorreo();
							}
							//]]>
						</script>
						<script language="javascript" type="text/javascript">
							...
							function cambiaIdProceso(){
								...
								if (f.fInformadoHast.value != ""){
									setTimeout(function(){
										$("#comboIdCandidato").show().load("jQuery.php",{sPG:"combocandidatosenvio",bBus:"0",multiple:"0",nLineas:"20",bObliga:"1",multiple:"1",fNombreCampo:"fIdCandidato",fJSProp:"cambiaIdCandidato",fIdEmpresa:f.fIdEmpresa.value,fIdProceso:f.fIdProceso.value,fInformado:f.fInformadoHast.value,vSelected:"<?php echo $cEntidad->getIdCandidato();?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"},function(){cambiaIdTipoCorreo();}).fadeIn("slow");
									},2000)
								}
							}
							//]]>
