...
function setPK(idAcceso)
{
	var f=document.forms[0];
	f.fIdAcceso.value=idAcceso;
}
...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="2" border="0" width="100%">
		<tr>
			...
			$sPinta = "";
			$sOrden = "ASC";
			if ($cEntidad->getOrderBy() == "idAcceso"){
			...
			?>
			<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities(constant("STR_ID_ACCESO"), ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='idAcceso';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?>  >>> <?php echo addslashes(constant("STR_ID_ACCESO"));?>';return true"><?php echo constant("STR_ID_ACCESO");?><?php echo $sPinta;?></a></td>
			<?php 
			$sPinta = "";
			$sOrden = "ASC";
		...
		<tr onmouseover="this.bgColor='<?php echo constant("ONMOUSEOVER");?>'" onmouseout="this.bgColor='<?php echo constant("ONMOUSEOUT");?>'" bgcolor="<?php echo constant("ONMOUSEOUT");?>">
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['idAcceso']));?>"><?php echo substr(str_replace("\n","<br />",strip_tags(html_entity_decode($lista->fields['idAcceso']), "<b><i><u><strong><br><br />")),0,11);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['IP']));?>"><?php echo substr(str_replace("\n","<br />",strip_tags(html_entity_decode($lista->fields['IP']), "<b><i><u><strong><br><br />")),0,255);?></td>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['login']));?>"><?php echo substr(str_replace("\n","<br />",strip_tags(html_entity_decode($lista->fields['login']), "<b><i><u><strong><br><br />")),0,255);?></td>
			<td class="tddatoslista" valign="top" align="center" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['fecAlta']));?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecAlta'];?>'));</script></td>
			<td class="tddatoslista" valign="top" align="center" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags(html_entity_decode($lista->fields['fecMod']));?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecMod'];?>'));</script></td>
			<td class="negro" align="center" valign="middle"><?php if($_bBorrar) {;?><a href="javascript:setPK('<?php echo addslashes($lista->fields['idAcceso']);?>');confBorrar(<?php echo constant("MNT_BORRAR");?>,'<?php echo constant("DEL_GENERICO");?>');"><img src="<?php echo constant("DIR_WS_GRAF");?>menos.gif" width="9" height="9" border="0" alt="<?php echo constant("STR_BORRAR");?>" /></a><?php };?></td>
		</tr>
		<?php $i++;
		$lista->MoveNext();
		} ?>
...
<?php $HELP="xx";
include (constant("DIR_WS_INCLUDE") . 'cabecera.php');
$aBuscador= $cEntidad->getBusqueda();?>
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width="10"><img src="<?php echo constant("DIR_WS_GRAF");?>sp.gif" width="10" height="1" border="0" alt="" /></td>
			<td width="100%" colspan="2" class="naranjab"><?php echo sprintf(constant("STR_LISTA_DE_"),str_replace('_', ' ', 'usuarios_accesos'));?> <font class="tdnaranjab"><a href="sqlToExcel.php?fSQLtoEXCEL=SQLUsuarios_accesosDB&fNombreFicheroExcel=<?php echo str_replace('_', ' ', 'usuarios_accesos');?>" ><img src="<?php echo constant("DIR_WS_GRAF");?>excel.gif" width="34" height="35" align="right" border="0" alt="<?php echo constant("STR_EXPORTAR_A_EXCEL");?>" /></a></td>
		</tr>
		<tr>
			<td width="10"><img src="<?php echo constant("DIR_WS_GRAF");?>sp.gif" width="10" height="1" border="0" alt="" /></td>
	...
	<br />
	<input type="hidden" name="fIdAcceso" value="" />
	<input type="hidden" name="ORIGEN" value="<?php echo constant("MNT_LISTAR");?>" />
	<input type="hidden" name="fReordenar" value="" />
	<input type="hidden" name="LSTIdAcceso" value="<?php echo (isset($_POST['LSTIdAcceso'])) ? $_POST['LSTIdAcceso'] : "";?>" />
	<input type="hidden" name="LSTIP" value="<?php echo (isset($_POST['LSTIP'])) ? $_POST['LSTIP'] : "";?>" />
	...
	<input type="hidden" name="usuarios_accesos_next_page" value="<?php echo (isset($_POST['usuarios_accesos_next_page'])) ? $cUtilidades->validaXSS($_POST['usuarios_accesos_next_page']) : "1";?>" />
</form>
<!-- Fin -->
