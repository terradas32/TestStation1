...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdEjemplo()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdEjemplo";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdEjemplo"]) && $_POST[$sPrefijo . "IdEjemplo"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idEjemplo=" . $conn->qstr($_POST[$sPrefijo . "IdEjemplo"], false);
		}
		...
		if ($bJoin){
			//$comboEJEMPLOS= new Combo($conn, $sPrefijo . "IdEjemplo", "DISTINCT(s.idEjemplo)", "s.descripcion", "descripcion", "ejemplos s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idEjemplo", "fecMod");
		}else{
			$comboEJEMPLOS= new Combo($conn, $sPrefijo . "IdEjemplo","idEjemplo","descripcion","descripcion","ejemplos","",constant("SLC_OPCION"),$sWhere, "idEjemplo", "fecMod");
		}
		...
		$comboEJEMPLOS->setNombre($sNombreCampo);
echo $comboEJEMPLOS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>