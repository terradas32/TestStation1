...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="1" border="0" alt="" /></td>
			<td width="100%" colspan="2" class="naranjab"><?php echo sprintf(constant("STR_LISTA_DE_"),str_replace('_', ' ', constant("STR_RESPUESTASPRUEBAS")));?>
<?php 
	$bPinto = false;
	$sURL_EXCEL = "";
	//Empresas que pueden exportar excel
	...
	if (in_array($_EmpresaLogada, $aDIRHU)){
		//$bPinto = true;
	}
	$sURL_EXCEL = constant("DIR_WS_GESTOR") . "Expecial_export_.php";
    //Buscamos si está en el array
	if ($bPinto)
	{
?>

			<input type="button" onclick="window.open('<?php echo $sURL_EXCEL;?>?call=<?php echo base64_encode("usr=" . $_EmpresaLogada); ?>', 'Excel', 'scrollbars=yes,status=no,resizable=yes')" class="botonesgrandes" title="Exportar Excel" value="Exportar Excel" name="Excel" />
<?php
	}
?>
			<a href="#_" onclick="javascript:document.forms[0].action='sqlToExcel.php';document.forms[0].submit();document.forms[0].action='<?php echo $_SERVER['PHP_SELF'];?>';"><img src="<?php echo constant('DIR_WS_GRAF');?>excel.gif" width="34" height="35" align="right" border="0" alt="<?php echo constant("STR_EXPORTAR_A_EXCEL");?>" /></a></td>
		</tr>
		<tr>
			<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="1" border="0" alt="" /></td>
