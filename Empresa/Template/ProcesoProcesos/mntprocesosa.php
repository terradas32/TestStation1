...
function enviar()
{
	...
	if (validaForm()){
		...
		if(hora.length!=5){
			jAlert("<?php echo constant("STR_FORMATO_HORA_INICIO_INCORRECTO_FORMATO_HH_MM");?>");
			return false;
		}
		...
		d=hora.charAt(3);
		if (a>=2 && b>3) {
			jAlert("<?php echo constant("STR_HORA_INICIO_INCORRECTA");?>");
			return false;
		}
		if(c!= ":"){
			jAlert("<?php echo constant("STR_FORMATO_HORA_INICIO_INCORRECTO_FORMATO_HH_MM");?>");
		...
		if (d>5) {
			jAlert("<?php echo constant("STR_MINUTOS_HORA_INICIO_INCORRECTA");?>");
			return false;
		}

		var	horaF = f.fHoraFin.value;
		if(horaF.length!=5){
			jAlert("<?php echo constant("STR_FORMATO_HORA_FIN_INCORRECTO_FORMATO_HH_MM");?>");
			return false;
		}
		...
		dF=horaF.charAt(3);
		if (aF>=2 && bF>3) {
			jAlert("<?php echo constant("STR_HORA_FIN_INCORRECTA");?>");
			return false;
		}
		if(cF!= ":"){
			jAlert("<?php echo constant("STR_FORMATO_HORA_FIN_INCORRECTO_FORMATO_HH_MM");?>");
		...
		if (dF>5) {
			jAlert("<?php echo constant("STR_MINUTOS_HORA_FIN_INCORRECTA");?>");
			return false;
		}
//		if (!CheckTime(hora)){
//			return false;
...
//	msg +=vString("<?php echo constant("STR_OBSERVACIONES");?>:",f.fObservaciones.value,4000,false);
	...
	if (msg == "") {
		...
		<?php	$sFecha = date("d/m/Y");	$sHoraJS = date("G");	$sMin = date("i");
			...
			$sMin = $aHOR[1];
		?>
		var HI= f.fHoraInicio.value;	var aI=HI.charAt(0);	var bI=HI.charAt(1);	var cI=HI.charAt(2);	var dI=HI.charAt(3);	var eI=HI.charAt(4);
		var horaI = aI + bI;
		var minI = dI + eI;

		var HF= f.fHoraFin.value;	var aF=HF.charAt(0);	var bF=HF.charAt(1);	var cF=HF.charAt(2);	var dF=HF.charAt(3);	var eF=HF.charAt(4);
		var horaF = aF + bF;
		var minF = dF + eF;

		var dFI = toJSDate(f.fFechaInicio.value,"dd/mm/yyyy", horaI, minI);	var dFF = toJSDate(f.fFechaFin.value,"dd/mm/yyyy", horaF, minF);
		var fDesde   = dFI;	var fHasta   = dFF;
		...
		var fSistema = toJSDate("<?php echo $sFecha?>","dd/mm/yyyy", <?php echo $sHoraJS?>, <?php echo $sMin?>);
			...
			if (f.MODO.value == "<?php echo constant("MNT_ALTA")?>"){
				if(!bCheckI){
					if (fDesde < fSistema){
						msg +="<?php echo constant("STR_NO_PUEDE_SER_MENOR_QUE_HOY");?>\n";
					}
				}
			}else{
		...
		if (fDesde > fHasta)
			msg +="<?php echo constant("STR_NO_PUEDE_SER_MAYOR_QUE_LA_FECHA_FIN");?>\n";
		if (msg != "")
			msg ="<?php echo constant("STR_FEC_INICIO");?>:\n" + msg;
	}
	if (msg == ""){
		if (dFI > dFF){
			msg ="<?php echo constant("STR_LA_FECHA_HORA_INICIO_NO_PUEDE_SER_MAYOR_QUE_LA_DE_FIN");?>\n";
		}
	}
if (msg != "") {
...
function getTimeZoneOK(obj){
	var f= document.forms[0];
	f.sPG.value="timezoneEmpresa";
	$("#timezone").hide(200).load("jQuery.php",$("form").serializeArray()).fadeIn("slow");
}
function getProcesoConfidencial(obj){
	var f= document.forms[0];
	f.sPG.value="procesoConfidencialEmpresa";
	$("#requestProcesoConfidencial").hide(200).load("jQuery.php",$("form").serializeArray()).fadeIn("slow");
}
				...
				<tr>
					...
					<td width="100%" style="border-bottom: 1px solid #000000;">
						<ul class="listaProceso">
							<li class="mActivo"><span><img src="./graf/iconsMenu/Tools.png" title="Datos proceso" alt="Datos proceso" /></span><b style="color:#FFB200"><?php echo constant("STR_DATOS_PROCESO");?></b></li>
							<li class="mArrow">&nbsp;</li>
				...
				<tr>
					<td width="100%" colspan="2" >
						<table cellspacing="0" cellpadding="0" width="100%" border="0">
							...
							<tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
								<td nowrap="nowrap" width="175" class="negrob" valign="top"><?php echo constant("STR_EMPRESA");?>&nbsp;</td>
								<?php
									$sReadOnly="";
								...
								?>
								<td><?php $comboEMPRESAS->setNombre("fIdEmpresa");?><?php echo $comboEMPRESAS->getHTMLComboMenu("1","obliga",$cEntidad->getIdEmpresa(), $sReadOnly . " onchange=\"getTimeZoneOK(this);getProcesoConfidencial(this);\"","");?></td>
							</tr>
 ...
 -->
							<tr>
								...
								if ($cEntidad->getFechaInicio() != "" && $cEntidad->getFechaInicio() != "0000-00-00" && $cEntidad->getFechaInicio() != "0000-00-00 00:00:00"){
									...
									$dt=new datetime("now",new datetimezone($cEmpresas->getTimezone()));
									$date = gmdate("Y-m-d H:i:s",(time()+$dt->getOffset()));	//Fecha actual de la Zona horaria
								...
								<td>
									...
									if($_POST['MODO'] == constant('MNT_MODIFICAR')){
										if($listaInformados->recordCount()>0){?>
											<img src="<?php echo constant('DIR_WS_GRAF');?>icon_calendario.gif" width="22" height="18" border="0" alt="<?php echo constant('STR_CALENDARIO');?>" align="bottom" />&nbsp;<input type="text" readonly="readonly" name="fFechaInicio" value="<?php echo $cEntidad->getFechaInicio();?>" class="obliga" style="width:75px;" onchange="javascript:trim(this);" />&nbsp;<input type="text" readonly="readonly" name="fHoraInicio" value="<?php echo $sHoraInicio;?>" class="obliga" style="width:40px;" onchange="javascript:trim(this);" />&nbsp;&nbsp;<?php echo constant('STR_VERIFICADO')?>:&nbsp;<input type="checkbox" checked="checked" name="fCheckHi" disabled="disabled"/>
									<?php
										}else{?>
											<a href="#" onclick="javascript:abrirCalendario('calendario.php?openerNombre=fFechaInicio','<?php echo constant("STR_CALENDARIO");?>');"><img src="<?php echo constant('DIR_WS_GRAF');?>icon_calendario.gif" width="22" height="18" border="0" alt="<?php echo constant('STR_CALENDARIO');?>" align="bottom" /></a>&nbsp;<input type="text" name="fFechaInicio" value="<?php echo $cEntidad->getFechaInicio();?>" class="obliga" style="width:75px;" onchange="javascript:trim(this);" />&nbsp;<input type="text" name="fHoraInicio" value="<?php echo $sHoraInicio;?>" class="obliga" style="width:40px;" onchange="javascript:trim(this);" />&nbsp;&nbsp;<?php echo constant('STR_VERIFICADO')?>:&nbsp;<input type="checkbox" name="fCheckHi" checked="checked" />
									<?php
										}
									}else{?>
							...
							<tr>
								...
								<td colspan="2">
									<table cellspacing="0" cellpadding="0" width="100%" border="0">
										<tr>
											<td nowrap="nowrap" width="175" class="negrob" valign="top"><?php echo constant("STR_FECHA_DE_FIN");?>&nbsp;</td>
											<?php
											$sHoraFin="";
              ...
              <?php
              $sDsiplayConfidencial="none";
              if ($cEmpresas->getProcesoConfidencial() != "")
              {
                $sDsiplayConfidencial="block";
              } ?>

              <tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
                <td colspan="2"><div id="requestProcesoConfidencial" style="display: none"></div>
                  <div id="ProcesoConfidencial" style="display: <?php echo $sDsiplayConfidencial; ?>;" >
                    <table width="100%" cellspacing="0" cellpadding="0" border="0">
                        <tr>
          								<td nowrap="nowrap" width="175" class="negrob" valign="top"><?php echo constant("STR_PROCESO_CONFIDENCIAL");?>&nbsp;</td>
          								<td>
          									<select size="1" name="fProcesoConfidencial">
          										<option style="color:#000000;" value="1" <?php echo ($cEntidad->getProcesoConfidencial() == 1) ? "selected" : "";?>><?php echo constant("STR_SI");?></option>
          										<option style="color:#000000;" value="0" <?php echo ($cEntidad->getProcesoConfidencial() <> 1) ? "selected" : "";?>><?php echo constant("STR_NO");?></option>
          									</select>
          								</td>
                        </tr>
                    </table>
