...
function cambiaidiomas(){
	...
	$("div#checkidiomas").load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdEmpresa: f.fIdEmpresa.value,MODO:"<?php echo constant('MNT_LISTAIDIOMAS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" });
}
function listapruebas(){
	var f = document.forms[0];
	var paginacargada = "ProcesoProcesos.php";
	$("div#listadepruebas").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,MODO:"<?php echo constant('MNT_AGREGAPRUEBA')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
}
function aniadepruebas(){
	var f = document.forms[0];
	var chequeado = false;
	var bActive_Fit = "<?php echo (!empty($cEmpresas->getpower_bi_active_fit())) ? $cEmpresas->getpower_bi_active_fit() : '0';?>";
	var bSelect_Fit = false;

	if (eval("document.forms[0].fIdTipoInforme1") !=null){
		for (i=1;i<200; i++){
			if (eval("document.forms[0].fIdTipoInforme"+i)!=null){
				obj = eval("document.forms[0].fIdTipoInforme"+i);
				if (obj.checked){
					chequeado = true;
					if (obj.value == "71"){
						bSelect_Fit=true;
	...
	var bEnvio = false;
	if (bActive_Fit == "0" && bSelect_Fit){
		bEnvio = false;
		jAlert("<?php echo constant("MSG_COMERCIAL_FIT");?>","<?php echo constant("STR_NOTIFICACION");?>");
	}else{
		bEnvio = true;
	}
	if (bEnvio){
		if (f.fPintaBaremo.value ==""){
			//No se valida el baremo
			if(f.fIdioma.value=="" || f.fIdPrueba.value=="" || f.fIdiomaInforme.value == "" || !chequeado){
				bEnvio = false;
				jAlert("<?php echo constant("ERR_FORM_ASIG_PRUEBA");?>");
			}else{
				bEnvio = true;
			}
		}else{
			//Sí se valida el baremo
			if(f.fIdioma.value=="" || f.fIdPrueba.value=="" || f.fIdBaremo.value=="" || f.fIdiomaInforme.value == "" || !chequeado){
				bEnvio = false;
				jAlert("<?php echo constant("ERR_FORM_ASIG_PRUEBA_BAREMO");?>");			
			}else{
				bEnvio = true;
			}
		}
	...
	if (bEnvio){
		f.fIdBaremo.disabled=false;
		var paginacargada = "ProcesoProcesos.php";
		f.fAniade.value="1";
		f.MODO.value="<?php echo constant('MNT_AGREGAPRUEBA')?>";
		$("div#listadepruebas").load(paginacargada, $("form").serializeArray()).fadeIn("slow");
	}
	
}
function borraprueba(idPrueba,codIdioma){
	var f = document.forms[0];

	jConfirm('<?php echo constant("DEL_GENERICO");?>', "<?php echo constant("STR_CONFIRMACION");?>", function(r) {  
		    if(r) {  
		var paginacargada = "ProcesoProcesos.php";
		$("div#listadepruebas").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,fIdPrueba: idPrueba,fIdioma: codIdioma,fIdBaremo: f.fIdBaremo.value,fBorra:"1",MODO:"<?php echo constant('MNT_AGREGAPRUEBA')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
				    }  
		}); 
...
<body onload="_body_onload();cambiaidiomas();listapruebas();block('<?php echo ($_POST["_block"] != "") ? $_POST["_block"] : "-1";?>');setClicado('<?php echo $_POST["_clicado"];?>');"  onunload="_body_onunload();">
				...
				<tr>
					...
					<td width="100%" style="border-bottom: 1px solid #000000;">
						<ul class="listaProceso">
							...
							<li class="mArrow">&nbsp;</li>
							<li  class="mActivo"><span><img src="./graf/iconsMenu/Folder.png" title="Pruebas" alt="Pruebas" /></span><b style="color:#FFB200"><?php echo constant("STR_PRUEBAS");?></b></li>
							<li class="mArrow">&nbsp;</li>
				...
				<tr>
					<td width="100%" colspan="2" >
						<table cellspacing="0" cellpadding="0" width="100%" border="0">
								...
								<td colspan="3">
									<div id="prueba" >
										<table cellspacing="0" cellpadding="0" border="0" width="100%">
											<tr>
												<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
												<td width="140px" nowrap="nowrap" class="negrob" valign="top"><?php echo constant("STR_PRUEBA");?>&nbsp;</td>
												<!--[if lte IE 8 ]>
												<style>
													.anchoS{width:75%;}
												</style>
												<![endif]-->
												<td class="anchoS" ><?php $comboPRUEBASGROUP->setNombre("fIdPrueba");?><?php echo $comboPRUEBASGROUP->getHTMLCombo("1","obliga",""," onchange=\"javascript:cambiaidiomas();\"","");?></td>
							...
							<tr>
								<td colspan="3" align="center"><input type="button" class="botones" id="bid-ok" name="fBtnOk" value="<?php echo constant("STR_ANIADIR");?>" onclick="javascript:aniadepruebas();"/></td>
							</tr>
							<tr>
								<td colspan="3" style="height:10px;" >
									&nbsp;
						...
						<table cellspacing="0" cellpadding="0" border="0">
							<tr>
								...
								<td><input type="button" class="botones" <?php echo ($_bModificar) ? '' : 'disabled="disabled"';?> id="bid-ok" name="fBtnOk" value="<?php echo constant("STR_SEGUIR");?>" onclick="javascript:enviarNuevo(<?php echo constant('MNT_ANIADECANDIDATOS')?>)"/></td>
							</tr>
						</table>
					</td>
				</tr>
