...
function listacandidato(){
	...
	$("div#listadecandidatos").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,MODO:"<?php echo constant('MNT_LISTACANDIDATOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
}
function aniadecandidato(){
	...
	if (msg != "") {
		...
		var paginacargada = "ProcesoProcesos.php";
		$("div#listadecandidatos").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,fNombre: f.fNombre.value, fApellido1: f.fApellido1.value, fApellido2: f.fApellido2.value, fDni: f.fDni.value, fMail: f.fMail.value,fPagoTpv: f.fPagoTpv.value,fAniade:"1",MODO:"<?php echo constant('MNT_LISTACANDIDATOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" },function(){var f = document.forms[0];f.fNombre.value="";f.fApellido1.value="";f.fApellido2.value="";f.fDni.value="";f.fMail.value="";}).fadeIn("slow");
	}
}
function aniadeblind(){
	var f = document.forms[0];
	var msg="";
	msg +=vNumber("<?php echo constant("STR_N_DE_ALTAS");?>:",f.fNumAltas.value,11,true);
	if (msg != "") {
		jAlert("<?php echo constant("ERR_FORM");?>:\n\n"+msg+"\n\n<?php echo constant("ERR_FORM_CORRIJA");?>.\n\n\t<?php echo constant("STR_MUCHAS_GRACIAS");?>.","<?php echo constant("STR_NOTIFICACION");?>");
	}else{
		jConfirm(f.fNumAltas.value + " <?php echo constant("STR_ALTAS_ANONIMAS");?>.\n\n\t<?php echo constant("STR_ESTA_SEGURO");?>", "<?php echo constant("STR_CONFIRMACION");?>", function(r) {
		    if(r) {
		    	var paginacargada = "ProcesoProcesos.php";
...
function volcar(){
	...
	var paginacargada = "ProcesoProcesos.php";
	$("div#listadecandidatos").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,fVolcar:"1",MODO:"<?php echo constant('MNT_LISTACANDIDATOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
...
function borracandidato(idCandidato){
	...
	jConfirm('<?php echo constant("DEL_GENERICO");?>', "<?php echo constant("STR_CONFIRMACION");?>", function(r) {
		    if(r) {
		var paginacargada = "ProcesoProcesos.php";
		$("div#listadecandidatos").hide().load(paginacargada,{fIdProceso: f.fIdProceso.value,fIdEmpresa: f.fIdEmpresa.value,fIdCandidato: idCandidato,fBorra:"1",MODO:"<?php echo constant('MNT_LISTACANDIDATOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
				    }
		});
...
function cargaalta(){
	...
	if(f.fTipoAlta.value==""){
		...
		$("div#carga").hide().load(paginacargada, {fTipoAlta:f.fTipoAlta.value, fIdEmpresa: f.fIdEmpresa.value, fIdProceso: f.fIdProceso.value,MODO:"<?php echo constant('MNT_ANIADECANDIDATOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	}
}
...
function aniadecandidatomasivo(){
	...
	if (f.fFichero.value != ""){
		...
		jAlert("Introduzca un fichero CSV.");
	}
}
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					...
					<td width="100%" style="border-bottom: 1px solid #000000; height:35px">
						<ul class="listaProceso">
							...
							<li class="mArrow">&nbsp;</li>
							<li class="mActivo"><span><img src="./graf/iconsMenu/User.png" title="Candidatos" alt="Candidatos" /></span><b style="color:#FFB200"><?php echo constant("STR_CANDIDATOS");?></b></li>
							<li class="mArrow">&nbsp;</li>
				...
				<tr>
					<td width="100%" colspan="2" >
						<table cellspacing="0" cellpadding="0" width="100%" border="0">
							<tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="10" border="0" alt="" /></td>
								<td colspan="2" align="center" class="negrob"><?php echo constant("STR_SELECCIONA_EL_TIPO_DE_CARGA_DE_CANDIDATOS");?></td>
							</tr>
							<tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="10" border="0" alt="" /></td>
							...
							<tr>
								...
								<td colspan="2" align="center">
									<select name="fTipoAlta" onchange="javascript:cargaalta();">
										...
										<option value="1"><?php echo constant("STR_CARGA_MASIVA");?></option>
										<option value="2"><?php echo constant("STR_ALTA_MANUAL");?></option>
										<?php
										if (strtoupper($cEmpTPV->getAltaCiega()) == "ON"){
											echo '<option value="3">' . constant("STR_ALTA_ANONIMA") . '</option>';
										}
										?>
									</select>
								</td>
							...
							<tr>
								...
								<td colspan="2" style="height:1px;" bgcolor="#000000">
								</td>
							</tr>
							<tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="10" border="0" alt="" /></td>
						...
						<table cellspacing="0" cellpadding="0" border="0">
							<tr>
								...
								<td><input type="button" class="botones" <?php echo ($_bModificar) ? '' : 'disabled="disabled"';?> id="bid-ok" name="fBtnOk" value="<?php echo constant("STR_SEGUIR");?>" onclick="javascript:enviarNuevo('<?php echo constant('MNT_ESCOGECORREOS')?>')"/></td>
							</tr>
						</table>
					</td>
				</tr>
