...
function enviar()
{
	...
	if (validaForm()){
		lon();
		f.LSTIdBloque.value=f.elements['LSTIdBloque'].value;
...
function validaForm()
{
	...
	msg +=vNumber("<?php echo constant("STR_BLOQUE");?>:",f.elements['LSTIdBloque'].value, 11, false);
	msg +=vNumber("<?php echo constant("STR_ID_ESCALA");?>:", f.LSTIdEscala.value, 11, false);
	msg +=vNumber("<?php echo constant("STR_ID_ESCALA");?> <?php echo constant("STR_HASTA");?>:", f.LSTIdEscalaHast.value, 11, false);
	msg +=vString("<?php echo constant("STR_NOMBRE");?>:",f.LSTNombre.value,255, false);
	msg +=vString("<?php echo constant("STR_DESCRIPCION");?>:",f.LSTDescripcion.value,1000, false);
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					...
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="LSTIdBloque"><?php echo constant("STR_BLOQUE");?></label>&nbsp;</td>
						<td><div id="comboIdBloque"><?php $comboBLOQUES->setNombre("LSTIdBloque");?><?php echo $comboBLOQUES->getHTMLComboNull("1","cajatexto",$cEntidad->getIdBloque(),"onchange=\"javascript:cambiaIdBloque()\" ","");?></div></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					...
					<td>
						<table cellspacing="0" cellpadding="0" border="0">
							<tr>
								...
								<td><input type="text" id="LSTIdEscala" name="LSTIdEscala" value="<?php echo $cEntidad->getIdEscala();?>" class="cajatexto" style="width:75px;" onchange="javascript:trim(this);" /></td>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
								<td nowrap="nowrap" class="negrob"><?php echo constant("STR_HASTA");?>&nbsp;</td>
								<td><input type="text" id="LSTIdEscalaHast" name="LSTIdEscalaHast" value="<?php echo $cEntidad->getIdEscalaHast();?>" class="cajatexto" style="width:75px;" onchange="javascript:trim(this);" /></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<select id="LSTOrderBy" name="LSTOrderBy" size="1" class="cajatexto">
							...
							<option style='color:#000000;' value='idEscala' <?php echo (!empty($sOrderBy) && $sOrderBy == 'idEscala') ? "selected='selected'" : "";?>><?php echo constant("STR_ID_ESCALA");?></option>
	...
	<input type="hidden" name="escalas_next_page" value="1" />
						<script language="javascript" type="text/javascript">
							//<![CDATA[
							function cambiaCodIdiomaIso2(){
								var f= document.forms[0];
								$("#comboIdBloque").show().load("jQuery.php",{sPG:"combobloques",bBus:"1",multiple:"0",nLineas:"1",bObliga:"0",bgColor:"<?php echo constant("BG_COLOR")?>",fNombreCampo:"LSTIdBloque",fJSProp:"cambiaIdBloque",LSTCodIdiomaIso2:f.LSTCodIdiomaIso2.value,vSelected:"<?php echo $cEntidad->getIdBloque();?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"}).fadeIn("slow");
							}
							//]]>
						</script>
						<script language="javascript" type="text/javascript">
