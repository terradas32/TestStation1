...
function enviar()
{
	...
	if (validaForm()){
		lon();
		f.fIdBloque.value=f.elements['fIdBloque'].value;
		return true;
	}else	return false;
}
function validaForm()
{
	...
	msg +=vNumber("<?php echo constant("STR_BLOQUE");?>:",f.elements['fIdBloque'].value,11,true);
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
			...
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					...
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fIdBloque"><?php echo constant("STR_BLOQUE");?></label>&nbsp;</td>
						<td><div id="comboIdBloque"><?php $comboBLOQUES->setNombre("fIdBloque");?><?php echo $comboBLOQUES->getHTMLComboNull("1","obliga",$cEntidad->getIdBloque(),"onchange=\"javascript:cambiaIdBloque()\" ","");?></div></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
	...
	<input type="hidden" name="escalas_next_page" value="<?php echo (isset($_POST['escalas_next_page'])) ? $cUtilidades->validaXSS($_POST['escalas_next_page']) : "1";?>" />
						<script language="javascript" type="text/javascript">
							//<![CDATA[
							function cambiaCodIdiomaIso2(){
								var f= document.forms[0];
								$("#comboIdBloque").show().load("jQuery.php",{sPG:"combobloques",bBus:"0",multiple:"0",nLineas:"1",bObliga:"1",bgColor:"<?php echo constant("BG_COLOR")?>",fNombreCampo:"fIdBloque",fJSProp:"cambiaIdBloque",fCodIdiomaIso2:f.fCodIdiomaIso2.value,vSelected:"<?php echo $cEntidad->getIdBloque();?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"}).fadeIn("slow");
							}
							//]]>
						</script>
						<script language="javascript" type="text/javascript">
