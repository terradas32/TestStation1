...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdComunidad()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdComunidad";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdPais"])){
		...
		if ($bJoin){
			//$comboWI_CCAA= new Combo($conn, $sPrefijo . "IdComunidad", "DISTINCT(s.idComunidad)", "s.descripcion", "descripcion", "wi_ccaa s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idComunidad", "fecMod");
		}else{
			$comboWI_CCAA= new Combo($conn, $sPrefijo . "IdComunidad","idComunidad","descripcion","descripcion","wi_ccaa","",constant("SLC_OPCION"),$sWhere, "idComunidad", "fecMod");
		}
		...
		$comboWI_CCAA->setNombre($sNombreCampo);
echo $comboWI_CCAA->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>