...
function validaForm()
{
	var f=document.forms[0];
	var msg="";
	msg +=vNumber("<?php echo constant("STR_BLOQUE");?>:",f.fIdBloque.value,11,true);
	msg +=vNumber("<?php echo constant("STR_ESCALA");?>:",f.elements['fIdEscala'].value,11,true);
...
function listaItems(){
	...
	var paginacargada = "Escalas_items.php";
	$("div#lItems").show().load(paginacargada,{
		idPrueba: f.fIdPrueba.value,
		idEscala: f.fIdEscala.value,
		idBloque: f.fIdBloque.value,
		MODO:"<?php echo constant('MNT_CARGAITEMS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" 
	}).fadeIn("slow");
...
function anadir(){
	...
	if(f.fIdPrueba.value!="" && f.fIdEscala.value!="" && f.fIdBloque.value && sItems!=""){
		var paginacargada = "Escalas_items.php";
		$("div#lItems").show().load(paginacargada,{
			idPrueba: f.fIdPrueba.value,
			idEscala: f.fIdEscala.value,
			...
			idsItems: sItems,
			fAniadir:"1",
			MODO:"<?php echo constant('MNT_GUARDAASIGNADOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" 
		}).fadeIn("slow");
	}else{
		jAlert("Debe seleccionar al menos un item del seleccionable 'Items pendientes de asignar'.","<?php echo constant("STR_NOTIFICACION");?>");
	}
}
function quitar(){
	var f=document.forms[0];
	...
	if(f.fIdPrueba.value!="" && f.fIdEscala.value!="" && f.fIdBloque.value && sItems!=""){
		...
		jAlert("Debe seleccionar al menos un item del seleccionable 'Items asignados'.","<?php echo constant("STR_NOTIFICACION");?>");
	}
...
function limpiar(){
	...
	if(f.fIdPrueba.value!="" && f.fIdEscala.value!="" && f.fIdBloque.value){
		var paginacargada = "Escalas_items.php";
		$("div#lItems").show().load(paginacargada,{
			idPrueba: f.fIdPrueba.value,
			idEscala: f.fIdEscala.value,
		...
		jAlert("Debe seleccionar un bloque, una escala y una prueba para poder limpiar items asignados.","<?php echo constant("STR_NOTIFICACION");?>");
	}
...
<body onload="_body_onload();cambiaIdBloque();block('<?php echo ($_POST["_block"] != "") ? $_POST["_block"] : "-1";?>');setClicado('<?php echo $_POST["_clicado"];?>');listaItems();" onunload="_body_onunload();">
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<input type="hidden" name="escalas_items_next_page" value="<?php echo (isset($_POST['escalas_items_next_page'])) ? $cUtilidades->validaXSS($_POST['escalas_items_next_page']) : "1";?>" />
						<script language="javascript" type="text/javascript">
							//<![CDATA[
