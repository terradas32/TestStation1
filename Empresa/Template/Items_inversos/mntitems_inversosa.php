...
function listaItems(){
	...
	var paginacargada = "Items_inversos.php";
...
function limpiar(){
	...
	if(f.fIdPrueba.value!=""){
		...
		jAlert("Debe seleccionar una prueba para poder limpiar items asignados.","<?php echo constant("STR_NOTIFICACION");?>");
	}
...
<body onload="_body_onload();cambiaIdPrueba();block('<?php echo ($_POST["_block"] != "") ? $_POST["_block"] : "-1";?>');setClicado('<?php echo $_POST["_clicado"];?>');listaItems();" onunload="_body_onunload();">
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<input type="hidden" name="items_inversos_next_page" value="<?php echo (isset($_POST['items_inversos_next_page'])) ? $cUtilidades->validaXSS($_POST['items_inversos_next_page']) : "1";?>" />
						<script language="javascript" type="text/javascript">
							//<![CDATA[
