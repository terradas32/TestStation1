...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdSeccion()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdSeccion";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdSeccion"]) && $_POST[$sPrefijo . "IdSeccion"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idSeccion=" . $conn->qstr($_POST[$sPrefijo . "IdSeccion"], false);
		}
		if (isset($_POST[$sPrefijo . "IdTipoInforme"])){
		...
		if ($bJoin){
			//$comboSECCIONES_INFORMES= new Combo($conn, $sPrefijo . "IdSeccion", "DISTINCT(s.idSeccion)", "s.nombre", "descripcion", "secciones_informes s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idSeccion", "fecMod");
		}else{
			$comboSECCIONES_INFORMES= new Combo($conn, $sPrefijo . "IdSeccion","idSeccion","nombre","descripcion","secciones_informes","",constant("SLC_OPCION"),$sWhere, "idSeccion", "fecMod");
		}
		...
		$comboSECCIONES_INFORMES->setNombre($sNombreCampo);
echo $comboSECCIONES_INFORMES->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>