...
function aniadebaremo(){
	...
	if(f.fIdBloque == null && f.fIdEscala == null){
		if(f.fIdPrueba.value==""){
			jAlert("Debe seleccionar una prueba.","<?php echo constant("STR_NOTIFICACION");?>");
		}else{
			$("div#aniadebaremo").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,MODO:"<?php echo constant('MNT_ANIADEBAREMO')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
		}
	}else{
		if(f.fIdPrueba.value!="" && f.fIdBloque.value!="" && f.fIdEscala.value!=""){
			$("div#aniadebaremo").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,MODO:"<?php echo constant('MNT_ANIADEBAREMO')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
		}else{
			jAlert("Debe seleccionar una prueba, un bloque y una escala.","<?php echo constant("STR_NOTIFICACION");?>");
		}
	}
}
function comprueba(){
	...
	if(f.fIdPrueba.value!=""){
		...
		$("div#escalas").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value, fIdBloque:idBloque, fIdEscala:idEscala, MODO:"<?php echo constant('MNT_COMPRUEBAESCALAS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
	}
}
function listabaremos(escala, compNo){
	...
	if(f.fIdBloque ==null && f.fIdEscala == null){
		if(f.fIdPrueba.value!=""){
			...
			if(compNo=="0"){
				$("div#listabaremos").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBloque: "0",fIdEscala: "0",MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
			}else{
				$("div#listabaremos").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value, fIdBloque: idBloque, fIdEscala: idEscala,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
			}
		...
		if(f.fIdPrueba.value!=""){
			if(compNo=="0"){
				...
				if(f.fIdBloque.value !="" && f.fIdEscala.value !=""){
					$("div#listabaremos").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBloque: f.fIdBloque.value,fIdEscala: f.fIdEscala.value,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
				}else{
					if(f.fIdBloqueOrigen.value !="" && f.fIdEscalaOrigen.value !=""){
						$("div#listabaremos").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBloque: f.fIdBloqueOrigen.value,fIdEscala: f.fIdEscalaOrigen.value,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
					}else{
						$("div#listabaremos").hide().load(paginacargada,{fIdPrueba: f.fIdPrueba.value,fIdBloque: "-1",fIdEscala: "-1",MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
					}
				}
			}
		}else{
...
function guardabaremos(){
	...
	if(f.fIdBloque ==null && f.fIdEscala == null){
		if(f.fIdPrueba.value!=""){
			$("div#listabaremos").hide().load(paginacargada,{fAniade:"1",fIdPrueba: f.fIdPrueba.value,fNombre: f.fNombre.value,fDescripcion: f.fDescripcion.value,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
		}else{
			$("div#listabaremos").empty();
		}
	}else{
		if(f.fIdPrueba.value!="" && f.fIdBloque.value!="" && f.fIdEscala.value!=""){
			$("div#listabaremos").hide().load(paginacargada,{fAniade:"1",fIdPrueba: f.fIdPrueba.value,fIdBloque: f.fIdBloque.value,fIdEscala: f.fIdEscala.value,fNombre: f.fNombre.value,fDescripcion: f.fDescripcion.value,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
		}else{
			$("div#listabaremos").empty();
		}
	}
...
function borrabaremo(idBaremo, idPrueba, idBloque, idEscala){
	var f = document.forms[0];
	jConfirm('<?php echo constant("DEL_GENERICO");?>', "<?php echo constant("STR_CONFIRMACION");?>", function(r) {  
		    if(r) {  
		var paginacargada = "Baremos.php";
		$("div#listabaremos").hide().load(paginacargada,{fBorra:"1",fIdPrueba: idPrueba,fIdBloque: idBloque,fIdEscala: idEscala,fIdBaremo: idBaremo,MODO:"<?php echo constant('MNT_LISTABAREMOS')?>", fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken()?>" }).fadeIn("slow");
				    }  
		}); 
...
function cambiaIdBloque(escala){
	var f= document.forms[0];
	$("#comboIdEscala").show().load("jQuery.php",{sPG:"comboescalas2",vSelected:escala,bBus:"0",multiple:"0",nLineas:"1",bObliga:"1",bgColor:"<?php echo constant("BG_COLOR")?>",fNombreCampo:"fIdEscala",fJSProp:"",fCodIdiomaIso2:f.fCodIdiomaIso2.value,fIdBloque:f.fIdBloque.value, fLang:"<?php echo $sLang;?>", sTK:"<?php echo $_cEntidadUsuarioTK->getToken();?>"}).fadeIn("slow");
}
