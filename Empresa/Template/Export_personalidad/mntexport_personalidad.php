...
<head>
	...
	<script language="javascript" type="text/javascript">
			...
			if (msg == "") {
				oSuggest = {
					...
					callback: function(obj) {
						document.getElementById('idProceso').value = obj.id;
					}
				};
		...
		function validaForm() {
			...
			//JOSH
			//var Obj=document.forms[0].elements["LSTTipoInforme0"];
			msg += vString("<?php echo constant("STR_TIPO_INFORME"); ?>:", Obj.value, 255, true);
			if (msg != "") {
				jAlert("<?php echo constant("ERR_FORM"); ?>:\n\n" + msg + "\n\n<?php echo constant("ERR_FORM_CORRIJA"); ?>.\n\n\t<?php echo constant("STR_MUCHAS_GRACIAS"); ?>.", "<?php echo constant("STR_NOTIFICACION"); ?>");
				return false;
			} else return true;
		...
		// TODO JOSH //llamar cuando cambias el tipo de prueba
		/* function anadirTipoInforme(nombre,texto,valor){
			if (valor !=""){
		...
		function cambiaPruebas() {
			...
			if (seleccionarMultiple('LSTIdEmpresa[]')) {
				if (seleccionarMultiple('LSTIdProceso[]')) {
					...
					f.LSTFecPrueba.value = cFechaFormat(f.LSTFecPrueba.value);
					f.LSTFecPruebaHast.value = cFechaFormat(f.LSTFecPruebaHast.value);
					$("#pruebas").hide().load("pruebasPersonalidad.php", $("form").serializeArray(), function(response, status, xhr) {

						//console.log("status del load de la function cambiaPruebas: " + xhr.status + " " + xhr.statusText + " " + JSON.stringify(($("form").serializeArray())));
		...
		/* function cambiaTipoInforme(){
			...
			if (seleccionarMultiple('LSTIdEmpresa[]')){
				if (seleccionarMultiple('LSTIdProceso[]')){
					...
					f.LSTFecPrueba.value=cFechaFormat(f.LSTFecPrueba.value);
					f.LSTFecPruebaHast.value=cFechaFormat(f.LSTFecPruebaHast.value);
					$("#pruebas").hide().load("tipoInformePersonalidad.php",$("form").serializeArray(),function(){
						var sDesde=formatThisDate(f.LSTFecPrueba.value);
						var sHasta=formatThisDate(f.LSTFecPruebaHast.value);
					    setTimeout(function(){
							f.LSTFecPrueba.value=sDesde;
		...
		/********************************************************************** */
		// NUEVAS FUNCIONES DE JOSH

		function validaSeleccionaPrueba(nombre) {
			var f = document.forms[0];
			texto = f.LSTDescProceso0.value;
			valor = f.idProceso.value;
			if (validaRepeProceso(nombre, f.LSTDescProceso0.value)) {
				seleccionarPrueba(nombre, texto, valor);
			} else {
				jAlert("<?php echo constant("STR_REPETIDO"); ?>", "<?php echo constant("STR_NOTIFICACION"); ?>");
			}
		}

		function seleccionarPrueba(nombre, texto, valor) {
			console.log("llamada a seleccionarPrueba");
			if (valor != "") {
				ObjCombo = document.forms[0].elements[nombre];
				ii = ObjCombo.length;
				bAniadir = true;
		...
		/* function cambiaInformes(){
			console.log("llamada a cambiaInformes");
			var f= document.forms[0];
			if (seleccionarMultiple('LSTIdEmpresa[]')){
				if (seleccionarMultiple('LSTIdProceso[]')){
					$("#bid-buscar").attr('disabled','disabled');
...
<body onload="_body_onload();block('<?php echo ($_POST["_block"] != "") ? $_POST["_block"] : "-1"; ?>');setClicado('<?php echo $_POST["_clicado"]; ?>');cambiaPruebas();" onunload="_body_onunload();">
				...
				<div id="contenido">
					<div style="width: 100%">
						<table cellspacing="0" cellpadding="0" width="100%" border="0">
							<tr>
								<td colspan="3" align="center" class="naranjab"><?php echo constant("STR_BUSCADOR"); ?></td>
							</tr>
							<tr>
							...
							<tr>
								<?php
								//$test = "'hi con var'";
								//$sAction = 'onchange="javascript:alert(' . $test . ');"';

								//$sAction = 'esto es un test';
								//var_dump($cEntidad);
								//die;
								//var_dump($cEntidad->getDescPrueba());
								//die;
								//echo '<script>console.log("ey", "'.$sCabecera.'");</script>';
								//echo '<script>console.log("ey2","'.$cEntidad->getDescPrueba().'");</script>';
								//echo '<script>console.log('.json_encode($comboPRUEBAS->getHTMLCombo("1","obliga",$cEntidad->getDescPrueba(),$sAction,"")).');</script>';
								//die;
								?>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF'); ?>sp.gif" width="5" height="20" border="0" alt="" /></td>
								<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="LSTDescPrueba0"><?php echo constant("STR_PRUEBA"); ?></label>&nbsp;</td>
								<td>
									<div id="pruebas">
										<table cellspacing="0" cellpadding="0" width="99%" border="0">
											<tr>
												<td width="48.5%" valign="top"><?php $comboPRUEBAS->setNombre("LSTDescPrueba0"); ?><?php echo $comboPRUEBAS->getHTMLCombo("1", "obliga", $cEntidad->getDescPrueba(), $sAction, ""); ?></td>
												<td width="50" valign="top">
													<table cellspacing="0" cellpadding="0" width="100%" border="0">
														<tr>
							...
							<!-- <button type="" onclik="javascript:alert('hi sin var');">TEST DE ACTUALIZACION DE TIPOS DE INFORME</button> -->
							<!-- JOSH -->
							<tr>
								<td width="5"><img src="<?php echo constant('DIR_WS_GRAF'); ?>sp.gif" width="5" height="20" border="0" alt="" /></td>
								<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="LSTTipoInforme0"><?php echo constant("STR_TIPO_INFORME"); ?></label>&nbsp;</td>
								<td>
									<div id="tipos_informe">
										<table cellspacing="0" cellpadding="0" width="99%" border="0">
											<tr>
												<td width="48.5%" valign="top"><?php $comboTIPOS_INFORMES->setTipoInforme("LSTTipoInforme0"); ?><?php echo $comboTIPOS_INFORMES->getHTMLCombo("1", "obliga", $cEntidad->getDescPrueba(), $sAction, ""); ?></td>
												<td width="50" valign="top">
													<table cellspacing="0" cellpadding="0" width="100%" border="0">
														<tr>
															<td align="center" width="32">&nbsp;</td>
								...
								<td>
									<table cellspacing="0" cellpadding="0" border="0">
										<tr>
											...
											<td><a href="#" onclick="javascript:abrirCalendario('calendario.php?openerNombre=LSTFecPrueba','<?php echo constant("STR_CALENDARIO"); ?>');"><img src="<?php echo constant('DIR_WS_GRAF'); ?>icon_calendario.gif" width="22" height="18" border="0" alt="<?php echo constant("STR_CALENDARIO"); ?>" align="bottom" /></a>&nbsp;<input type="text" id="LSTFecPrueba" name="LSTFecPrueba" value="<?php echo $cEntidad->getFecPrueba(); ?>" class="obliga" style="width:75px;" onchange="javascript:trim(this);" /></td>
											<td width="5"><img src="<?php echo constant('DIR_WS_GRAF'); ?>sp.gif" width="5" height="20" border="0" alt="" /></td>
 ...
 -->
							...
							<tr>
								<td colspan="3"><?php echo constant("MSG_ANTIGUEDAD_EXPORT_INFO"); ?> </td>
							</tr>
							<tr>
								<td colspan="3"><img src="<?php echo constant('DIR_WS_GRAF'); ?>sp.gif" width="1" height="1" border="0" alt="" /></td>
							</tr>
