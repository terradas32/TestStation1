...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdEscala()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdEscala";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdBloque"])){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idBloque=" . $conn->qstr($_POST[$sPrefijo . "IdBloque"], false);
		}
		if (isset($_POST[$sPrefijo . "IdEscala"]) && $_POST[$sPrefijo . "IdEscala"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idEscala=" . $conn->qstr($_POST[$sPrefijo . "IdEscala"], false);
		}
		...
		if ($bJoin){
			//$comboESCALAS= new Combo($conn, $sPrefijo . "IdEscala", "DISTINCT(s.idBloque)", "s.nombre", "descripcion", "escalas s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idEscala", "fecMod");
		}else{
			$comboESCALAS= new Combo($conn, $sPrefijo . "IdEscala","idEscala","nombre","descripcion","escalas","",constant("SLC_OPCION"),$sWhere, "idEscala", "fecMod");
		}
		...
		$comboESCALAS->setNombre($sNombreCampo);
		
echo $comboESCALAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>