...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdBloque()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdBloque";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdBloque"]) && $_POST[$sPrefijo . "IdBloque"] != ""){
		...
		if ($bJoin){
			//$comboBLOQUES= new Combo($conn, $sPrefijo . "IdBloque", "DISTINCT(s.idBloque)", "s.nombre", "descripcion", "bloques s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idBloque", "fecMod");
		}else{
			$comboBLOQUES= new Combo($conn, $sPrefijo . "IdBloque","idBloque","nombre","descripcion","bloques","",constant("SLC_OPCION"),$sWhere, "idBloque", "fecMod");
		}
		...
		$comboBLOQUES->setNombre($sNombreCampo);
echo $comboBLOQUES->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>