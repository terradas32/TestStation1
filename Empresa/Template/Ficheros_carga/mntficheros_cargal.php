...
function setPK(idFichero)
{
	var f=document.forms[0];
	f.fIdFichero.value=idFichero;
}
...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="2" border="0" width="100%">
		<tr>
			...
			$sPinta = "";
			$sOrden = "ASC";
			if ($cEntidad->getOrderBy() == "idFichero"){
			...
			?>
			<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities(constant("STR_TIPO"), ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='tipo';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes(constant("STR_TIPO"));?>';return true"><?php echo constant("STR_TIPO");?><?php echo $sPinta;?></a></td>
			<?php 
			$sPinta = "";
			$sOrden = "ASC";
		...
		<tr onmouseover="this.bgColor='<?php echo constant("ONMOUSEOVER");?>'" onmouseout="this.bgColor='<?php echo constant("ONMOUSEOUT");?>'" bgcolor="<?php echo constant("ONMOUSEOUT");?>">
			...
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFichero']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['idFichero']);?>"><?php echo substr(str_replace("\n","<br />",strip_tags($lista->fields['idFichero'], "<b><i><u><strong><br><br />")),0,11);?></td>
		<?php	if (!empty($lista->fields['fichero']))
			{
				...
				$sLlamada = '<td class="tddatoslista" valign="top" onclick="javascript:setPK(\'' . addslashes($lista->fields['idFichero']) . '\');enviar(' . constant("MNT_CONSULTAR") . ');" title="">&nbsp;</td>';
			}
		echo $sLlamada; 
		?>
			<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFichero']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['tipo']);?>"><?php echo substr(str_replace("\n","<br />",strip_tags($lista->fields['tipo'], "<b><i><u><strong><br><br />")),0,255);?></td>
			<td class="tddatoslista" valign="top" align="center" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idFichero']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['fecAlta']);?>"><script language="javascript" type="text/javascript">document.write(formatThisDate('<?php echo $lista->fields['fecAlta'];?>'));</script></td>
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width="10"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="10" height="1" border="0" alt="" /></td>
			<td width="100%" colspan="2" class="naranjab"><?php echo sprintf(constant("STR_LISTA_DE_"),str_replace('_', ' ', constant("STR_FICHEROSCARGA")));?><a href="sqlToExcel.php?fSQLtoEXCEL=<?php echo base64_encode($sql . constant("CHAR_SEPARA") . "ficheros_carga");?>"><img src="<?php echo constant('DIR_WS_GRAF');?>excel.gif" width="34" height="35" align="right" border="0" alt="<?php echo constant("STR_EXPORTAR_A_EXCEL");?>" /></a></td>
		</tr>
		<tr>
			<td width="10"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="10" height="1" border="0" alt="" /></td>
	...
	<br />
	<input type="hidden" name="fIdFichero" value="" />
	...
	<input type="hidden" name="LSTTipo" value="<?php echo (isset($_POST['LSTTipo'])) ? $cUtilidades->validaXSS($_POST['LSTTipo']) : "";?>" />
	...
	<input type="hidden" name="ficheros_carga_next_page" value="<?php echo (isset($_POST['ficheros_carga_next_page'])) ? $cUtilidades->validaXSS($_POST['ficheros_carga_next_page']) : "1";?>" /></div>
