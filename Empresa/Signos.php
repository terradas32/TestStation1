...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new SignosDB($conn);  // Entidad DB
	$cEntidad	= new Signos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['signos_next_page']) && $_POST['signos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'signos');
					...
					$lista=$pager->getRS();
					include('Template/Signos/mntsignosl.php');
				}else{
					$cEntidad	= new Signos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Signos/mntsignosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Signos/mntsignos.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		global $cUtilidades;
		$cEntidad->setIdSigno((isset($_POST["fIdSigno"])) ? $_POST["fIdSigno"] : "");
		$cEntidad->setSigno((isset($_POST["fSigno"])) ? $_POST["fSigno"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdSigno((isset($_POST["LSTIdSigno"]) && $_POST["LSTIdSigno"] != "") ? $_POST["LSTIdSigno"] : "");	$cEntidad->setBusqueda(constant("STR_ID_SIGNO"), (isset($_POST["LSTIdSigno"]) && $_POST["LSTIdSigno"] != "" ) ? $_POST["LSTIdSigno"] : "");
		$cEntidad->setIdSignoHast((isset($_POST["LSTIdSignoHast"]) && $_POST["LSTIdSignoHast"] != "") ? $_POST["LSTIdSignoHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_SIGNO") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdSignoHast"]) && $_POST["LSTIdSignoHast"] != "" ) ? $_POST["LSTIdSignoHast"] : "");
		$cEntidad->setSigno((isset($_POST["LSTSigno"]) && $_POST["LSTSigno"] != "") ? $_POST["LSTSigno"] : "");	$cEntidad->setBusqueda(constant("STR_SIGNO"), (isset($_POST["LSTSigno"]) && $_POST["LSTSigno"] != "" ) ? $_POST["LSTSigno"] : "");
