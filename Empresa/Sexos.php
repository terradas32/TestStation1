...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Sexos/SexosDB.php");
	require_once(constant("DIR_WS_COM") . "Sexos/Sexos.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new SexosDB($conn);  // Entidad DB
	$cEntidad	= new Sexos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['sexos_next_page']) && $_POST['sexos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'sexos');
					...
					$lista=$pager->getRS();
					include('Template/Sexos/mntsexosl.php');
				}else{
					$cEntidad	= new Sexos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Sexos/mntsexosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Sexos/mntsexos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdSexo((isset($_POST["LSTIdSexo"]) && $_POST["LSTIdSexo"] != "") ? $_POST["LSTIdSexo"] : "");	$cEntidad->setBusqueda(constant("STR_ID_SEXO"), (isset($_POST["LSTIdSexo"]) && $_POST["LSTIdSexo"] != "" ) ? $_POST["LSTIdSexo"] : "");
		$cEntidad->setIdSexoHast((isset($_POST["LSTIdSexoHast"]) && $_POST["LSTIdSexoHast"] != "") ? $_POST["LSTIdSexoHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_SEXO") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdSexoHast"]) && $_POST["LSTIdSexoHast"] != "" ) ? $_POST["LSTIdSexoHast"] : "");
