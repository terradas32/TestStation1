...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Notificaciones_tiposDB($conn);  // Entidad DB
	$cEntidad	= new Notificaciones_tipos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['notificaciones_tipos_next_page']) && $_POST['notificaciones_tipos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'notificaciones_tipos');
					...
					$lista=$pager->getRS();
					include('Template/Notificaciones_tipos/mntnotificaciones_tiposl.php');
				}else{
					$cEntidad	= new Notificaciones_tipos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Notificaciones_tipos/mntnotificaciones_tiposa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Notificaciones_tipos/mntnotificaciones_tipos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdTipoNotificacion((isset($_POST["LSTIdTipoNotificacion"]) && $_POST["LSTIdTipoNotificacion"] != "") ? $_POST["LSTIdTipoNotificacion"] : "");	$cEntidad->setBusqueda(constant("STR_ID"), (isset($_POST["LSTIdTipoNotificacion"]) && $_POST["LSTIdTipoNotificacion"] != "" ) ? $_POST["LSTIdTipoNotificacion"] : "");
		$cEntidad->setIdTipoNotificacionHast((isset($_POST["LSTIdTipoNotificacionHast"]) && $_POST["LSTIdTipoNotificacionHast"] != "") ? $_POST["LSTIdTipoNotificacionHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdTipoNotificacionHast"]) && $_POST["LSTIdTipoNotificacionHast"] != "" ) ? $_POST["LSTIdTipoNotificacionHast"] : "");
