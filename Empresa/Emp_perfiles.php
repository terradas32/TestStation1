...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Emp_perfiles/Emp_perfilesDB.php");
	require_once(constant("DIR_WS_COM") . "Emp_perfiles/Emp_perfiles.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Emp_perfilesDB($conn);  // Entidad DB
	$cEntidad	= new Emp_perfiles();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['emp_perfiles_next_page']) && $_POST['emp_perfiles_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'emp_perfiles');
					...
					$lista=$pager->getRS();
					include('Template/Emp_perfiles/mntemp_perfilesl.php');
				}else{
					$cEntidad	= new Emp_perfiles();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Emp_perfiles/mntemp_perfilesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Emp_perfiles/mntemp_perfiles.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdPerfil((isset($_POST["LSTIdPerfil"]) && $_POST["LSTIdPerfil"] != "") ? $_POST["LSTIdPerfil"] : "");	$cEntidad->setBusqueda(constant("STR_ID_PERFIL"), (isset($_POST["LSTIdPerfil"]) && $_POST["LSTIdPerfil"] != "" ) ? $_POST["LSTIdPerfil"] : "");
		$cEntidad->setIdPerfilHast((isset($_POST["LSTIdPerfilHast"]) && $_POST["LSTIdPerfilHast"] != "") ? $_POST["LSTIdPerfilHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_PERFIL") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdPerfilHast"]) && $_POST["LSTIdPerfilHast"] != "" ) ? $_POST["LSTIdPerfilHast"] : "");
