...
//	echo('modo:' . $_POST['MODO']);
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			$cEmpresaTramita->setIdEmpresa($_EmpresaLogada);
			$cEmpresaTramita = $cEmpresasDB->readEntidad($cEmpresaTramita);

			$cEmpresaOrigen = new Empresas();
			$cEmpresaOrigen->setIdEmpresa($cEntidad->getIdEmpresaReceptora());
			$cEmpresaOrigen = $cEmpresasDB->readEntidad($cEmpresaOrigen);

			$cEmpresaDestino = new Empresas();
			$cEmpresaDestino->setIdEmpresa($cEntidad->getIdEmpresa());
			$cEmpresaDestino = $cEmpresasDB->readEntidad($cEmpresaDestino);

			$cEntidad->setEstado('Movimiento de ' . $cEntidad->getNDongles() . ' unidades realizado por: ' . $cEmpresaTramita->getNombre() . ' (' . $cEmpresaTramita->getIdEmpresa() . ') Descontadas de: ' . $cEmpresaOrigen->getNombre() . ', asignadas a: ' . $cEmpresaDestino->getNombre());
			//
			$newId	= $cEntidadDB->insertarMovimiento($cEntidad);
			if (!empty($newId)){
				?><script language="javascript" type="text/javascript">alert("Su solicitud ha sido tramitada correctamente.");</script><?php

				$cEntidad	= new Peticiones_dongles();  // inicializamos la Entidad
				$_POST['MODO']    = constant("MNT_ALTA");
				include('Template/Mover_dongles/mntmover_donglesa.php');
		...
		case constant("MNT_MODIFICAR"):
			...
			if ($cEntidadDB->modificar($cEntidad))
			{
				...
				if (!empty($_POST['mover_dongles_next_page']) && $_POST['mover_dongles_next_page'] > 1 ){
					$bInit=false;
				...
				$pager = new ADODB_Pager($conn,$sql,'mover_dongles');
				...
				$lista=$pager->getRS();
				include('Template/Mover_dongles/mntmover_donglesl.php');
			}else{
		...
		case constant("MNT_CONSULTAR"):
			...
			break;
		case 555:
			if((isset($_POST['fIdEmpresaOrigen']) && $_POST['fIdEmpresaOrigen']!=""))
			{
				$cEmpresas	= new Empresas();
				$cEmpresas->setIdEmpresa($_POST['fIdEmpresaOrigen']);
				$cEmpresas = $cEmpresasDB->readEntidad($cEmpresas);
				echo '<input type="text" disabled="disabled" id="fNDonglesOrigen" name="fNDonglesOrigen" readonly="readonly" value="' . $cEmpresas->getDongles() . '" class="obliga" style="width:75px;" onchange="javascript:trim(this);" />';
				if ($cEmpresas->getDongles() <= 0){
					echo '&nbsp;&nbsp;&nbsp;&nbsp;No se pueden mover unidades, La empresa ORIGEN tiene: <b class="rojob">' . $cEmpresas->getDongles() . '</b> Unidades.';
				}
			}
			if((isset($_POST['fIdEmpresaDestino']) && $_POST['fIdEmpresaDestino']!=""))
			{
				$cEmpresas	= new Empresas();
				$cEmpresas->setIdEmpresa($_POST['fIdEmpresaDestino']);
				$cEmpresas = $cEmpresasDB->readEntidad($cEmpresas);
				echo '<input type="text" disabled="disabled" id="fNDonglesDestino" name="fNDonglesDestino" readonly="readonly" value="' . $cEmpresas->getDongles() . '" class="obliga" style="width:75px;" onchange="javascript:trim(this);" />';
			}
			break;
		case constant("MNT_BUSCAR"):
			$_POST['MODO']    = constant("MNT_LISTAR");
			$cEntidad = readLista($cEntidad);
			include('Template/Mover_dongles/mntmover_dongles.php');
			break;
