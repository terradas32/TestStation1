...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Tratamientos/TratamientosDB.php");
	require_once(constant("DIR_WS_COM") . "Tratamientos/Tratamientos.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new TratamientosDB($conn);  // Entidad DB
	$cEntidad	= new Tratamientos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">jAlert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php 
				...
				{
					if (!empty($_POST['tratamientos_next_page']) && $_POST['tratamientos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'tratamientos');
					...
					$lista=$pager->getRS();
					include('Template/Tratamientos/mnttratamientosl.php');
				}else{
					$cEntidad	= new Tratamientos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Tratamientos/mnttratamientosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Tratamientos/mnttratamientos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdTratamiento((isset($_POST["LSTIdTratamiento"]) && $_POST["LSTIdTratamiento"] != "") ? $_POST["LSTIdTratamiento"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TRATAMIENTO"), (isset($_POST["LSTIdTratamiento"]) && $_POST["LSTIdTratamiento"] != "" ) ? $_POST["LSTIdTratamiento"] : "");
		$cEntidad->setIdTratamientoHast((isset($_POST["LSTIdTratamientoHast"]) && $_POST["LSTIdTratamientoHast"] != "") ? $_POST["LSTIdTratamientoHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_TRATAMIENTO") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdTratamientoHast"]) && $_POST["LSTIdTratamientoHast"] != "" ) ? $_POST["LSTIdTratamientoHast"] : "");
