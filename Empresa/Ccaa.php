...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Ccaa/CcaaDB.php");
	require_once(constant("DIR_WS_COM") . "Ccaa/Ccaa.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new CcaaDB($conn);  // Entidad DB
	$cEntidad	= new Ccaa();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['ccaa_next_page']) && $_POST['ccaa_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'ccaa');
					...
					$lista=$pager->getRS();
					include('Template/Ccaa/mntccaal.php');
				}else{
					$cEntidad	= new Ccaa();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Ccaa/mntccaaa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Ccaa/mntccaa.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdComunidad((isset($_POST["LSTIdComunidad"]) && $_POST["LSTIdComunidad"] != "") ? $_POST["LSTIdComunidad"] : "");	$cEntidad->setBusqueda(constant("STR_ID_COMUNIDAD"), (isset($_POST["LSTIdComunidad"]) && $_POST["LSTIdComunidad"] != "" ) ? $_POST["LSTIdComunidad"] : "");
