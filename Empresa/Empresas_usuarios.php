...
//    $cUsuariosDB	= new UsuariosDB($conn);  // Entidad DB
//	$cUsuarios	= new Usuarios();  // Entidad

    $cEntidadDB	= new Empresas_usuariosDB($conn);  // Entidad DB
	$cEntidad	= new Empresas_usuarios();  // Entidad
	...
	$comboUSUARIOS	= new Combo($conn,"fUsuAlta","idEmpresa","usuario","Descripcion","empresas","",constant("SLC_OPCION"),"");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
            ...
            $bEncontradoUsuario = $cUtilidades->chkChar($cEntidad->getUsuario());
            $bEncontradoPassword = $cUtilidades->chkChar($cEntidad->getPassword());
//            echo "<br />Usario:" . $cEntidad->getUsuario();
//            echo "<br />Pass:" . $cEntidad->getPassword();
            if (!$bEncontradoPassword)
            {
                if (!$bEncontradoUsuario)
                {
//                	echo "<br />Antes insert";
        			$newId	= $cEntidadDB->insertar($cEntidad);
//        			echo "<br />Antes insert new " . $newId;
        			if (!empty($newId)){
        			?><script>alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
        				...
        				{
        					if (!empty($_POST['empresas_usuarios_next_page']) && $_POST['empresas_usuarios_next_page'] > 1 ){
        						$bInit=false;
        					...
        					$pager = new ADODB_Pager($conn,$sql,'empresas_usuarios');
        					if ($bInit)	$pager->curr_page=1;
        					$pager->showPageLinks = true;
        					$lista=$pager->getRS();
        					include('Template/Empresas_usuarios/mntempresas_usuariosl.php');
        				}else{
        					$cEntidad	= new Empresas_usuarios();  // inicializamos la Entidad
        					$_POST['MODO']    = constant("MNT_ALTA");
        					include('Template/Empresas_usuarios/mntempresas_usuariosa.php');
        				}
        			}else{
      ...
      $cEntidad = $cEntidadDB->readEntidad($cEntidad);
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Empresas_usuarios/mntempresas_usuarios.php');
			break;
	...
	*/
	function readLista($cEntidad){
        global $conn;
        global $cUtilidades;
        $cEntidad->setIdEmpresa((!empty($_POST["LSTIdEmpresa"])) ? $_POST["LSTIdEmpresa"] : "");	$cEntidad->setBusqueda(constant("STR_EMPRESA"), (!empty($_POST["LSTIdEmpresa"])) ? $_POST["LSTIdEmpresa"] : "");
		...
		$cEntidad->setUsuario((!empty($_POST["LSTUsuario"])) ? $_POST["LSTUsuario"] : "");	$cEntidad->setBusqueda(constant("STR_USUARIO"), (!empty($_POST["LSTUsuario"])) ? $_POST["LSTUsuario"] : "");
		...
		$cEntidad->setApellido2((!empty($_POST["LSTApellido2"])) ? $_POST["LSTApellido2"] : "");	$cEntidad->setBusqueda(constant("STR_SEGUNDO_APELLIDO"), (!empty($_POST["LSTApellido2"])) ? $_POST["LSTApellido2"] : "");
		...
		$cEntidad->setIdEmpresa((!empty($_POST["LSTIdEmpresa"])) ? $_POST["LSTIdEmpresa"] : "");	$cEntidad->setBusqueda(constant("STR_EMPRESA"), (!empty($_POST["LSTIdEmpresa"])) ? $comboEMPRESAS->getDescripcionCombo($_POST["LSTIdEmpresa"]) : "");
