...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Textos_secciones/Textos_seccionesDB.php");
	require_once(constant("DIR_WS_COM") . "Textos_secciones/Textos_secciones.php");
	
include_once ('include/conexion.php');
	
	$cUtilidades	= new Utilidades();
	
	$cEntidadDB	= new Textos_seccionesDB($conn);  // Entidad DB
	$cEntidad	= new Textos_secciones();  // Entidad
	...
	$comboTIPOS_INFORMES	= new Combo($conn,"fIdTipoInforme","idTipoInforme","nombre","Descripcion","tipos_informes","",constant("SLC_OPCION"),"","","fecMod");
	$comboSECCIONES_INFORMES	= new Combo($conn,"fIdSeccion","idSeccion","nombre","Descripcion","secciones_informes","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">jAlert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php 
				...
				{
					if (!empty($_POST['textos_secciones_next_page']) && $_POST['textos_secciones_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'textos_secciones');
					...
					$lista=$pager->getRS();
					include('Template/Textos_secciones/mnttextos_seccionesl.php');
				}else{
					$cEntidad	= new Textos_secciones();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Textos_secciones/mnttextos_seccionesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Textos_secciones/mnttextos_secciones.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setIdSeccion((isset($_POST["fIdSeccion"])) ? $_POST["fIdSeccion"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdTipoInforme((isset($_POST["LSTIdTipoInforme"]) && $_POST["LSTIdTipoInforme"] != "") ? $_POST["LSTIdTipoInforme"] : "");	$cEntidad->setBusqueda(constant("STR_TIPO_INFORME"), (isset($_POST["LSTIdTipoInforme"]) && $_POST["LSTIdTipoInforme"] != "" ) ? $comboTIPOS_INFORMES->getDescripcionCombo($_POST["LSTIdTipoInforme"]) : "");
		global $comboSECCIONES_INFORMES;
		$cEntidad->setIdSeccion((isset($_POST["LSTIdSeccion"]) && $_POST["LSTIdSeccion"] != "") ? $_POST["LSTIdSeccion"] : "");	$cEntidad->setBusqueda(constant("STR_SECCION"), (isset($_POST["LSTIdSeccion"]) && $_POST["LSTIdSeccion"] != "" ) ? $comboSECCIONES_INFORMES->getDescripcionCombo($_POST["LSTIdSeccion"]) : "");
