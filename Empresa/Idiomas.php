...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new IdiomasDB($conn);  // Entidad DB
	$cEntidad	= new Idiomas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['idiomas_next_page']) && $_POST['idiomas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'idiomas');
					if ($bInit)	$pager->curr_page=1;
					$pager->showPageLinks = true;
					$lista=$pager->getRS();
					include('Template/Idiomas/mntidiomasl.php');
				}else{
					$cEntidad	= new Idiomas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Idiomas/mntidiomasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Idiomas/mntidiomas.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
        ...
        global $cUtilidades;
		$cEntidad->setCodIso2((isset($_POST["fCodIso2"])) ? $_POST["fCodIso2"] : "");
		$cEntidad->setNombre((isset($_POST["fNombre"])) ? $_POST["fNombre"] : "");
		$cEntidad->setPathImagen((isset($_POST["fPathImagen"])) ? $_POST["fPathImagen"] : "");
		$cEntidad->setOrden((isset($_POST["fOrden"])) ? $_POST["fOrden"] : "");
		$cEntidad->setActivoFront((isset($_POST["fActivoFront"])) ? $_POST["fActivoFront"] : "");
		$cEntidad->setActivoBack((isset($_POST["fActivoBack"])) ? $_POST["fActivoBack"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setCodIso2((isset($_POST["LSTCodIso2"]) && $_POST["LSTCodIso2"] != "") ? $_POST["LSTCodIso2"] : "");	$cEntidad->setBusqueda(constant("STR_COD_ISO2"), (isset($_POST["LSTCodIso2"]) && $_POST["LSTCodIso2"] != "" ) ? $_POST["LSTCodIso2"] : "");
		...
		$cEntidad->setPathImagen((isset($_POST["LSTPathImagen"]) && $_POST["LSTPathImagen"] != "") ? $_POST["LSTPathImagen"] : "");	$cEntidad->setBusqueda(constant("STR_IMAGEN"), (isset($_POST["LSTPathImagen"]) && $_POST["LSTPathImagen"] != "" ) ? $_POST["LSTPathImagen"] : "");
		...
		$cEntidad->setActivoFront((isset($_POST["LSTActivoFront"]) && $_POST["LSTActivoFront"] != "") ? $_POST["LSTActivoFront"] : "");	$cEntidad->setBusqueda(constant("STR_ACTIVO_EN_EL_FRONT"), (isset($_POST["LSTActivoFront"]) && $_POST["LSTActivoFront"] != "" ) ? $_POST["LSTActivoFront"] : "");
		$cEntidad->setActivoBack((isset($_POST["LSTActivoBack"]) && $_POST["LSTActivoBack"] != "") ? $_POST["LSTActivoBack"] : "");	$cEntidad->setBusqueda(constant("STR_ACTIVO_EN_EL_BACK"), (isset($_POST["LSTActivoBack"]) && $_POST["LSTActivoBack"] != "" ) ? $_POST["LSTActivoBack"] : "");
		...
		$cEntidad->setUsuAlta((isset($_POST["LSTUsuAlta"]) && $_POST["LSTUsuAlta"] != "") ? $_POST["LSTUsuAlta"] : "");	$cEntidad->setBusqueda(constant("STR_USU_ALTA"), (isset($_POST["LSTUsuAlta"]) && $_POST["LSTUsuAlta"] != "" ) ? $comboUSUARIOS->getDescripcionCombo($_POST["LSTUsuAlta"]) : "");
		$cEntidad->setUsuMod((isset($_POST["LSTUsuMod"]) && $_POST["LSTUsuMod"] != "") ? $_POST["LSTUsuMod"] : "");	$cEntidad->setBusqueda(constant("STR_USU_MOD"), (isset($_POST["LSTUsuMod"]) && $_POST["LSTUsuMod"] != "" ) ? $comboUSUARIOS->getDescripcionCombo($_POST["LSTUsuMod"]) : "");
	...
	*/
	function quitaImg($cEntidad, $cEntidadDB, $bBorrar= false){
		...
		if (!empty($_POST['cfPathImagen']) && strtoupper($_POST['cfPathImagen']) == 'ON'){
			$cEntidad->setPathImagen($_POST['cfPathImagen']);
			$bLlamada=true;
		}
		if ($bLlamada){
