<?php
@require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "gif.php");
class Upload
{
	...
	function __construct($sFormName){
		...
		if (($this->src_file != '') && ($this->src_file != 'none')){
			...
			$this->tmpConvertFile	= constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . '/tmpIMG/';
	...
	function UploadFile($server_path){
		...
		$this->dst_path_WS = str_replace (DIR_FS_DOCUMENT_ROOT_ADMIN, '', $this->dst_path);
		...
		if ($this->bImg){ //Miramos a ver si hay que hacer algo con ella.
			if ($this->image_convert != '') { //Vemos si hay que hacer algun proceso de conversión.
				preg_match('/\.([^\.]*$/)', $this->dst_name, $extension);
							...
							$this->image_mini )
			{
				...
				if ($bRetorno && $image_src)
				{
					...
					if ($this->image_resize)
					{
						...
						{
							if ($this->creaFirma()){
								require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Rwatermark.php");
	...
	*/
	function checkea_nombre($the_name) {
		$bRetorno	= true;
		if ($the_name != "") {
			if (!preg_match("/^([a-zA-Z0-9_-]+\.)*[a-zA-Z0-9_-]+\.[a-zA-Z0-9]{1,4}$/", $the_name)) {
	...
	*/
	...
	function copiarFichero(){
		...
		if ($this->getSobreEscribir()){
			if (!@copy($_FILES[$this->sFormName]['tmp_name'], $this->dst_pathname)) {
				if (!@move_uploaded_file($_FILES[$this->sFormName]['tmp_name'], $this->dst_pathname)){
