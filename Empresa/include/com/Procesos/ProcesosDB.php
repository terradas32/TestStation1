...
class ProcesosDB
{
	...
	************************************************************************/
	 ...
	 *  en la base de datos
	 ************************************************************************/
	function modificarEnvioProgramado($cEntidad)
	{
		$aux			= $this->conn;
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][EnvioProgramado][ProcesosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
