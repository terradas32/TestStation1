...
* la tabla respuestas_pruebas_items
**/
class Respuestas_pruebas_itemsDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr($cEntidad->getIdItem(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDescItem(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIdOpcion(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDescOpcion(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Respuestas_pruebas_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	/***********************************************************************
	* Devuelve la SQL de inserción máxiva
	* @param entidad Entidad a insertar con Datos
	* @return string SQL
	* @exception Exception Error al ejecutar la acción
	*  en la base de datos
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM respuestas_pruebas_items ";
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Respuestas_pruebas_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE respuestas_pruebas_items SET ";
		...
		$sql .= "descItem=" . $aux->qstr($cEntidad->getDescItem(), false) . ", ";
		$sql .= "idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . ", ";
		$sql .= "descOpcion=" . $aux->qstr($cEntidad->getDescOpcion(), false) . ", ";
		$sql .= "orden=" . $aux->qstr($cEntidad->getOrden(), false) . ", ";
		$sql .= "valor=" . $aux->qstr($cEntidad->getValor(), false) . ", ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Respuestas_pruebas_itemsDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
...
//		echo "<br />" . $sql;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getDescItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(descItem) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescItem() . "%") . ")";
		}
		if ($cEntidad->getIdOpcion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false);
		}
		if ($cEntidad->getDescOpcion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
	...
	*************************************************************************/
	function readListaIN($cEntidad)
	{
		...
		if ($cEntidad->getIdCandidato() != ""){
			...
			$and = true;
//			$sql .="idCandidato=" . $aux->qstr($cEntidad->getIdCandidato(), false);
			$sql .="idCandidato IN(" . $cEntidad->getIdCandidato() . ") ";
		}
		if ($cEntidad->getDescCandidato() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getIdItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idItem IN (" . $cEntidad->getIdItem() . ")";
		}
		if ($cEntidad->getDescItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function readListaAciertosErrores($cEntidad)
	{
		$aux			= $this->conn;

		$sql="";
		$and = false;
		$sql.="SELECT rpi.* FROM respuestas_pruebas_items rpi, respuestas_pruebas rp  ";
		$sql.="WHERE rpi.idEmpresa=rp.idEmpresa	AND rpi.idProceso=rp.idProceso AND rpi.idCandidato=rp.idCandidato AND rpi.codIdiomaIso2=rp.codIdiomaIso2 AND rpi.idPrueba=rp.idPrueba AND rp.finalizado=1";
		$and = true;
		if ($cEntidad->getIdEmpresa() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.idEmpresa IN(" . $cEntidad->getIdEmpresa() . ") ";
		}
		if ($cEntidad->getDescEmpresa() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descEmpresa) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescEmpresa() . "%") . ")";
		}
		if ($cEntidad->getIdProceso() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false);
		}
		if ($cEntidad->getDescProceso() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descProceso) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescProceso() . "%") . ")";
		}
		if ($cEntidad->getIdCandidato() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
//			$sql .="idCandidato=" . $aux->qstr($cEntidad->getIdCandidato(), false);
			$sql .="rpi.idCandidato IN(" . $cEntidad->getIdCandidato() . ") ";
		}
		if ($cEntidad->getDescCandidato() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descCandidato) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescCandidato() . "%") . ")";
		}
		if ($cEntidad->getCodIdiomaIso2() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false);
		}
		if ($cEntidad->getDescIdiomaIso2() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descIdiomaIso2) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescIdiomaIso2() . "%") . ")";
		}
		if ($cEntidad->getIdPrueba() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.idPrueba IN (" . $cEntidad->getIdPrueba() . ") ";
		}
		if ($cEntidad->getDescPrueba() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descPrueba) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescPrueba() . "%") . ")";
		}
		if ($cEntidad->getIdItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.idItem IN (" . $cEntidad->getIdItem() . ")";
		}
		if ($cEntidad->getDescItem() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descItem) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescItem() . "%") . ")";
		}
		if ($cEntidad->getIdOpcion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false);
		}
		if ($cEntidad->getDescOpcion() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rpi.descOpcion) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getDescOpcion() . "%") . ")";
		}
		if ($cEntidad->getOrden() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.orden>=" . $aux->qstr($cEntidad->getOrden(), false);
		}
		if ($cEntidad->getOrdenHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.orden<=" . $aux->qstr($cEntidad->getOrdenHast(), false);
		}
		if ($cEntidad->getFecAlta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.fecAlta>=" . $aux->qstr($cEntidad->getFecAlta(), false);
		}
		if ($cEntidad->getFecAltaHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.fecAlta<=" . $aux->qstr($cEntidad->getFecAltaHast(), false);
		}
		if ($cEntidad->getFecMod() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.fecMod>=" . $aux->qstr($cEntidad->getFecMod(), false);
		}
		if ($cEntidad->getFecModHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.fecMod<=" . $aux->qstr($cEntidad->getFecModHast(), false);
		}
		if ($cEntidad->getUsuAlta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.usuAlta=" . $aux->qstr($cEntidad->getUsuAlta(), false);
		}
		if ($cEntidad->getUsuMod() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rpi.usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false);
		}
		if ($cEntidad->getOrderBy() != ""){
			$sql .=" ORDER BY " . $cEntidad->getOrderBy();
			if ($cEntidad->getOrder() != ""){
...
}//Fin de la Clase Respuestas_pruebas_itemsDB
?>
