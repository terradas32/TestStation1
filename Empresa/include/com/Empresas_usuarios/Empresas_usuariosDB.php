...
* la tabla Empresas_usuarios
**/
class Empresas_usuariosDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		if ($iCont > 0 ){
			...
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Usuario Existe [" . constant("MNT_ALTA") . "][Empresas_usuariosDB]";
		...
		if ($iCont > 0 ){
			//Existe un usuario con ese Login
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Usuario Existe [" . constant("MNT_ALTA") . "][Empresas_usuariosDB]see[Empresas]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
			...
			$_cEntidadEmpresasDB	= new EmpresasDB($aux);
			$_cEntidadEmpresas		= new Empresas();
			$_cEntidadEmpresas->setIdEmpresa($cEntidad->getUsuAlta());
			$_cEntidadEmpresasDB->readEntidad($_cEntidadEmpresas);
			...
			$cEntidadHistorico_cambios->setIdUsuario($_cEntidadEmpresas->getIdEmpresa());
			$cEntidadHistorico_cambios->setLogin($_cEntidadEmpresas->getUsuario());
			$cEntidadHistorico_cambios->setNombre($_cEntidadEmpresas->getNombre());
			$cEntidadHistorico_cambios->setApellido1($_cEntidadEmpresas->getApellido1());
			$cEntidadHistorico_cambios->setApellido2($_cEntidadEmpresas->getApellido2());
			$cEntidadHistorico_cambios->setEmail($_cEntidadEmpresas->getMail());
			...
			if (empty($HistId)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error [GUARDANDO HISTORICO][" . constant("MNT_ALTA") . "][Empresas_usuariosDB]";
				$this->msg_Error[]	= $sTypeError;
		...
		$cEmpPerfil = new Empresas_usuarios_perfiles();
		$cEmpPerfilDB = new Empresas_usuarios_perfilesDB($aux);

		$cEmpPerfil->setIdEmpresa($cEntidad->getIdEmpresa());
		$cEmpPerfil->setIdUsuario($newId);

		//Sólo borramos los perfiles generales por si tienen otros agregados.
		$cEmpPerfil->setIdPerfil("0");
		$cEmpPerfilDB->borrar($cEmpPerfil);
		$cEmpPerfil->setIdPerfil("1");
		$cEmpPerfilDB->borrar($cEmpPerfil);

		if($cEmpresa->getDistribuidor()=="1") {
			$cEmpPerfil->setIdPerfil("0");
		}else{
			$cEmpPerfil->setIdPerfil("1");
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idUsuario) AS Max FROM empresas_usuarios ";
		$sql  .= "WHERE idEmpresa= " . $aux->qstr($cEntidad->getIdEmpresa(), false);
//echo "<br />siguienteID:" . $sql;exit;
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " AND ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
		...
		$cEmpPerfil->setIdEmpresa($cEntidad->getIdEmpresa());
		$cEmpPerfil->setIdUsuario($cEntidad->getIdUsuario());
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$and			= false;
		$retorno			= true;
		$retorno = is_file(constant("DIR_WS_COM") . "Empresas_usuarios_perfiles/Empresas_usuarios_perfilesDB.php");
		if (!$retorno){
			$retorno=false;
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Class Not Found [" . constant("MNT_BORRAR") . "][Empresas_usuariosDB::Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->	" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		...
		if ($retorno)
		{
			...
			$cEmpresas_usuarios_perfilesDB	= new Empresas_usuarios_perfilesDB($aux);  // Entidad DB
			$cEmpresas_usuarios_perfiles	= new Empresas_usuarios_perfiles();  // Entidad
			$cEmpresas_usuarios_perfiles->setIdEmpresa($cEntidad->getIdEmpresa());
			$cEmpresas_usuarios_perfiles->setIdUsuario($cEntidad->getIdUsuario());
			if (!$cEmpresas_usuarios_perfilesDB->borrar($cEmpresas_usuarios_perfiles))
			{
				$retorno=false;
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Empresas_usuariosDB::Empresas_usuarios_perfilesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->	" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}
		...
		if ($retorno){
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Empresas_usuariosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM empresas_usuarios WHERE ";
		$sql  .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$sql  .="AND idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdUsuario($arr['idEmpresa']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
	...
	*************************************************************************/
	function Login($cEntidad)
	{
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [Login][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
	...
	function getLogin($cEntidad)
	{
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [getLogin][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
	...
	************************************************************************/
	...
	function ActualizaToken($cEntidad)
	{
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "-Token][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
	 ...
	 * de la base de datos
	 * @return Empresas_usuarios Informacion recuperada
	 *************************************************************************/
	function usuarioPorToken($cEntidad)
	{
		...
		$sql = "SELECT * FROM empresas_usuarios ";
		$sql  .= "WHERE ";
		$sql  .= "token=" . $aux->qstr($cEntidad->getToken(), false) . " ";
		$sql  .= "AND ";
		$sql  .= "bajaLog=0";
//		echo "<br />" . $sql;
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [usuarioPorToken][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
	...
	function isUsuarioActivo($cEntidad)
	{
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [isUsuarioActivo][Empresas_usuariosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
		}
...
}//Fin de la Clase Empresas_usuariosDB
?>
