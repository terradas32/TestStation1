...
* la tabla opciones_valores
**/
class Opciones_valoresDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $this->getSiguienteId($cEntidad);
                ...
                if($cEntidad->getIdValor() != ""){
                    $newId = $cEntidad->getIdValor();
        		}else{
                    $newId = $this->getSiguienteId($cEntidad);
        		}
		$sql = "INSERT INTO opciones_valores (";
		$sql .= "codIdiomaIso2" . ",";
		$sql .= "idPrueba" . ",";
		$sql .= "idItem" . ",";
		$sql .= "idOpcion" . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Opciones_valoresDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idValor) AS Max FROM opciones_valores ";
		$sql  .=" WHERE codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " AND idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Opciones_valoresDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE opciones_valores SET ";
		...
		$sql .= "idValor=" . $aux->qstr($cEntidad->getIdValor(), false) . ", ";
		...
		$sql .= " WHERE ";
		$sql .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " AND idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " AND idValor=" . $aux->qstr($cEntidad->getIdValor(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Opciones_valoresDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		$sql .= "bajaLog='1'";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Opciones_valoresDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM opciones_valores WHERE ";
		$sql  .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idItem=" . $aux->qstr($cEntidad->getIdItem(), false) . " AND idOpcion=" . $aux->qstr($cEntidad->getIdOpcion(), false) . " AND idValor=" . $aux->qstr($cEntidad->getIdValor(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setIdValor($arr['idValor']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Opciones_valoresDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM opciones_valores ";
		if ($cEntidad->getCodIdiomaIso2() != ""){
...
}//Fin de la Clase Opciones_valoresDB
?>