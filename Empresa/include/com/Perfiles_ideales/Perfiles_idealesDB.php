...
* la tabla perfiles_ideales
**/
class Perfiles_idealesDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $cEntidad->getCodIdiomaIso2();
		$iCont = 0;
		$sql  = "SELECT COUNT(codIdiomaIso2) AS Max FROM perfiles_ideales ";
		$sql .= "WHERE ";
		$sql .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Perfiles_idealesDB]";
		...
		$sql = "INSERT INTO perfiles_ideales (";
		$sql .= "codIdiomaIso2" . ",";
		$sql .= "idPrueba" . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Perfiles_idealesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM perfiles_ideales ";
		$sql  .=" WHERE codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Perfiles_idealesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE perfiles_ideales SET ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Perfiles_idealesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM perfiles_ideales ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Perfiles_idealesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM perfiles_ideales WHERE ";
		$sql  .="codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Perfiles_idealesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM perfiles_ideales ";
		if ($cEntidad->getCodIdiomaIso2() != ""){
		...
		if ($cEntidad->getPuntuacionMin() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntuacionMin>=" . $aux->qstr($cEntidad->getPuntuacionMin(), false);
		}
		if ($cEntidad->getPuntuacionMinHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntuacionMin<=" . $aux->qstr($cEntidad->getPuntuacionMinHast(), false);
		}
		if ($cEntidad->getPuntuacionMax() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntuacionMax>=" . $aux->qstr($cEntidad->getPuntuacionMax(), false);
		}
		if ($cEntidad->getPuntuacionMaxHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
	...
	function quitaImagen($cEntidad)
	{
		...
		$cEntidadAnterior	= new Perfiles_ideales();
		$cEntidadAnterior->setCodIdiomaIso2($cEntidad->getCodIdiomaIso2());
		$cEntidadAnterior->setIdPrueba($cEntidad->getIdPrueba());
		$cEntidadAnterior->setIdTipoCompetencia($cEntidad->getIdTipoCompetencia());
		$cEntidadAnterior->setIdCompetencia($cEntidad->getIdCompetencia());
...
}//Fin de la Clase Perfiles_idealesDB
?>