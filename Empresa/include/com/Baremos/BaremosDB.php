...
class BaremosDB
{
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idBaremo) AS Max FROM baremos ";
		$sql  .=" WHERE idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
		if($cEntidad->getIdEscala()!="" && $cEntidad->getIdBloque()!=""){
			$sql  .=" AND idBloque=" . $aux->qstr($cEntidad->getIdBloque(), false) . " AND idEscala=" . $aux->qstr($cEntidad->getIdEscala(), false) . " ";
		}
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
...
/*************************************************************************
	...
	*************************************************************************/
	function readEntidadCompleta($cEntidad)
	{
		...
		$sql  .="idBloque=" . $aux->qstr($cEntidad->getIdBloque(), false) . " AND idEscala=" . $aux->qstr($cEntidad->getIdEscala(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdBaremo($arr['idBaremo']);
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM baremos ";
		if ($cEntidad->getIdBaremo() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idBaremo IN (" . $cEntidad->getIdBaremo() . ")";
		}
		if ($cEntidad->getIdPrueba() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
