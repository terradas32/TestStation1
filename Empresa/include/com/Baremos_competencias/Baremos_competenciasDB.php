...
* la tabla baremos_competencias
**/
class Baremos_competenciasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql = "INSERT INTO baremos_competencias (";
		$sql .= "idBaremo" . ",";
		$sql .= "idPrueba" . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][BaremosDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idBaremo) AS Max FROM baremos_competencias ";
		$sql  .=" WHERE idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
		if($cEntidad->getIdCompetencia()!="" && $cEntidad->getIdTipoCompetencia()!=""){
			$sql  .=" AND idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " ";
		}
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][BaremosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE baremos_competencias SET ";
		...
		$sql .= " WHERE ";
		$sql .="idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][BaremosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM baremos_competencias ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][BaremosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM baremos_competencias WHERE ";
		$sql  .="idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][BaremosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
...
/*************************************************************************
	...
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function readEntidadCompleta($cEntidad)
	{
		$aux			= $this->conn;
	
		$sql = "SELECT *  FROM baremos_competencias WHERE ";
		$sql  .="idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND ";
		$sql  .="idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . " AND idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdBaremo($arr['idBaremo']);
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM baremos_competencias ";
		if ($cEntidad->getIdBaremo() != ""){
		...
		if ($cEntidad->getIdCompetencia() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idCompetencia=" . $aux->qstr($cEntidad->getIdEscala(), false);
		}
