...
class Peticiones_donglesDB
{
	...
	*  en la base de datos
	***********************************************************************/
	function insertarMovimiento($cEntidad)
	{
		$aux			= $this->conn;
		$bOk=true;

		$sConcepto = $cEntidad->getEstado();
		$cEntidad->setEstado('1');
		require_once(constant("DIR_WS_COM") . "Empresas/EmpresasDB.php");
		require_once(constant("DIR_WS_COM") . "Empresas/Empresas.php");
		$cEntidadEmpresaDestinoDB	= new EmpresasDB($aux);  // Entidad DB Historico_cambios
		$cEntidadEmpresaDestino		= new Empresas();  // Entidad Historico_cambios
		$cEntidadEmpresaDestino->setIdEmpresa($cEntidad->getIdEmpresa());
		$cEntidadEmpresaDestino = $cEntidadEmpresaDestinoDB->readEntidad($cEntidadEmpresaDestino);
		$NDonglesDestino = ($cEntidadEmpresaDestino->getDongles() + $cEntidad->getNDongles());
		$cEntidadEmpresaDestino->setDongles($NDonglesDestino);
		$cEntidadEmpresaDestino->setPassword('');	//Para que no modifique la contraseña
		$cEntidadEmpresaDestinoDB->modificar($cEntidadEmpresaDestino);

		$newId = $this->getSiguienteId($cEntidad);
		// Apunte de suma unidades
		$sql = "INSERT INTO peticiones_dongles (";
		$sql .= "idPeticion" . ",";
		$sql .= "idEmpresa" . ",";
		$sql .= "idEmpresaReceptora" . ",";
		...
		if($bOk){
			require_once(constant("DIR_WS_COM") . "Empresas/EmpresasDB.php");
			require_once(constant("DIR_WS_COM") . "Empresas/Empresas.php");
			$cEntidadEmpresaOrigenDB	= new EmpresasDB($aux);  // Entidad DB Historico_cambios
			$cEntidadEmpresaOrigen		= new Empresas();  // Entidad Historico_cambios
			$cEntidadEmpresaOrigen->setIdEmpresa($cEntidad->getIdEmpresaReceptora());
			$cEntidadEmpresaOrigen = $cEntidadEmpresaOrigenDB->readEntidad($cEntidadEmpresaOrigen);
			$NDonglesOrigen = ($cEntidadEmpresaOrigen->getDongles() - $cEntidad->getNDongles());
			$cEntidadEmpresaOrigen->setDongles($NDonglesOrigen);
			$cEntidadEmpresaOrigen->setPassword('');	//Para que no modifique la contraseña
			$cEntidadEmpresaOrigenDB->modificar($cEntidadEmpresaOrigen);

			$newId = $this->getSiguienteId($cEntidad);
			// Apunte de resta unidades
			$sql = "INSERT INTO peticiones_dongles (";
			$sql .= "idPeticion" . ",";
			$sql .= "idEmpresa" . ",";
			$sql .= "idEmpresaReceptora" . ",";
	...
	function modificar($cEntidad)
	{
		...
		if($retorno){
			...
			if($cEntidad->getEstado()=="1"){
				require_once(constant("DIR_WS_COM") . "Empresas/EmpresasDB.php");
				require_once(constant("DIR_WS_COM") . "Empresas/Empresas.php");
				//Creamos una entidad q contendrá a la empresa donante de Dongles para comprobar que tenga
				//Dongles suficientes a suministrar a la empresa.
				$cEmpresaDB = new EmpresasDB($aux);
				$cEmpresaDonante = new Empresas();
				$cEmpresaDonante->setIdEmpresa($cEntidad->getIdEmpresaReceptora());
				$cEmpresaDonante = $cEmpresaDB->readEntidadSinPass($cEmpresaDonante);

				if(($cEmpresaDonante->getDongles()-$cEntidad->getNDongles()) >=0){
					...
					$cEmpresaDonante->setDongles($cEmpresaDonante->getDongles() - $cEntidad->getNDongles());

					$cEmpresaDB->modificar($cEmpresaDonante);
				}else{
					$bEnvia=false;
					$retorno=false;
					$this->msg_Error	= array();
					$sTypeError	=	date('d/m/Y H:i:s') . " "  . constant("ERR_SIN_DONGLES");
					$this->msg_Error[]	= $sTypeError;
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
...
//		exit;
	...
	function enviaEmailAceptacionRechazo($cEntidad){
		...
		try {
			...
			echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}, {$mail}";exit;
		}
	    return $exito;
	}
	function enviaEmailPeticion($cEntidad){
		...
		$cEmpresaPEDB = new EmpresasDB($aux);
		...
		$cEmpresaPE = $cEmpresaTramitaDB->readEntidad($cEmpresaPE);
		...
		try {
			...
			if($cEmpresaTramita->getMail2()!=""){
				$mail->AddAddress($cEmpresaTramita->getMail2(), $cEmpresaTramita->getNombre());
			}
			if($cEmpresaTramita->getMail3()!=""){
				$mail->AddAddress($cEmpresaTramita->getMail3(), $cEmpresaTramita->getNombre());
			}
			...
			//Ponemos las cuentas de PE
			$mail->AddAddress($cEmpresaPE->getMail(), $cEmpresaPE->getNombre());
			if($cEmpresaPE->getMail2()!=""){
				$mail->AddAddress($cEmpresaPE->getMail2(), $cEmpresaPE->getNombre());
			}
			if($cEmpresaPE->getMail3()!=""){
				$mail->AddAddress($cEmpresaPE->getMail3(), $cEmpresaPE->getNombre());
			}
