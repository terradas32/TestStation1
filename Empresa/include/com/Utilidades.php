...
class Utilidades
{
    ...
    *********************************************/
    public function chkChar($sCadena)
    {
        $aChars = array('\"','<','>','&','|','\'', '%', '?', '=', '/', '\\');
        $bRetorno = false;
    ...
    *********************************************/
    ...
    public function delcaracteresp($sCadena)
    {
    ...
    public function quitar_espacios_extra($str)
    {
    ...
    * @return String Cadena convertida.
    *********************************************/
    public function cCambiaAcentos($sCadena)
    {
    ...
    * @return String Cadena convertida.
    *********************************************/
    public function posicion_letrasMAYS($sCadena)
    {
        $u = 0;
        $d = 0;
        $n = strlen($sCadena);
    ...
    * @return String Cadena convertida.
    *********************************************/
    public function SEOTitulo($sCadena)
    {
    ...
    *********************************************/
    public function verifica_url($url)
    {
    ...
    * 				[1] --> Alto.
    *********************************************/
    public function thumb($imagewidth=150, $imageheight=150, $ancho)
    {
    ...
    */
    public function PrintDate($fecha = "", $mask = "%d de %B de %Y")
    {
        if ($fecha == "") {
            $fecha = time();
        } else {
    ...
    *  @param String Zona horaria de DateTimeZone STRING timezone
    *  @return boolean True si la fecha sistema está entre las dos fechas,
    */
    public function isCurrent2Dates($fecInicio, $fecFin, $timezone)
    {
        $dt=new datetime("now", new datetimezone($timezone));
        $fecActual = gmdate("Y-m-d H:i:s", (time()+$dt->getOffset()));	//Fecha actual de la Zona horaria
        //		echo "<br />FECHA actual en :" . $timezone . " : " . $fecActual;
        $fecActual = strtotime($fecActual);
        $fecInicio = strtotime($fecInicio);
        $fecFin = strtotime($fecFin);
        // 		echo "<br />FECHA (strtotime) ACTUAL :" . $fecActual;
        // 		echo "<br />FECHA (strtotime) INICIO :" . $fecInicio;
        // 		echo "<br />FECHA (strtotime) FIN :" . $fecFin;
        // 		exit;
    ...
    */
    public function getValorCalculadoPRUEBAS($rsLine, $cOpciones = null, $conn)
    {
        ...
        switch ($rsLine->fields['idPrueba']) {
            ...
            case 72:	//Flash Las 7 maravillas
            case 73:	//Redacción ESADE
...
//				echo "<br />Empresa::" . $rsLine->fields['idEmpresa'] . " Proceso::" . $rsLine->fields['idProceso'] . " Prueba::" . $rsLine->fields['idPrueba'] . " Candidato::" . $rsLine->fields['idCandidato'] . " **->orden::" . $rsLine->fields['orden'] . " " . $cItem->getCorrecto() . " == " . $cOpcion->getCodigo() . " ->valor::" . $sValor;
    ...
    /*
     * Crea dinámicamente las zonas horarios con la diferencia UTC teniendo en cuenta horario de verano.
     */
    public function getOptiosSelect_timezone_list($sKeySelected="")
    {
        ...
        //Default option
        $holder .= '<option value="">'.constant("SLC_OPCION").'</option>';
        ...
        while ($i < $count) {
            ...
            foreach ($timezone_list as $key => $val) {
                ...
                $holder .= '<option ' . $sSelected . ' value="'.$key.'">'.$val.'</option>';
            }
            $holder .= '</optgroup>';
            ++$i;
        }
