...
class Combo
{
	 ...
	 **/
	function __construct(&$conn, $sNombre, $sIdKey, $sPipe, $sDescKey, $sTabla, $sPadre, $sDefault, $sWhere, $sAsIdKey = "", $sOrderBy = "", $sGroupBy = ""/*, $sTipoInforme=""*/)
	{
	...
	/**
	 * establece el valor de la propiedad sTipoInforme
	 * @sNombre
	 */
	function setTipoInforme($sCadena)
	{
		//echo'<script>console.log("setTipoInforme")</script>';
		$this->sTipoInforme = $sCadena;
	}
	 ...
	 * @return String
	 *//*                  "1","obliga",$cEntidad->getDescPrueba(),"",""*/
	function getHTMLCombo($sSize, $sClass, $sValor, $sAction, $sMoreOptions = "", $companyProcessZipCase = false)
	{
		...
		// In company report request the process field and the company field must be required (Jairo López - Xeridia S.L.)
		if($companyProcessZipCase){
			$sHTML = str_replace("cajatexto", "obliga", $sHTML);
		}
	...
	function setCabeceraCombo($sIdSelect, $sSize, $sClass, $sAction)
	{
		...
		$sCabecera .= "<select id='$sIdSelect' name='$sIdSelect'";
		if ($sSize != "")
		...
		if (is_array($cadena)) {
			foreach ($cadena as $sValor) {
				...
				while (!$this->vDatos->EOF) {
					$sAux = $this->vDatos->fields[$this->sIdKey];
					if ($sValor != "" && (strtoupper(trim($sValor)) == strtoupper(trim($sAux)))) {
						$sDescripcion .=  $this->vDatos->fields[$this->sDescKey] . "<br /> ";
						break;
					}
					$this->vDatos->MoveNext();
				}
