...
//("Specified height (".$this->iHeightFactor.") for gantt bar is out of range.");
	...
	if( $this->iShadow ) {
	    ...
	    $prect->SetPos(new RectangleJP($xt+1,$yt+1,$xb-$xt-$this->iShadowWidth-2,$yb-$yt-$this->iShadowWidth-2));
	    $prect->Stroke($aImg);
	}
	else {
	    $prect->SetPos(new RectangleJP($xt,$yt,$xb-$xt+1,$yb-$yt+1));
	    $prect->Stroke($aImg);
	    $aImg->SetColor($this->iFrameColor);
	...
	if( $this->progress->iProgress > 0 ) {
		...
		$marg = ceil(($barheight-$progressheight)/2);
	    	$pos = new RectangleJP($xtp,$yt + $marg, $len,$barheight-2*$marg);
		$prog->SetPos($pos);
		$prog->Stroke($aImg);
	    }
	}
