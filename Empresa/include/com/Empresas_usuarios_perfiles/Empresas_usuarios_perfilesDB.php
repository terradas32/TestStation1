...
* la tabla empresas_usuarios_perfiles
**/
class Empresas_usuarios_perfilesDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iCont = 0;
		$sql  = "SELECT COUNT(idEmpresa) AS Max FROM empresas_usuarios_perfiles ";
		$sql .= "WHERE ";
		$sql .="idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " ";
		$sql .="AND idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " AND idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
		...
		if ($iCont > 0 ){
			//Existe el registro
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error Registro Existe [" . constant("MNT_ALTA") . "][Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}

		$sql = "INSERT INTO empresas_usuarios_perfiles (";
		$sql .= "idEmpresa" . ",";
		$sql .= "idUsuario" . ",";
		$sql .= "idPerfil" . ",";
		$sql .= "fecAlta" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getIdPerfil(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			return 0;
		}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM empresas_usuarios_perfiles ";
		$sql  .=" WHERE idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " AND idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " ";
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE empresas_usuarios_perfiles SET ";
		...
		$sql .= " WHERE ";
		$sql .="idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " AND idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " ";
		$sql .=" AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
		$retorno=true;
		//En la parte de empresas no necesita modificar nada 22/01/2016
		return $retorno;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		$and			= false;
		$retorno			= true;
		//En la parte de empresas no necesita borrar nada 22/01/2016
		return $retorno;
		if ($retorno){
			//Borramos el registro de la Entidad.
			$sql  ="DELETE FROM empresas_usuarios_perfiles ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Empresas_usuarios_perfilesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT  idEmpresa, idUsuario, idPerfil, fecAlta, fecMod, usuAlta, usuMod FROM empresas_usuarios_perfiles WHERE ";
		$sql  .="idUsuario=" . $aux->qstr($cEntidad->getIdUsuario(), false) . " AND idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " ";
		$sql  .=" AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " ";
//		echo "<br />**//-->" . $sql;
		$rs = $aux->Execute($sql);
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Empresas_usuarios_perfilesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	...
	{
		$aux			= $this->conn;
//	echo "<br />ENTRO readlista*******************:" . $cEntidad->getIdEmpresa();
		$sql="";
		$and = false;
		$sql.="SELECT * FROM empresas_usuarios_perfiles ";
		if ($cEntidad->getIdEmpresa() != ""){
...
//		echo "<br />ENTRO readlista::SQL:" . $sql;
		if ($cEntidad->getIdPerfil() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false);
...
}//Fin de la Clase Empresas_usuarios_perfilesDB
?>
