...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Paises/PaisesDB.php");
	require_once(constant("DIR_WS_COM") . "Paises/Paises.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new PaisesDB($conn);  // Entidad DB
	$cEntidad	= new Paises();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['paises_next_page']) && $_POST['paises_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'paises');
					...
					$lista=$pager->getRS();
					include('Template/Paises/mntpaisesl.php');
				}else{
					$cEntidad	= new Paises();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Paises/mntpaisesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Paises/mntpaises.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setCodPaisIso3((isset($_POST["fCodPaisIso3"])) ? $_POST["fCodPaisIso3"] : "");
		$cEntidad->setCodPaisIso2((isset($_POST["fCodPaisIso2"])) ? $_POST["fCodPaisIso2"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdPais((isset($_POST["LSTIdPais"]) && $_POST["LSTIdPais"] != "") ? $_POST["LSTIdPais"] : "");	$cEntidad->setBusqueda(constant("STR_ID_PAIS"), (isset($_POST["LSTIdPais"]) && $_POST["LSTIdPais"] != "" ) ? $_POST["LSTIdPais"] : "");
		...
		$cEntidad->setCodPaisIso3((isset($_POST["LSTCodPaisIso3"]) && $_POST["LSTCodPaisIso3"] != "") ? $_POST["LSTCodPaisIso3"] : "");	$cEntidad->setBusqueda(constant("STR_CODIGO_PAIS_ISO3"), (isset($_POST["LSTCodPaisIso3"]) && $_POST["LSTCodPaisIso3"] != "" ) ? $_POST["LSTCodPaisIso3"] : "");
		$cEntidad->setCodPaisIso2((isset($_POST["LSTCodPaisIso2"]) && $_POST["LSTCodPaisIso2"] != "") ? $_POST["LSTCodPaisIso2"] : "");	$cEntidad->setBusqueda(constant("STR_CODIGO_PAIS_ISO2"), (isset($_POST["LSTCodPaisIso2"]) && $_POST["LSTCodPaisIso2"] != "" ) ? $_POST["LSTCodPaisIso2"] : "");
