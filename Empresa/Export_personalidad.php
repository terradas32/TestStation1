...
include_once ('include/conexion.php');
	...
	$comboTIPOS_INFORMES	= new Combo($conn,"fIdTipoInforme","idTipoInforme","nombre","descripcion","tipos_informes","",constant("SLC_OPCION"),"codIdiomaIso2=" . $conn->qstr(constant("LENGUAJEDEFECTO"), false),"","nombre");
	// Aquí recibo el campo de tipo de informe para realizar el filtrado correspondiente
	//var_dump(intval($_POST['fIdTipoInforme']));
	//die;
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					if (!empty($_POST['export_personalidad_next_page']) && $_POST['export_personalidad_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'export_personalidad');
					...
					$lista=$pager->getRS();
					include('Template/Export_personalidad/mntexport_personalidadl.php');
				}else{
					$cEntidad	= new Export_personalidad();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Export_personalidad/mntexport_personalidada.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Export_personalidad/mntexport_personalidad.php');
			break;
		case constant("MNT_LISTAR"):
			...
			// Aquí añadimos a la consulta el tipo de informe para el filtro de la sql
			if(!empty($_POST['fIdTipoInforme'])){
				$newSql = "WHERE idTipoInforme = ".intval($_POST['fIdTipoInforme'])." AND ";
				$sql = str_replace("WHERE", $newSql, strval($sql));
			}
		...
		default:
			//TODO JOSH
			$cEntidad->setOrderBy("idEmpresa, idProceso, idCandidato, idPrueba, idBaremo");
			$cEntidad->setBusqueda(constant("STR_ORDENAR_POR"), "Empresa, Proceso, Candidato, Prueba");
			$cEntidad->setOrder("DESC");
			$cEntidad->setBusqueda(constant("STR_ORDEN"), "DESC");
			$_POST['MODO']    = constant("MNT_LISTAR");
			include('Template/Export_personalidad/mntexport_personalidad.php');
	...
	function readLista($cEntidad){
		...
		if (isset($_POST["LSTIdEmpresa"])){
			if (is_array($_POST["LSTIdEmpresa"])){
				$aEmpresas = explode(",", $_POST["LSTIdEmpresa"][0]);
				if (!in_array(" ", $aEmpresas)){
					$_Empresas = implode(",", $aEmpresas);
					if (empty($_Empresas)){
						$_Empresas = $_cEntidadUsuarioTK->getIdEmpresa();
					}
				}
			}else{
...
//		$cEntidad->setDescProceso((isset($_POST["LSTDescProceso"]) && $_POST["LSTDescProceso"] != "") ? $_POST["LSTDescProceso"] : "");	$cEntidad->setBusqueda(constant("STR_PROCESO"), (isset($_POST["LSTDescProceso"]) && $_POST["LSTDescProceso"] != "" ) ? $_POST["LSTDescProceso"] : "");
		...
		if (isset($_POST["LSTDescPrueba"])){
			if (is_array($_POST["LSTDescPrueba"])){
				if (!in_array(" ", $_POST["LSTDescPrueba"])){
					echo("<script>console.log('El modo MNT_LISTAR 1 es: ".$_POST['MODO'].", ". json_encode($_POST) ."');</script>");
		...
		$cEntidad->setDescPrueba($_Pruebas);	
		$cEntidad->setBusqueda(constant("STR_PRUEBA"), (isset($_POST["LSTDescPrueba"]) && $_POST["LSTDescPrueba"] != "" ) ? str_replace("'","", $_Pruebas) : "");
...
include_once ('include/conexion.php');
	...
	switch ($_POST['MODO'])
	{
		...
		case constant("MNT_LISTAR"):
			...
			break;
		default:
			$cEntidad->setOrderBy("idEmpresa, idProceso, idCandidato, idPrueba, idTipoInforme, idBaremo");
			$cEntidad->setBusqueda(constant("STR_ORDENAR_POR"), "Empresa, Proceso, Candidato, Prueba");
			$cEntidad->setOrder("DESC");
			$cEntidad->setBusqueda(constant("STR_ORDEN"), "DESC");
			$_POST['MODO']    = constant("MNT_LISTAR");
