...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Opciones_ejemplosDB($conn);  // Entidad DB
	$cEntidad	= new Opciones_ejemplos();  // Entidad
	...
	$comboEJEMPLOS	= new Combo($conn,"fIdEjemplo","idEjemplo","descripcion","Descripcion","ejemplos","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['opciones_ejemplos_next_page']) && $_POST['opciones_ejemplos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'opciones_ejemplos');
					...
					$lista=$pager->getRS();
					include('Template/Opciones_ejemplos/mntopciones_ejemplosl.php');
				}else{
					$cEntidad	= new Opciones_ejemplos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Opciones_ejemplos/mntopciones_ejemplosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Opciones_ejemplos/mntopciones_ejemplos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		global $comboEJEMPLOS;
		$cEntidad->setIdEjemplo((isset($_POST["LSTIdEjemplo"]) && $_POST["LSTIdEjemplo"] != "") ? $_POST["LSTIdEjemplo"] : "");	$cEntidad->setBusqueda(constant("STR_EJEMPLO"), (isset($_POST["LSTIdEjemplo"]) && $_POST["LSTIdEjemplo"] != "" ) ? $comboEJEMPLOS->getDescripcionCombo($_POST["LSTIdEjemplo"]) : "");
		$cEntidad->setIdOpcion((isset($_POST["LSTIdOpcion"]) && $_POST["LSTIdOpcion"] != "") ? $_POST["LSTIdOpcion"] : "");	$cEntidad->setBusqueda(constant("STR_ID_OPCION"), (isset($_POST["LSTIdOpcion"]) && $_POST["LSTIdOpcion"] != "" ) ? $_POST["LSTIdOpcion"] : "");
		$cEntidad->setIdOpcionHast((isset($_POST["LSTIdOpcionHast"]) && $_POST["LSTIdOpcionHast"] != "") ? $_POST["LSTIdOpcionHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_OPCION") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdOpcionHast"]) && $_POST["LSTIdOpcionHast"] != "" ) ? $_POST["LSTIdOpcionHast"] : "");
