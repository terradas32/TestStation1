...
//			if (!empty($vaPruebas)){
		...
		case constant("MNT_LISTAR"):
			$cEntidad = readLista($cEntidad);
			if (isset($_POST['ORIGEN']) && $_POST['ORIGEN'] == constant("MNT_LISTAR")){
...
*/
		...
		case constant("MNT_NUEVOCORREO"):
			...
			if(isset($_POST['fConsulta']) && $_POST['fConsulta']!=""){
				...
				include('Template/ProcesoProcesos/datosConsultaCorreo.php');
				break;
			}
			...
			if(isset($_POST['fConsultaAsignados']) && $_POST['fConsultaAsignados']!=""){
				...
				include('Template/ProcesoProcesos/datosCorreoAsignado.php');
				break;
			}
		case constant("MNT_PROGRAMAR_ENVIO"):
			$cEntidad	= readEntidad($cEntidad);
			$cEntidad->setUsuMod($_cEntidadUsuarioTK->getIdEmpresa());
			if ($cEntidadDB->modificarEnvioProgramado($cEntidad)){
				echo "OK";
			}else{
				echo "Error";
			}
...
//			$cEntidad = readLista($cEntidad);
	...
	*/
	function readEntidad($cEntidad){
		...
		$sFechaHoraProgramado =(isset($_POST["fFecEnvioProgramado"])) ? $_POST["fFecEnvioProgramado"] : "";
		$sHoraProgramado =(isset($_POST["fHoraInicioProgramado"])) ? $_POST["fHoraInicioProgramado"] : "";
		if (!empty($sFechaHoraProgramado) && !empty($sHoraProgramado)){
			//2011-03-30 00:00:00
			$sFechaHoraProgramado .= " " . $sHoraProgramado . ":00";
		}
		$cEntidad->setFecEnvioProgramado($sFechaHoraProgramado);
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setBajaLog((isset($_POST["LSTBajaLog"]) && $_POST["LSTBajaLog"] != "") ? $_POST["LSTBajaLog"] : "");	$cEntidad->setBusqueda(constant("STR_VISIBLE"), (isset($_POST["LSTBajaLog"]) && $_POST["LSTBajaLog"] != "" ) ? $_POST["LSTBajaLog"] : "");
