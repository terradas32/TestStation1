...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Empresas_perfilesDB($conn);  // Entidad DB
	$cEntidad	= new Empresas_perfiles();  // Entidad
	...
	$comboEMP_PERFILES	= new Combo($conn,"fIdPerfil","idPerfil","descripcion","Descripcion","emp_perfiles","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['empresas_perfiles_next_page']) && $_POST['empresas_perfiles_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'empresas_perfiles');
					...
					$lista=$pager->getRS();
					include('Template/Empresas_perfiles/mntempresas_perfilesl.php');
				}else{
					$cEntidad	= new Empresas_perfiles();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Empresas_perfiles/mntempresas_perfilesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Empresas_perfiles/mntempresas_perfiles.php');
			break;
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdPerfil((isset($_POST["LSTIdPerfil"]) && $_POST["LSTIdPerfil"] != "") ? $_POST["LSTIdPerfil"] : "");	$cEntidad->setBusqueda(constant("STR_PERFIL"), (isset($_POST["LSTIdPerfil"]) && $_POST["LSTIdPerfil"] != "" ) ? $comboEMP_PERFILES->getDescripcionCombo($_POST["LSTIdPerfil"]) : "");
