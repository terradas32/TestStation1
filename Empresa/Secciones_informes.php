...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Secciones_informes/Secciones_informesDB.php");
	require_once(constant("DIR_WS_COM") . "Secciones_informes/Secciones_informes.php");

include_once ('include/conexion.php');

	$cUtilidades	= new Utilidades();

	$cEntidadDB	= new Secciones_informesDB($conn);  // Entidad DB
	$cEntidad	= new Secciones_informes();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['secciones_informes_next_page']) && $_POST['secciones_informes_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'secciones_informes');
					...
					$lista=$pager->getRS();
					include('Template/Secciones_informes/mntsecciones_informesl.php');
				}else{
					$cEntidad	= new Secciones_informes();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Secciones_informes/mntsecciones_informesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Secciones_informes/mntsecciones_informes.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdSeccion((isset($_POST["LSTIdSeccion"]) && $_POST["LSTIdSeccion"] != "") ? $_POST["LSTIdSeccion"] : "");	$cEntidad->setBusqueda(constant("STR_IDSECCION"), (isset($_POST["LSTIdSeccion"]) && $_POST["LSTIdSeccion"] != "" ) ? $_POST["LSTIdSeccion"] : "");
		$cEntidad->setIdSeccionHast((isset($_POST["LSTIdSeccionHast"]) && $_POST["LSTIdSeccionHast"] != "") ? $_POST["LSTIdSeccionHast"] : "");	$cEntidad->setBusqueda(constant("STR_IDSECCION") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdSeccionHast"]) && $_POST["LSTIdSeccionHast"] != "" ) ? $_POST["LSTIdSeccionHast"] : "");
