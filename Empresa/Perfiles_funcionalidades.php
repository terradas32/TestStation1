...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Perfiles_funcionalidadesDB($conn);  // Entidad DB
	$cEntidad	= new Perfiles_funcionalidades();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script>alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['perfiles_funcionalidades_next_page']) && $_POST['perfiles_funcionalidades_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'perfiles_funcionalidades');
					if ($bInit)	$pager->curr_page=1;
					$pager->showPageLinks = true;
					$lista=$pager->getRS();
					include('Template/Perfiles_funcionalidades/mntperfiles_funcionalidadesl.php');
				}else{
					$cEntidad	= new Perfiles_funcionalidades();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Perfiles_funcionalidades/mntperfiles_funcionalidadesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Perfiles_funcionalidades/mntperfiles_funcionalidades.php');
			break;
