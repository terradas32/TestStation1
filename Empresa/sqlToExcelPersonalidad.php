...
//	session_start();
	...
	if($_REQUEST['LSTTipoInforme'] == 71){
		// Ejemplo de empresa de CANAL ISABEL II
		$vDatos = $conn->Execute($InitSQL);
		$vDatosDist = $conn->Execute($InitSQL);
		$nomMatchings = array();

		while (!$vDatosDist->EOF){
			if(!in_array($vDatosDist->fields['nomMatching'], $nomMatchings)){
				//echo $vDatosDist->fields['nomMatching'] . "</br>";
				array_push($nomMatchings, $vDatosDist->fields['nomMatching']);
			}
			$vDatosDist->MoveNext();
		}
		$vDatosDist->MoveFirst();
	}else{
		$vDatos = $conn->Execute($InitSQL);
	}
	...
	if($_REQUEST['LSTTipoInforme'] == 71){
		...
		// Josh
		$DatosHeadEscalas = getHTMLFit($vDatos, $rowsParticipants, $nomMatchings);
		//$vDatos->Move();
	...
	/* if($_REQUEST['LSTTipoInforme'] == 71){
		//$inicio = microtime(true); // Tiempo inicial en segundos con microsegundos
		$auxFunction = countParticipantsAndRows($vDatos);
		$counterParticipants =  $auxFunction[0];
		$rowsParticipants =  $auxFunction[1];
		
		//echo $counterParticipants;
		//die;
		$vDatos->MoveFirst();
		$counterAux = 0;
		for($k=0; $k<$counterParticipants; $k++){
			...
			$idEmpresa = $vDatos->fields['idEmpresa'];
			$idProceso = $vDatos->fields['idProceso'];
			$idCandidato = $vDatos->fields['idCandidato'];

			foreach ($nomMatchings as $nomMatching) {
				$vDatos->MoveFirst();
				while (!$vDatos->EOF){
					$existNomMatching = false;
					if($idEmpresa == $vDatos->fields['idEmpresa'] &&
					$idProceso == $vDatos->fields['idProceso'] &&
					$idCandidato == $vDatos->fields['idCandidato']){
						if ($vDatos->fields['nomMatching'] == $nomMatching) {
							if($vDatos->fields['idEmpresa'] == 5897){
								//$puntuacion = round($vDatos->fields['puntuacion']);
								if($vDatos->fields['puntuacion'] <= 3){
									$puntuacion = "1.00";
								}else if($vDatos->fields['puntuacion'] > 3 && $vDatos->fields['puntuacion'] <= 6){
									$puntuacion = "2.00";
								}else if($vDatos->fields['puntuacion'] > 6 && $vDatos->fields['puntuacion'] <= 8){
									$puntuacion = "3.00";
								}else{
									$puntuacion = "4.00";
								}
							}else{
								$puntuacion = $vDatos->fields['puntuacion'];
							}

							// Encontrado, generamos casilla en su posición
							$datosComunes .= "<td align=\"center\">" . $puntuacion . "</td>";
							$l++;

							$existNomMatching = true;
							break;
						}
					}
					$vDatos->MoveNext();
				}
				if($existNomMatching == false){
					// NO Encontrado, generamos casilla VACIA en su posición
					$datosComunes .= "<td align=\"center\"></td>";
					$l++;
				}


				if($l >= count($nomMatchings)){
					$l=0;
					$vDatos->MoveFirst();
					break;
				}	
...
//$fin = microtime(true); // Tiempo final en segundos con microsegundos
...
/**
 * Calcula la puntuación basada en reglas específicas.
 */
function calcularPuntuacion($puntuacion) {
    return match (true) {
        $puntuacion <= 3 => "1.00",
        $puntuacion > 3 && $puntuacion <= 6 => "2.00",
        $puntuacion > 6 && $puntuacion <= 8 => "3.00",
        default => "4.00",
    };
}

if ($_REQUEST['LSTTipoInforme'] == 71) {
    // Cargar todos los datos en memoria una sola vez
    $dataArray = [];
    while (!$vDatos->EOF) {
        $dataArray[] = $vDatos->fields;
        $vDatos->MoveNext();
    }

    // Indexar los datos por combinación de `idEmpresa`, `idProceso`, `idCandidato`
    $indexedData = [];
    foreach ($dataArray as $row) {
        $key = $row['idEmpresa'] . '|' . $row['idProceso'] . '|' . $row['idCandidato'];
        $indexedData[$key][] = $row;
    }

    $datosComunes = '';
    $counterParticipants = count($indexedData); // Total de participantes
    $rowsParticipants = ceil(count($dataArray) / $counterParticipants); // Estimación de filas por participante
    $counterAux = 0;

    // Iterar sobre cada participante
    foreach (array_keys($indexedData) as $index => $key) {
        $participantData = $indexedData[$key];
        $firstRow = $participantData[0]; // Primer registro del participante

        // Agregar datos comunes al HTML
        $datosComunes .= "<tr>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . ($index + 1) . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descEmpresa'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descProceso'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['nombre'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['apellido1'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['apellido2'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['email'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['dni'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descPrueba'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descTipoInforme'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descBaremo'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $_codIdiomaInforme . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['fecPrueba'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['fecAltaProceso'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descSexo'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descEdad'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descFormacion'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descNivel'] . "</font></td>";
        $datosComunes .= "<td align=\"left\"><font color=\"blue\">" . $firstRow['descArea'] . "</font></td>";
        $datosComunes .= "<td align=\"center\"><font color=\"blue\">" . $firstRow['codIso2PaisProcedencia'] . "</font></td>";

        // Iterar sobre los matchings
        foreach ($nomMatchings as $nomMatching) {
            $matchingFound = false;
            foreach ($participantData as $row) {
                if ($row['nomMatching'] === $nomMatching) {
                    $puntuacion = ($row['idEmpresa'] == 5897)
                        ? calcularPuntuacion($row['puntuacion'])
                        : $row['puntuacion'];
                    $datosComunes .= "<td align=\"center\">{$puntuacion}</td>";
                    $matchingFound = true;
                    break;
        ...
        $counterAux += count($participantData);
        $datosComunes .= "</tr>";
    }
}
		...
		while (!$vDatos->EOF){
			...
			//$sqlESCALASIn .= $sOrder;
			$sqlESCALASIn .= " ORDER BY idBloque ASC, idEscala ASC ";
			...
			$sqlCOMPETENCIASIn .= " ORDER BY idTipoCompetencia ASC, idCompetencia ASC ";
	...
	function getHTMLBloquesDatos($vDatos){
		...
		while (!$vDatos->EOF){
			...
			$sqlESCALASIn .= " AND idBaremo=" . $vDatos->fields['idBaremo'] . " ";
			$sqlESCALASIn .= " AND idTipoInforme=" . $vDatos->fields['idTipoInforme'] . " ";
			$sqlESCALASIn .= " ORDER BY idBloque ASC ";
			...
			while (!$vEscalasCandidato->EOF){
				if ($sBloque != $vEscalasCandidato->fields['nomBloque']){
					...
					$sqlBloquesIn .= " ORDER BY idEscala ASC ";
		...
		while (!$vDatos->EOF){
			...
			while (!$vEscalasCandidato->EOF){
				if ($sEscala != $vEscalasCandidato->fields['nomEscala']){
					...
					$sqlBloquesIn .= " ORDER BY idBloque ASC, idEscala ASC ";
	...
	//Josh
	function getHTMLFit($vDatos, $rowsParticipants, $nomMatchings){
		$sRetorno = "";
		foreach($nomMatchings as $nomMatching){
		
				$sRetorno .= '<td align="center" colspan="" ><b>' . $nomMatching . "</b></td>";
		}
		...
		while (!$vDatos->EOF){
			...
			while (!$vCompetenciasCandidato->EOF){
				if ($sTiposCompetencias != $vCompetenciasCandidato->fields['nomTipoCompetencia']){
					...
					$sqlBloquesIn .= " ORDER BY idTipoCompetencia ASC, idCompetencia ASC ";
