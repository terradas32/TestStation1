...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Zonas/ZonasDB.php");
	require_once(constant("DIR_WS_COM") . "Zonas/Zonas.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new ZonasDB($conn);  // Entidad DB
	$cEntidad	= new Zonas();  // Entidad
	...
	$comboWI_MUNICIPIOS	= new Combo($conn,"fIdMunicipio","idMunicipio","descripcion","Descripcion","wi_municipios","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">jAlert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php 
				...
				{
					if (!empty($_POST['zonas_next_page']) && $_POST['zonas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'zonas');
					...
					$lista=$pager->getRS();
					include('Template/Zonas/mntzonasl.php');
				}else{
					$cEntidad	= new Zonas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Zonas/mntzonasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Zonas/mntzonas.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdZona((isset($_POST["LSTIdZona"]) && $_POST["LSTIdZona"] != "") ? $_POST["LSTIdZona"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ZONA"), (isset($_POST["LSTIdZona"]) && $_POST["LSTIdZona"] != "" ) ? $_POST["LSTIdZona"] : "");
