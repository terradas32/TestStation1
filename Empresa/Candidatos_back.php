...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Candidatos_backDB($conn);  // Entidad DB
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					if (!empty($_POST['candidatos_back_next_page']) && $_POST['candidatos_back_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'candidatos_back');
					...
					$lista=$pager->getRS();
					include('Template/Candidatos_back/mntcandidatos_backl.php');
				}else{
					$cEntidad	= new Candidatos_back();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Candidatos_back/mntcandidatos_backa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Candidatos_back/mntcandidatos_back.php');
			break;
