						 	...
						 	if($nEscalas > 0){
						 		while(!$listaEscalas->EOF){
									...
									while (!$vVectorCandidatos->EOF){
								        ...
								        $sNoMostrar = $listaEscalas->fields['idBloque'] . "-" . $listaEscalas->fields['idEscala'];
								        if ($bPrimeraVuelta){
								        	if ($sNoMostrar != "40-1"){
										        $sHtml.='
...
//				       	echo "<br />---------->[" . $sPosi . "][" . $iPBaremada . "]";
		 ...
		 {
		 	case 10:	//Tipo Prisma TEC
		 		$consistencia = baremo_C_TEC(number_format(sqrt($iPGlobal/10)*100 ,0));
