...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Pruebas_ayudasDB($conn);  // Entidad DB
	$cEntidad	= new Pruebas_ayudas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>","<?php echo constant("STR_NOTIFICACION");?>");</script><?php
				...
				{
					if (!empty($_POST['pruebas_ayudas_next_page']) && $_POST['pruebas_ayudas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'pruebas_ayudas');
					...
					$lista=$pager->getRS();
					include('Template/Pruebas_ayudas/mntpruebas_ayudasl.php');
				}else{
					$cEntidad	= new Pruebas_ayudas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Pruebas_ayudas/mntpruebas_ayudasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Pruebas_ayudas/mntpruebas_ayudas.php');
			break;
