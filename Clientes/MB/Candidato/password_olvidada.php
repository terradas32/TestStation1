...
if (isset($_POST['fGo']))
{
	if (!empty($_POST['fLogin']))
	{
		...
		if (!$bEncontradoUsuario)
        {
    		...
    		if (sizeof($rowUser) > 0 && $rowUser["mail"] == $_POST['fLogin'])
    		{
                if (!empty($rowUser["idCandidato"]))
                {
...
<?php
	function enviaEmail($cEntidad, $cEmpresa)
	{
