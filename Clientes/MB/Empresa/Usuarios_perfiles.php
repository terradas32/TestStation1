...
include_once ('include/conexion.php');
    ...
    $cEntidadDB	= new Usuarios_perfilesDB($conn);  // Entidad DB
	$cEntidad	= new Usuarios_perfiles();  // Entidad
	
	$_cEntidadUsuarioTK = getUsuarioToken($conn);
    $sCol1='';
    $sCol2='';
	
	$comboUSUARIOS	= new Combo($conn,"fIdUsuario","idUsuario","login","Descripcion","wi_usuarios","",constant("SLC_OPCION"),"");
	$comboPERFILES	= new Combo($conn,"fIdPerfil","idPerfil","descripcion","Descripcion","wi_perfiles","",constant("SLC_OPCION"),"");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script>alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['usuarios_perfiles_next_page']) && $_POST['usuarios_perfiles_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'usuarios_perfiles');
					if ($bInit)	$pager->curr_page=1;
					$pager->showPageLinks = true;
					$lista=$pager->getRS();
					include('Template/Usuarios_perfiles/mntusuarios_perfilesl.php');
				}else{
					$cEntidad	= new Usuarios_perfiles();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Usuarios_perfiles/mntusuarios_perfilesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Usuarios_perfiles/mntusuarios_perfiles.php');
			break;
