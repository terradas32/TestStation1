		...
		case constant("MNT_MODIFICAR"):
			...
			if ($cEntidadDB->modificar($cEntidad))
			{
				...
				if ($cEntidad->getCertificada() == "1")
				{
					...
					//Buscamos el informe
					$_idPruebaEncontrada="";
					$_idPrueba = "87";	//CUESTIONARIO DE PERSONALIDAD LABORAL (SEAS)
					...
					if (empty($_idPruebaEncontrada)){
						$_idPrueba = "83";	//CUESTIONARIO PERSONALIDAD COMERCIAL (SERC)
						$cRespuestas_pruebas->setIdEmpresa($cEntidad->getIdEmpresa());
						$cRespuestas_pruebas->setIdProceso($cEntidad->getIdProceso());
						$cRespuestas_pruebas->setIdCandidato($cEntidad->getIdCandidato());
						$cRespuestas_pruebas->setIdPrueba($_idPrueba);
					...
					if ($bSubidoInforme)
					{
						...
						//En caso MB sólo un informe, se guarda en el campo report1
					...
					if ($bEmpresaFormacion)
					{
						...
						$cEmpresaCliente->setIdEmpresa("5140");	// Empresa FormacionMB
						...
						//Revisamos cuantos usuarios Cliente de MB hay en AV y mandamos correos
						$sQUERY = 'SELECT * FROM users WHERE client_id = 15 ';
						...
						$cEmpresaFORMACION->setIdEmpresa("5140");	// Empresa FormacionMB
