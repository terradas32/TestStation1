...
function validaForm()
{
	...
	msg +=vString("<?php echo constant("STR_LITERAL");?>:",f.fLiteral.value,255,true);
	msg +=vString("<?php echo constant("STR_DESCRIPCION");?>:",f.fDescripcion.value,255,true);
	aSistema = document.forms[0].fSistema;
	sId = "";
	for(i=0; i < aSistema.length; i++ ){
		if (aSistema[i].type == "radio" && aSistema[i].name == "fSistema"){
			if (aSistema[i].checked)
			{
				sId = aSistema[i].value;
			}
		}
	}
	msg +=vNumber("<?php echo constant("STR_SISTEMA");?>:",sId,1,true);
	aVisible = document.forms[0].fVisible;
	sId = "";
	for(i=0; i < aVisible.length; i++ ){
		if (aVisible[i].type == "radio" && aVisible[i].name == "fVisible"){
			if (aVisible[i].checked)
			{
				sId = aVisible[i].value;
			}
		}
	}
	msg +=vNumber("<?php echo constant("STR_VISIBLE");?>:",sId,1,true);
if (msg != "") {
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	<div id="envoltura">
		...
		<div style="width: 100%">
			<table cellspacing="0" cellpadding="0" width="100%" border="0">
				...
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fLiteral"><?php echo constant("STR_LITERAL");?></label>&nbsp;</td>
					<td><input type="text" id="fLiteral" name="fLiteral" value="<?php echo $cEntidad->getLiteral();?>" class="obliga"  onchange="javascript:trim(this);" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
				...
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fSistema"><?php echo constant("STR_SISTEMA");?></label>&nbsp;</td>
					<td><input type="radio" name="fSistema" id="fSistema1" value="1"  <?php echo ($cEntidad->getSistema() != "" && strtoupper($cEntidad->getSistema()) == "1") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fSistema1">Sí</label>&nbsp;<input type="radio" name="fSistema" id="fSistema0" value="0"  <?php echo ($cEntidad->getSistema() != "" && strtoupper($cEntidad->getSistema()) == "0") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fSistema0">No</label>&nbsp;<input type="hidden" name="fSistemaHast" value="<?php echo $cEntidad->getSistema();?>" /></td>
				</tr>
				<tr>
					<td width="5"><img src="<?php echo constant('DIR_WS_GRAF');?>sp.gif" width="5" height="20" border="0" alt="" /></td>
					<td nowrap="nowrap" width="140" class="negrob" valign="top"><label for="fVisible"><?php echo constant("STR_VISIBLE");?></label>&nbsp;</td>
					<td><input type="radio" name="fVisible" id="fVisible1" value="1"  <?php echo ($cEntidad->getVisible() != "" && strtoupper($cEntidad->getVisible()) == "1") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fVisible1">Sí</label>&nbsp;<input type="radio" name="fVisible" id="fVisible0" value="0"  <?php echo ($cEntidad->getVisible() != "" && strtoupper($cEntidad->getVisible()) == "0") ? "checked=\"checked\"" : "";?> />&nbsp;<label for="fVisible0">No</label>&nbsp;<input type="hidden" name="fVisibleHast" value="<?php echo $cEntidad->getVisible();?>" /></td>
				</tr>
	...
	<input type="hidden" name="fIdLiteral" value="<?php echo (empty($_POST['AddIdioma'])) ? $cEntidad->getIdLiteral() : $_POST['fIdLiteral'];?>" />
...
<?php include (constant("DIR_WS_INCLUDE") . "pie.php");?>
	...
	<input type="hidden" name="correos_literales_next_page" value="<?php echo (isset($_POST['correos_literales_next_page'])) ? $cUtilidades->validaXSS($_POST['correos_literales_next_page']) : "1";?>" />