...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaCodIdiomaIso2()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "CodIdiomaIso2";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "CodIdiomaIso2"]) && $_POST[$sPrefijo . "CodIdiomaIso2"] != ""){
		...
		if (isset($_POST[$sPrefijo . "PathImagen"]) && $_POST[$sPrefijo . "PathImagen"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "pathImagen=" . $conn->qstr($_POST[$sPrefijo . "PathImagen"], false);
		}
		if (isset($_POST[$sPrefijo . "Orden"]) && $_POST[$sPrefijo . "Orden"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "orden=" . $conn->qstr($_POST[$sPrefijo . "Orden"], false);
		}
		if (isset($_POST[$sPrefijo . "ActivoFront"]) && $_POST[$sPrefijo . "ActivoFront"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "activoFront=" . $conn->qstr($_POST[$sPrefijo . "ActivoFront"], false);
		}
		if (isset($_POST[$sPrefijo . "ActivoBack"]) && $_POST[$sPrefijo . "ActivoBack"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "activoBack=" . $conn->qstr($_POST[$sPrefijo . "ActivoBack"], false);
		}
		...
		if ($bJoin){
			//$comboWI_IDIOMAS= new Combo($conn, $sPrefijo . "CodIdiomaIso2", "DISTINCT(s.codIdiomaIso2)", "s.nombre", "descripcion", "wi_idiomas s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "codIdiomaIso2", "fecMod");
		}else{
			$comboWI_IDIOMAS= new Combo($conn, $sPrefijo . "CodIdiomaIso2","codIdiomaIso2","nombre","descripcion","wi_idiomas","",constant("SLC_OPCION"),$sWhere, "codIdiomaIso2", "fecMod");
		}
		...
		$comboWI_IDIOMAS->setNombre($sNombreCampo);
echo $comboWI_IDIOMAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>