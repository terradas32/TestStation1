...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Proceso_baremosDB($conn);  // Entidad DB
	$cEntidad	= new Proceso_baremos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					if (!empty($_POST['proceso_baremos_next_page']) && $_POST['proceso_baremos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'proceso_baremos');
					...
					$lista=$pager->getRS();
					include('Template/Proceso_baremos/mntproceso_baremosl.php');
				}else{
					$cEntidad	= new Proceso_baremos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Proceso_baremos/mntproceso_baremosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Proceso_baremos/mntproceso_baremos.php');
			break;
