...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new NotificacionesDB($conn);  // Entidad DB
	$cEntidad	= new Notificaciones();  // Entidad
	
	$_cEntidadUsuarioTK = getUsuarioToken($conn);
	$sCol1='';
	$sCol2='';
	
	$comboNOTIFICACIONES_TIPOS	= new Combo($conn,"fIdTipoNotificacion","idTipoNotificacion","nombre","Descripcion","notificaciones_tipos","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['notificaciones_next_page']) && $_POST['notificaciones_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'notificaciones');
					...
					$lista=$pager->getRS();
					include('Template/Notificaciones/mntnotificacionesl.php');
				}else{
					$cEntidad	= new Notificaciones();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Notificaciones/mntnotificacionesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Notificaciones/mntnotificaciones.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		global $cUtilidades;
		$cEntidad->setIdNotificacion((isset($_POST["fIdNotificacion"])) ? $_POST["fIdNotificacion"] : "");
		$cEntidad->setIdTipoNotificacion((isset($_POST["fIdTipoNotificacion"])) ? $_POST["fIdTipoNotificacion"] : "");
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdNotificacion((isset($_POST["LSTIdNotificacion"]) && $_POST["LSTIdNotificacion"] != "") ? $_POST["LSTIdNotificacion"] : "");	$cEntidad->setBusqueda(constant("STR_ID"), (isset($_POST["LSTIdNotificacion"]) && $_POST["LSTIdNotificacion"] != "" ) ? $_POST["LSTIdNotificacion"] : "");
		$cEntidad->setIdNotificacionHast((isset($_POST["LSTIdNotificacionHast"]) && $_POST["LSTIdNotificacionHast"] != "") ? $_POST["LSTIdNotificacionHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdNotificacionHast"]) && $_POST["LSTIdNotificacionHast"] != "" ) ? $_POST["LSTIdNotificacionHast"] : "");
		global $comboNOTIFICACIONES_TIPOS;
		$cEntidad->setIdTipoNotificacion((isset($_POST["LSTIdTipoNotificacion"]) && $_POST["LSTIdTipoNotificacion"] != "") ? $_POST["LSTIdTipoNotificacion"] : "");	$cEntidad->setBusqueda(constant("STR_TIPO_DE_NOTIFICACION"), (isset($_POST["LSTIdTipoNotificacion"]) && $_POST["LSTIdTipoNotificacion"] != "" ) ? $comboNOTIFICACIONES_TIPOS->getDescripcionCombo($_POST["LSTIdTipoNotificacion"]) : "");
