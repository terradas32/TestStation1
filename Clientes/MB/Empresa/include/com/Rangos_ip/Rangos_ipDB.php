...
class Rangos_ipDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr($cEntidad->getIdRangoIr(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getRangoSup(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getRangoInf(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Rangos_ipDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idRangoIp) AS Max FROM rangos_ip ";
		$sql  .=" WHERE idRangoIr=" . $aux->qstr($cEntidad->getIdRangoIr(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Rangos_ipDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE rangos_ip SET ";
		$sql .= "idRangoIp=" . $aux->qstr($cEntidad->getIdRangoIp(), false) . ", ";
		$sql .= "idRangoIr=" . $aux->qstr($cEntidad->getIdRangoIr(), false) . ", ";
		$sql .= "rangoSup=" . $aux->qstr($cEntidad->getRangoSup(), false) . ", ";
		$sql .= "rangoInf=" . $aux->qstr($cEntidad->getRangoInf(), false) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false) ;
		$sql .= " WHERE ";
		$sql .="idRangoIp=" . $aux->qstr($cEntidad->getIdRangoIp(), false) . " AND idRangoIr=" . $aux->qstr($cEntidad->getIdRangoIr(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Rangos_ipDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM rangos_ip ";
			$sql  .="WHERE ";
			if ($cEntidad->getIdRangoIp() != ""){
				$sql .= $this->getSQLAnd($and);
				$and = true;
				$sql  .="idRangoIp=" . $aux->qstr($cEntidad->getIdRangoIp(), false) . " ";
			}
			if ($cEntidad->getIdRangoIr() != ""){
				$sql .= $this->getSQLAnd($and);
				$and = true;
				$sql  .="idRangoIr=" . $aux->qstr($cEntidad->getIdRangoIr(), false) . " ";
			}
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Rangos_ipDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM rangos_ip WHERE ";
		$sql  .="idRangoIp=" . $aux->qstr($cEntidad->getIdRangoIp(), false) . " AND idRangoIr=" . $aux->qstr($cEntidad->getIdRangoIr(), false) . " ";
//		echo "<br />" . $sql;
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdRangoIp($arr['idRangoIp']);
					$cEntidad->setIdRangoIr($arr['idRangoIr']);
					$cEntidad->setRangoSup($arr['rangoSup']);
					$cEntidad->setRangoInf($arr['rangoInf']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Rangos_ipDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getRangoSup() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rangoSup) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getRangoSup() . "%") . ")";
		}
		if ($cEntidad->getRangoInf() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(rangoInf) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getRangoInf() . "%") . ")";
		}
