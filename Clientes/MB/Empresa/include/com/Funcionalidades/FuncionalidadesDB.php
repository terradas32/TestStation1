		...
		while (!$lista->EOF)
		{
			...
			if ($lista->fields['indentacion'] == 0)
			{
				...
				$sMenus .= '
					...
					<table id="tb' . $lista->fields['idFuncionalidad'] . '" cellspacing="0" width="100%" cellpadding="0" border="0">
					';
					if (empty($lista->fields['popUp'])){
						...
						if ("134" == $lista->fields['idFuncionalidad']){	//Videotutoriales para MB
							$sUrl = constant("HTTP_LINK_ACCESO") . "/Empresa/videotutoriales/";
						}
						$sMenus .= '
				...
				if (empty($lista->fields['url'])){
					...
					$mystring = $lista->fields['url'];
					$findme   = 'http';
					$pos = strpos($mystring, $findme);
					if ($pos === false) {
							$sUrl = constant("HTTPS_SERVER") . $lista->fields['url'];
					} else {
					    $sUrl = $lista->fields['url'];
					}
				}
				$sMenus .= '
					<table id="tb' . $lista->fields['idFuncionalidad'] . '" cellspacing="1" width="100%" cellpadding="1" border="0">';
				...
				if (empty($lista->fields['popUp'])){
					$sMenus .= '
							<td class="submenu" width="100%" bgcolor="' . $bgColor . '" onclick="javascript:cambia(' . $i . ',2);setTitulo(\'' . addslashes($comboFUNCIONALIDADES->getDescripcionCombo($lista->fields['idPadre']) . ' - ' . $str_menu) . '\');enviarMenu(\'' . $sUrl . '\',\'' . $sModo . '\',\'' . $lista->fields['bgColor'] . '\');" onmouseout="window.status=\'\';return true" onmouseover="window.status=\'' . addslashes($str_menu) . '\';return true" title="' . $str_desc_menu . '"><img src="graf/sp.gif" width="' . 5*$lista->fields['indentacion'] . '" style="height:5px" border="0" alt="" />' . $str_menu . '</td>
					...
					$sMenus .= '
							<td class="submenu" width="100%" bgcolor="' . $bgColor . '" onclick="javascript:expandingWindow(\'' . $sUrl . '\');"><img src="graf/sp.gif" width="' . 5*$lista->fields['indentacion'] . '" style="height:5px" border="0" alt="" /><a href="#" onmouseout="window.status=\'\';return true" onmouseover="window.status=\'' . addslashes($str_menu) . '\';return true" title="' . $str_desc_menu . '">' . $str_menu . '</a></td>
							';
				}
				$sMenus .= '
						</tr>
