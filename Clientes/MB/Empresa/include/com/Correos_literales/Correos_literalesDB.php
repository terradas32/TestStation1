...
* la tabla correos_literales
**/
class Correos_literalesDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$newId = $this->getSiguienteId($cEntidad);
                ...
                if($cEntidad->getIdLiteral() != ""){
                    $newId = $cEntidad->getIdLiteral();
        		}else{
                    $newId = $this->getSiguienteId($cEntidad);
        		}
		$sql = "INSERT INTO correos_literales (";
		$sql .= "idLiteral" . ",";
		$sql .= "codIdiomaIso2" . ",";
		$sql .= "literal" . ",";
		$sql .= "descripcion" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getLiteral(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getDescripcion(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getSistema(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getVisible(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Correos_literalesDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idLiteral) AS Max FROM correos_literales ";
		...
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Correos_literalesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE correos_literales SET ";
		$sql .= "idLiteral=" . $aux->qstr($cEntidad->getIdLiteral(), false) . ", ";
		$sql .= "codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . ", ";
		$sql .= "literal=" . $aux->qstr($cEntidad->getLiteral(), false) . ", ";
		$sql .= "descripcion=" . $aux->qstr($cEntidad->getDescripcion(), false) . ", ";
		$sql .= "sistema=" . $aux->qstr($cEntidad->getSistema(), false) . ", ";
		$sql .= "visible=" . $aux->qstr($cEntidad->getVisible(), false) . ", ";
		$sql .= "fecMod=" . $aux->sysTimeStamp . ",";
		$sql .= "usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false) ;
		$sql .= " WHERE ";
		$sql .="idLiteral=" . $aux->qstr($cEntidad->getIdLiteral(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Correos_literalesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Correos_literalesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM correos_literales WHERE ";
		$sql  .="idLiteral=" . $aux->qstr($cEntidad->getIdLiteral(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdLiteral($arr['idLiteral']);
					$cEntidad->setCodIdiomaIso2($arr['codIdiomaIso2']);
					$cEntidad->setLiteral($arr['literal']);
					$cEntidad->setDescripcion($arr['descripcion']);
					$cEntidad->setSistema($arr['sistema']);
					$cEntidad->setVisible($arr['visible']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Correos_literalesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM correos_literales ";
		if ($cEntidad->getIdLiteral() != ""){
		...
		if ($cEntidad->getLiteral() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(literal) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getLiteral() . "%") . ")";
		}
		...
		if ($cEntidad->getSistema() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="sistema>=" . $aux->qstr($cEntidad->getSistema(), false);
		}
		if ($cEntidad->getSistemaHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="sistema<=" . $aux->qstr($cEntidad->getSistemaHast(), false);
		}
		if ($cEntidad->getVisible() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="visible>=" . $aux->qstr($cEntidad->getVisible(), false);
		}
		if ($cEntidad->getVisibleHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="visible<=" . $aux->qstr($cEntidad->getVisibleHast(), false);
		}
...
}//Fin de la Clase Correos_literalesDB
?>