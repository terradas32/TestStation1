...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Opciones_valores/Opciones_valoresDB.php");
	require_once(constant("DIR_WS_COM") . "Opciones_valores/Opciones_valores.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Opciones_valoresDB($conn);  // Entidad DB
	$cEntidad	= new Opciones_valores();  // Entidad
	...
	$comboITEMS	= new Combo($conn,"fIdItem","idPrueba","descripcion","Descripcion","items","",constant("SLC_OPCION"),"","","fecMod");
	$comboOPCIONES	= new Combo($conn,"fIdOpcion","idOpcion","descripcion","Descripcion","opciones","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['opciones_valores_next_page']) && $_POST['opciones_valores_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'opciones_valores');
					...
					$lista=$pager->getRS();
					include('Template/Opciones_valores/mntopciones_valoresl.php');
				}else{
					$cEntidad	= new Opciones_valores();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Opciones_valores/mntopciones_valoresa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Opciones_valores/mntopciones_valores.php');
			break;
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setIdValor((isset($_POST["fIdValor"])) ? $_POST["fIdValor"] : "");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setIdValor((isset($_POST["LSTIdValor"]) && $_POST["LSTIdValor"] != "") ? $_POST["LSTIdValor"] : "");	$cEntidad->setBusqueda(constant("STR_ID_VALOR"), (isset($_POST["LSTIdValor"]) && $_POST["LSTIdValor"] != "" ) ? $_POST["LSTIdValor"] : "");
		$cEntidad->setIdValorHast((isset($_POST["LSTIdValorHast"]) && $_POST["LSTIdValorHast"] != "") ? $_POST["LSTIdValorHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_VALOR") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdValorHast"]) && $_POST["LSTIdValorHast"] != "" ) ? $_POST["LSTIdValorHast"] : "");
