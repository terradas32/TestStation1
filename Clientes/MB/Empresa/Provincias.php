...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Provincias/ProvinciasDB.php");
	require_once(constant("DIR_WS_COM") . "Provincias/Provincias.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new ProvinciasDB($conn);  // Entidad DB
	$cEntidad	= new Provincias();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['provincias_next_page']) && $_POST['provincias_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'provincias');
					...
					$lista=$pager->getRS();
					include('Template/Provincias/mntprovinciasl.php');
				}else{
					$cEntidad	= new Provincias();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Provincias/mntprovinciasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Provincias/mntprovincias.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdProvincia((isset($_POST["LSTIdProvincia"]) && $_POST["LSTIdProvincia"] != "") ? $_POST["LSTIdProvincia"] : "");	$cEntidad->setBusqueda(constant("STR_ID_PROVINCIA"), (isset($_POST["LSTIdProvincia"]) && $_POST["LSTIdProvincia"] != "" ) ? $_POST["LSTIdProvincia"] : "");
