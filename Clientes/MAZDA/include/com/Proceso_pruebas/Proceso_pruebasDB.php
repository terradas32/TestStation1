...
class Proceso_pruebasDB
{
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  .=" WHERE idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql .= " WHERE ";
		$sql .="idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false) . " AND idEmpresa=" . $aux->qstr($cEntidad->getIdEmpresa(), false) . " AND idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " AND codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$this->sSQL=$sql;
		$_SESSION['SQLProceso_pruebasDB'] = $sql;
		return $sql;
	}
	...
	{
		$this->sSQL = $_SESSION['SQLProceso_pruebasDB'];
		return $this->sSQL;
	}
