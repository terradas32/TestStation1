...
class BaremosDB
{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			//Borramos todos los resultados para este baremo-prueba.
			
			$sql1  ="DELETE FROM baremos_resultados ";
			$sql1  .="WHERE ";
			if ($cEntidad->getIdBaremo() != ""){

				$sql1  .="idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . " ";
			}
			if ($cEntidad->getIdPrueba() != ""){
				$sql1 .= $this->getSQLAnd($and);
				$and = true;
				$sql1  .="idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
			}

			if($aux->Execute($sql1) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][BaremosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql1 . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
	...
	*************************************************************************/
	...
	{
		$this->sSQL = $_SESSION['SQLBaremosDB'];
		return $this->sSQL;
	}
