...
* la tabla perfiles_funcionalidades
**/
class Perfiles_funcionalidadesDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$iSize = sizeof($aPerfilFuncionalidades);
		for ($i=0; $i < $iSize; $i++){
			$aPsFs = explode("|", $aPerfilFuncionalidades[$i]);
			$cEntidad->setIdPerfil($aPsFs[0]);
			$cEntidad->setIdFuncionalidad($aPsFs[1]);
			if (!$this->borrar($cEntidad)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . ":::" . constant("MNT_BORRAR") . "][Perfiles_funcionalidadesDB]";
				...
				if($aux->Execute($sql) === false){
					$this->msg_Error	= array();
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Perfiles_funcionalidadesDB]";
					$this->msg_Error[]	= $sTypeError;
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					return 0;
				}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM wi_perfiles_funcionalidades ";
		$sql  .=" WHERE idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " AND idFuncionalidad=" . $aux->qstr($cEntidad->getIdFuncionalidad(), false) . " ";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
...
/*		$aux			= $this->conn;
		...
		$sql = "UPDATE wi_perfiles_funcionalidades SET ";
		...
		$sql .= " WHERE ";
		$sql .="idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " AND idFuncionalidad=" . $aux->qstr($cEntidad->getIdFuncionalidad(), false) . " ";
		$retorno=true;
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Perfiles_funcionalidadesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		return $retorno;
*/
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM wi_perfiles_funcionalidades ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Perfiles_funcionalidadesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	...
	{
		$aux			= $this->conn;
	$cEntidad->getPerfilFuncionalidades();
		$sql = "SELECT  idPerfil, idFuncionalidad, modificar, borrar, fecAlta, fecMod, usuAlta, usuMod FROM wi_perfiles_funcionalidades WHERE ";
		$sql  .="idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " AND idFuncionalidad=" . $aux->qstr($cEntidad->getIdFuncionalidad(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setPerfilFuncionalidades($arr['idPerfil'] . "|" . $arr['idFuncionalidad'] . "|" . $arr['modificar'] . "|" . $arr['borrar']);
			}
		}else{
			echo("Error SQL:\n" . $sql);
			exit;
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM wi_perfiles_funcionalidades ";
		if ($cEntidad->getIdPerfil() != ""){
...
//			$sql .="idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false);
		...
		$this->sSQL=$sql;
		$_SESSION['SQLPerfiles_funcionalidadesDB'] = $sql;
		return $sql;
	}
	...
	{
		$this->sSQL = $_SESSION['SQLPerfiles_funcionalidadesDB'];
		return $this->sSQL;
	}
...
}//Fin de la Clase Perfiles_funcionalidadesDB
?>