...
include_once ('include/conexion.php');
  ...
  if (!$bLogado){
	...
	if (!empty($_POST["fAceptaCondiciones"])){
		//Marcamos que la Empresa ha aceptado las condiciones.
		$sSQL = "UPDATE empresas SET aceptaCondiciones=" . $conn->qstr($_POST["fAceptaCondiciones"], false) . " WHERE idEmpresa=" . $_cEntidadUsuarioTK->getIdEmpresa();
		$conn->Execute($sSQL);
	}
	if (!empty($_POST["fAceptaTerminos"])){
		//Marcamos que la Empresa ha aceptado las condiciones.
		$sSQL = "UPDATE empresas SET aceptaTerminos=" . $conn->qstr($_POST["fAceptaTerminos"], false) . " WHERE idEmpresa=" . $_cEntidadUsuarioTK->getIdEmpresa();
		$conn->Execute($sSQL);
	}
?>
