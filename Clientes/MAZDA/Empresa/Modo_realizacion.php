...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Modo_realizacion/Modo_realizacionDB.php");
	require_once(constant("DIR_WS_COM") . "Modo_realizacion/Modo_realizacion.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Modo_realizacionDB($conn);  // Entidad DB
	$cEntidad	= new Modo_realizacion();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['modo_realizacion_next_page']) && $_POST['modo_realizacion_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'modo_realizacion');
					...
					$lista=$pager->getRS();
					include('Template/Modo_realizacion/mntmodo_realizacionl.php');
				}else{
					$cEntidad	= new Modo_realizacion();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Modo_realizacion/mntmodo_realizaciona.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Modo_realizacion/mntmodo_realizacion.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdModoRealizacion((isset($_POST["LSTIdModoRealizacion"]) && $_POST["LSTIdModoRealizacion"] != "") ? $_POST["LSTIdModoRealizacion"] : "");	$cEntidad->setBusqueda(constant("STR_ID_MODO_REALIZACION"), (isset($_POST["LSTIdModoRealizacion"]) && $_POST["LSTIdModoRealizacion"] != "" ) ? $_POST["LSTIdModoRealizacion"] : "");
		$cEntidad->setIdModoRealizacionHast((isset($_POST["LSTIdModoRealizacionHast"]) && $_POST["LSTIdModoRealizacionHast"] != "") ? $_POST["LSTIdModoRealizacionHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_MODO_REALIZACION") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdModoRealizacionHast"]) && $_POST["LSTIdModoRealizacionHast"] != "" ) ? $_POST["LSTIdModoRealizacionHast"] : "");
