	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
				...
				while (!$rsEmpresas->EOF)
				{
					...
					{
						//Miramos si ya se le ha enviado aviso a los 30 días
						...
						//echo $sSQL;exit;
						$rsAvisos30 = $conn->Execute($sSQL);
						$bAvisar=false;
						if ($rsAvisos30->NumRows() <= 0)
						{
							...
							$intervalo = new DateInterval('P1M'); // Más 1 mes
							$fecha->add($intervalo);
							$sFecha30 = $fecha->format('Y-m-d');
							$dFecha30 = strtotime($sFecha30);
							$now = date('Y-m-d');
							$dAhora = strtotime($now);
							if ($now == $sFecha30){
								$bAvisar=true;
								$sSQL = 'INSERT INTO umarkedcandidatesforhiring (idEmpresa, idProceso, idCandidato, fechaInicio, UltimaFecha) VALUES ';
								$sSQL .= ' (' . $rsCandidatos->fields['idEmpresa'] . ', ' . $rsCandidatos->fields['idProceso'] . ', ' . $rsCandidatos->fields['idCandidato'] . ', \'' . $sFecha30 . '\', NULL)';
								$conn->Execute($sSQL);
							}
