...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdMunicipio()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdMunicipio";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdComunidad"]) && $_POST[$sPrefijo . "IdComunidad"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idComunidad=" . $conn->qstr($_POST[$sPrefijo . "IdComunidad"], false);
		}
		if (isset($_POST[$sPrefijo . "IdProvincia"])){
		...
		if ($bJoin){
			//$comboWI_MUNICIPIOS= new Combo($conn, $sPrefijo . "IdMunicipio", "DISTINCT(s.idMunicipio)", "s.descripcion", "descripcion", "wi_municipios s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idMunicipio", "fecMod");
		}else{
			$comboWI_MUNICIPIOS= new Combo($conn, $sPrefijo . "IdMunicipio","idMunicipio","descripcion","descripcion","wi_municipios","",constant("SLC_OPCION"),$sWhere, "idMunicipio", "fecMod");
		}
		...
		$comboWI_MUNICIPIOS->setNombre($sNombreCampo);
echo $comboWI_MUNICIPIOS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>