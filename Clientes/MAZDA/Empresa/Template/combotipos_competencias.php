...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdTipoCompetencia()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdTipoCompetencia";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdTipoCompetencia"]) && $_POST[$sPrefijo . "IdTipoCompetencia"] != ""){
		...
		if ($bJoin){
			//$comboTIPOS_COMPETENCIAS= new Combo($conn, $sPrefijo . "IdTipoCompetencia", "DISTINCT(s.idTipoCompetencia)", "s.nombre", "descripcion", "tipos_competencias s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idTipoCompetencia", "fecMod");
		}else{
			$comboTIPOS_COMPETENCIAS= new Combo($conn, $sPrefijo . "IdTipoCompetencia","idTipoCompetencia","nombre","descripcion","tipos_competencias","",constant("SLC_OPCION"),$sWhere, "idTipoCompetencia", "fecMod");
		}
		...
		$comboTIPOS_COMPETENCIAS->setNombre($sNombreCampo);
echo $comboTIPOS_COMPETENCIAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>