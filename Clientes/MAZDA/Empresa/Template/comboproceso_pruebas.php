...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		if (isset($_POST[$sPrefijo . "IdEmpresa"]) && $_POST[$sPrefijo . "IdEmpresa"] != ""){
		...
		if ($bJoin){
			//$comboPROCESO_PRUEBAS= new Combo($conn, $sPrefijo . "IdPrueba", "DISTINCT(s.idPrueba)", "s.", "descripcion", "proceso_pruebas s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idPrueba", "fecMod");
		}else{
			$comboPROCESO_PRUEBAS= new Combo($conn, $sPrefijo . "IdPrueba","idPrueba","","descripcion","proceso_pruebas","",constant("SLC_OPCION"),$sWhere, "idPrueba", "fecMod");
		}
		...
		$comboPROCESO_PRUEBAS->setNombre($sNombreCampo);
echo $comboPROCESO_PRUEBAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>