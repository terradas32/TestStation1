...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdOpcion()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdOpcion";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdItem"])){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idItem=" . $conn->qstr($_POST[$sPrefijo . "IdItem"], false);
		}
		if (isset($_POST[$sPrefijo . "IdOpcion"]) && $_POST[$sPrefijo . "IdOpcion"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idOpcion=" . $conn->qstr($_POST[$sPrefijo . "IdOpcion"], false);
		}
		...
		if ($bJoin){
			//$comboOPCIONES= new Combo($conn, $sPrefijo . "IdOpcion", "DISTINCT(s.idOpcion)", "s.descripcion", "descripcion", "opciones s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idOpcion", "fecMod");
		}else{
			$comboOPCIONES= new Combo($conn, $sPrefijo . "IdOpcion","idOpcion","descripcion","descripcion","opciones","",constant("SLC_OPCION"),$sWhere, "idOpcion", "fecMod");
		}
		...
		$comboOPCIONES->setNombre($sNombreCampo);
echo $comboOPCIONES->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>