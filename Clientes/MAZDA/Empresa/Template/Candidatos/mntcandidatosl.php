...
<?php ob_start();?>
	<table cellspacing="1" cellpadding="2" border="0" width="99%">
      ...
      $sPinta = "";
      $sOrden = "ASC";
      if ($cEntidad->getOrderBy() == "certificada"){
      ...
      ?>
      <td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities("Contratado", ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='certificada';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes("Contratado");?>';return true"><?php echo "Contratado";?><?php echo $sPinta;?></a></td>
      <?php
			...
			?>
				...
				$sPinta = "";
				$sOrden = "ASC";
				if ($cEntidad->getOrderBy() == "aceptaMazda"){
					if ($cEntidad->getOrder() == "ASC"){
						$sOrden = "DESC";
						$sPinta = $strOrderASC;
					}else{
				...
				?>
				<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities("aceptaMazda", ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='aceptaMazda';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes("aceptaMazda");?>';return true"><?php echo "Acepta Mazda";?><?php echo $sPinta;?></a></td>
			<?php
			 }
			 ?>
			...
			<?php
			if ($_cEntidadUsuarioTK->getIdEmpresa() == "*pedroIdEmpresa")
			{
			?>
				<?php
				$sPinta = "";
				$sOrden = "ASC";
				if ($cEntidad->getOrderBy() == "envDiploma"){
					if ($cEntidad->getOrder() == "ASC"){
						$sOrden = "DESC";
						$sPinta = $strOrderASC;
					}else{
				...
				?>
				<td class="tdnaranjab" align="center"><a title="<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo htmlentities("EnvDiploma", ENT_COMPAT, "UTF-8");?>" href="#" class="tdnaranjab" style="color:#ffffff" onclick="javascript:document.forms[0].fReordenar.value=1;document.forms[0].LSTOrderBy.value='envDiploma';document.forms[0].LSTOrder.value='<?php echo $sOrden;?>';enviar('<?php echo constant("MNT_LISTAR");?>');" onmouseout="window.status='';return true" onmouseover="window.status='<?php echo constant("STR_ORDENAR_POR");?> >>> <?php echo addslashes("EnvDiploma");?>';return true"><?php echo "Env. Diploma";?><?php echo $sPinta;?></a></td>
			<?php
			 }
			 ?>
...
?>
			...
			{
			?>
				<td class="tddatoslista" valign="top" onclick="javascript:setPK('<?php echo addslashes($lista->fields['idCandidato']);?>','<?php echo addslashes($lista->fields['idEmpresa']);?>','<?php echo addslashes($lista->fields['idProceso']);?>');enviar(<?php echo constant("MNT_CONSULTAR");?>);" title="<?php echo strip_tags($lista->fields['envDiploma']);?>"><?php echo ($lista->fields['envDiploma'] > 0 ) ? constant("STR_SI") : constant("STR_NO");?></td>
			<?php
			}
			?>
...
<?php include (constant("DIR_WS_INCLUDE") . "cabecera.php");?>
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			...
			{
			?>
				<a href="sqlToExcel.php?fSQLtoEXCEL=<?php echo base64_encode($sql . constant("CHAR_SEPARA") . "candidatos");?>"><img src="<?php echo constant('DIR_WS_GRAF');?>excel.gif" width="34" height="35" align="right" border="0" alt="<?php echo constant("STR_EXPORTAR_A_EXCEL");?>" /></a>
			<?php
			}
			?>
			</td>
