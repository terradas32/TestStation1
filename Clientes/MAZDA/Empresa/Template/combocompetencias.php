...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:cambiaIdCompetencia()\"";
		$sPrefijo		= "f";
		$sNombreCampo	= $sPrefijo . "IdCompetencia";
		$sWhere			= "";
		$bJoin			= false;
		$sFrame			= "";
		$sBgColor		= "";
		...
		if (isset($_POST[$sPrefijo . "IdTipoCompetencia"])){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idTipoCompetencia=" . $conn->qstr($_POST[$sPrefijo . "IdTipoCompetencia"], false);
		}
		if (isset($_POST[$sPrefijo . "IdCompetencia"]) && $_POST[$sPrefijo . "IdCompetencia"] != ""){
			$sWhere.=(empty($sWhere) ? " " : " AND ") . "idCompetencia=" . $conn->qstr($_POST[$sPrefijo . "IdCompetencia"], false);
		}
		...
		if ($bJoin){
			//$comboCOMPETENCIAS= new Combo($conn, $sPrefijo . "IdCompetencia", "DISTINCT(s.idTipoCompetencia)", "s.nombre", "descripcion", "competencias s, suscriptorestemasboletins stm", "", constant("SLC_OPCION"), $sWhere, "idCompetencia", "fecMod");
		}else{
			$comboCOMPETENCIAS= new Combo($conn, $sPrefijo . "IdCompetencia","idCompetencia","nombre","descripcion","competencias","",constant("SLC_OPCION"),$sWhere, "idCompetencia", "fecMod");
		}
		...
		$comboCOMPETENCIAS->setNombre($sNombreCampo);
echo $comboCOMPETENCIAS->getHTMLCombo($lineas, $sObliga, $sValor, $sComboProp, $sOption);
?>