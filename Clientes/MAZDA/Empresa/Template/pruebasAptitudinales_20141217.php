...
if (!defined ("DIR_FS_DOCUMENT_ROOT")){
    ...
    if (!$bLogado){
  	...
  	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Combo.php");
		...
		$sOption			= "";
		$lineas			= "1";
		$sComboProp		= "onchange=\"javascript:pruebasAptitudinales()\"";
		$sPrefijo		= "f";
		...
		if ($bJoin){
			...
			if (!empty($sPRUEBASPROCESO)){
				...
				$sWhere2.=(empty($sWhere2) ? " " : " AND ") . "idPrueba IN (" . $sPRUEBASPROCESO . ") AND bajaLog=0 AND idTipoPrueba=2";
			}else {
				$sWhere2.=(empty($sWhere2) ? " " : " AND ") . "idPrueba='-1'";
			}
			$comboPRUEBASGROUP	= new Combo($conn, $sPrefijo . "IdPrueba","idPrueba","nombre","Descripcion","pruebas","","",$sWhere2, "idprueba", "fecMod", "idprueba");
		}
