...
class Respuestas_pruebasDB
{
	...
	* de uno o varios registros.
	* @param entidad Entidad con la información de la Búsqueda
	* @return String Lista recuperada
	*************************************************************************/
	...
	* @return String Lista recuperada
	*************************************************************************/
	function readListaAceptaMazdaIN($cEntidad)
	{
		$aux			= $this->conn;

		$sql="";
		$and = true;
		$sql.='SELECT * FROM respuestas_pruebas rp, candidatos c 
		WHERE c.idEmpresa=rp.idEmpresa
		AND c.idProceso=rp.idProceso
		AND c.idCandidato=rp.idCandidato ';
		if ($cEntidad->getIdEmpresa() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.idEmpresa IN(" . $cEntidad->getIdEmpresa() . ") ";
		}
		if ($cEntidad->getDescEmpresa() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getIdProceso() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.idProceso=" . $aux->qstr($cEntidad->getIdProceso(), false);
		}
		if ($cEntidad->getDescProceso() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getIdCandidato() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.idCandidato=" . $aux->qstr($cEntidad->getIdCandidato(), false);
		}
		if ($cEntidad->getDescCandidato() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getCodIdiomaIso2() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.codIdiomaIso2=" . $aux->qstr($cEntidad->getCodIdiomaIso2(), false);
		}
		if ($cEntidad->getDescIdiomaIso2() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getIdPrueba() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false);
		}
		if ($cEntidad->getDescPrueba() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getFinalizado() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.finalizado>=" . $aux->qstr(intval($cEntidad->getFinalizado()), false);
		}
		if ($cEntidad->getFinalizadoHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.finalizado<=" . $aux->qstr($cEntidad->getFinalizadoHast(), false);
		}
		if ($cEntidad->getLeidoInstrucciones() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.leidoInstrucciones>=" . $aux->qstr($cEntidad->getLeidoInstrucciones(), false);
		}
		if ($cEntidad->getLeidoInstruccionesHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.leidoInstrucciones<=" . $aux->qstr($cEntidad->getLeidoInstruccionesHast(), false);
		}
		if ($cEntidad->getLeidoEjemplos() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.leidoEjemplos>=" . $aux->qstr($cEntidad->getLeidoEjemplos(), false);
		}
		if ($cEntidad->getLeidoEjemplosHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.leidoEjemplos<=" . $aux->qstr($cEntidad->getLeidoEjemplosHast(), false);
		}
		/* if ($cEntidad->getMinutos_test() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
		...
		if ($cEntidad->getFecAlta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.fecAlta>=" . $aux->qstr($cEntidad->getFecAlta(), false);
		}
		if ($cEntidad->getFecAltaHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.fecAlta<=" . $aux->qstr($cEntidad->getFecAltaHast(), false);
		}
		if ($cEntidad->getFecMod() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.fecMod>=" . $aux->qstr($cEntidad->getFecMod(), false);
		}
		if ($cEntidad->getFecModHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.fecMod<=" . $aux->qstr($cEntidad->getFecModHast(), false);
		}
		if ($cEntidad->getUsuAlta() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.usuAlta=" . $aux->qstr($cEntidad->getUsuAlta(), false);
		}
		if ($cEntidad->getUsuMod() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="rp.usuMod=" . $aux->qstr($cEntidad->getUsuMod(), false);
		}
		
		$sql .=" AND (c.aceptaMazda=1 OR c.aceptaMazda IS NULL) ";
		
		if ($cEntidad->getOrderBy() != ""){
			$sOrder= "rp." . $cEntidad->getOrderBy();
			$cEntidad->setOrderBy($sOrder);
			$sql .=" ORDER BY " . $cEntidad->getOrderBy();
			if ($cEntidad->getOrder() != ""){
				$sql .=" " . $cEntidad->getOrder();
			}
