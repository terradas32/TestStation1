...
//				$sql.= "cabecera='imgPruebas/16/es/cabecera.jpg', ";
				...
				//Borrmos aquellas pruebas que no esten en algún idioma en la plataforma vieja
				//Ejemplo:
				// CEC			(3)-->	es
				// CECCASBEGA	(2)-->	es
				// CEL16		(12)-->	es
	...
	*/
	...
	function cargaOpciones_ItemsESP(){
		...
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [insertsOpcionesVipsEsp.sql][ProcesoTablas]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
		...
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [itemsVipsEsp.sql][ProcesoTablas]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
	...
	function actualizaItems_Nips(){
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/actualizaItemsNips.sql","r");
		$content="";
		while(!feof($DescriptorFichero)){
	    	$content .= fgets($DescriptorFichero,4096);
		}
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [actualizaItemsNips][ProcesoTablas]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
