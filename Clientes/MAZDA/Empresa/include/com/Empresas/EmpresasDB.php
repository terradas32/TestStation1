...
class EmpresasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		//Para todas las empresas se introduce la imagen de MAZDA
		if ($cEntidad->getPathLogo() == ""){
			$cEntidad->setPathLogo("imgEmpresas/Mazda.jpg");
		}
		...
		$sql .= $aux->qstr($cEntidad->getRazonSocial(), false) . ",";
		...
		if($cEntidad->getDistribuidor()=="1") {
			...
			$cEmpPerfil->setIdPerfil("8"); //Perfil Para concesionarios
		}

		$cEmpPerfilDB->insertar($cEmpPerfil);

		//Se pone por defecto las plantillas de correo de Mazda
		$sSQL = "INSERT INTO `correos` (`idCorreo`, `idTipoCorreo`, `idEmpresa`, `nombre`, `asunto`, `cuerpo`, `descripcion`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod`) ";
		$sSQL .= "SELECT `idCorreo`, `idTipoCorreo`, " . $newId . ", `nombre`, `asunto`, `cuerpo`, `descripcion`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod` ";
		$sSQL .= "FROM `correos` ";
		$sSQL .= "WHERE  idEmpresa=5650";	//Matriz de Mazda
		if($aux->Execute($sSQL) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][EmpresasDB][Plantillas correo M]";
			$this->msg_Error[]	= $sTypeError;
...
**************************************************************************************************/
	function modificar($cEntidad)
	{
			...
			($cEntidad->getDespuesDe() == $cEntidadBack->getOrden()) )
		{
			...
			$sql .= "razonSocial=" . $aux->qstr($cEntidad->getRazonSocial(), false) . ", ";
	...
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setRazonSocial($arr['razonSocial']);
...
if ($cEntidad->getRazonSocial() != ""){
	$sql .= $this->getSQLWhere($and);
	$and = true;
	$sql .="UPPER(razonSocial) LIKE UPPER('%" . $cEntidad->getRazonSocial() . "%')";
}
		if ($cEntidad->getCif() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
...
if ($cEntidad->getRazonSocial() != ""){
	...
	************************************************************************/
	 ...
	 *  en la base de datos
	 ************************************************************************/
	function ActualizaAccion($cEntidad)
	{
		$aux			= $this->conn;
