...
* la tabla Baremos_resultados_competencias
**/
class Baremos_resultados_competenciasDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql = "INSERT INTO baremos_resultados_competencias (";
		$sql .= "idResultado" . ",";
		$sql .= "idBaremo" . ",";
		$sql .= "idPrueba" . ",";
		$sql .= "idTipoCompetencia" . ",";
		$sql .= "idCompetencia" . ",";
		$sql .= "puntMin" . ",";
		$sql .= "puntMax" . ",";
		$sql .= "puntBaremada" . ",";
		...
		$sql .= $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getIdCompetencia(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPuntMin(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPuntMax(), false) . ",";
		$sql .= $aux->qstr($cEntidad->getPuntBaremada(), false) . ",";
		...
		if($aux->Execute($sql) === false){
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Baremos_resultadosDB]";
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX(idResultado) AS Max FROM baremos_resultados_competencias ";
		$sql  .="";
		$rs = $aux->Execute($sql);
		$newId=0;
		if ($rs){
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [newID][Baremos_resultadosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	************************************************************************/
	function modificar($cEntidad)
	{
		...
		$sql = "UPDATE baremos_resultados_competencias SET ";
		$sql .= "idResultado=" . $aux->qstr($cEntidad->getIdResultado(), false) . ", ";
		$sql .= "idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . ", ";
		$sql .= "idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . ", ";
		$sql .= "idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false) . ", ";
		$sql .= "idCompetencia=" . $aux->qstr($cEntidad->getIdCompetencia(), false) . ", ";
		$sql .= "puntMin=" . $aux->qstr($cEntidad->getPuntMin(), false) . ", ";
		$sql .= "puntMax=" . $aux->qstr($cEntidad->getPuntMax(), false) . ", ";
		$sql .= "puntBaremada=" . $aux->qstr($cEntidad->getPuntBaremada(), false) . ", ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Baremos_resultadosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		else{
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM baremos_resultados_competencias ";
			$sql  .="WHERE ";
			if ($cEntidad->getIdResultado() != ""){
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Baremos_resultadosDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	function readEntidad($cEntidad)
	{
		...
		$sql = "SELECT *  FROM baremos_resultados_competencias WHERE ";
		$sql  .="idResultado=" . $aux->qstr($cEntidad->getIdResultado(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdResultado($arr['idResultado']);
					$cEntidad->setIdBaremo($arr['idBaremo']);
					$cEntidad->setIdPrueba($arr['idPrueba']);
					$cEntidad->setIdTipoCompetencia($arr['idTipoCompetencia']);
					$cEntidad->setIdCompetencia($arr['idCompetencia']);					
					$cEntidad->setPuntMin($arr['puntMin']);
					$cEntidad->setPuntMax($arr['puntMax']);
					$cEntidad->setPuntBaremada($arr['puntBaremada']);
			...
			echo(constant("ERR"));
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [readEntidad][Baremos_resultadosDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			exit;
	...
	/*************************************************************************
	* Lo utilizamos para obtener la puntuación baremada de una prueba
	* @param entidad Entidad con la información basica a consultar
	* @exception Exception Error al ejecutar la acción
	*  en la base de datos
	* @return cEntidad con la información recuperada.
	*************************************************************************/
	function readEntidadTransformada($cEntidad)
	{
		$aux			= $this->conn;
	
		$sql = "SELECT *  FROM baremos_resultados_competencias WHERE ";
		$sql  .="idPrueba=" . $aux->qstr($cEntidad->getIdPrueba(), false) . " ";
		$sql  .="AND idBaremo=" . $aux->qstr($cEntidad->getIdBaremo(), false) . " ";
		$sql  .="AND puntMin>=" . $aux->qstr($cEntidad->getPuntMin(), false) . " ";
		$sql  .="AND puntMax<=" . $aux->qstr($cEntidad->getPuntMax(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
					$cEntidad->setIdResultado($arr['idResultado']);
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getIdTipoCompetencia() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="idTipoCompetencia=" . $aux->qstr($cEntidad->getIdTipoCompetencia(), false);
		}
		...
		if ($cEntidad->getPuntMin() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntMin>=" . $aux->qstr($cEntidad->getPuntMin(), false);
		}
		if ($cEntidad->getPuntMinHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntMin<=" . $aux->qstr($cEntidad->getPuntMinHast(), false);
		}
		if ($cEntidad->getPuntMax() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntMax>=" . $aux->qstr($cEntidad->getPuntMax(), false);
		}
		if ($cEntidad->getPuntMaxHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntMax<=" . $aux->qstr($cEntidad->getPuntMaxHast(), false);
		}
		if ($cEntidad->getPuntBaremada() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntBaremada>=" . $aux->qstr($cEntidad->getPuntBaremada(), false);
		}
		if ($cEntidad->getPuntBaremadaHast() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="puntBaremada<=" . $aux->qstr($cEntidad->getPuntBaremadaHast(), false);
		}
	...
	* @return Vector Lista recuperada
	*************************************************************************/
	function readListaTransforma($cEntidad)
	{
		$aux			= $this->conn;
...
}//Fin de la Clase Baremos_resultadosDB
?>