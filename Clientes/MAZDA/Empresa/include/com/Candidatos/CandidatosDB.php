...
class CandidatosDB
{
	...
	***********************************************************************/
	function insertar($cEntidad)
	{
		...
		$sql .= $aux->qstr($cEntidad->getCertificada(), false) . ",";
...
//		echo $sql;exit;
	...
	function readEntidad($cEntidad)
	{
		...
		if ($rs){
			while ($arr = $rs->FetchRow()){
					...
					$cEntidad->setCertificada($arr['certificada']);
					$cEntidad->setEnvDiploma($arr['envDiploma']);
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		if ($cEntidad->getCertificada() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(certificada) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getCertificada() . "%") . ")";
		}

		if ($cEntidad->getEnvDiploma() != ""){
			$sql .= $this->getSQLWhere($and);
			$and = true;
			$sql .="UPPER(envDiploma) LIKE UPPER(" . $aux->qstr("%" . $cEntidad->getEnvDiploma() . "%") . ")";
		}
