...
// | PHP Version 5                                                        |
// +----------------------------------------------------------------------+
// | Copyright (c) 2005 - Azul Pomodoro                                |
// +----------------------------------------------------------------------+
// | Este proceso se encarga de actualizar las tablas de testStation      |
// | de la Base de datos Antigua a la nueva.                              |
// +----------------------------------------------------------------------+
// | Author: Pedro Borregán <pborregan@azulpomodoro.com>               |
// +----------------------------------------------------------------------+
//
// $Id: Expecial_export.php,v 1.0 11/01/2011
//
class Expecial_export
{
	/**
	...
	**/
	...
	function creaSQL($result, $connMssql, $sTablaSQLSERVER)
	{
		...
		for ($j=0, $max=sizeof($arr); $j < $max; $j++){
			...
			$this->crea_fichero(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "SQLFiles/" . $aRetorno["sTablaMySQL"] . ".sql",str_replace("\r\n","\\n", $sql) . ";\n", $j, $aRetorno["sTablaMySQL"]);
		}
	...
	function crea_fichero($file, $content, $i, $sTabla)
	{
		if ($i > 0 ){
			...
			if (!$this->vaciarTabla($sTabla)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]::vaciarTabla()";
		...
		if (!$this->cargaMySQL($content)){
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]::cargaMySQL()";
		...
		switch ($sTablaMySQL)
		{
			case "empresas":
				...
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]";
				...
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_NUEVO") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
					exit;
				}
			...
			case "wi_usuarios":
				...
				$sql .=" VALUES ";
				$sql .=" (0, 0, 'Admin', '" . md5("mordor01") . "', 'Administrador', 'Sistema', 'sistemas@azulpomodoro.com', now(), now());";
			...
			case "pruebas":
				...
				$sql.= "IdPrueba =2;\n ";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
					exit;
				}
...
//				$sql.= "cabecera='imgPruebas/16/es/cabecera.jpg', ";
				...
				$sql.= "observaciones=CONCAT(observaciones,'\nEspecífico de Mazda, pero no sé si esta es la ultima versión') ";
				$sql.= " WHERE ";
				$sql.= "IdPrueba =19;\n ";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Expecial_export]";
				    	...
				    	if ($k == "grafico" ||
				    		...
				    		$k == "grafico4" ){
				    			if (empty($v)){
									...
									$this->chk_dir(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . $sDir);
									if (!@copy($sOrigenFile, constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . $sDestinoFile)){
										@file_put_contents(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . $sDestinoFile, file_get_contents($sOrigenFile));
									}
	...
	*/
	...
	/*
					//BORRAMOS TODAS LAS OPCIONES VALORES DE ESA PRUEBA - Se ha borrado la tabla
	...
	*/
	...
	function cargaBaremos($sIdPrueba){
		...
		switch ($sIdPrueba)
		{
			...
			case 16:	//nips
				...
				$sql = "DELETE FROM `baremos_resultados` WHERE idPrueba='" . $sIdPrueba . "'";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
				...
				$sql="";
				$sql.="INSERT INTO `baremos_resultados` (`idResultado`, `idBaremo`, `idPrueba`, `idBloque`, `idEscala`, `puntMin`, `puntMax`, `puntBaremada`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod`) VALUES";
				$sql.="(23, 1, 16, 0, 0, 0, 1, 1, '2011-02-08 08:24:47', '2011-02-08 08:24:47', 0, 0),";
				$sql.="(24, 1, 16, 0, 0, 2, 2, 2, '2011-02-08 08:25:13', '2011-02-08 08:25:13', 0, 0),";
				$sql.="(25, 1, 16, 0, 0, 3, 3, 5, '2011-02-08 08:25:24', '2011-02-08 08:25:24', 0, 0),";
				$sql.="(26, 1, 16, 0, 0, 4, 4, 12, '2011-02-08 08:25:34', '2011-02-08 08:25:34', 0, 0),";
				$sql.="(27, 1, 16, 0, 0, 5, 5, 21, '2011-02-08 08:25:42', '2011-02-08 08:25:42', 0, 0),";
				$sql.="(28, 1, 16, 0, 0, 6, 6, 34, '2011-02-08 08:25:51', '2011-02-08 08:25:51', 0, 0),";
				$sql.="(29, 1, 16, 0, 0, 7, 7, 50, '2011-02-08 08:26:01', '2011-02-08 08:26:01', 0, 0),";
				$sql.="(30, 1, 16, 0, 0, 8, 8, 66, '2011-02-08 08:26:10', '2011-02-08 08:26:10', 0, 0),";
				$sql.="(31, 1, 16, 0, 0, 9, 9, 79, '2011-02-08 08:26:19', '2011-02-08 08:26:19', 0, 0),";
				$sql.="(32, 1, 16, 0, 0, 10, 10, 88, '2011-02-08 08:26:30', '2011-02-08 08:26:30', 0, 0),";
				$sql.="(33, 1, 16, 0, 0, 11, 11, 95, '2011-02-08 08:26:47', '2011-02-08 08:26:47', 0, 0),";
				$sql.="(34, 1, 16, 0, 0, 12, 12, 98, '2011-02-08 08:26:56', '2011-02-08 08:26:56', 0, 0),";
				$sql.="(35, 1, 16, 0, 0, 13, 15, 99, '2011-02-08 08:27:14', '2011-02-08 08:27:14', 0, 0);";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_NUEVO") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
			...
			case 26:	//vips
				...
				$sql="";
				$sql.="INSERT INTO `baremos` (`idBaremo`, `idPrueba`, `idBloque`, `idEscala`, `nombre`, `descripcion`, `observaciones`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod`) VALUES";
				$sql.="(1, 26, 0, 0, 'Baremo Estándar', 'Este baremo será el estandar aplicado a la prueba', '', '2011-02-07 16:50:30', '2011-02-07 16:50:30', 0, 0);";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_NUEVO") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
				...
				$sql="";
				$sql.="INSERT INTO `baremos_resultados` (`idResultado`, `idBaremo`, `idPrueba`, `idBloque`, `idEscala`, `puntMin`, `puntMax`, `puntBaremada`, `fecAlta`, `fecMod`, `usuAlta`, `usuMod`) VALUES";
				$sql.="(1, 1, 26, 0, 0, 0, 11, 1, '2011-02-08 08:09:16', '2011-02-08 08:09:16', 0, 0),";
				$sql.="(2, 1, 26, 0, 0, 12, 12, 2, '2011-02-08 08:09:28', '2011-02-08 08:09:28', 0, 0),";
				$sql.="(3, 1, 26, 0, 0, 13, 13, 3, '2011-02-08 08:09:46', '2011-02-08 08:09:46', 0, 0),";
				$sql.="(4, 1, 26, 0, 0, 14, 14, 5, '2011-02-08 08:09:59', '2011-02-08 08:09:59', 0, 0),";
				$sql.="(5, 1, 26, 0, 0, 15, 15, 8, '2011-02-08 08:10:07', '2011-02-08 08:10:07', 0, 0),";
				$sql.="(6, 1, 26, 0, 0, 16, 16, 12, '2011-02-08 08:10:21', '2011-02-08 08:10:21', 0, 0),";
				$sql.="(7, 1, 26, 0, 0, 17, 17, 16, '2011-02-08 08:10:32', '2011-02-08 08:10:32', 0, 0),";
				$sql.="(8, 1, 26, 0, 0, 18, 18, 24, '2011-02-08 08:10:43', '2011-02-08 08:10:43', 0, 0),";
				$sql.="(9, 1, 26, 0, 0, 19, 19, 31, '2011-02-08 08:10:53', '2011-02-08 08:10:53', 0, 0),";
				$sql.="(10, 1, 26, 0, 0, 20, 20, 38, '2011-02-08 08:11:05', '2011-02-08 08:11:05', 0, 0),";
				$sql.="(11, 1, 26, 0, 0, 21, 21, 46, '2011-02-08 08:11:19', '2011-02-08 08:11:19', 0, 0),";
				$sql.="(12, 1, 26, 0, 0, 22, 22, 54, '2011-02-08 08:11:36', '2011-02-08 08:11:36', 0, 0),";
				$sql.="(13, 1, 26, 0, 0, 23, 23, 66, '2011-02-08 08:11:47', '2011-02-08 08:11:47', 0, 0),";
				$sql.="(14, 1, 26, 0, 0, 24, 24, 73, '2011-02-08 08:12:01', '2011-02-08 08:12:01', 0, 0),";
				$sql.="(15, 1, 26, 0, 0, 25, 25, 79, '2011-02-08 08:12:11', '2011-02-08 08:12:11', 0, 0),";
				$sql.="(16, 1, 26, 0, 0, 26, 26, 84, '2011-02-08 08:12:27', '2011-02-08 08:12:27', 0, 0),";
				$sql.="(17, 1, 26, 0, 0, 27, 27, 88, '2011-02-08 08:12:40', '2011-02-08 08:12:40', 0, 0),";
				$sql.="(18, 1, 26, 0, 0, 28, 28, 93, '2011-02-08 08:13:01', '2011-02-08 08:13:01', 0, 0),";
				$sql.="(19, 1, 26, 0, 0, 29, 29, 96, '2011-02-08 08:13:14', '2011-02-08 08:13:14', 0, 0),";
				$sql.="(20, 1, 26, 0, 0, 30, 30, 97, '2011-02-08 08:13:32', '2011-02-08 08:13:32', 0, 0),";
				$sql.="(21, 1, 26, 0, 0, 31, 31, 98, '2011-02-08 08:13:40', '2011-02-08 08:13:40', 0, 0),";
				$sql.="(22, 1, 26, 0, 0, 32, 39, 99, '2011-02-08 08:14:15', '2011-02-08 08:14:15', 0, 0);";
				if($oconn->Execute($sql) === false){
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_NUEVO") . "][Expecial_export]";
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					echo(constant("ERR"));
	...
	function cargaInstrucciones_pruebas(){
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/sqlInstrucciones_pruebas.sql","r");
		$content="";
		...
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [cargaInstrucciones_pruebas][Expecial_export]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
	...
	function cargaOpciones_ItemsESP(){
		$oconn	= $this->conn;
		$sql = "DELETE FROM `opciones` WHERE idPrueba=26 AND codIdiomaIso2='es';";
		if($oconn->Execute($sql) === false){
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]";
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			echo(constant("ERR"));
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/insertsOpcionesVipsEsp.sql","r");
		$content="";
		while(!feof($DescriptorFichero)){
	    	$content .= fgets($DescriptorFichero,4096);
		}
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [insertsOpcionesVipsEsp.sql][Expecial_export]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
		...
		$sql = "DELETE FROM `items` WHERE idPrueba=26 AND codIdiomaIso2='es';";
		if($oconn->Execute($sql) === false){
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Expecial_export]";
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			echo(constant("ERR"));
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/itemsVipsEsp.sql","r");
		$content="";
		while(!feof($DescriptorFichero)){
	    	$content .= fgets($DescriptorFichero,4096);
		}
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [itemsVipsEsp.sql][Expecial_export]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
	...
	function cargaEjemplos_ayudas(){
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/ejemplos_ayudas.sql","r");
		$content="";
		while(!feof($DescriptorFichero)){
	    	$content .= fgets($DescriptorFichero,4096);
		}
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [cargaEjemplos_ayudas][Expecial_export]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
	...
	function cargaPruebas_ayudas(){
		...
		$DescriptorFichero = fopen(constant("DIR_FS_DOCUMENT_ROOT_ADMIN") . "sqls/pruebas_ayudas.sql","r");
		$content="";
		while(!feof($DescriptorFichero)){
	    	$content .= fgets($DescriptorFichero,4096);
		}
		if (!empty($content)){
			if (!$this->cargaMySQL($content)){
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [cargaPruebas_ayudas][Expecial_export]::cargaMySQL()";
				error_log($sTypeError . " ->\t" . $content . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				echo(constant("ERR"));
				exit;
			}
...
}//Fin de la Clase Expecial_export
?>
