...
* la tabla Empresas_perfiles_funcionalidades
**/
class Empresas_perfiles_funcionalidadesDB
{
	...
	function insertar($cEntidad)
	{
		...
		for ($i=0; $i < $iSize; $i++){
			...
			if (!$this->borrar($cEntidad)){
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . ":::" . constant("MNT_BORRAR") . "][Empresas_perfiles_funcionalidadesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
				return 0;
			}else{
				//Alta
				$sql = "INSERT INTO empresas_perfiles_funcionalidades (";
				$sql .= "idPerfil" . ",";
				$sql .= "idFuncionalidad" . ",";
				$sql .= "modificar" . ",";
				$sql .= "borrar" . ",";
				...
				if($aux->Execute($sql) === false){
					$this->msg_Error	= array();
					$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_ALTA") . "][Empresas_perfiles_funcionalidadesDB]";
					$this->msg_Error[]	= $sTypeError;
					error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					return 0;
				}
	...
	*****************************************************************************************/
	function getSiguienteId($cEntidad)
	{
		...
		$sql  = "SELECT MAX() AS Max FROM empresas_perfiles_funcionalidades ";
...
/*		$aux			= $this->conn;
		...
		$sql = "UPDATE empresas_perfiles_funcionalidades SET ";
		...
		if($aux->Execute($sql) === false){
			$retorno=false;
			$this->msg_Error	= array();
			$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_MODIFICAR") . "][Empresas_perfiles_funcionalidadesDB]";
			$this->msg_Error[]	= $sTypeError;
			error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
		}
		return $retorno;
*/
	...
	************************************************************************/
	function borrar($cEntidad)
	{
		...
		if ($retorno){
			...
			$sql  ="DELETE FROM empresas_perfiles_funcionalidades ";
			$sql  .="WHERE ";
			...
			if($aux->Execute($sql) === false){
				$retorno=false;
				$this->msg_Error	= array();
				$sTypeError	=	date('d/m/Y H:i:s') . " Error SQL [" . constant("MNT_BORRAR") . "][Empresas_perfiles_funcionalidadesDB]";
				$this->msg_Error[]	= $sTypeError;
				error_log($sTypeError . " ->\t" . $sql . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
			}
		}else $retorno=false;
	...
	*************************************************************************/
	...
	$cEntidad->getPerfilFuncionalidades();
		$sql = "SELECT * FROM empresas_perfiles_funcionalidades WHERE ";
		$sql  .="idPerfil=" . $aux->qstr($cEntidad->getIdPerfil(), false) . " AND idFuncionalidad=" . $aux->qstr($cEntidad->getIdFuncionalidad(), false) . " ";
		$rs = $aux->Execute($sql);
		if ($rs){
			while ($arr = $rs->FetchRow()){
	...
	*************************************************************************/
	function readLista($cEntidad)
	{
		...
		$sql="";
		$and = false;
		$sql.="SELECT * FROM empresas_perfiles_funcionalidades ";
		if ($cEntidad->getIdPerfil() != ""){
...
}//Fin de la Clase Empresas_perfiles_funcionalidadesDB
?>