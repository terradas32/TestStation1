...
ob_start();
	...
	require_once(constant("DIR_WS_COM") . "Areas/AreasDB.php");
	require_once(constant("DIR_WS_COM") . "Areas/Areas.php");
...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new AreasDB($conn);  // Entidad DB
	$cEntidad	= new Areas();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['areas_next_page']) && $_POST['areas_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'areas');
					...
					$lista=$pager->getRS();
					include('Template/Areas/mntareasl.php');
				}else{
					$cEntidad	= new Areas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Areas/mntareasa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Areas/mntareas.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdArea((isset($_POST["LSTIdArea"]) && $_POST["LSTIdArea"] != "") ? $_POST["LSTIdArea"] : "");	$cEntidad->setBusqueda(constant("STR_ID_AREA"), (isset($_POST["LSTIdArea"]) && $_POST["LSTIdArea"] != "" ) ? $_POST["LSTIdArea"] : "");
		$cEntidad->setIdAreaHast((isset($_POST["LSTIdAreaHast"]) && $_POST["LSTIdAreaHast"] != "") ? $_POST["LSTIdAreaHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_AREA") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdAreaHast"]) && $_POST["LSTIdAreaHast"] != "" ) ? $_POST["LSTIdAreaHast"] : "");
