...
ob_start();
	...
	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Baremos_empresas/Baremos_empresasDB.php");
	require_once(constant("DIR_FS_DOCUMENT_ROOT") . constant("DIR_WS_COM") . "Baremos_empresas/Baremos_empresas.php");
...
include_once ('include/conexion.php');
	...
	if (isset($_POST['fCodIdiomaIso2Prueba']) && isset($_POST['fIdPrueba']))
	{
	...
	if (isset($_POST['fIdEmpresa']) && isset($_POST['fIdProceso']) && isset($_POST['fIdCandidato']))
	{
...
//exit;
	...
	switch ($_POST['MODO'])
	{
		...
		case constant("MNT_LISTAR"):
			...
			{
				$sql = $cEntidadDB->readListaAceptaMazdaIN($cEntidad);
				//echo $sql;exit;
            }
			$pager = new ADODB_Pager($conn,$sql,'respuestas_pruebas');
			if ($bInit)	$pager->curr_page=1;
...
//				$cTipos_informes->setCodIdiomaIso2($sLang);
				$sSQLTInformes = $cTipos_informesDB->readLista($cTipos_informes);
				$listaTInformes = $conn->Execute($sSQLTInformes);
				$sTipoInformes = "";
//				echo $sSQLTInformes;
				while(!$listaTInformes->EOF){
					$sTipoInformes .= "," . $listaTInformes->fields['idTipoInforme'];
					$listaTInformes->MoveNext();
				}
				if (!empty($sTipoInformes)){
					$sTipoInformes  = substr($sTipoInformes , 1);
				}
				$cInformes_pruebas	= new Informes_pruebas();
				$cInformes_pruebas->setIdPrueba($_POST['fIdPrueba']);
                if (!empty($_POST['fCodIdiomaInforme'])){
                    $cInformes_pruebas->setCodIdiomaIso2($_POST['fCodIdiomaInforme']);
                }
//				$cInformes_pruebas->setCodIdiomaIso2('es');
//mal				$cInformes_pruebas->setCodIdiomaIso2($sLang);
//				$cInformes_pruebas->setIdTipoInforme($sTipoInformes);
				$sqlTipos= $cInformes_pruebasDB->readLista($cInformes_pruebas);
//				$sqlTipos = str_replace(")", ") GROUP BY idTipoInforme ", $sqlTipos);
				$sqlTipos .= " GROUP BY idTipoInforme ";
//				echo $sqlTipos;
		...
		case constant("MNT_LISTABAREMOS"):
			if(isset($_POST['fIdPrueba']) && $_POST['fIdPrueba']!=""){
				...
				{
					$cBaremos_empresas = new Baremos_empresas();
					$cBaremos_empresas->setIdEmpresa($_POST['fIdEmpresa']);
					$cBaremos_empresas->setIdPrueba($_POST['fIdPrueba']);
					...
					$sBaremosEmpresaPrueba = "";
					while(!$listaBaremos_empresas->EOF){
						$sBaremosEmpresaPrueba .= "," . $listaBaremos_empresas->fields['idBaremo'];
						$listaBaremos_empresas->MoveNext();
					}
					if (!empty($sBaremosEmpresaPrueba)){
						$sBaremosEmpresaPrueba = substr($sBaremosEmpresaPrueba, 1);
					}else{
						//Si no tiene asignado Baremo lo inicializamos para que no saque
						$sBaremosEmpresaPrueba = "-1";
					}
					$cBaremos->setIdBaremo($sBaremosEmpresaPrueba);
				}
		...
		case constant("MNT_EXPORTA"):
				...
				$bDescargar=false;
				if ($_sPrepago == 0 || $_EmpresaLogada == constant("EMPRESA_PE")){
									...
									if ($sLlamada != "Candidato" )
 									{
										if (isset($aSQLPuntuacionesPPL) || isset($aSQLPuntuacionesC))
										{
											...
											{
												$sValidar = "SELECT * FROM export_personalidad_laboral WHERE ";
												$sValidar .= "idEmpresa=" . $cCandidato->getIdEmpresa();
												$sValidar .= "AND idProceso=" . $cCandidato->getIdProceso();
												$sValidar .= "AND idCandidato=" . $cCandidato->getIdCandidato();
												$sValidar .= "AND idPrueba=" . $cPruebas->getIdPrueba();
												$rsValidar = $conn->Execute($sValidar);
												echo "<br />/**/-->" . $rsValidar->recordCount();
											...
											{
												$sValidar = "SELECT * FROM export_personalidad_competencias WHERE ";
												$sValidar .= "idEmpresa=" . $cCandidato->getIdEmpresa();
												$sValidar .= "AND idProceso=" . $cCandidato->getIdProceso();
												$sValidar .= "AND idCandidato=" . $cCandidato->getIdCandidato();
												$sValidar .= "AND idPrueba=" . $cPruebas->getIdPrueba();
...
// 										echo "<br />" . $sSQLUPDATE;
		...
		//					print  ( $binDatosExcel);
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_EXPORTA") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
...
//							echo "pPercentil: " . $iPercentil . "<br />";
		...
		//					print  ( $binDatosExcel);
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " RE-Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_EXPORTA_HTML") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
						//Si ya está generado sacamos el enlace
...
//							echo "pPercentil: " . $iPercentil . "<br />";
		...
		//					print  ( $binDatosExcel);
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_EXPORTA_WORD") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
						//Si ya está generado sacamos el enlace
						//Miramos si hay que descontar y cuanto para informar al usuario.
...
//							echo "<a href=\"#_\" title=\"Descargar\" onclick=\"javascript:abrirVentana('0','". base64_encode(constant('HTTP_SERVER'). $sDirImg . $sNombre)."');\">" . constant("STR_DESCARGAR") . "</a>";
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_DESCUENTA_DONGLES") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
						//Si ya está generado sacamos el enlace
						//Miramos si hay que descontar y cuanro para informar al usuario.
...
//							echo "<a href=\"#_\" title=\"Descargar\" onclick=\"javascript:abrirVentana('0','". base64_encode(constant('HTTP_SERVER'). $sDirImg . $sNombre)."');\">" . constant("STR_DESCARGAR") . "</a>";
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_DESCUENTA_DONGLES_HTML") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
						//Si ya está generado sacamos el enlace
						//Miramos si hay que descontar y cuanro para informar al usuario.
...
//							echo "<a href=\"#_\" title=\"Descargar\" onclick=\"javascript:abrirVentana('0','". base64_encode(constant('HTTP_SERVER'). $sDirImg . $sNombre)."');\">" . constant("STR_DESCARGAR") . "</a>";
						...
						$sTypeError	=	"\n" . date('d/m/Y H:i:s') . " Generado desde [" . $_SERVER['REMOTE_ADDR'] . "] [" . $sNombre . "][" . constant("MNT_DESCUENTA_DONGLES_WORD") . "][Empresa]";
						error_log($sTypeError . "\n", 3, constant("DIR_FS_PATH_NAME_LOG"));
					}else{
						//Si ya está generado sacamos el enlace
						//Miramos si hay que descontar y cuanro para informar al usuario.
