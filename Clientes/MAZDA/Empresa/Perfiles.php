...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new PerfilesDB($conn);  // Entidad DB
	$cEntidad	= new Perfiles();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['perfiles_next_page']) && $_POST['perfiles_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'perfiles');
					if ($bInit)	$pager->curr_page=1;
					$pager->showPageLinks = true;
					$lista=$pager->getRS();
					include('Template/Perfiles/mntperfilesl.php');
				}else{
					$cEntidad	= new Perfiles();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Perfiles/mntperfilesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Perfiles/mntperfiles.php');
			break;
		case constant("MNT_LISTAR"):
			...
			if ($cEntidad->getIdPerfil() != "")
			{
				...
				if ($cEntidad->getIdPerfil() != ""){
					...
					?><script language="javascript" type="text/javascript">alert("<?php echo constant("ERR_FORM_ERROR");?>\n<?php echo constant("ERR_FORM_NO_EXISTEN_DATOS");?>");</script><?php 
					$_POST['MODO']=constant("MNT_LISTAR");
					include('Template/Perfiles/mntperfiles.php');
				}
			}else{
	...
	*/
	function readLista($cEntidad){
        global $conn;
        global $cUtilidades;
		$cEntidad->setIdPerfil((isset($_POST["LSTIdPerfil"])) ? $_POST["LSTIdPerfil"] : "");	$cEntidad->setBusqueda(constant("STR_ID_PERFIL"), (isset($_POST["LSTIdPerfil"])) ? $_POST["LSTIdPerfil"] : "");
