...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new Empresas_accesosDB($conn);  // Entidad DB
	$cEntidad	= new Empresas_accesos();  // Entidad
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				...
				{
					if (!empty($_POST['empresas_accesos_next_page']) && $_POST['empresas_accesos_next_page'] > 1 ){
						$bInit=false;
					...
					$pager = new ADODB_Pager($conn,$sql,'empresas_accesos');
					...
					$lista=$pager->getRS();
					include('Template/Empresas_accesos/mntempresas_accesosl.php');
				}else{
					$cEntidad	= new Empresas_accesos();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Empresas_accesos/mntempresas_accesosa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Empresas_accesos/mntempresas_accesos.php');
			break;
	...
	*/
	function readLista($cEntidad){
		global $conn;
		global $cUtilidades;
		$cEntidad->setIdAcceso((isset($_POST["LSTIdAcceso"]) && $_POST["LSTIdAcceso"] != "") ? $_POST["LSTIdAcceso"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ACCESO"), (isset($_POST["LSTIdAcceso"]) && $_POST["LSTIdAcceso"] != "" ) ? $_POST["LSTIdAcceso"] : "");
		$cEntidad->setIdAccesoHast((isset($_POST["LSTIdAccesoHast"]) && $_POST["LSTIdAccesoHast"] != "") ? $_POST["LSTIdAccesoHast"] : "");	$cEntidad->setBusqueda(constant("STR_ID_ACCESO") . " " . constant("STR_HASTA"), (isset($_POST["LSTIdAccesoHast"]) && $_POST["LSTIdAccesoHast"] != "" ) ? $_POST["LSTIdAccesoHast"] : "");
