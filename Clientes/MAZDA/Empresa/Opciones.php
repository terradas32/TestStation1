...
include_once ('include/conexion.php');
	...
	$cEntidadDB	= new OpcionesDB($conn);  // Entidad DB
	$cEntidad	= new Opciones();  // Entidad
	...
	$comboITEMS	= new Combo($conn,"fIdItem","idItem","descripcion","Descripcion","items","",constant("SLC_OPCION"),"","","fecMod");
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			?><script language="javascript" type="text/javascript">alert("<?php echo constant("CONF_ALTA") . $newId;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php 
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					$lista=$pager->getRS();
					include('Template/Opciones/mntopcionesl.php');
				}else{
					$cEntidad	= new Opciones();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Opciones/mntopcionesa.php');
				}
			}else{
		...
		case constant("MNT_BUSCAR"):
			...
			include('Template/Opciones/mntopciones.php');
			break;
