...
include_once ('include/conexion.php');
	...
	if ($cAutologin->getToken() != ""){
		...
		if (empty($_POST["MODO"])){
			$_POST["MODO"]= constant("MNT_NUEVO");
		}
		$cAutologinDB->ActualizaAccion($cAutologin);
	}else{
		$token =md5(uniqid('', true));
		$cAutologin->setToken($token);
		$cAutologinDB->ActualizaToken($cAutologin);
	}
	...
	switch ($_POST['MODO'])
	{
		case constant("MNT_ALTA"):
			...
			$sUsr= $cEntidadDB->getSiguienteId($cEntidad);
			$cEntidad->setUsuario($sUsr);
...
///////////////////////////////////////////////////////////////////
				$sMensajeAltaMB = 'Hemos recibido sus datos correctamente,\nen un plazo no superior a 24 h.\nestará activada la plataforma.';
			?><script language="javascript" type="text/javascript">alert("<?php echo $sMensajeAltaMB;?>\n<?php echo $cEntidadDB->ver_errores();?>");</script><?php
				if ($_POST['ORIGEN'] == constant("MNT_LISTAR"))
				{
					...
					$lista=$pager->getRS();
					include('Template/Concesionarios/mntempresasl.php');
				}else{
					$cEntidad	= new Empresas();  // inicializamos la Entidad
					$_POST['MODO']    = constant("MNT_ALTA");
					include('Template/Concesionarios/mntempresasa.php');
				}
			}else{
...
//			$_POST["LSTOrder"] = "ASC";
		...
		case constant("MNT_BUSCAR"):
			...
			$cEntidad = readLista($cEntidad);
			include('Template/Concesionarios/mntempresas.php');
			break;
...
//			$cEntidad = readLista($cEntidad);
	...
	*/
	function readEntidad($cEntidad){
		...
		$cEntidad->setRazonSocial((isset($_POST["fRazonSocial"])) ? $_POST["fRazonSocial"] : "");
		...
		//echo "->" . $_cEntidadUsuarioTK->getOrden();
		$cEntidad->setDentroDe($_cEntidadUsuarioTK->getOrden());
		$cEntidad->setDespuesDe("");
	...
	*/
	function readLista($cEntidad){
		...
		$cEntidad->setRazonSocial((isset($_POST["LSTRazonSocial"]) && $_POST["LSTRazonSocial"] != "") ? $_POST["LSTRazonSocial"] : "");	$cEntidad->setBusqueda(constant("STR_NOMBRE"), (isset($_POST["LSTRazonSocial"]) && $_POST["LSTRazonSocial"] != "" ) ? $_POST["LSTRazonSocial"] : "");
		...
		$cEntidad->setPersonaContacto((isset($_POST["LSTPersonaContacto"]) && $_POST["LSTPersonaContacto"] != "") ? $_POST["LSTPersonaContacto"] : "");	$cEntidad->setBusqueda("Persona Contacto", (isset($_POST["LSTPersonaContacto"]) && $_POST["LSTPersonaContacto"] != "" ) ? $_POST["LSTPersonaContacto"] : "");
		$cEntidad->setTlfContacto((isset($_POST["LSTTlfContacto"]) && $_POST["LSTTlfContacto"] != "") ? $_POST["LSTTlfContacto"] : "");	$cEntidad->setBusqueda("Tlf. Contacto", (isset($_POST["LSTTlfContacto"]) && $_POST["LSTTlfContacto"] != "" ) ? $_POST["LSTTlfContacto"] : "");
